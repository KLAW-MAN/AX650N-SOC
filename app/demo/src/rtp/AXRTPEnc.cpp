...
*/
static AX_S32 AX_RTPENC_H264(AX_RTP_ENC_T *pRtpEnc, AX_U8 *inBuf, AX_S32 inLen, AX_U32 pcr, AX_U64 u64SeqNum, const AX_MEDIA_DATA_T& mediaData);
...
void *RtpEnc_MediaData_Proc(void *args)
{
    ...
    while(pRtpEnc->nThreadStart)
    {
        ...
        AX_RTPENC_H264(pRtpEnc, stMediaData.mediaData, stMediaData.mediaDataLen, stMediaData.timestamp, stMediaData.u64SeqNum, stMediaData);

        if (pRtpEnc->cbSent) {
            pRtpEnc->cbSent(stMediaData);
        }
...
AX_S32 AX_RTPENC_Init(AX_RTP_ENC_T *pRtpEnc, const AX_CHAR *ipAddr, AX_U32 ipPort, RTP_PREP_CALLBACK cbPrep, RTP_SENT_CALLBACK cbSent) {
    AX_S32 sockfd;
    AX_U32 sendBufSize = 1024*1024;
    AX_S32 s32Ret;

    memset(pRtpEnc, 0, sizeof(AX_RTP_ENC_T));
    pRtpEnc->cbPrep = cbPrep;
    pRtpEnc->cbSent = cbSent;
    ...
    s32Ret = setsockopt(sockfd, SOL_SOCKET, SO_SNDBUF, (char *)&sendBufSize, sizeof(sendBufSize));
    if(s32Ret != 0)
    {
    ...
    strcpy(pRtpEnc->ip, ipAddr);
    pRtpEnc->sockPort = ipPort;
...
AX_S32 AX_RTPENC_DeInit(AX_RTP_ENC_T *pRtpEnc) {
    ...
    AX_VOID* threadResult;
    pthread_join(pRtpEnc->mediaDataPid, &threadResult);
...
AX_S32 AX_RTPENC_Proc(AX_RTP_ENC_T *pRtpEnc, const AX_U8 *inBuf, AX_S32 inLen, AX_S32 nType, AX_U64 u64PTS, AX_U64 u64SeqNum, AX_U64 nUserData) {
    ...
    if (!stMediaData.mediaData) {
        return -1;
    }

    memcpy(stMediaData.mediaData, inBuf, inLen);
    stMediaData.mediaDataLen = inLen;
    stMediaData.timestamp = (AX_U32)u64PTS;
    stMediaData.type = nType; /* AX_PAYLOAD_TYPE_E */
    stMediaData.u64SeqNum = u64SeqNum;
    stMediaData.u64UserData = nUserData;
...
AX_S32 AX_RTPENC_H264(AX_RTP_ENC_T *pRtpEnc, AX_U8 *inBuf, AX_S32 inLen, AX_U32 pcr, AX_U64 u64SeqNum, const AX_MEDIA_DATA_T& mediaData)
{
	AX_U8 sendBuf[2048];
	AX_S32 sendBufLen;
	RTP_HEADER *rtpHdr;
	...
	rtpHdr->ssrc = (0xCDBA0000 | pRtpEnc->sockPort);
	rtpHdr->timestamp = htonl(pcr);
    rtpHdr->u64SeqNum = u64SeqNum;

#ifdef __SEND_LATENCY_DEBUG_DATA__
    if (pRtpEnc->cbPrep) {
        pRtpEnc->cbPrep(mediaData, &sendBuf[0], sizeof(sendBuf));
    }
#endif
