...
AX_BOOL CVo::Init(const VO_ATTR_T& stAttr) {
    ...
    const AX_HDMI_ID_E enHdmi = (1 == m_stAttr.voDev) ? AX_HDMI_ID1 : AX_HDMI_ID0;
    ...
    try {
        ...
        if (0 != ret) {
            ...
            LOG_M_C(VO, "layer %d: [(%d, %d) %dx%d], dispatch mode %d, layer depth %d, part mode %d, tolerance %d", m_stAttr.voLayer,
        ...
        if (m_stAttr.fbLayer != INVALID_LAYER_ID) {
            ret = AX_VO_BindGraphicLayer(m_stAttr.fbLayer, m_stAttr.voDev);
            if (0 != ret) {
                LOG_M_E(VO, "AX_VO_BindGraphicLayer(layer %d dev %d) fail, ret = 0x%x", m_stAttr.fbLayer, m_stAttr.voDev, ret);
                throw 1;
            } else {
                nState |= FBLAYER_BINDED;
            }
        }
...
AX_BOOL CVo::GetDispInfoFromIntfSync(AX_VO_INTF_SYNC_E eIntfSync, AX_VO_RECT_T& stArea, AX_U32& nHz) {
    ...
    switch (eIntfSync) {
        ...
        case AX_VO_OUTPUT_4096x2160_60:
            ...
            nHz = 60;
            break;
        case AX_VO_OUTPUT_1080x1920_60:
            stArea.u32Width = 1080;
            stArea.u32Height = 1920;
            nHz = 60;
            break;
        default:
...
AX_BOOL CVo::DpmsOnOff(AX_BOOL bOn) {
    ...
    AX_S32 ret;
    if (bOn) {
        ret = AX_VO_DpmsOn(m_stAttr.voDev);
        if (0 != ret) {
            LOG_MM_E(VO, "AX_VO_DpmsOn(dev %d) fail, ret = 0x%x", m_stAttr.voDev, ret);
            return AX_FALSE;
        }

        LOG_MM_W(VO, "dev %d dpms on", m_stAttr.voDev);
    } else {
        ret = AX_VO_DpmsOff(m_stAttr.voDev);
        if (0 != ret) {
            LOG_MM_E(VO, "AX_VO_DpmsOff(dev %d) fail, ret = 0x%x", m_stAttr.voDev, ret);
            return AX_FALSE;
        }

        LOG_MM_W(VO, "dev %d dpms off", m_stAttr.voDev);
    }
