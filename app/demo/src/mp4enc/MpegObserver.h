...
public:
    ...
    AX_BOOL OnDataChange(OBS_TARGET_TYPE_E eTarget, AX_U32 nGrp, AX_U32 nChn, OBS_CHANGE_DATA_TYPE_E eDataType, AX_VOID* pData) override {
        ...
        if (nullptr == pData) {
            LOG_M_E(MPEG4_OBS, "pData nullptr for eDataType: %d", eDataType);
            return AX_FALSE;
        }

        switch(eDataType) {
            case E_OBS_CHANGE_DATA_TYPE_FRAMERATE:
                m_pSink->UpdateFrameRate(*(AX_U32 *)pData);
                break;
            case E_OBS_CHANGE_DATA_TYPE_RESOLUTION:
                {
                    OBS_TRANS_RESOLUTION_PTR pResolution = (OBS_TRANS_RESOLUTION_PTR)pData;
                    m_pSink->UpdateResolution(pResolution->nWidth, pResolution->nHeight);
                }
                break;
            case E_OBS_CHANGE_DATA_TYPE_PAYLOADTYPE:
                m_pSink->UpdateVideoPayloadType((AX_PAYLOAD_TYPE_E)(*(AX_U32 *)pData));
                break;
            case E_OBS_CHANGE_DATA_TYPE_BITRATE:
                m_pSink->UpdateBitRate(*(AX_U32 *)pData);
                break;
            default:
                LOG_M_W(MPEG4_OBS, "Unsupport eDataType: %d", eDataType);
                break;
        }
        return AX_TRUE;
    }
