...
#define MAX_WS_CONN_NUM (64)
#define AX_WEB_VENC_RING_BUFF_COUNT (2)
#define AX_WEB_JENC_RING_BUFF_COUNT (2)
#define AX_WEB_MJENC_RING_BUFF_COUNT (2)
#define AX_WEB_EVENTS_RING_BUFF_COUNT (5)
#define AX_WEB_AENC_RING_BUFF_COUNT (10)
#define AX_WEB_MAX_PREV_SNS_NUM (8)
...
typedef enum {
    ...
    E_WEB_OPERATION_TYPE_FLASH_SETTING,
    E_WEB_OPERATION_TYPE_EIS,
    E_WEB_OPERATION_TYPE_SHUTTER_TYPE,
    E_WEB_OPERATION_TYPE_VSUBCHN_SWITCH,
    E_WEB_OPERATION_TYPE_GET_DUMPRAWPIPE,
    E_WEB_OPERATION_TYPE_RECORDER_SWITCH,
...
typedef struct _WEB_OPR_EIS_SWITCH {
    AX_BOOL bOn;

    _WEB_OPR_EIS_SWITCH() {
        bOn = AX_TRUE;
    }
} WEB_OPR_EIS_SWITCH_T;

typedef struct _WEB_OPR_SHUTTERTYPE {
    AX_U32 nShutterType;

    _WEB_OPR_SHUTTERTYPE() {
        nShutterType = 0;
    }
} WEB_OPR_SHUTTERTYPE_T;
typedef struct _WEB_OPR_VSUBCHN_SWITCH {
    AX_BOOL bOn;

    _WEB_OPR_VSUBCHN_SWITCH() {
        bOn = AX_TRUE;
    }
} WEB_OPR_VSUBCHN_SWITCH_T;

typedef struct _WEB_OPR_RESERVED_RAW_PIPE {
    AX_U8 nPipeId;

    _WEB_OPR_RESERVED_RAW_PIPE() {
        nPipeId = 0;
    }
} WEB_OPR_DUMP_RAW_PIPE_T;

typedef struct _WEB_OPR_RECORDER_SWITCH {
    AX_BOOL bOn;
    AX_U8   nRecordType;

    _WEB_OPR_RECORDER_SWITCH() {
        bOn = AX_FALSE;
        nRecordType = 0;
    }
} WEB_OPR_RECORDER_SWITCH_T;
typedef enum {
...
typedef struct _WEB_OPR_FLASH_ATTR_T {
    AX_S8 nLaneId{-1};
    AX_S8 nFlashId{-1};
    AX_S8 nTriggerCount{-1};
    AX_S8 nFrameInterval{-1};
} WEB_OPR_FLASH_ATTR_T;
...
typedef struct _WEB_REQ_OPERATION_T {
    ...
    union {
        ...
        WEB_OPR_EIS_SWITCH_T tEIS;
        WEB_OPR_SHUTTERTYPE_T tShutter;
        WEB_OPR_VSUBCHN_SWITCH_T tVSubChnEnable;
        WEB_OPR_DUMP_RAW_PIPE_T tDumpRawPipe;
        WEB_OPR_RECORDER_SWITCH_T tRecorderSwitch;
    };
...
typedef struct _TRIGGER_INFO_T{
    AX_U64 nUserData;
    AX_U64 nPts;
    AX_U64 nSeqNum;
    _TRIGGER_INFO_T(){
...
}TRIGGER_INFO_T, *TRIGGER_INFO_PTR;
...
public:
    ...
    AX_VOID SendCaptureData(AX_U8 nSnsID, AX_U8 nUniChn, AX_VOID* data, AX_U64 nUserData, AX_U32 size, AX_U64 nPts = 0, AX_BOOL bIFrame = AX_TRUE,
                            JPEG_DATA_INFO_T* pJpegInfo = nullptr);
    ...
    AX_VOID RegistUniCaptureChn(AX_U8 nSnsId, AX_S8 nCaptureChn, JPEG_TYPE_E eType = JPEG_TYPE_CAPTURE);
    ...
    AX_BOOL IsCaptrueEnable(AX_U8 nSnsID) const;
    ...
    AX_VOID SetTriggerPicPath(std::string path) {
        m_strTriggerPicPath = path;
    }

    AX_VOID EnableTriggerPicStore(AX_BOOL enable) {
        m_bEnableStoreTriggerPic = enable;
    }
...
private:
    ...
    CAXRingElementEx* GetCaptureData(AX_S32 &nCaptureChn);
...
private:
    ...
    AX_BOOL m_arrCaptureEnable[AX_WEB_MAX_PREV_SNS_NUM]{AX_FALSE};
    ...
    std::string m_strTriggerPicPath;
    AX_BOOL m_bEnableStoreTriggerPic{AX_FALSE};

    AX_U8 m_nCaptureIndex{0};
    std::map<AX_U8, AX_U8> m_mapCaptureSnsChn;
};