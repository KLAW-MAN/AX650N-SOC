...
#define PARAM_KEY_SNS_SELECT "snsSelect"
using namespace std;
...
static std::map<string, AX_U32> g_mapToken2Data;
...
static AX_U8 GetUniChnFromToken(cchar* token, AX_U8 nSnsID) {
    ...
    AX_U32 nData = g_mapToken2Data[(string)token];
...
static AX_VOID SaveTokenData(cchar* token, AX_U8 nSnsID, AX_U8 nUniChn) {
    ...
    AX_U32 nData = 0;
    std::map<string, AX_U32>::iterator itFind = g_mapToken2Data.find(token);
...
inline picojson::object ConstructResponseJsonObj(AX_S32 nHttpStatusCode, cchar* pszToken) {
...
AX_VOID ReadAndPlayAudioData(HttpConn* conn) {
    static AX_U8 nTalkChannel = s_pWebInstance->GetTalkChannel();
    MprBuf* pWSData = (MprBuf*)httpGetWebSocketData(conn);
    AX_U8 nChannel = 0;
    if (!pWSData) {
        return;
    } else {
...
static AX_VOID LoginAction(HttpConn* conn) {
    ...
    mprWriteJson(mprReadJsonObj(pResponseBody, "data"), PARAM_KEY_SNS_MODE, (bDualSnsMode ? "1" : "0"), MPR_JSON_STRING);

    AX_BOOL bSnsSelect = (APP_WEB_SHOW_SENSOR_COUNT() >= 4) ? AX_TRUE : AX_FALSE;
    mprWriteJson(mprReadJsonObj(pResponseBody, "data"), PARAM_KEY_SNS_SELECT, (bSnsSelect ? "1" : "0"), MPR_JSON_STRING);

    string strPanoSnsId = (APP_WEB_PANO_SENSOR_ID() == -1 ) ? "-1" : to_string(APP_WEB_PANO_SENSOR_ID());
    mprWriteJson(mprReadJsonObj(pResponseBody, "data"), PARAM_KEY_PANO_SNS_ID, strPanoSnsId.c_str(), MPR_JSON_STRING);
...
static constexpr AX_U8 nPrevInfoMaxCharLen = 24;
static AX_VOID PreviewInfoAction(HttpConn* conn) {
    ...
    if (strcmp(conn->rx->method, "GET") == 0) {
        ...
        string strStream = "[";
        for (AX_U8 i = 0; i < AX_WEB_MAX_PREV_SNS_NUM; i++) {
            AX_CHAR chStream[nPrevInfoMaxCharLen + 1] = {0};
            if (i < nSnsCnt) {
                AX_U8 nPrevChnCount = m_sMapPrevChn2UniChn[i].size();
                if (1 == nPrevChnCount) {
                    sprintf(chStream, "[%d]", 0);
                } else if (2 == nPrevChnCount) {
                    sprintf(chStream, "[%d, %d]", 0, 1);
                } else if (3 == nPrevChnCount) {
                    sprintf(chStream, "[%d, %d, %d]", 0, 1, 2);
                } else {
                    sprintf(chStream, "[0, 1, 2]");
                }
            } else {
                sprintf(chStream, "[0, 1, 2]");
            }
        ...
        string strCodecType = "[";
        for (AX_U8 i = 0; i < AX_WEB_MAX_PREV_SNS_NUM; i++) {
            AX_CHAR chCodecType[nPrevInfoMaxCharLen + 1] = {0};
            if (i < nSnsCnt) {
                AX_U8 nPrevChnCount = m_sMapPrevChn2UniChn[i].size();
                if (1 == nPrevChnCount) {
                    sprintf(chCodecType, "[%d]", std::get<1>(m_sMapPrevChn2UniChn[i][0]));
                } else if (2 == nPrevChnCount) {
                    sprintf(chCodecType, "[%d, %d]", std::get<1>(m_sMapPrevChn2UniChn[i][0]), std::get<1>(m_sMapPrevChn2UniChn[i][1]));
                } else if (3 == nPrevChnCount) {
                    sprintf(chCodecType, "[%d, %d, %d]", std::get<1>(m_sMapPrevChn2UniChn[i][0]),
                            std::get<1>(m_sMapPrevChn2UniChn[i][1]), std::get<1>(m_sMapPrevChn2UniChn[i][2]));
                } else {
                    sprintf(chCodecType, "[0, 0, 1]");
                }
            } else {
                sprintf(chCodecType, "[0, 0, 1]");
            }

            strCodecType += chCodecType;
            if (i < (AX_WEB_MAX_PREV_SNS_NUM - 1)) {
                strCodecType += ",";
            }
        }
        ...
        string strVideoFPS = "[";
        for (AX_U8 i = 0; i < AX_WEB_MAX_PREV_SNS_NUM; i++) {
            AX_CHAR chVideoFPS[nPrevInfoMaxCharLen + 1] = {0};
            if (i < nSnsCnt) {
                CWebOptionHelper::GetInstance()->GetVideoFramerateStr(i, chVideoFPS, nPrevInfoMaxCharLen);
            } else {
                sprintf(chVideoFPS, "[15, 15, 15]");
            }
            strVideoFPS += chVideoFPS;
            if (i < (AX_WEB_MAX_PREV_SNS_NUM - 1)) {
                strVideoFPS += ",";
            }
        }
        ...
        sprintf(szData,
                ...
                "%s:%s,%s:%s,%s:%d,%s:%d,%s:%s,%s:%s,%s:%s,%s:%s,%s:%s}",
                "sns_num", nSnsCnt, "sns_codec", strCodecType.c_str(), /* 0: VENC; 1: MJPEG */
                "sns_video_fps", strVideoFPS.c_str(),
                "stream_list", strStream.c_str(),
                "ai_enable", "false", "searchimg", "false", "detect_mode", "hvcfp", "osd_enable", "true", "auto_capture_enable",
                szAutoCapture.c_str(), "aenc_enable", szAudioCapture, "aenc_type", &aencType[0], "aenc_sample_rate", stEncodeAttr.eSampleRate,
                "aenc_bit_width", stEncodeAttr.eBitWidth, "trigger_enable", szTrigger.c_str(), "ezoom_enable", szEZoomEnable.c_str(), "ezoom", szEZoom.c_str(),
                "flash_enable", szFlash.c_str(), "snap_shot_enable", szSnapShotEnable.c_str());

        LOG_MM_I(WEB, "szData:%s, len:%d", szData, strlen(szData));

        AX_U16 nRespCode = RESPONSE_STATUS_FAILURE_CODE;
        if (0 != mprWriteJsonObj(mprReadJsonObj(pResponseBody, "data"), "info", mprParseJson(szData))) {
            LOG_MM_E(WEB, "mprWriteJsonObj failed");
            nRespCode = RESPONSE_STATUS_FAILURE_CODE;
        } else {
            httpSetContentType(conn, "application/json");
            httpWrite(conn->writeq, mprJsonToString(pResponseBody, MPR_JSON_QUOTES));

            //LOG_MM_I(WEB, "resp:%s", mprJsonToString(pResponseBody, MPR_JSON_QUOTES));
            nRespCode = RESPONSE_STATUS_OK_CODE;
        }

        httpSetStatus(conn, nRespCode);
        httpFinalize(conn);
    }
...
static AX_VOID WSPreviewAction(HttpConn* conn) {
    ...
    if (nSrcID >= APP_WEB_SHOW_SENSOR_COUNT()) { // Invalid id
...
static AX_VOID CameraAction(HttpConn* conn) {
    ...
    if (strcmp(conn->rx->method, "GET") == 0) {
        ...
        WEB_CAMERA_ATTR_T attr = CWebOptionHelper::GetInstance()->GetCamera(nSnsID);
        if (attr.bTriggerEnable) {
            mprWriteJsonObj(mprReadJsonObj(pResponseBody, "data"), "flash_attr_enable", mprParseJson("true"));
        } else {
            mprWriteJsonObj(mprReadJsonObj(pResponseBody, "data"), "flash_attr_enable", mprParseJson("false"));
        }

        auto jsonFlashAttr = CWebOptionHelper::GetInstance()->GetFlashAttrJson(nSnsID);
        if (jsonFlashAttr) {
            mprWriteJsonObj(mprReadJsonObj(pResponseBody, "data"), "flash_attr", jsonFlashAttr);
        }

        SENSOR_CONFIG_T tSnsCfg;
        if (!APP_SENSOR_CONFIG(nSnsID, tSnsCfg)) {
            LOG_M_E(WEB, "Failed to get sensor(%d) config.", nSnsID);
            nHttpStatusCode = RESPONSE_STATUS_FAILURE_CODE;
        } else {
            if (tSnsCfg.strPNFrameRate.empty()) {
                LOG_M_I(WEB, "strPNFrameRate is empty");
                ...
                mprWriteJsonObj(mprReadJsonObj(pResponseBody, "data"), "framerate_opts",
                            mprParseJson(tSnsCfg.strPNFrameRate.c_str()));
            }
        }
...
static AX_VOID AudioAction(HttpConn* conn) {
    ...
    if (strcmp(conn->rx->method, "GET") == 0) {
        ...
        const WEB_AUDIO_ATTR_T tAudioAttr = CWebOptionHelper::GetInstance()->GetAudio();
...
static AX_VOID VideoAction(HttpConn* conn) {
    ...
    if (strcmp(conn->rx->method, "GET") == 0) {
        auto root = ConstructResponseJsonObj(RESPONSE_STATUS_OK_CODE, 0);
        AX_U8 nSnsID = atoi(httpGetParam(conn, PARAM_KEY_PREVIEW_SNS_SRC, "0"));
...
static AX_VOID RecordAction(HttpConn* conn) {
    if (!IsAuthorized(conn, AX_TRUE)) {
        ResponseUnauthorized(conn);
        return;
    }
    ...
    if (strcmp(conn->rx->method, "GET") == 0) {
        ...
        auto jsonRecordAttr = CWebOptionHelper::GetInstance()->GetRecordJson(nSnsID);
        if (jsonRecordAttr) {
            mprWriteJsonObj(mprReadJsonObj(pResponseBody, "data"), "record_attr", jsonRecordAttr);
        }
        ...
        LOG_MM_C(WEB, "...");
        nHttpStatusCode = YieldProcessWebOpr(conn, E_REQ_TYPE_RECORD);
        ResponseStatusCode(conn, nHttpStatusCode);
    }
}
...
static AX_VOID Switch3ASyncRatioAction(HttpConn* conn) {
    LOG_MM_C(WEB, "...");
    if (!IsAuthorized(conn, AX_TRUE)) {
        ResponseUnauthorized(conn);
        return;
    ...
    if (!bActionProcessing) {
        ...
        nHttpStatusCode = YieldProcessWebOpr(conn, E_REQ_TYPE_SWITCH_3A_SYNCRATIO);
...
const HTTP_ACTION_INFO g_httpActionInfo[] = {{"/action/login", LoginAction},
                                             ...
                                             {"/action/setting/record", RecordAction},
...
CWebServer::CWebServer() : m_bServerStarted(AX_FALSE), m_pAppwebThread(nullptr) {
    memset(m_arrConnStatus, AX_FALSE, sizeof(m_arrConnStatus));
    memset(m_arrCaptureEnable, AX_TRUE, sizeof(m_arrCaptureEnable));
}
...
AX_VOID CWebServer::Destroy() {
    if (m_pAppwebThread) {
        mprSetExitStatus(-1);
        mprSetExitTimeout(MPR_EXIT_TIMEOUT);
        mprDestroy();
...
AX_BOOL CWebServer::RequestRingbuf(AX_U32 nUniChn, AX_U32 nElementBuffSize, AX_U32 nBuffCount, string strName) {
    ...
    LOG_MM_I(WEB, "New ring buffer for %s with size(%d) and count(%d)", strName.c_str(), nElementBuffSize, nBuffCount);
...
AX_VOID CWebServer::SendWSData(AX_VOID) {
    ...
    for (AX_S32 next = 0; (client = (HttpConn*)MprListGetNextItem(g_pClients, &next)) != 0;) {
        ...
        if (nSnsID == -1 || nSnsID >= AX_WEB_MAX_PREV_SNS_NUM) {
            LOG_MM_W(WEB, "connect %p nSnsID = %d is invalid", client, nSnsID);
            continue;
        }

        nUniChannel = GetChnFromWS(client);
        if (nUniChannel == -1 || nUniChannel >= MAX_WS_CONN_NUM) {
            // LOG_MM_W(WEB, "connect %p nUniChannel = %d is invalid", client, nUniChannel);
            continue;
        }
        ...
        CAXRingElementEx* pData = nullptr;
        if (nUniChannel == m_nCaptureChannel) {
            pData = pWebServer->GetCaptureData(nUniChannel);
        } else {
            pData = pWebServer->m_arrChannelData[nUniChannel].pRingBuffer->Get();
        }
        if (!pData) {
            continue;
        }
...
AX_VOID CWebServer::SendCaptureData(AX_U8 nSnsID, AX_U8 nUniChn, AX_VOID* data, AX_U64 nUserData, AX_U32 size, AX_U64 nPts /*= 0*/,
                                    AX_BOOL bIFrame /*= AX_TRUE*/, JPEG_DATA_INFO_T* pJpegInfo /*= nullptr*/) {
    if (!IsCaptrueEnable(nSnsID)) {
        return;
    }
    ...
    AX_CHAR cFileName[128] = {0};
    if (nullptr != pJpegInfo) {
        AX_CHAR szJsonData[256] = {0};
        /* construct json data info */
        if (JPE_TYPE_FLASH == eType) {
            if (0 != nUserData) {
                TRIGGER_INFO_PTR ptr = (TRIGGER_INFO_PTR)nUserData;
                sprintf(szJsonData, "{\"snsId\": %d, \"type\": %d, \"attribute\": {\"src\": %d, \"width\": %d, \"height\": %d, \"pts\": %lld, \"seqnum\": %lld, \"userdata\": %lld}}", nSnsID, eType,
                    pJpegInfo->tCaptureInfo.tHeaderInfo.nSnsSrc, pJpegInfo->tCaptureInfo.tHeaderInfo.nWidth,
                    pJpegInfo->tCaptureInfo.tHeaderInfo.nHeight, ptr->nPts, ptr->nSeqNum, ptr->nUserData);
                sprintf(cFileName, "trigger_pic_%lld_%lld_%lld.jpg", ptr->nUserData, ptr->nSeqNum, ptr->nPts);
            }else {
                sprintf(szJsonData, "{\"snsId\": %d, \"type\": %d, \"attribute\": {\"src\": %d, \"width\": %d, \"height\": %d, \"pts\": %d, \"seqnum\": %d, \"userdata\": %d}}", nSnsID, eType,
                    pJpegInfo->tCaptureInfo.tHeaderInfo.nSnsSrc, pJpegInfo->tCaptureInfo.tHeaderInfo.nWidth,
                    pJpegInfo->tCaptureInfo.tHeaderInfo.nHeight, 255, 255, 255);
                sprintf(cFileName, "trigger_pic_%d_%d_%d.jpg", 255, 255, 255);
            }
        } else {
        ...
        CAXRingElementEx _ele((AX_U8*)data, size, nPts, bIFrame, (AX_U8*)&tJpegHead, tJpegHead.nTotalLen);
        m_arrChannelData[nUniChn].pRingBuffer->Put(_ele);

        if ((JPE_TYPE_FLASH == eType) && m_bEnableStoreTriggerPic) {
            const AX_U16 nPathNameLen = 512;
            AX_CHAR cPathName[nPathNameLen] = {0};
            if (!m_strTriggerPicPath.empty()) {
                if ((m_strTriggerPicPath.size() + strlen(cFileName)) >= nPathNameLen) {
                    LOG_MM_E(WEB, "pathname is out of range");
                    return;
                } else {
                    sprintf(cPathName, "%s%s", m_strTriggerPicPath.c_str(),cFileName);
                }
            } else {
                sprintf(cPathName, "./%s", cFileName);
            }
            std::ofstream ofTriggerPic;
            ofTriggerPic.open(cPathName, std::ofstream::binary);
            if (ofTriggerPic.is_open()) {
                ofTriggerPic.write((const char *)data, size);
                ofTriggerPic.close();
            } else {
                LOG_MM_E(WEB, "open file %s failed", cPathName);
            }
        }
    } else {
        CAXRingElementEx ele((AX_U8*)data, size, nPts, bIFrame);
        m_arrChannelData[nUniChn].pRingBuffer->Put(ele);
    }
}
...
static AX_U8 s_arrSnsPrevInnerIndex[AX_WEB_MAX_PREV_SNS_NUM] = {0};
...
AX_VOID CWebServer::RegistUniCaptureChn(AX_U8 nSnsId, AX_S8 nCaptureChn, JPEG_TYPE_E eType) {
    if (-1 == m_nCaptureChannel) {
        /* Applies the first setting value */
        m_nCaptureChannel = nCaptureChn;
    }
    m_mapJencChnType[nCaptureChn] = eType;
    m_mapCaptureSnsChn[nSnsId] = nCaptureChn;
}
...
AX_BOOL CWebServer::IsCaptrueEnable(AX_U8 nSnsID) const {
    if (!m_bServerStarted) {
        return AX_FALSE;
    }

    if (nSnsID < sizeof(m_arrCaptureEnable)) {
        return m_arrCaptureEnable[nSnsID];
    } else {
        return AX_FALSE;
    }
}
...
CAXRingElementEx* CWebServer::GetCaptureData(AX_S32 &nCaptureChn) {
    AX_U8 nSnsCount = m_mapCaptureSnsChn.size();
    if (m_nCaptureIndex >= nSnsCount) {
        m_nCaptureIndex = 0;
        return nullptr;
    }

    CAXRingElementEx* pData = nullptr;
    for (AX_U8 i = 0; i < nSnsCount; i++) {
        auto it = m_mapCaptureSnsChn.begin();
        std::advance(it, m_nCaptureIndex);

        AX_U8 nChn = it->second;
        pData = m_arrChannelData[nChn].pRingBuffer->Get();
        m_nCaptureIndex = (m_nCaptureIndex + 1) % nSnsCount;
        if (pData || m_arrCaptureEnable[it->first]) {
            nCaptureChn = nChn;
            break;
        }
    }
