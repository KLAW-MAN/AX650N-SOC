...
public:
    AX_BOOL OnRecvData(OBS_TARGET_TYPE_E eTarget, AX_U32 nGrp, AX_U32 nChn, AX_VOID* pData) override {
        ...
        if (E_OBS_TARGET_TYPE_VENC == eTarget) {
            ...
            m_pSink->SendCaptureData(nGrp, nChn, (AX_U8*)pVencPack->pu8Addr, pVencPack->u64UserData, pVencPack->u32Len, 0, AX_TRUE, &tJpegInfo);
    ...
    AX_BOOL OnRegisterObserver(OBS_TARGET_TYPE_E eTarget, AX_U32 nGrp, AX_U32 nChn, OBS_TRANS_ATTR_PTR pParams) override {
        ...
        if (E_OBS_TARGET_TYPE_VENC == eTarget) {
            ...
            AX_U32 nBufCount = COptionHelper::GetInstance()->GetVencOutBuffCount();
            if (0 == nBufCount) {
                nBufCount = AX_WEB_VENC_RING_BUFF_COUNT;
            }
            sprintf(szName, "VENC_CH%d", nChn);

            return m_pSink->RequestRingbuf(nChn, nBuffSize, nBufCount, szName);
        } else if (E_OBS_TARGET_TYPE_JENC == eTarget) {
            AX_U32 nWidth = pParams->nWidth;
            AX_U32 nHeight = pParams->nHeight;
            AX_F32 fRatio = COptionHelper::GetInstance()->GetJencOutBuffRatio();
            AX_U32 nBuffSize = nWidth * nHeight * 3 / 2 * fRatio;
            AX_U32 nBufCount = COptionHelper::GetInstance()->GetJencOutBuffCount();
            if (0 == nBufCount) {
                nBufCount = AX_WEB_JENC_RING_BUFF_COUNT;
            }
            ...
            AX_U32 nBufCount = COptionHelper::GetInstance()->GetJencOutBuffCount();
            if (0 == nBufCount) {
                nBufCount = AX_WEB_MJENC_RING_BUFF_COUNT;
            }
            nBuffSize += sizeof(JpegHead);
            ...
            AX_U32 nBufCount = COptionHelper::GetInstance()->GetAencOutFrmCount();
            if (0 == nBufCount) {
                nBufCount = AX_WEB_AENC_RING_BUFF_COUNT;
            }
            sprintf(szName, "AENC_CH%d", nChn);
            return m_pSink->RequestRingbuf(m_pSink->GetAencChannel(), nBuffSize, nBufCount, szName);
        } else if (E_OBS_TARGET_TYPE_EVENT == eTarget) {
            // do nothing
        }
