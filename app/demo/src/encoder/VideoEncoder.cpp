...
AX_BOOL CVideoEncoder::Start(STAGE_START_PARAM_PTR pStartParams) {
    ...
    /* set VENC_STREAM_CACHED 0x2 for async AX_VENC_DestroyChn/AX_VENC_GetStream/AX_VENC_ReleaseStream */
    m_tVencChnAttr.stVencAttr.flag |= 0x2;
    ...
    if (m_gdrNum > 0) {
        ...
        param.u32RefreshNum = m_gdrNum;
        AX_S32 ret = AX_VENC_SetIntraRefresh(GetChannel(), &param);
        if (0 != ret) {
            LOG_M_E(VENC, "[%d] AX_VENC_SetIntraRefresh(%d) failed, ret=0x%x", GetChannel(), m_gdrNum, ret);
            return AX_FALSE;
        } else {
            LOG_MM_C(VENC, "veChn %d GDR actived, num: %d", GetChannel(), m_gdrNum);
        }
    }
...
AX_BOOL CVideoEncoder::NotifyAll(AX_U32 nChannel, AX_VOID* pStream) {
    if (nullptr == pStream) {
        return AX_TRUE;
    }
    ...
    for (vector<IObserver*>::iterator it = m_vecObserver.begin(); it != m_vecObserver.end(); it++) {
        if (!(*it)->OnRecvData(E_OBS_TARGET_TYPE_VENC, m_tVideoConfig.nPipeSrc, nChannel, pStream)) {
            bRet = AX_FALSE;
        }
    }
...
AX_BOOL CVideoEncoder::OnDataChange(AX_U32 nChannel, OBS_CHANGE_DATA_TYPE_E eDataType, AX_VOID* pData) {
    if (nullptr == pData) {
        return AX_TRUE;
    }
    ...
    AX_BOOL bRet = AX_TRUE;
    for (vector<IObserver*>::iterator it = m_vecObserver.begin(); it != m_vecObserver.end(); it++) {
        if (!(*it)->OnDataChange(E_OBS_TARGET_TYPE_VENC, m_tVideoConfig.nPipeSrc, nChannel, eDataType, pData)) {
            bRet = AX_FALSE;
        }
    }
...
AX_BOOL CVideoEncoder::InitParams() {
    ...
    LOG_M_I(VENC, "[%d] u32MaxPicWidth: %d, u32MaxPicHeight: %d",
    ...
    m_tVencChnAttr.stVencAttr.u32PicWidthSrc = m_tCurResolution.nWidth;
    m_tVencChnAttr.stVencAttr.u32PicHeightSrc = m_tCurResolution.nHeight;
...
AX_BOOL CVideoEncoder::UpdateChnResolution(AX_U32 nNewWidth, AX_U32 nNewHeight) {
    m_tCurResolution.nWidth = nNewWidth;
    m_tCurResolution.nHeight = nNewHeight;
    ...
    AX_S32 s32Ret = AX_VENC_SetChnAttr(GetChannel(), &tAttr);
    if (AX_SUCCESS != s32Ret) {
        LOG_M_E(VENC, " [%d]AX_VENC_SetChnAttr fail(0x%x) for Reset res: (w: %d, h: %d) max(w:%d, h:%d)",
                         GetChannel(),
                         s32Ret,
                         tAttr.stVencAttr.u32PicWidthSrc,
                         tAttr.stVencAttr.u32PicHeightSrc,
                         tAttr.stVencAttr.u32MaxPicWidth,
                         tAttr.stVencAttr.u32MaxPicHeight);
    }

    if (AX_SUCCESS == s32Ret) {
        OBS_TRANS_RESOLUTION_T stObsTransResolution;
        stObsTransResolution.nWidth = tAttr.stVencAttr.u32PicWidthSrc;
        stObsTransResolution.nHeight = tAttr.stVencAttr.u32PicHeightSrc;
        OnDataChange(GetChannel(), E_OBS_CHANGE_DATA_TYPE_RESOLUTION, &stObsTransResolution);
    }
    ...
    return (AX_SUCCESS == s32Ret) ? AX_TRUE : AX_FALSE;
}
...
AX_BOOL CVideoEncoder::UpdateChnBitrate(AX_U32 nBitrate) {
    ...
    if (nRet == AX_SUCCESS) {
        OnDataChange(GetChannel(), E_OBS_CHANGE_DATA_TYPE_BITRATE, &nBitrate);
    }
...
AX_BOOL CVideoEncoder::UpdateRcInfo(RC_INFO_T& tRcInfo) {
    ...
    if (-1 != tRcParam.s32FirstFrameStartQp) {
        if ((AX_S32)tRcInfo.nMinIQp > tRcParam.s32FirstFrameStartQp) {
            tRcParam.s32FirstFrameStartQp = tRcInfo.nMinIQp;
        } else if ((AX_S32)tRcInfo.nMaxIQp < tRcParam.s32FirstFrameStartQp) {
            tRcParam.s32FirstFrameStartQp = tRcInfo.nMaxIQp;
        }
    }
