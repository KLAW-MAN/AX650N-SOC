...
AX_BOOL CJpegEncoder::InitParams() {
    ...
    m_tJencChnAttr.stVencAttr.u32MaxPicWidth = m_tJpegConfig.nMaxWidth;
    if (0 == m_tJpegConfig.nMaxWidth) {
        m_tJencChnAttr.stVencAttr.u32MaxPicWidth = ALIGN_UP(AX_MAX(m_tJpegConfig.nWidth, m_tJpegConfig.nHeight), AX_ENCODER_FBC_STRIDE_ALIGN_VAL);
    }
    m_tJencChnAttr.stVencAttr.u32MaxPicHeight = m_tJpegConfig.nMaxHeight;
    if (0 == m_tJpegConfig.nMaxHeight) {
        m_tJencChnAttr.stVencAttr.u32MaxPicHeight = m_tJencChnAttr.stVencAttr.u32MaxPicWidth; /* Make max height same as width for the case of rotation 90  */
    }
    LOG_M_I(JENC, "[%d] u32MaxPicWidth: %d, u32MaxPicHeight: %d",
                   GetChannel(), m_tJencChnAttr.stVencAttr.u32MaxPicWidth,
                   m_tJencChnAttr.stVencAttr.u32MaxPicHeight);

    if (!m_tCurResolution.nWidth && !m_tCurResolution.nHeight) {
        m_tCurResolution.nWidth = m_tJpegConfig.nWidth;
        m_tCurResolution.nHeight = m_tJpegConfig.nHeight;
    }

    m_tJencChnAttr.stVencAttr.u32PicWidthSrc = m_tCurResolution.nWidth;
    m_tJencChnAttr.stVencAttr.u32PicHeightSrc = m_tCurResolution.nHeight;
...
AX_BOOL CJpegEncoder::UpdateChnResolution(AX_U32 nNewWidth, AX_U32 nNewHeight) {
    ...
    if (AX_SUCCESS != s32Ret) {
        LOG_M_E(JENC, " [%d]AX_VENC_SetChnAttr fail(0x%x) for Reset res: (w: %d, h: %d) max(w:%d, h:%d)",
                         GetChannel(),
                         s32Ret,
