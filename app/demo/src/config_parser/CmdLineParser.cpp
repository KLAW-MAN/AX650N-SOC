...
AX_S32 CCmdLineParser::Parse(int argc, const char* const argv[]) {
    ...
    pParser->add<AX_S32>(AX_IPC_CMD_KEY_COREDUMP, 'q', "0:disable coredump path config others:enable...", false, 1);
...
string CCmdLineParser::ScenarioEnum2Str(AX_U8 nScenario) {
    ...
    switch ((AX_IPC_SCENARIO_E)nScenario) {
        ...
        case E_PPL_SCENRIO_6:
        case E_PPL_SCENRIO_7:
        case E_PPL_SCENRIO_8:
        case E_PPL_SCENRIO_9:
        case E_PPL_SCENRIO_10:
        case E_PPL_SCENRIO_11:
...
AX_S32 CCmdLineParser::GetPPLIndex() {
    AX_S32 nIndex = -1;
    if (!GetIntValue(AX_IPC_CMD_KEY_PPL, nIndex)) {
        nIndex = -1;
    }
    return nIndex;
}

AX_S32 CCmdLineParser::GetScenario() {
    AX_S32 nScenario = -1;
    if (!GetIntValue(AX_IPC_CMD_KEY_SCENARIO, nScenario)) {
        nScenario = -1;
    }
    return nScenario;
}
