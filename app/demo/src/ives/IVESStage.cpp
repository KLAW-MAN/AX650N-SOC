...
AX_BOOL CIVESStage::ProcessFrame(CAXFrame* pAxFrame) {
    ...
    do {
        ...
        LOG_MM_D(IVES, "nSnsID: %d, Seq: %lld, w:%d, h:%d, size:%u, PhyAddr:%lld", nSnsID, pAxFrame->stFrame.stVFrame.stVFrame.u64SeqNum,
...
AX_BOOL CIVESStage::UpdateRotation(AX_U8 nRotation) {
    ...
    if (m_spODInstance) {
        m_spODInstance->Cleanup();
        m_spODInstance->Startup(nSnsID, (AX_U32)m_stAttr.fSrcFramerate, nFrmW, nFrmH);
        const AX_U32 nAreaW = nFrmW / 2;
        const AX_U32 nAreaH = nFrmH / 2;
