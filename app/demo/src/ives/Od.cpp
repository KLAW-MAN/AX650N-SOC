...
AX_BOOL COD::Startup(AX_S32 nSnsID, AX_U32 nFrameRate, AX_U32 nWidth, AX_U32 nHeight) {
    ...
    if (!APP_SENSOR_CONFIG(nSnsID, tSensorCfg)) {
        LOG_M_E(OD, "Failed to get sensor(%d) config.", nSnsID);
        return AX_FALSE;
    }

    m_nPipeID = tSensorCfg.arrPipeAttr[0].nPipeID;
...
const std::vector<AX_U8> &COD::ProcessFrame(const AX_U32 nSnsID, const CAXFrame *pFrame) {
    ...
    if (0 == AX_ISP_IQ_GetAeStatus(m_nPipeID, &tAE)) {
        stImg.u32Lux = tAE.tAlgStatus.nLux;
    } else {
        stImg.u32Lux = 0;
        LOG_M_E(OD, "AX_ISP_IQ_GetAeStatus(sns: %d)(pipe: %d) fail", nSnsID, m_nPipeID);
    }
