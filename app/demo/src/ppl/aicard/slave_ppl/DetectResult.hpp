...
public:
    ...
    AX_BOOL Pop(AX_S32 nGrp, DETECT_RESULT_T& fhvp) {
        std::lock_guard<std::mutex> lck(m_mtx);
        if (m_mapRlts.end() == m_mapRlts.find(nGrp) || 0 == m_mapRlts[nGrp].nCount) {
            return AX_FALSE;
        }

        fhvp = m_mapRlts[nGrp];
        m_mapRlts[nGrp].nCount = 0;
        memset(&m_mapRlts[nGrp].item, 0, sizeof(DETECT_RESULT_ITEM_T) * MAX_DETECT_RESULT_COUNT);
        return AX_TRUE;
    }
