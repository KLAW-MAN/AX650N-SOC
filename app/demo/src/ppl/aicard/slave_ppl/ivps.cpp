...
AX_BOOL CIVPS::Start(AX_VOID) {
    ...
    if (0 != ret) {
        ...
        LOG_MM_E(TAG, "[%d] AX_IVPS_SetPipelineAttr OK", m_ivGrp);
    }
    ...
    for (AX_U32 i = 0; i < m_stAttr.nChnNum; ++i) {
        ...
        if (0 != ret) {
            ...
            LOG_MM_E(TAG, "[%d] AX_IVPS_EnableChn OK", m_ivGrp);
        }
    }
    ...
    if (0 != ret) {
        ...
        LOG_MM_E(TAG, "[%d] AX_IVPS_StartGrp OK", m_ivGrp);
    }
    ...
    for (auto&& m : m_dispatchers) {
        if (m) {
            if (!m->Start()) {
                ...
                LOG_MM_E(TAG, "[%d] Start dispatcher OK", m_ivGrp);
            }
        }
    }
