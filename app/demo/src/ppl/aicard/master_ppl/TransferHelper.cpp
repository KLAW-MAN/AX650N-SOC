...
AX_BOOL CTransferHelper::SendAiAttr(AI_CARD_AI_SWITCH_ATTR_T& tAiAttr) {
    ...
    if (CPcieAdapter::GetInstance()->SendCtrlCommand(tAiAttr, m_tIniAttr.nSendTimeout)) {
        ...
        NotifyStateObservers((AX_U32)STATE::STATE_ENABLE);
    }
...
AX_VOID CTransferHelper::RecvDetectResultThread(AX_VOID* pArg) {
    ...
    while (m_vecThreadRecvDetRet[nIndex]->IsRunning()) {
        ...
        CDetectResult::GetInstance()->Push(nVideoIndex, *pResult);
...
AX_VOID CTransferHelper::RecvCtrlCmdResultThread(AX_VOID* pArg) {
    ...
    while (m_threadRecvCmdRet.IsRunning()) {
        ...
        NotifyStateObservers((AX_U32)STATE::STATE_DISABLE);
    }
...
AX_BOOL CTransferHelper::RegisterAiSwitchStateChangeObs(CAiSwitchStateObs* pObs) {
    for (auto &m : m_vecStateChangeObs) {
        if (m == pObs) {
            LOG_MM_W(TAG, "AiSwitchObserver already registered.");
            return AX_FALSE;
        }
    }

    m_vecStateChangeObs.push_back(pObs);

    return AX_TRUE;
}

AX_BOOL CTransferHelper::UnregisterAiSwitchStateChangeObs(CAiSwitchStateObs* pObs) {
    std::vector<CAiSwitchStateObs*>::iterator it;
    for (it = m_vecStateChangeObs.begin(); it != m_vecStateChangeObs.end(); it++) {
        if (pObs == *it) {
            m_vecStateChangeObs.erase(it);
            return AX_TRUE;
        }
    }
...
AX_VOID CTransferHelper::NotifyStateObservers(AX_U32 nState) {
    for (auto &m : m_vecStateChangeObs) {
        m->OnStateChanged(nState);
    }
}