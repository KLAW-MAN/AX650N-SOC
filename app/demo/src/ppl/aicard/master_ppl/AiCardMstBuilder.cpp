...
AX_BOOL CAiCardMstBuilder::Init(AX_VOID) {
    ...
    if (!InitDispatcher(dispVoConfig.strBmpPath)) {
        ...
        m_arrAiSwitchStateObs.resize(m_nDecodeGrpCount);
        for (AX_U32 i = 0; i < m_nDecodeGrpCount; ++i) {
            m_arrAiSwitchStateObs[i] = CStateObserverMaker::CreateStateObserver<CAiSwitchStateObs>(m_arrDispatcher[i].get());
        }
    }
    ...
    if (!InitTransHelper()) {
        ...
        for (AX_U32 i = 0; i < m_nDecodeGrpCount; ++i) {
            m_transHelper->RegisterAiSwitchStateChangeObs((CAiSwitchStateObs*)m_arrAiSwitchStateObs[i].get());
        }
    }
