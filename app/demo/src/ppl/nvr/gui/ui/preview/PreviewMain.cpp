...
void PreviewMain::showEvent(QShowEvent *event) {
    do {
        ...
        if (m_bPlaybackInterrupting) {
            m_hPlaybackStopEvent.ResetEvent();
            emit signal_force_playback_terminated(); /* Force interrupted playback to continue and terminated */
        }
...
void PreviewMain::OnPlaybackInterrupting() {
    m_bPlaybackInterrupting = true;
}
...
void PreviewMain::update_display(const ax_nvr_channel_vector &vecViewChn, const vector<AX_NVR_RECT_T> &vecVoRect,
    ...
    std::thread thread([this, interface, vecViewChn, vecVoRect, enChangeType, res]() mutable {
        ...
        LOG_MM_C(TAG, "AppPreviewUpd thread action %d +++", enChangeType);
        res.enResult = AX_NVR_PREVIEW_RES_TYPE::PREVIEW_ERR;
        ...
        do {
            ...
            // Update VO attr
            if (!pPrimaryDispCtrl->UpdateView(vecViewChn, vecVoRect, AX_NVR_VIEW_TYPE::PREVIEW, enChangeType, AX_FALSE)) {
            ...
            if (!bRet) {
                LOG_MM_C(TAG, "AppPreviewUpd thread action %d failed", enChangeType);
                break;
            }
