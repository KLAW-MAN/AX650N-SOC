    ...
    , m_pSettingsMain(new SettingsMain)
{
    ...
    if (type == 1) {
        ...
        AX_NVR_DISPVO_CONFIG_T tConfig = CNVRConfigParser::GetInstance()->GetPrimaryDispConfig();
        if (-1 != tConfig.nFBMouse) {
            /* Open mouse framebuffer device */
            AX_CHAR fbPath[32];
            snprintf(fbPath, sizeof(fbPath), "/dev/fb%d", tConfig.nFBMouse);
            AX_S32 hFbMouse = open(fbPath, O_RDWR);
            if (hFbMouse < 0) {
                LOG_MM_E(TAG, "open %s failed, err:%s", fbPath, strerror(errno));
            } else {
                m_nFbMouseIndex = tConfig.nFBMouse;
                m_hFbMouse = hFbMouse;
            }
        }
        ...
        connect(m_pPreviewMain, &PreviewMain::signal_force_playback_terminated, m_pPlaybackMain, &PlaybackMain::OnForceTerminated);
        ...
        connect(m_pPlaybackMain, &PlaybackMain::signal_status_interrupting, m_pPreviewMain, &PreviewMain::OnPlaybackInterrupting);

        // settings
        connect(m_pSettingsLeftToolbar, &SettingsLeftToolbar::signal_change_page, m_pSettingsMain, &SettingsMain::OnChangePage);
        connect(m_pSettingsMain, &SettingsMain::signal_res_changed, this, &NVRMainWindow::OnVoIfSyncChanged);
...
void NVRMainWindow::OnVoIfSyncChanged(AX_S32 nSelVoIfSyncIndex) {
    AX_U32 nWidth {1920};
    AX_U32 nHeight {1080};
    switch ((AX_NVR_VO_IF_SYNC)nSelVoIfSyncIndex) {
        case AX_NVR_VO_IF_SYNC::VO_IF_SYNC_MAX: {
            ...
            if (8/* 1920x1080P60 */ == tPrimaryCfg.nHDMI) {
                nWidth = 1920;
                nHeight = 1080;
            } else {
                /* 3840x2160P30, 3840x2160P60 */
                nWidth = 1920;
                nHeight = 1080;
            }
        ...
        default: {
            LOG_MM_W(TAG, "Unknown resolution index: %d", nSelVoIfSyncIndex);
            return;
        }
    }

    /* setGeometry() is not valid if widget contains layout components */
    // this->setGeometry(0, 0, nWidth, nHeight);
    this->setFixedSize(nWidth, nHeight);

    CAXNVRFramework::GetInstance()->OnVoIfSyncChanged((AX_NVR_VO_IF_SYNC)nSelVoIfSyncIndex);
}
...
AX_VOID NVRMainWindow::DrawFBMouse(QPoint& pos) {
    if (-1 == m_hFbMouse) {
        return;
    }
    ...
    stPos.u16X = pos.x();
    stPos.u16Y = pos.y();
    AX_S32 s32Ret = ioctl(m_hFbMouse, AX_FBIOPUT_CURSOR_POS, &stPos);
    if (s32Ret < 0) {
        LOG_MM_E("TAG", "put cursor pos fb%d failed", m_nFbMouseIndex);
    }
}