...
signals:
    ...
    void signal_status_interrupting();
...
private:
    ...
    /* Playback would be interrupted when switching to setting page, and continues next time back to playback page.
       And playback would be terminated immediately when enterring preview page */
    AX_BOOL m_bInterrupted {AX_FALSE};
    AX_BOOL m_bPaused {AX_FALSE};
    AX_BOOL m_bPlaying {AX_FALSE};
    PLAYBACK_ACTION_TYPE m_ePlayStatus {PLAYBACK_ACTION_TYPE::STOP};
    AX_S32 m_nPlayStatus {0}; // 0: STOP; 1: Forward play; 2: Backword play
private:
