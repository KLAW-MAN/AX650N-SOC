...
void PlaybackMain::resizeEvent(QResizeEvent *event) {
    ...
    m_pPlayBackTimeLine->updateGeometry();
...
void PlaybackMain::showEvent(QShowEvent *event) {
    do {
        ...
        if (m_bInterrupted) {
            enChangeType = AX_NVR_VIEW_CHANGE_TYPE::CONTINUE;
        }
...
void PlaybackMain::hideEvent(QHideEvent *event) {
    do {
        ...
        emit signal_status_interrupting();
...
void PlaybackMain::OnChangeSplitVideo(SPLIT_TYPE enSplitType) {
    do {
        ...
        if (m_watcherPreview.isRunning()) {
            LOG_MM_W(TAG, "Last operation still active, wait for finished...");
...
void PlaybackMain::OnChangeMainSub1(void) {
    do {
        ...
        m_pScaleLabelSeled->setFocus(Qt::OtherFocusReason);
        ...
        std::thread thread([this, interface, vecViewChn, enActionType, nYYYYMMDD, nHHMMSS, res]() mutable {
            LOG_MM_C(TAG, "Change MainSub thread Action +++");
...
void PlaybackMain::OnDoPlaybackAction(PLAYBACK_ACTION_TYPE enActionType) {
    ...
    do {
        ...
        if (PLAYBACK_ACTION_TYPE::PLAY == enActionType || PLAYBACK_ACTION_TYPE::REVERSE == enActionType) {
            ...
            if (vecViewChnSelected.size() == 0) {
                LOG_MM_W(TAG, "Please select devices first and try again.");
                break;
            }

            if (m_vecViewChnSelected.size() > 0) {
                if (m_ePlayStatus == enActionType && vecViewChnSelected == m_vecViewChnSelected) {
                    res.enResult = AX_NVR_PREVIEW_RES_TYPE::PREVIEW_OK;
                    LOG_MM_W(TAG, "%s playback has already started, operation ignored.", PLAYBACK_ACTION_TYPE::PLAY == m_ePlayStatus ? "Forward" : "Backward");
                    break;
                }

                if (m_ePlayStatus != PLAYBACK_ACTION_TYPE::STOP) {
                    AX_BOOL bNeedStopFirst = AX_FALSE;
                    for (auto &m : vecViewChnPlayed) {
                        if (std::get<1>(m) != -1) {
                            bNeedStopFirst = AX_TRUE;
                            break;
                        }
                    }

                    if (bNeedStopFirst) {
                        LOG_MM_W(TAG, "Playback is running, STOP is called in advance.");
                        pPlaybak->Stop(vecViewChnPlayed);

                        if (m_ePlayStatus != enActionType && vecViewChnSelected == m_vecViewChnSelected) {
                            nYYYYMMDD = 0; // Clear time info and it will be filled with current displayed time within channel component
                            nHHMMSS = 0;
                            nTick = 0;
                        }

                        m_ePlayStatus = PLAYBACK_ACTION_TYPE::STOP;
                        m_bPaused = AX_FALSE;
                    }
                }
            }
        ...
        } else if (enActionType == PLAYBACK_ACTION_TYPE::REPEAT) {
            break;
        }
        ...
        std::thread thread([this, interface, vecViewChn, enActionType, nYYYYMMDD, nHHMMSS, nTick, res]() mutable {
            pthread_setname_np(pthread_self(), "AppPBAction");
            LOG_MM_C(TAG, "AppPBAction thread action %d +++", enActionType);
            ...
            do {
                ...
                switch (enActionType) {
                    case PLAYBACK_ACTION_TYPE::PLAY:
                        bRet = pPlaybak->Start(vecViewChn, nYYYYMMDD, nHHMMSS, nTick, AX_FALSE, m_pPlayBackCtrl->IsRepeatPlayback());
                        m_ePlayStatus = PLAYBACK_ACTION_TYPE::PLAY;
                        break;
                    case PLAYBACK_ACTION_TYPE::REVERSE:
                        if (nHHMMSS == 0) {
                            /* Adapter for testsuite to automatically start without timeline setting */
                            nHHMMSS = 235959;
                        }

                        bRet = pPlaybak->Start(vecViewChn, nYYYYMMDD, nHHMMSS, nTick, AX_TRUE, m_pPlayBackCtrl->IsRepeatPlayback());
                        m_ePlayStatus = PLAYBACK_ACTION_TYPE::REVERSE;
                        break;
                    ...
                    case PLAYBACK_ACTION_TYPE::PAUSE:
                        ...
                        if (bRet) {
                            m_bPaused = m_bPaused ? AX_FALSE : AX_TRUE;
                        }
                        break;
                    case PLAYBACK_ACTION_TYPE::SLOW_SPEED: {
                        ...
                        bRet = pPlaybak->SetSpeed(vecViewChn, m_fCurrentSpeed, AX_TRUE);
                        break;
                    }
                ...
                m_pPlayBackCtrl->EnableRepeatBtn(m_ePlayStatus != PLAYBACK_ACTION_TYPE::STOP ? false : true);

                if (!bRet) {
                    LOG_MM_C(TAG, "AppPBAction thread action %d failed", enActionType);
                    break;
                }
                res.enResult = AX_NVR_PREVIEW_RES_TYPE::PREVIEW_OK;
            } while (0);
...
void PlaybackMain::OnForceTerminated() {
    AX_NVR_ACTION_RES_T res;
    QFutureInterface<AX_NVR_ACTION_RES_T> interface;
    interface.reportStarted();
    m_watcherPreview.setFuture(interface.future());
    ...
    std::thread thread([this, interface, res]() mutable {
        pthread_setname_np(pthread_self(), "AppPBForceInterr");
        ...
        do {
            ...
            bRet = pPlaybak->ContinueVo();
            if (!bRet) {
                LOG_MM_E(TAG, "OnForceTerminated: Forced to continue vo link failed.");
            } else {
                LOG_MM_W(TAG, "OnForceTerminated: Forced to continue vo link successfully.");
            }

            bRet = pPlaybak->StopAll();
            if (!bRet) {
                LOG_MM_E(TAG, "OnForceTerminated: Forced to stop playback failed.");
            } else {
                LOG_MM_W(TAG, "OnForceTerminated: Forced to stop playback successfully.");
            }
...
void PlaybackMain::update_display(const ax_nvr_channel_vector &vecViewChn,
    ...
    std::thread thread([this, interface, vecViewChn, vecVoRect, enChangeType, res]() mutable {
        ...
        LOG_MM_C(TAG, "AppPBUpdDisp thread action %d +++", enChangeType);
        res.enResult = AX_NVR_PREVIEW_RES_TYPE::PREVIEW_ERR;
        do {
            AX_BOOL bRet = AX_FALSE;
            CAXNVRDisplayCtrl *pPrimaryDispCtrl = CAXNVRFramework::GetInstance()->PrimaryDispCtrl();
            ...
            case AX_NVR_VIEW_CHANGE_TYPE::HIDE:
                m_bInterrupted = AX_TRUE;
                res.enPlaybackActionType = PLAYBACK_ACTION_TYPE::STOP;
                pPlaybak->PreStopVo();
                // 4 is an empirical value that in none blocking mode(after unlinked ivps->vo),
                // datastream_play sending frames in almost 100FPS, which means 4-times speed for a 25FPS video
                pPlaybak->SetSpeed(vecViewChn, 4);
                break;
            case AX_NVR_VIEW_CHANGE_TYPE::CONTINUE:
                pPlaybak->ContinueVo();
                pPlaybak->SetSpeed(vecViewChn, m_fCurrentSpeed);
                m_bInterrupted = AX_FALSE;
                break;
            ...
            // Update VO attr
            if (!pPrimaryDispCtrl->UpdateView(vecViewChn, vecVoRect, AX_NVR_VIEW_TYPE::PLAYBACK, enChangeType, m_bPaused)) {
            ...
            case AX_NVR_VIEW_CHANGE_TYPE::MIN:
                ...
                break;
            case AX_NVR_VIEW_CHANGE_TYPE::HIDE:
                res.enPlaybackActionType = PLAYBACK_ACTION_TYPE::HIDE;
                bRet = AX_TRUE;
                break;
            case AX_NVR_VIEW_CHANGE_TYPE::CONTINUE:
                bRet = pPlaybak->Update(vecViewChn);
                if (m_bPaused) {
                    // Restore channel state to PAUSED because this state would be clear within DISP resolution changing process
                    pPlaybak->RestorePauseState(vecViewChn);
                }
                break;
            case AX_NVR_VIEW_CHANGE_TYPE::UPDATE:
            case AX_NVR_VIEW_CHANGE_TYPE::SHOW:
            ...
            if (!bRet) {
                LOG_MM_E(TAG, "AppPBUpdDisp thread action %d failed", enChangeType);
                break;
            }
