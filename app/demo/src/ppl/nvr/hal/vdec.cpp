...
AX_BOOL CVDEC::SendStream(CONST AX_U8* pStream, AX_U32 nLen, AX_U64 u64PTS, AX_U64 nPrivData /* = 0 */,
    ...
    if (0 != m_nLastSendCode) {
        if ((AX_ERR_VDEC_BUF_FULL == m_nLastSendCode) || (AX_ERR_VDEC_QUEUE_FULL == m_nLastSendCode)) {
            LOG_MM_W(TAG, "vdGrp %d input buffer is full, abandon %d bytes, pts %lld", m_vdGrp, nLen, u64PTS);
        } else if (AX_ERR_VDEC_NOT_PERM == m_nLastSendCode) {
            LOG_MM_W(TAG, "AX_VDEC_SendStream(vdGrp %d, len %d, pts %lld, timeout %d) not permitted, ret = 0x%x", m_vdGrp, nLen, u64PTS, nTimeOut,
                m_nLastSendCode);
        } else {
            LOG_MM_E(TAG, "AX_VDEC_SendStream(vdGrp %d, len %d, pts %lld, timeout %d) fail, ret = 0x%x", m_vdGrp, nLen, u64PTS, nTimeOut,
                m_nLastSendCode);
        }
