...
AX_BOOL CDataStreamIndFile::Init(const AX_CHAR* pFilePath, AXIF_OPEN_FLAG_E eOpenFlag, AX_S32 nDate /*= 0*/, AX_S32 nTime /*= 0*/, AX_S32 nTick /*= 0*/, AX_BOOL bGopMode /*= AX_FALSE*/) {
    LOG_MM_I(DSIF, "+++");
...
AX_BOOL CDataStreamIndFile::CreateAndSave(AXIF_FILE_INFO_T& tIFFileInfo) {
    ...
    if (AX_DS_INVALID_HANDLE == m_hFD) {
        ...
        if (!WriteFileHeader(m_tFileHeader)) {
            ...
            LOGBUF(APP_LOG_DEBUG, &m_tFileHeader, sizeof(AXIF_FILE_HEADER_T), APP_LOG_SYNC_SEND);
            LOG_MM_I(DSIF, "Load existing index file header info:");
            LOG_MM_I(DSIF, "Total Size: %d, Data File Count: %d, Total Time: %lld, Total Frame: %d, Total I Frame: %d"
...
AX_BOOL CDataStreamIndFile::FindFrameLocationByTime(AXDS_DATETIME_T& tTargetTime, AXIF_FRAME_LOCATION_T& tLocation, AX_BOOL bIFrameOnly /*= AX_FALSE*/, AX_BOOL bReverse /*= AX_FALSE*/) {
    AX_BOOL bFindResult = AX_FALSE;
    AXDS_DATETIME_T tFileStartTime = {0, 0};
    AXDS_DATETIME_T tFileEndTime = {0, 0};
    ...
    for (; itStart != itEnd; ++itStart) {
        info = *itStart;
        tFileStartTime = info.tInfo.tStartTime;
        tFileEndTime = info.tInfo.tEndTime;
        ...
        do {
            if (tTargetTime >= tFileStartTime && tTargetTime <= tFileEndTime) {
                ...
                if (bReverse) {
                    for (AX_S32 i = info.tInfo.uIFrameCount; i > 0; --i) {
                        ...
                        if (pHeader->tTimeStamp <= tTargetTime) {
                            nFrmIndexWithinFile = i - 1;
                            LOG_MM_I("DSIF", "Find iframe index %d(0, %d), Time %lld, TargetTime %lld", nFrmIndexWithinFile, info.tInfo.uIFrameCount - 1, pHeader->tTimeStamp.Value(), tTargetTime.Value());
                            break;
                        }
                    }
                } else {
                    nFrmIndexWithinFile = dsf->FindFrmIndexByTime(tTargetTime, bIFrameOnly, AX_TRUE);
                }
            ...
            } else if ((!bReverse && tTargetTime < tFileStartTime) || (bReverse && tTargetTime > tFileEndTime)) {
                AXDS_FRAME_HEADER_T* pHeader = nullptr;
                AX_S32 nFrmIndexWithinFile = -1;
                if (bReverse) {
                    for (AX_S32 i = info.tInfo.uIFrameCount; i > 0; --i) {
                        ...
                        if (pHeader->tTimeStamp <= tTargetTime) {
                            nFrmIndexWithinFile = i - 1;
                            LOG_MM_I("DSIF", "Find iframe index %d(0, %d), nFileIndex=%d", nFrmIndexWithinFile, info.tInfo.uIFrameCount, nFileIndex);
                            break;
                        }
                    }
                } else {
                    nFrmIndexWithinFile = dsf->FindFrmIndexByTime(tTargetTime, bIFrameOnly);
                }
...
CDSIterator CDataStreamIndFile::begin() {
    if (m_nStartDate > 0) {
        std::pair<time_t, AX_U32> pairSecAndTick = CElapsedTimer::GetTimeTValEx(m_nStartDate, m_nStartTime, m_nStartTick);
        AXDS_DATETIME_T tDateTime {(AX_U32)pairSecAndTick.first, pairSecAndTick.second};
        AXIF_FRAME_LOCATION_T tLocation;
        if (FindFrameLocationByTime(tDateTime, tLocation, AX_TRUE)) {
            return CDSIterator(this, CDSIterator::BEGIN).Relocate(tLocation);
        } else {
            LOG_MM_W(DSIF, "Can not locate at time(%d %d %d).", m_nStartDate, m_nStartTime, m_nStartTick);
...
CDSIterator CDataStreamIndFile::rbegin() {
    if (m_nStartDate > 0) {
        ...
        AXDS_DATETIME_T tDateTime {(AX_U32)pairSecAndTick.first, pairSecAndTick.second};
        AXIF_FRAME_LOCATION_T tLocation;
        if (FindFrameLocationByTime(tDateTime, tLocation, AX_TRUE, AX_TRUE)) {
