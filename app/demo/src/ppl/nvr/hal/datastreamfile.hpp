...
typedef struct AXDS_FRAME_HEADER {
    AX_U32 uMagic;  // Magic number
    AX_U32 uHeaderSize;
    AX_U32 uFrameSize; // Total size of frame data + frame header
    AXDS_DATETIME_T tTimeStamp;
    AX_U8  uFrameType;   // 0x02: Video SPS; 0x03: Video PPS; 0x04: Video VPS; 0x05: Video I；0x06：Video P/B；0x11：Audio
    AX_U8  uFrameSubType; // reserved
    AX_U16 uExtDataLen;   // Extended information is not needed for now, set the length to 0.
    AX_U8  uExtInfo[8]; //Extended information for I-frames or audio frames is not needed at the moment.
...
typedef struct AXDS_FILE_TAIL {
    AX_U32 uMagic;  // Magic number
    AX_U32 uFrameCount;
    AX_U32* pBufFrameOffset; //Offset of the frame position in the file
...
typedef struct AXDSF_FRAME_LOCATION {
    AX_S32 nCurrFrameIndexWithinFile; /* Current traversed frame index */
    AX_U32 nTotalFrameCount;  /* Total number of frames in the current data file */
...
public:
    ...
    AX_S32 FindFrmIndexByTime(AXDS_DATETIME_T& tTargetTime, AX_BOOL bOnlyIFrame = AX_FALSE, AX_BOOL bReverseSearch = AX_FALSE);
