...
int main(int argc, char *argv[]) {
    ...
    LOG_M_C(TAG, "VDEC %s, PP VB num = %d, depth = %d, bVarRand = %d, TOTAL_STREAM_COUNT = %d",
            (1 == playBackMode) ? "playback" : "preview", ppVBCnt, ppDepth, bVarRand, TOTAL_STREAM_COUNT);
    ...
    while (0 == loopCount || loop < loopCount) {
        ...
        for (AX_U32 i = 0; i < layOut; ++i) {
            ...
            if (!ppls[i].Create(stAttr)) {
                ...
                LOG_MM_E(TAG, "create ppl %d fail", i);
                return 1;
            }
        }
        ...
        for (AX_U32 i = 0; i < layOut; ++i) {
            v.emplace_back(
                [](PPL *ppl, int i) {
                    if (!ppl->Destory()) {
                        LOG_MM_E(TAG, "destory ppl %d fail", i);
                    }
                },
                &ppls[i], i);
        }
