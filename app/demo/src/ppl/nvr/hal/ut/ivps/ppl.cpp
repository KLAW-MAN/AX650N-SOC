...
AX_BOOL PPL::Create(const PPL_ATTR_T &stAttr) {
    ...
    do {
        ...
        if (!m_vo->SetChnFps(m_stAttr.voChn, (AX_F32)(m_stream->GetStreamInfo().stVideo.nFps))) {
            break;
        }

        if (!m_vo->EnableChn(m_stAttr.voChn, m_stAttr.voChnAttr)) {
            break;
        }
...
AX_BOOL PPL::Destory(AX_VOID) {
    ...
    if (m_stream) {
        ...
        if (!m_stream->Stop()) {
            LOG_MM_E(TAG, "stop stream %d fail", m_stAttr.voChn);
            return AX_FALSE;
        }

        if (!DestoryStream()) {
            LOG_MM_E(TAG, "destory stream %d fail", m_stAttr.voChn);
            return AX_FALSE;
        }
    }

    if (!m_vo->DisableChn(m_stAttr.voChn)) {
        LOG_MM_E(TAG, "disable voChn %d fail", m_stAttr.voChn);
        return AX_FALSE;
    }

    if (m_ivps) {
        if (!m_ivps->Stop()) {
            LOG_MM_E(TAG, "stop ivps %d fail", m_stAttr.voChn);
            return AX_FALSE;
        }

        if (!DestoryIVPS()) {
            LOG_MM_E(TAG, "destory ivps %d fail", m_stAttr.voChn);
            return AX_FALSE;
        }
    }
    ...
    if (m_vdec) {
        if (!m_vdec->Stop()) {
            LOG_MM_E(TAG, "stop vdec %d fail", m_stAttr.voChn);
            return AX_FALSE;
        }

        if (!DestoryVDEC()) {
            LOG_MM_E(TAG, "destory vdec %d fail", m_stAttr.voChn);
            return AX_FALSE;
        }
    }
...
AX_BOOL PPL::CreateIVPS(AX_VOID) {
    ...
    stChn.nWidth = m_stAttr.voChnAttr.stRect.u32Width;
    stChn.nHeight = m_stAttr.voChnAttr.stRect.u32Height;
