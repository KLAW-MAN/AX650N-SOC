...
AX_BOOL CDataStreamFile::WriteFrame(const AX_VOID* pData, AX_U32 nSize, AXDS_FRAME_HEADER_T& tFrameHeader) {
    ...
    if (tFrameHeader.uFrameType == 0x05/* IDR */) {
        m_nIndexWithinGop = 0;
    } else {
        m_nIndexWithinGop++;
    }
    ...
    tFrameHeader.uFrameSubType = m_nIndexWithinGop;
...
AX_S32 CDataStreamFile::FindFrmIndexByTime(AXDS_DATETIME_T& tTargetTime, AX_BOOL bOnlyIFrame /*= AX_FALSE*/, AX_BOOL bReverseSearch/*= AX_FALSE*/) {
    if (!IsOpened()) {
        return -1;
    }
    ...
    if (bReverseSearch) {
        for (AX_S32 i = nFrmCount - 1; i >= 0; --i) {
