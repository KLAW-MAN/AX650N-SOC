...
AX_BOOL CVideoStreamTransfer::SendStream(AX_S32 nStream, CONST AX_VENC_PACK_T& tVencPacked, AX_BOOL bGopStart /*= AX_TRUE*/, AX_S32 nTimeOut /*= INFINITE*/) {
    ...
    if (!m_pStreamContainer->Push(tVencPacked, nTimeOut)) {
        LOG_MM_E(TAG, "Fill stream data(size: %d, GopStart: %d) to container failed", tVencPacked.u32Len, bGopStart);
        return AX_FALSE;
    } else {
        LOG_MM_D(TAG, "Fill stream data(size: %d, PTS: %lld, GopStart: %d, Left: %d) to container.", tVencPacked.u32Len, tVencPacked.u64PTS, bGopStart, m_pStreamContainer->GetLeftCnt());
    }
...
AX_BOOL CVideoStreamTransfer::Pause() {
    LOG_MM_I(TAG, "+++");
    if (m_threadDispatch.IsRunning()) {
        m_threadDispatch.Pause();
    }
    LOG_MM_I(TAG, "---");
    return AX_TRUE;
}

AX_BOOL CVideoStreamTransfer::Resume() {
    LOG_MM_I(TAG, "+++");
    if (m_threadDispatch.IsPausing()) {
        m_threadDispatch.Resume();
    }
    LOG_MM_I(TAG, "---");
    return AX_TRUE;
}
...
AX_VOID CVideoStreamTransfer::DispatchThread(AX_VOID*) {
    ...
    while (m_threadDispatch.IsRunning() || m_threadDispatch.IsPausing()) {
        while (m_threadDispatch.IsPausing()) {
            std::this_thread::sleep_for(std::chrono::milliseconds(1));
        }

        VIDEO_STREAM_INFO_T tVencStream;
        if (!m_pStreamContainer->Pop(tVencStream, 10) || 0 == tVencStream.u32Len) {
            std::this_thread::sleep_for(std::chrono::milliseconds(1));
            continue;
        }
