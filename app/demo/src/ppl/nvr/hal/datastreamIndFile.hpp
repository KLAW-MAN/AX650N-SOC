...
typedef struct AXIF_FRAME_LOCATION {
    std::string strDataFile; /* Global location of the frame in the data file. # Global location refers to the positioning within all video files of a specific channel on the same day. */
    AX_S32 nFileIndex {-1}; /* Global location: The index of the data file where the frame is located. */
    AX_S32 nGlobalFrameIndex {-1}; /* Global location: The index of the frame.*/
    AX_S32 nFrameIndexWithinFile {-1}; /* Global location: The position of the frame in the current file. */
    AX_S32 nFileStartFrmIndex {-1}; /* The position of the first frame in the current file within the global location. */
    AX_S32 nFileEndFrmIndex {-1}; /* The position of the last frame in the current file within the global location. */
...
typedef struct AXIF_FRAME_RELOCATION_INFO {
    AX_S32 nFileIndex {-1}; /* The index of the data file where the frame is located in the global location. */
    AX_S32 nFrameIndexWithinFile {-1}; /* The position of the frame within the current file in the global location. */
...
typedef struct AXIF_FILE_HEADER {
    ...
    AX_U32 uTotalSize;
    AX_U32 uFileCount;
    AX_U32 uTotalFrameCount;
    AX_U32 uTotalIFrameCount;
    AX_U64 uTotalTime; //us
    AXIF_FILE_HEADER() {
...
typedef struct AXIF_FILE_INFO {
    AX_U32  uSize; // The actual size of the stored file information, excluding the size of the pIFrameOffsetStart pointer, and including the I-frame offset data.
    ...
    AX_U32 uIFrameCount;
    AX_U32* pIFrameOffsetStart;  //Records the offset of I-frames, with a variable length.
...
public:
    ...
    AX_BOOL Init(const AX_CHAR* pFilePath, AXIF_OPEN_FLAG_E eOpenFlag, AX_S32 nDate = 0, AX_S32 nTime = 0, AX_S32 nTick = 0, AX_BOOL bGopMode = AX_FALSE);
    AX_BOOL DeInit();
...
private:
    ...
    AX_BOOL FindFrameLocationByTime(AXDS_DATETIME_T& tTargetTime, AXIF_FRAME_LOCATION_T& tLocation, AX_BOOL bIFrameOnly = AX_FALSE, AX_BOOL bReverse = AX_FALSE);

private:
    AX_CHAR m_szFilePath[276];
    AX_S32 m_nStartDate {-1};
    AX_S32 m_nStartTime {-1};
    AX_S32 m_nStartTick {-1};
    ...
    CDSIterator(CDataStreamIndFile* pIFInstance, AX_S32 nIndex, AX_BOOL bOnlyIFrame = AX_FALSE) : m_pDSIF(pIFInstance) {
        ...
        if (BEGIN == nIndex) {
            ...
            if (!OpenDataFile((*m_itDSIF).tInfo.szFilePath)) {
                ...
                LOG_MM_I("DS_ITER", "Open data file %s (FrmTotal %d).", (*m_itDSIF).tInfo.szFilePath, (*m_itDSIF).tInfo.uFrameCount);
            }
