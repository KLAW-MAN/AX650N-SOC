...
AX_BOOL CVO::Init(VO_ATTR_T& stAttr) {
    ...
    m_stAttr = stAttr;
    if (AX_VO_OUTPUT_BUTT == m_stAttr.enMaxIntfSync) {
        m_stAttr.enMaxIntfSync = m_stAttr.enIntfSync;
    }

    const AX_HDMI_ID_E enHdmi = (1 == m_stAttr.dev) ? AX_HDMI_ID1 : AX_HDMI_ID0;
    if (!m_hdmiEvent.CheckPlugged(enHdmi)) {
        LOG_M_W(VO, "please connect HDMI %d to monitor", (AX_S32)enHdmi);
    }
    ...
    stPubAttr.enIntfFmt = m_stAttr.enIntfFmt;
    stPubAttr.stHdmiAttr = m_stAttr.stHdmiAttr;
    ...
    do {
        ...
        if (m_stAttr.s32FBIndex[2] != -1) {
            ret = AX_VO_BindGraphicLayer(m_stAttr.s32FBIndex[2], m_stAttr.dev);
            if (0 != ret) {
                LOG_M_E(VO, "AX_VO_BindGraphicLayer(layer %d dev %d) fail, ret = 0x%x", m_stAttr.s32FBIndex[2], m_stAttr.dev, ret);
                break;
            } else {
        ...
        if (m_stAttr.s32FBIndex[1] != -1) {
            ...
            if (-1 != m_stAttr.nGraphicLayerVFresh) {
                ret = AX_VO_SetGraphicLayerVrefresh(m_stAttr.dev, m_stAttr.s32FBIndex[1], m_stAttr.nGraphicLayerVFresh);
                if (0 != ret) {
                    LOG_MM_E(VO, "AX_VO_SetGraphicLayerVrefresh(dev %d layer %d fps %d) fail, ret = 0x%x", m_stAttr.dev, m_stAttr.s32FBIndex[1], m_stAttr.nGraphicLayerVFresh, ret);
                    break;
                }
            }
        }
        ...
        if (m_stAttr.s32FBIndex[0] != -1) {
            ...
            if (-1 != m_stAttr.nGraphicLayerVFresh) {
                ret = AX_VO_SetGraphicLayerVrefresh(m_stAttr.dev, m_stAttr.s32FBIndex[0], m_stAttr.nGraphicLayerVFresh);
                if (0 != ret) {
                    LOG_MM_E(VO, "AX_VO_SetGraphicLayerVrefresh(dev %d layer %d fps %d) fail, ret = 0x%x", m_stAttr.dev, m_stAttr.s32FBIndex[0], m_stAttr.nGraphicLayerVFresh, ret);
                    break;
                }
            }
        }
        ...
        if (AX_VO_OUT_FMT_YUV422_10 == m_stAttr.enIntfFmt) {
        ...
        if (m_stAttr.bListenHotPlug) {
            if (!m_hdmiEvent.IsPlugged()) {
                (AX_VOID) DpmsOnOff(AX_FALSE);
            }

            (AX_VOID) m_hdmiEvent.Start(enHdmi, std::bind(&CVO::HdmiPlugEventCallback, this, std::placeholders::_1));
        }
    ...
    if (UILAYER_BINDED == (nState & UILAYER_BINDED)) {
        ...
        if (m_stAttr.s32FBIndex[2] != -1) {
            ret = AX_VO_UnBindGraphicLayer(m_stAttr.s32FBIndex[2], m_stAttr.dev);
            if (0 != ret) {
                LOG_M_E(VO, "AX_VO_UnBindGraphicLayer(layer %d dev %d) fail, ret = 0x%x", m_stAttr.s32FBIndex[2], m_stAttr.dev, ret);
            }
        }
    }
...
AX_BOOL CVO::DeInit(AX_VOID) {
    ...
    /* always set dpms on before disable device */
    if (!DpmsOnOff(AX_TRUE)) {
        return AX_FALSE;
    }
...
AX_BOOL CVO::BatchUpdate(AX_BOOL bEnable, CONST VO_CHN_INFO_T& stChnInfo) {
    ...
    if (0 != ret) {
        LOG_MM_E(VO, "AX_VO_BatchBegin(layer %d) fail, ret = 0x%x", m_stAttr.voLayer, ret);
        return AX_FALSE;
    }
    ...
    if (0 != ret) {
        LOG_MM_E(VO, "AX_VO_BatchEnd(layer %d) fail, ret = 0x%x", m_stAttr.voLayer, ret);
        return AX_FALSE;
    }
...
AX_BOOL CVO::GetDispInfoFromIntfSync(AX_VO_INTF_SYNC_E eIntfSync, AX_VO_RECT_T& stArea, AX_U32& nHz) {
    ...
    stArea.u32Y = 0;
    switch (eIntfSync) {
        case AX_VO_OUTPUT_1024x768_60:
            stArea.u32Width = 1024;
            stArea.u32Height = 768;
            nHz = 60;
            break;
        case AX_VO_OUTPUT_1280x1024_60:
            stArea.u32Width = 1280;
            stArea.u32Height = 1024;
            nHz = 60;
            break;
        case AX_VO_OUTPUT_1366x768_60:
            stArea.u32Width = 1366;
            stArea.u32Height = 768;
            nHz = 60;
            break;
        ...
        if (voChn == m_stAttr.pipChn) {
            ...
            if (stChnCropRect.u32Width > 0 && stChnCropRect.u32Height > 0) {
                ...
                if (tRect.nX < stChnCropRect.u32X && tRect.nY < stChnCropRect.u32Y &&
                    ...
                    /* Remove the full surrounding border */
                    continue;
                }
...
AX_BOOL CVO::GenRegionsForPipRect(VO_RECT_ATTR_T& tRect, std::vector<VO_RECT_ATTR_T>& vecOutConvertedRect,
    ...
    if (m_tPipRect.nW == 0 || m_tPipRect.nH == 0) {
        /* PIP is disable，return original RECT */
    ...
    if (PointInRect(ptLeftTop, m_tPipRect)) {
        ...
        /* Completely cover the PIP channel. */
        vecOutConvertedRect.emplace_back(tRect);
...
AX_BOOL CVO::GetVideoLayerCSC(AX_VO_CSC_T& csc) {
    AX_S32 ret = AX_VO_GetVideoLayerCSC(m_stAttr.voLayer, &csc);
    if (0 != ret) {
        LOG_M_E(VO, "AX_VO_GetVideoLayerCSC(layer %d) fail, ret = 0x%x", m_stAttr.voLayer, ret);
        return AX_FALSE;
    }
...
AX_BOOL CVO::SetVideoLayerCSC(CONST AX_VO_CSC_T& csc) {
    AX_S32 ret = AX_VO_SetVideoLayerCSC(m_stAttr.voLayer, &csc);
    if (0 != ret) {
        LOG_M_E(VO, "AX_VO_SetVideoLayerCSC(layer %d) fail, ret = 0x%x", m_stAttr.voLayer, ret);
        return AX_FALSE;
    } else {
        LOG_M_I(VO, "set video layer %d csc: matrix %d, luma %d, contrast %d, hue %d, satuature %d", m_stAttr.voLayer, csc.enCscMatrix,
                csc.u32Luma, csc.u32Contrast, csc.u32Hue, csc.u32Satuature);
    }

    return AX_TRUE;
}

AX_BOOL CVO::DpmsOnOff(AX_BOOL bOn) {
    if (m_bDpmsOn == bOn) {
        return AX_TRUE;
    }
    ...
    AX_S32 ret;
    if (bOn) {
        ret = AX_VO_DpmsOn(m_stAttr.dev);
        if (0 != ret) {
            LOG_MM_E(VO, "AX_VO_DpmsOn(dev %d) fail, ret = 0x%x", m_stAttr.dev, ret);
            return AX_FALSE;
        }

        LOG_MM_W(VO, "dev %d dpms on", m_stAttr.dev);
    } else {
        ret = AX_VO_DpmsOff(m_stAttr.dev);
        if (0 != ret) {
            LOG_MM_E(VO, "AX_VO_DpmsOff(dev %d) fail, ret = 0x%x", m_stAttr.dev, ret);
            return AX_FALSE;
        }

        LOG_MM_W(VO, "dev %d dpms off", m_stAttr.dev);
    }
...
AX_BOOL CVO::HdmiPlugEventCallback(AX_HDMI_EVENT_TYPE_E enType) {
    return DpmsOnOff((AX_HDMI_EVENT_HOTPLUG == enType) ? AX_TRUE : AX_FALSE);
}
