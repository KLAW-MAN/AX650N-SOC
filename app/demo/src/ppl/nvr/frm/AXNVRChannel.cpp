...
AX_BOOL CAXNVRChannel::StartFile(AX_U32 nDate, AX_U32 nTime, AX_U32 nTick, AX_S32 nReverse /*= -1*/, AX_S32 nRepeat /*= -1*/) {
    AX_BOOL bRet = AX_FALSE;
    LOG_MM_I(TAG, "[%02d:%02d][state:%d] +++", m_stAttr.nDevID, m_stAttr.enIndex, m_enState);
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    do {
        ...
        if (-1 == nRepeat) {
            bRepeat = m_stAttr.bRepeat;
        } else {
            bRepeat = (AX_BOOL)nRepeat;
        }
...
AX_BOOL CAXNVRChannel::StartDisp(VO_LAYER nVoLayer, VO_CHN nVoChannel, const AX_VO_RECT_T &stRect) {
    ...
    do {
        ...
        if (AX_NVR_CHN_STATE::STARTED == m_enState && m_stAttr.enStreamSrcType == AX_NVR_CHN_SRC_TYPE::RECORD) {
            m_stAttr.pPlayback->ChangeDirection(m_stAttr.nDevID, (AX_U8)m_stAttr.enIndex, m_stAttr.bReverse, m_stAttr.bRepeat);
            bRet = AX_TRUE;
            break;
        }
        ...
        // 6. file stream start
        if (AX_NVR_CHN_SRC_TYPE::RECORD == m_stAttr.enStreamSrcType) {
            if (!m_stAttr.pPlayback->StartPlay(m_stAttr.nDevID, (AX_U8)m_stAttr.enIndex, m_stAttr.nStartDate, m_stAttr.nStartTime, m_stAttr.nStartTick,
                                               m_stAttr.bReverse, m_stAttr.bRepeat)) {
...
AX_BOOL CAXNVRChannel::StopDisp(AX_BOOL bDispValid /*= AX_TRUE*/) {
    AX_BOOL bRet = AX_FALSE;
    LOG_MM_I(TAG, "[%02d:%02d][state:%d][reverse:%d] +++", m_stAttr.nDevID, m_stAttr.enIndex, m_enState, m_stAttr.bReverse);
    std::lock_guard<std::mutex> lock(mutex_);
    ...
    do {
        if (AX_NVR_CHN_STATE::PAUSED == m_enState) {
            if (!this->pause_resume(AX_FALSE, bDispValid)) {
                break;
            }
            bRet = AX_TRUE;
        }
        ...
        // unregister playback stream observer and stop stream
...
AX_BOOL CAXNVRChannel::PreStopVo(AX_VOID) {
    ...
    AX_S32 ret = AX_SYS_UnLink(&m_stModeInfoIvps, &m_stModeInfoVo);
    if (0 != ret) {
        LOG_MM_W(TAG, "unlink IVPS %d %d to DISP %d %d fail, ret = 0x%x", m_stModeInfoIvps.s32GrpId, m_stModeInfoIvps.s32ChnId,
                m_stModeInfoVo.s32GrpId, m_stModeInfoVo.s32ChnId, ret);
    } else {
        LOG_MM_W(TAG, "unlink IVPS %d %d to DISP %d %d OK", m_stModeInfoIvps.s32GrpId, m_stModeInfoIvps.s32ChnId,
                m_stModeInfoVo.s32GrpId, m_stModeInfoVo.s32ChnId);
    }

    if (m_stAttr.bReverse && m_stAttr.enStreamSrcType == AX_NVR_CHN_SRC_TYPE::RECORD && !m_stAttr.pPlayback->GetAttr().bOnlyIFrameOnReverse) {
        m_objStreamTrans.Pause();
    }

    return AX_TRUE;
}

AX_BOOL CAXNVRChannel::ContinueVo(AX_VOID) {
    if (m_enState != AX_NVR_CHN_STATE::STARTED && m_enState != AX_NVR_CHN_STATE::PAUSED) {
        return AX_TRUE;
    }
    ...
    AX_S32 ret = AX_SYS_Link(&m_stModeInfoIvps, &m_stModeInfoVo);
    if (0 != ret) {
        LOG_MM_W(TAG, "link IVPS %d %d to DISP %d %d fail, ret = 0x%x", m_stModeInfoIvps.s32GrpId, m_stModeInfoIvps.s32ChnId,
                m_stModeInfoVo.s32GrpId, m_stModeInfoVo.s32ChnId, ret);
    } else {
        LOG_MM_W(TAG, "link IVPS %d %d to DISP %d %d OK", m_stModeInfoIvps.s32GrpId, m_stModeInfoIvps.s32ChnId,
                m_stModeInfoVo.s32GrpId, m_stModeInfoVo.s32ChnId);
    }

    if (m_stAttr.bReverse && m_stAttr.enStreamSrcType == AX_NVR_CHN_SRC_TYPE::RECORD && !m_stAttr.pPlayback->GetAttr().bOnlyIFrameOnReverse) {
        m_objStreamTrans.Resume();
    }
...
AX_BOOL CAXNVRChannel::UpdateRect(const AX_VO_RECT_T &stRect) {
    ...
    do {
        ...
        if (!m_objIvps.UpdateChnAttr(m_stAttr.nLinkIvpsVoooChn, stChnAttr)) {
            ...
            LOG_MM_W(TAG, "[%d] Update ivps rect(%d, %d)", m_stAttr.nLinkIvpsVoooChn, stChnAttr.nWidth, stChnAttr.nHeight);
        }
...
AX_BOOL CAXNVRChannel::RestorePauseState() {
    std::lock_guard<std::mutex> lock(mutex_);
    AX_S32 ret = AX_VO_PauseChn(m_stAttr.nVoLayer, m_stAttr.nVoChannel);
    if (0 != ret) {
...
AX_BOOL CAXNVRChannel::Step(AX_BOOL bReverse) {
    ...
    do {
        if (AX_NVR_CHN_STATE::PAUSED != m_enState) {
            LOG_MM_W(TAG, "Not in PAUSED state, force to PAUSE automatically.");
            this->pause_resume();
        }
...
std::tuple<AX_U32, AX_U32, AX_U32> CAXNVRChannel::GetCurrDisplayedTime(AX_VOID) {
    std::lock_guard<std::mutex> lock(mutex_);
    if (m_stAttr.pPlayback) {
        LOG_MM_W(TAG, "Get displayed time for layer:%d, channel:%d", m_stAttr.nVoLayer, m_stAttr.nVoChannel);
        return m_stAttr.pPlayback->GetCurrDisplayedTime(m_stAttr.nDevID, (AX_U8)m_stAttr.enIndex, m_stAttr.nVoLayer, m_stAttr.nVoChannel);
    }

    return make_tuple<AX_U32, AX_U32, AX_U32>(0, 0, 0);
}
...
AX_BOOL CAXNVRChannel::init_venc(AX_PAYLOAD_TYPE_E enPayload, AX_U32 nWidth, AX_U32 nHeight, AX_U32 nFPS, VENC_GRP veGrp /*= INVALID_VENC_GRP*/) {
    ...
    do {
        ...
        tVencAttr.nBitrate = 60000; /* Not used now */
        ...
        tVencAttr.arrRcConfig[0].stRCInfo[0].nBitrate = 60000; /* When GOP=1, the recommended value is frame size * fps. */
        ...
        tVencAttr.arrRcConfig[0].stRCInfo[1].nBitrate = 60000;
        /* H265 RC config */
        ...
        tVencAttr.arrRcConfig[1].stRCInfo[0].nBitrate = 60000;
        ...
        tVencAttr.arrRcConfig[1].stRCInfo[1].nBitrate = 60000;
...
AX_VOID CAXNVRChannel::set_state(AX_NVR_CHN_STATE enStatus) {
    LOG_MM_I(TAG, "[%02d]change state %d to %d", m_stAttr.nDevID, m_enState, enStatus);
    m_enState = enStatus;
}
...
AX_BOOL CAXNVRChannel::pause_resume(AX_BOOL bForceResume /*= AX_FALSE*/, AX_BOOL bDispValid /*= AX_TRUE*/) {
    AX_BOOL bRet = AX_FALSE;
    do {
        AX_S32 ret = -1;
        if (AX_NVR_CHN_STATE::STARTED == m_enState) {
            if (!bForceResume) {
                ...
                if (bDispValid) { /* DISP maybe reset after resolution changed */
