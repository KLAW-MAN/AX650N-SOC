...
public:
    ...
    AX_BOOL StartFile(AX_U32 nDate, AX_U32 nTime, AX_U32 nTick = 0, AX_S32 nReverse = -1, AX_S32 nRepeat = -1);
    AX_BOOL StopFile(AX_VOID);

    AX_BOOL StartDisp(VO_LAYER nVoLayer, VO_CHN nVoChannel, const AX_VO_RECT_T &stRect);
    AX_BOOL StopDisp(AX_BOOL bDispValid = AX_TRUE);
    ...
    AX_BOOL RestorePauseState(AX_VOID);
    ...
    AX_BOOL PreStopVo(AX_VOID);
    AX_BOOL ContinueVo(AX_VOID);

    //
    AX_NVR_CHN_RES_T GetResolution(AX_VOID);
    std::tuple<AX_U32, AX_U32, AX_U32> GetCurrDisplayedTime(AX_VOID);
...
private:
    AX_BOOL pause_resume(AX_BOOL bForceResume = AX_FALSE, AX_BOOL bDispValid = AX_TRUE);
