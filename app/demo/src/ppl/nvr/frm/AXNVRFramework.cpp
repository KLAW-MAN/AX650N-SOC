...
AX_VOID CAXNVRFramework::OnVoIfSyncChanged(AX_NVR_VO_IF_SYNC eVoIfSync) {
    if (m_pPrimaryDispCtrl) {
        m_pPrimaryDispCtrl->UpdateVoIfSync(eVoIfSync, AX_TRUE);
    }
}
...
CAXNVRDisplayCtrl *CAXNVRFramework::initPrimaryDispCtrl(AX_VOID) {
    ...
    stPrimaryAttr.stVoAttr.s32FBIndex[2] = m_stPrimaryCfg.nFBMouse;
    ...
    stPrimaryAttr.stVoAttr.nGraphicLayerVFresh = m_stPrimaryCfg.nGraphicLayerVFresh;
