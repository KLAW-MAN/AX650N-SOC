...
AX_BOOL CAXNVRDisplayCtrl::Init(const AX_NVR_VO_INFO_T &stAttr) {
    ...
    do {
        ...
        if (m_stAttr.stVoAttr.s32FBIndex[2] != -1) {
            ...
            AX_U16 show = 1;
            AX_U32 u32FbIndex = m_stAttr.stVoAttr.s32FBIndex[2];
            AX_U32 u32StartX = 0;
            AX_U32 u32StartY = 0;
            AX_U32 u32Width = 16;
            AX_U32 u32Height = 16;
            ...
            do {
                ...
                if (s32Fd < 0) {
                    LOG_MM_E(TAG, "open %s failed, err:%s", fbPath, strerror(errno));
                    break;
                }
                ...
                if (s32Ret < 0) {
                    LOG_MM_E(TAG, "get fix screen info from fb%d failed", u32FbIndex);
                    break;
                }
                ...
                if (s32Ret < 0) {
                    LOG_MM_E(TAG, "get variable screen info from fb%d failed", u32FbIndex);
                    break;
                }
                ...
                */
                ...
                if (stVar.xres < u32Width || stVar.yres < u32Height) {
                    LOG_MM_E(TAG, "Curse res(%dx%d) is larger than fb res(%dx%d)", u32Width, u32Height, stVar.xres, stVar.yres);
                    break;
                }
                ...
                if (s32Ret < 0) {
                    LOG_MM_E(TAG, "put variable screen info to fb%d failed", u32FbIndex);
                    break;
                }
                ...
                if (pShowScreen == (AX_U8 *) - 1) {
                    LOG_MM_E(TAG, "map fb%d failed", u32FbIndex);
                    break;
                }

                if (![](char *buf) -> AX_BOOL {
                    /* load cursor file, must be bgra format */
                    std::ifstream ifs;
                    ifs.open("./config/cursor.bgra", std::ifstream::in | std::ifstream::binary);
                    if (ifs.is_open()) {
                        ifs.seekg (0, ifs.end);
                        int len = ifs.tellg();
                        ifs.seekg (0, ifs.beg);
                        ...
                        ifs.close();
                        return (len > 0) ? AX_TRUE : AX_FALSE;
                    } else {
                        return AX_FALSE;
                    }
                }((char *)pShowScreen)) {
                    /* no cursor file, replace with red squal */
                    for (AX_U32 i = 0; i < u32Height; i++) {
                        u32Pixel = (AX_U32 *)(pShowScreen + i * stFix.line_length);
                        for (AX_U32 j = 0; j < u32Width; j++) {
                ...
                if (s32Ret < 0) {
                    LOG_MM_E(TAG, "put cursor pos fb%d failed", u32FbIndex);
                    break;
                }
                ...
                if (s32Ret < 0) {
                    LOG_MM_E(TAG, "put cursor res fb%d failed", u32FbIndex);
                    break;
                }
                ...
                if (s32Ret < 0) {
                    LOG_MM_E(TAG, "put cursor show fb%d failed", u32FbIndex);
                    break;
                }

                LOG_MM_W(TAG, "Initialize cursor fb%d (rect [%d %d %d %d] line_length:%d) finished.", u32FbIndex, u32StartX, u32StartY, u32Width,
...
AX_BOOL CAXNVRDisplayCtrl::UpdateVoIfSync(AX_NVR_VO_IF_SYNC eVoIfSync, AX_BOOL bReset /*= AX_FALSE*/) {
    AX_VO_INTF_SYNC_E eVoIntfSync = AX_VO_OUTPUT_BUTT;
    switch (eVoIfSync) {
        case AX_NVR_VO_IF_SYNC::VO_IF_SYNC_MAX: {
            if (AX_VO_OUTPUT_BUTT != m_stAttr.stVoAttr.enMaxIntfSync) {
                eVoIntfSync = m_stAttr.stVoAttr.enMaxIntfSync;
            } else {
                eVoIntfSync = AX_VO_OUTPUT_1080P60;
            }

            break;
        }
        case AX_NVR_VO_IF_SYNC::VO_IF_SYNC_1280_1024_P60: {
            eVoIntfSync = AX_VO_OUTPUT_1280x1024_60;
            break;
        }
        case AX_NVR_VO_IF_SYNC::VO_IF_SYNC_1024_768_P60: {
            eVoIntfSync = AX_VO_OUTPUT_1024x768_60;
            break;
        }
        default: {
            LOG_MM_W(TAG, "Unknown resolution index: %d", eVoIfSync);
            return AX_FALSE;
        }
    }

    if (eVoIntfSync == m_stAttr.stVoAttr.enIntfSync) {
        return AX_TRUE;
    }

    VO_ATTR_T attr = m_pVo->GetAttr();
    m_stAttr.stVoAttr = std::move(m_pVo->GetAttr());

    AX_BOOL bRet = AX_FALSE;
    if (bReset) {
        DeInit();

        m_stAttr.stVoAttr.enIntfSync = eVoIntfSync;

        bRet = Init(m_stAttr);
        if (!bRet) {
            LOG_MM_E(TAG, "Reset vo attributes failed.");
            return AX_FALSE;
        }
    }

    m_stAttr.bIfSyncReset = AX_TRUE;
...
AX_BOOL CAXNVRDisplayCtrl::UpdateView(const ax_nvr_channel_vector vecViewChns,
                    ...
                    AX_BOOL bPaused) {
    AX_BOOL bRet = AX_FALSE;
    LOG_M_I(TAG, "[%s][%d][%d] +++", __func__, __LINE__, m_stAttr.enDispDevType);
    ...
    do {
        ...
        if (AX_NVR_DISPDEV_TYPE::PRIMARY == m_stAttr.enDispDevType) {
            if (AX_NVR_VIEW_CHANGE_TYPE::HIDE == enChangeType) {
                if (AX_NVR_VIEW_TYPE::PLAYBACK == enViewType && !bPaused) {
                    VO_ATTR_T attr = m_pVo->GetAttr();
            ...
            else if (AX_NVR_VIEW_CHANGE_TYPE::CONTINUE == enChangeType) {
                if (m_stAttr.bIfSyncReset) { /* Case if resolution changed in setting page after turn back to playback page */
                    ...
                    for (unsigned int i = 0; i < attr.stChnInfo.nCount; ++i) {
                        ...
                        LOG_MM_I(TAG, "[%d] %d,%d %dx%d", nVoChn,
                    ...
                    if (!m_pVo->UpdateChnInfo(attr.stChnInfo)) {
                        ...
                        LOG_MM_C(TAG, "Refresh vo channel attr for DISP resolution changed.");
                    }
                }
            }
...
AX_S32 CAXNVRDisplayCtrl::initFramebuffer(AX_U32 u32Width, AX_U32 u32Height, AX_U32 u32FbIndex) {
    ...
    do {
        ...
        */
        stVar.xres          = u32Width;
        stVar.xres_virtual  = 1920; /* Fixed now, must be same as fb size configured in run.sh */
        stVar.yres          = u32Height;
        stVar.yres_virtual  = 1080 * 2;
        stVar.transp        = a;
        stVar.red           = r;
        stVar.green         = g;
        stVar.blue          = b;
        ...
        LOG_MM_W(TAG, "FBIOPUT_VSCREENINFO => (%d, %d)", stVar.xres, stVar.yres);
        ...
        s32Ret = ioctl(s32Fd, FBIOGET_FSCREENINFO, &stFix);
        if (s32Ret < 0) {
            // LOG_MM_E(TAG, "get fix screen info from fb%d failed\n", u32FbIndex);
            break;
        }
...
AX_BOOL CAXNVRDisplayCtrl::getDispInfoFromIntfSync(AX_VO_INTF_SYNC_E eIntfSync, AX_VO_RECT_T& stArea, AX_U32& nHz) {
    ...
    switch (eIntfSync) {
        case AX_VO_OUTPUT_1024x768_60:  nHz = 60; break;
        case AX_VO_OUTPUT_1280x1024_60: nHz = 60; break;
        case AX_VO_OUTPUT_1366x768_60:  nHz = 60; break;
