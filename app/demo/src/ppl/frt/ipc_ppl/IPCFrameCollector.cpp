...
AX_VOID CFrameCollector::RegObserver(IObserver* pObserver) {
    if (nullptr != pObserver) {
        ...
        tTransAttr.nCfgWidth = m_stAttr.nCfgWidth;
        tTransAttr.nCfgHeight = m_stAttr.nCfgHeight;
        tTransAttr.nSnsSrc = m_stAttr.nSnsSrc;
        tTransAttr.arrOutFBC[0] = m_stAttr.arrFBC[0];
        tTransAttr.arrOutFBC[1] = m_stAttr.arrFBC[1];
        tTransAttr.fFramerate = m_stAttr.fFramerate;
        LOG_MM_I(COLLECT, "m_nGroup:%d, tTransAttr.nWidth:%d, height:%d, frameRate:%lf, nSnsSrc: %d",
                           m_nGroup, tTransAttr.nWidth, tTransAttr.nHeight, tTransAttr.fFramerate, tTransAttr.nSnsSrc);
...
AX_BOOL CFrameCollector::ChangeData(OBS_CHANGE_DATA_TYPE_E eDataType, AX_VOID* pData) {
    if (nullptr == pData) {
        return AX_TRUE;
    }
    ...
    for (vector<IObserver*>::iterator it = m_vecObserver.begin(); it != m_vecObserver.end(); it++) {
        if (!(*it)->OnDataChange(E_OBS_TARGET_TYPE_COLLECT, m_nGroup, 0, eDataType, pData)) {
            bRet = AX_FALSE;
        }
    }
