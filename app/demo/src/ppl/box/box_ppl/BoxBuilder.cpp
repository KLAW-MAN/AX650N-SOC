...
AX_BOOL CBoxBuilder::InitDisplay(AX_DISPDEV_TYPE enDispDev, DISPVO_CONFIG_T &dispVoConfig, AX_U32 nChnCount) {
    if (dispVoConfig.nDevId == -1) {
        return AX_TRUE;
    }

    VO_LAYER fbLayer = INVALID_LAYER_ID;
    if (dispVoConfig.bShowFB) {
        if (AX_VO_OUTPUT_3840x2160_30 == dispVoConfig.nHDMI || AX_VO_OUTPUT_3840x2160_60 == dispVoConfig.nHDMI) {
            if (!InitFrameBuffer(3840, 2160, 0)) {
                return AX_FALSE;
            }

            fbLayer = 0;
        } else if (AX_VO_OUTPUT_1080P60 == dispVoConfig.nHDMI) {
            if (!InitFrameBuffer(1920, 1080, 0)) {
                return AX_FALSE;
            }

            fbLayer = 0;
        } else {
            LOG_MM_W(BOX, "unsupport fb configuration");
            dispVoConfig.bShowFB = AX_FALSE;
        }
    }
    ...
    stAttr.fbLayer = fbLayer;

    if (dispVoConfig.nMipiDSI >= 0) {
        stAttr.enIntfType = AX_VO_INTF_DSI;
        stAttr.enIntfSync = (AX_VO_INTF_SYNC_E)dispVoConfig.nMipiDSI;
        stAttr.bListenHotPlug = AX_FALSE;
        LOG_M_C(BOX, "MIPI DSI, timing %d", dispVoConfig.nMipiDSI);
    } else {
        stAttr.enIntfType = AX_VO_INTF_HDMI;
        stAttr.enIntfSync = (AX_VO_INTF_SYNC_E)dispVoConfig.nHDMI;
        LOG_M_C(BOX, "HDMI, timing %d", dispVoConfig.nHDMI);
    }
    stAttr.nBgClr = 0x000000;
    if (AX_DISPDEV_TYPE::SECONDARY == enDispDev) {
        stAttr.nBgClr = 0x0000ff;
    }
...
AX_BOOL CBoxBuilder::InitFrameBuffer(AX_U32 width, AX_U32 height, AX_U8 fb) {
    AX_CHAR name[32] = {0};
    sprintf(name, "/dev/fb%d", fb);
    AX_S32 fd = open(name, O_RDWR);
    if (fd < 0) {
        LOG_MM_E(BOX, "open %s fail, %s", name, strerror(errno));
        return AX_FALSE;
    }
    ...
    do {
        struct fb_var_screeninfo vsc = {0};
        struct fb_fix_screeninfo fsc = {0};
        ret = ioctl(fd, FBIOGET_VSCREENINFO, &vsc);
        if (ret < 0) {
            LOG_MM_E(BOX, "get VSCREENINFO fail, %s", strerror(errno));
            break;
        }

        vsc.xres = width;
        vsc.xres_virtual = width;
        vsc.yres = height;
        vsc.yres_virtual = height;
        vsc.transp = {0, 8, 0};
        vsc.red = {24, 8, 0};
        vsc.green = {16, 8, 0};
        vsc.blue = {8, 8, 0};
        vsc.bits_per_pixel = 32; /* ARGB888 */
        ret = ioctl(fd, FBIOPUT_VSCREENINFO, &vsc);
        if (ret < 0) {
            LOG_MM_E(BOX, "put VSCREENINFO fail, %s", strerror(errno));
            break;
        }

        ret = ioctl(fd, FBIOGET_FSCREENINFO, &fsc);
        if (AX_SUCCESS != ret) {
            LOG_MM_E(BOX, "get FSCREENINFO, %s", strerror(errno));
            break;
        }

        size_t sz = vsc.xres * vsc.yres * vsc.bits_per_pixel / 8;
        AX_U8 *buf = (AX_U8 *)mmap(0, sz, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
        if (!buf) {
            LOG_MM_E(BOX, "mmap %d fail, %s", sz, strerror(errno));
            break;
        } else {
            memset(buf, 0, sz);
        }

        AX_S32 bytesPerPixel = vsc.bits_per_pixel / 8;

        constexpr AX_U32 x = 0;
        constexpr AX_U32 y = 0;
        AX_U32 pos = x * bytesPerPixel + y * fsc.line_length;

        for (AX_U32 i = 0; i < width; ++i) {
            for (AX_U32 j = 0; j < height; ++j) {
                buf[pos + i * bytesPerPixel + j * fsc.line_length] = 64;      /* A */
                buf[pos + i * bytesPerPixel + j * fsc.line_length + 1] = 128; /* B */
                buf[pos + i * bytesPerPixel + j * fsc.line_length + 2] = 255; /* G */
                buf[pos + i * bytesPerPixel + j * fsc.line_length + 3] = 128; /* R */
            }
        }

        ret = munmap(buf, sz);
        if (AX_SUCCESS != ret) {
            LOG_MM_E(BOX, "munmap %p, %s", buf, strerror(errno));
            break;
        }
