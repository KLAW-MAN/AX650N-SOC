...
STREAM_CONFIG_T CBoxConfig::GetStreamConfig(AX_VOID) {
    ...
    if (nCount > 0) {
        AX_CHAR szChipID[64] = {0};
        if (CSysUtils::GetChipID(szChipID, 64)) {
            if (!strncmp(szChipID, "AX650C_CHIP", sizeof("AX650C_CHIP") - 1)) {
                if (nCount > 16) {
                    nCount = 16;
                }
            }
...
DISPVO_CONFIG_T CBoxConfig::GetDispVoConfig(const std::string &SECT) {
    ...
    conf.nMipiDSI = m_Ini.GetIntValue(SECT, "MIPI DSI", -1);
    ...
    conf.bShowFB = (AX_BOOL)m_Ini.GetIntValue(SECT, "show fb", 0);
