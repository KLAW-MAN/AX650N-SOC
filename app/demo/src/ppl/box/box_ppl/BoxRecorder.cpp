...
AX_BOOL CBoxRecorder::CreateEncoderInstance(AX_VOID) {
    ...
    do {
        if (m_stAttr.nGdrNum > 0 && m_stAttr.nGdrNum < conf.nGOP) {
            m_pEncoder->ActiveGDR(m_stAttr.nGdrNum);
        }
...
AX_BOOL CBoxRecorder::CreateMuxerInstance(AX_VOID) {
    ...
    stMp4.stVideoChnInfo.nVideoChn = m_stAttr.veChn;
    stMp4.stVideoChnInfo.bOn = AX_TRUE;
    ...
    LOG_M_C(REC, "MP4 chn %d, %dx%d, fps %d, bps %d, 264(%d), max file size %d MB, path %s", stMp4.stVideoChnInfo.nVideoChn, stMp4.stVideoAttr.nfrWidth, stMp4.stVideoAttr.nfrHeight,
