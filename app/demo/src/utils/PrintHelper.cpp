...
AX_VOID CPrintHelper::PrintThreadFunc(CPrintHelper* pCaller) {
    ...
    while (pThis->m_bPrintThreadWorking) {
        ...
        if ((pThis->m_nTickEnd - pThis->m_nTickStart) >= PRINT_INTERVAL * 1000) {
            ...
            m_stVinYUVStatisticsInfo.Print();
            m_stVinRawStatisticsInfo.Print();
    ...
    m_stVinYUVStatisticsInfo.FinalPrint();
    m_stVinRawStatisticsInfo.FinalPrint();
}
...
AX_VOID CPrintHelper::Stop() {
    ...
    m_stVinYUVStatisticsInfo.ClearAll();
    m_stVinRawStatisticsInfo.ClearAll();
...
AX_VOID CPrintHelper::Add(PRINT_HELPER_MOD_E eModType, AX_U32 nChn) {
    ...
    switch (eModType) {
        ...
        case E_PH_MOD_MJENC:
            ...
            break;
        case E_PH_MOD_VIN_RAW:
            m_stVinRawStatisticsInfo.Add(nChn); // reuse nChn as pipe, so pass pipe when vin raw call
            break;
        default:
            break;
    }
...
AX_VOID CPrintHelper::Reset(PRINT_HELPER_MOD_E eModType, AX_U32 nChn) {
    ...
    switch (eModType) {
        ...
        case E_PH_MOD_MJENC:
            ...
            break;
        case E_PH_MOD_VIN_RAW:
            m_stVinRawStatisticsInfo.Reset(nChn); // reuse nChn as pipe, so pass pipe when vin raw call
            break;
        default:
            break;
    }
...
AX_VOID CPrintHelper::Add(PRINT_HELPER_MOD_E eModType, AX_U32 nPipe, AX_U32 nChn) {
    if (!m_hPrintThread.joinable()) {
        return;
    }
    ...
    m_bEnableStart = AX_TRUE;
    switch (eModType) {
        case E_PH_MOD_VIN_YUV:
            m_stVinYUVStatisticsInfo.Add(nPipe, nChn);
            break;
        default:
            break;
    }
...
AX_VOID CPrintHelper::Reset(PRINT_HELPER_MOD_E eModType, AX_U32 nPipe, AX_U32 nChn) {
    if (!m_hPrintThread.joinable()) {
        return;
    }
    ...
    switch (eModType) {
        case E_PH_MOD_VIN_YUV:
            m_stVinYUVStatisticsInfo.Reset(nPipe, nChn);
            break;
        default:
            break;
    }
