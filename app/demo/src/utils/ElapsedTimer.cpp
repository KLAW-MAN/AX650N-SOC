...
wchar_t *CElapsedTimer::GetCurrDateStr(wchar_t *szOut, AX_U16 nDateFmt, AX_S32 &nOutCharLen) {
    ...
    switch (eDateFmt) {
        ...
        case OSD_DATE_FORMAT_YYMMDD2: {
            nOutCharLen = swprintf(szOut, nDateLen, L"%04dY%02dM%02dD", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday);
            break;
        }
        case OSD_DATE_FORMAT_MMDDYY2: {
            nOutCharLen = swprintf(szOut, nDateLen, L"%02dM%02dD%04dY", tm.tm_mon + 1, tm.tm_mday, tm.tm_year + 1900);
            break;
        }
        case OSD_DATE_FORMAT_DDMMYY2: {
            nOutCharLen = swprintf(szOut, nDateLen, L"%02dD%02dM%04dY", tm.tm_mday, tm.tm_mon + 1, tm.tm_year + 1900);
            break;
        }
        ...
        case OSD_DATE_FORMAT_YYMMDDWW1: {
            const wchar_t *wday[] = {L"SUN", L"MON", L"TUS", L"WEN", L"THU", L"FRI", L"SAT"};
...
std::pair<time_t, AX_U32> CElapsedTimer::GetTimeTValEx(AX_S32 nDate, AX_S32 nTime, AX_S32 nTick) {
    AX_CHAR szDateTime[64] = {0};
    sprintf(szDateTime, "%04d-%02d-%02d %02d:%02d:%02d", nDate / 10000, (nDate % 10000) / 100, nDate % 100, nTime / 10000, (nTime % 10000) / 100, nTime % 100);
    return std::make_pair<AX_U32, AX_U32>(StringToDatetime(szDateTime), nTick);
}