...
AX_BOOL CVideoDecoder::ResetGrp(AX_VDEC_GRP vdGrp, AX_S32 nTryCount /* = AICARD_VDEC_RESET_GRP_TRY_COUNT*/) {
    ...
    while (nIndex++ < nTryCount) {
        ...
        if (0 != ret) {
            if (AX_ERR_VDEC_BUSY == ret) {
                LOG_M_W(VDEC, "[%d] vdGrp %d is busy, try again to reset", nIndex, vdGrp);
            } else {
                LOG_M_E(VDEC, "[%d] Try AX_VDEC_ResetGrp(vdGrp %d) fail, ret = 0x%x", nIndex, vdGrp, ret);
            }
    ...
    if (AX_SUCCESS != ret) {
        LOG_M_E(VDEC, "AX_VDEC_ResetGrp(vdGrp %d) failed after try %d times, ret = 0x%x", vdGrp, nTryCount, ret);
        return AX_FALSE;
    }

    LOG_M_N(VDEC, "vdGrp %d is reset", vdGrp);
