...
static void continueAfterDESCRIBE(RTSPClient *rtspClient, int resultCode, char *resultString) {
    ...
    /* An unrecoverable error occurred with this stream.
       ...
       - if network is linked down, sendDescribeCommand fail by setup socket, so continueAfterDESCRIBE is NOT invoked, rtspClient remain not
       deleted That makes confused  to distinguish when to invoke CAXRTSPClient::Stop(), so delete rtspClient by CAXRTSPClient::Stop()
       explicitly
    */
    ...
    if (PT_H264 == m_ePayload) {
        ...
        if (H264_NAL_SEI == naluType) {
            ...
            if (m_cacheBufLen + frameSize > m_cacheBufCap) {
                dumpCacheBuf(m_frameBuf, frameSize);
                envir() << "rtsp cache 264 buffer is not enough, realloc from " << m_nReceiveBufferSize;
            ...
            if (m_cacheBufLen > 0) {
                /* normal seq: SPS -> PPS -> IDR
                   but abnormal seq would be SPS -> PPS -> non-IDR, abandon VPS + SPS + PPS to avoid reallocBuf */
                m_cacheBufLen = 0;
            }
        ...
        if (HEVC_NAL_SEI_PREFIX == naluType || HEVC_NAL_SEI_SUFFIX == naluType) {
            ...
            if (m_cacheBufLen + frameSize > m_cacheBufCap) {
                dumpCacheBuf(m_frameBuf, frameSize);
                envir() << "rtsp cache 265 buffer is not enough, realloc from " << m_nReceiveBufferSize;
                reallocBuf(m_frameBuf, frameSize);
                envir() << " to " << m_nReceiveBufferSize << "\n";
            } else {
                memmove(&m_cacheBuf[m_cacheBufLen], m_frameBuf, frameSize);
            ...
            if (m_cacheBufLen > 0) {
                /* normal seq: VPS -> SPS -> PPS -> IDR
                   but abnormal seq would be VPS -> SPS -> PPS -> non-IDR, abandon VPS + SPS + PPS to avoid reallocBuf */
                m_cacheBufLen = 0;
            }
...
Boolean DummySink::continuePlaying() {
    ...
    fSource->getNextFrame(&m_frameBuf[NALU_START_CODE_LENGTH], m_frameBufCap - NALU_START_CODE_LENGTH, afterGettingFrame, this,
...
void DummySink::dumpCacheBuf(unsigned char *buf, unsigned int sz) {
    timeval tv;
    struct tm t;
    ...
    char fpath[128];
    sprintf(fpath, "./nalu%d_%p_%02u_%02u_%02u_%02u_%02u.%d.%d.cached", m_ePayload, this, t.tm_mon + 1, t.tm_mday, t.tm_hour, t.tm_min,
            t.tm_sec, m_cacheBufLen, sz);
    std::ofstream ofs(fpath, std::ofstream::out | std::ofstream::binary);
    if (!ofs) {
        envir() << "open " << fpath << " fail\n";
        return;
    }
    ofs.write((const char *)m_cacheBuf, m_cacheBufLen);
    ofs.write((const char *)buf, sz);
    ofs.close();

    envir() << fpath << " has been dumped\n";
}