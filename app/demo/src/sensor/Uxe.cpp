...
AX_BOOL CUxe::Init(AX_VOID) {
    ...
    ret = AX_VIN_Init();
    if (0 != ret) {
        LOG_MM_E(SENSOR, "AX_VIN_Init fail, ret = 0x%x", ret);
        return AX_FALSE;
    }

    ret = AX_MIPI_RX_Init();
    if (0 != ret) {
        LOG_MM_E(SENSOR, "AX_MIPI_RX_Init fail, ret = 0x%x", ret);
        return AX_FALSE;
    }
...
AX_BOOL CUxe::Open() {
    LOG_MM_I(SENSOR, "[Dev:%d] +++", m_tSnsCfg.nDevID);

    LOG_MM(SENSOR, "UXE => w: %d h: %d fps: %.2f snsMode: %d rawType: %d", m_tSnsAttr.nWidth, m_tSnsAttr.nHeight, m_tSnsAttr.fFrameRate,
           m_tSnsAttr.eSnsMode, m_tSnsAttr.eRawType);
    ...
    switch (m_eImgFormat) {
        ...
        case AX_FORMAT_YUV422_INTERLEAVED_YUYV: /* mipi 4 lane */
            ...
            break;
        default:
            LOG_MM_E(SENSOR, "unsupport pixel fmt %d", m_eImgFormat);
            return AX_FALSE;
    }

    AX_U8 nDevID = m_tSnsCfg.nDevID;

    if (m_nMipiDev < MAX_MIPI_RX_DEV_ID_BUTT) {
        nMipiDev = m_nMipiDev;
    }

    LOG_MM(SENSOR, "UXE => [sif %d mipi %d] w: %d h: %d fps: %.2f snsMode: %d rawType: %d", nDevID, nMipiDev, m_tSnsAttr.nWidth,
           m_tSnsAttr.nHeight, m_tSnsAttr.fFrameRate, m_tSnsAttr.eSnsMode, m_tSnsAttr.eRawType);

    // MIPI RX
    ret = AX_MIPI_RX_SetLaneCombo(emode);
    if (0 != ret) {
        LOG_MM_E(SENSOR, "AX_MIPI_RX_SetLaneCombo fail, ret = 0x%x", ret);
        return AX_FALSE;
    }

    ret = AX_MIPI_RX_SetAttr(nMipiDev, &m_tMipiRxDev);
    if (0 != ret) {
        LOG_MM_E(SENSOR, "AX_MIPI_RX_SetAttr fail, ret = 0x%x", ret);
        return AX_FALSE;
    }

    ret = AX_MIPI_RX_Reset(nMipiDev);
    if (0 != ret) {
        LOG_MM_E(SENSOR, "AX_MIPI_RX_Reset fail, ret = 0x%x", ret);
        return AX_FALSE;
    }

    ret = AX_MIPI_RX_Start(nMipiDev);
    if (0 != ret) {
        LOG_MM_E(SENSOR, "AX_MIPI_RX_Start fail, ret = 0x%x", ret);
        return AX_FALSE;
    }

    // DEV
    ret = AX_VIN_CreateDev(nDevID, &m_tDevAttr);
    if (0 != ret) {
        LOG_MM_E(SENSOR, "AX_VIN_CreateDev fail, ret = 0x%x", ret);
        return AX_FALSE;
    }

    ret = AX_VIN_SetDevAttr(nDevID, &m_tDevAttr);
    if (0 != ret) {
        LOG_MM_E(SENSOR, "AX_VIN_SetDevAttr fail, ret = 0x%x", ret);
        return AX_FALSE;
    }
    ...
    ret = AX_VIN_SetDevBindPipe(nDevID, &tDevBindPipe);
    if (0 != ret) {
        LOG_MM_E(SENSOR, "AX_VIN_SetDevBindPipe fail, ret = 0x%x", ret);
        return AX_FALSE;
    }

    ret = AX_VIN_SetDevBindMipi(nDevID, nMipiDev);
    if (0 != ret) {
        LOG_MM_E(SENSOR, "AX_VIN_SetDevBindMipi fail, ret = 0x%x", ret);
        return AX_FALSE;
    }

    /* configure the attribute of early reporting of frame interrupts */
    if (AX_VIN_DEV_WORK_MODE_1MULTIPLEX < m_tDevAttr.eDevWorkMode) {
        ret = AX_VIN_SetDevFrameInterruptAttr(nDevID, &m_tDevFrmIntAttr);
        if (0 != ret) {
            LOG_MM_E(SENSOR, "AX_VIN_SetDevFrameInterruptAttr fail, ret = 0x%x", ret);
            return AX_FALSE;
        }
    }
    ...
    if (AX_VIN_DEV_OFFLINE == m_tDevAttr.eDevMode) {
        ...
        if (0 != ret) {
            LOG_MM_E(SENSOR, "AX_VIN_SetDevDumpAttr fail, ret = 0x%x", ret);
            return AX_FALSE;
        }
    }

    ret = AX_VIN_EnableDev(nDevID);
    if (0 != ret) {
        LOG_MM_E(SENSOR, "AX_VIN_EnableDev fail, ret = 0x%x", ret);
        return AX_FALSE;
    }

    LOG_MM_I(SENSOR, "[%d] ---", m_tSnsCfg.nDevID);
...
AX_BOOL CUxe::Close() {
    ...
    if (0 != ret) {
        LOG_MM_E(SENSOR, "AX_VIN_DisableDev fail, ret = 0x%x", ret);
        return AX_FALSE;
    }
    ...
    if (0 != ret) {
        LOG_MM_E(SENSOR, "AX_MIPI_RX_Stop fail, ret = 0x%x", ret);
        return AX_FALSE;
    }

    ret = AX_VIN_DestroyDev(nDevID);
    if (0 != ret) {
        LOG_MM_E(SENSOR, "AX_VIN_DestroyDev fail, ret = 0x%x", ret);
        return AX_FALSE;
    }
...
AX_BOOL CUxe::DeInit(AX_VOID) {
    ...
    ret = AX_VIN_Deinit();
    if (0 != ret) {
        LOG_MM_E(SENSOR, "AX_VIN_Deinit failed, ret=0x%x", ret);
        return AX_FALSE;
    }

    ret = AX_MIPI_RX_DeInit();
    if (0 != ret) {
        LOG_MM_E(SENSOR, "AX_MIPI_RX_DeInit failed, ret=0x%x", ret);
        return AX_FALSE;
    }
...
AX_VOID CUxe::InitSnsAttr() {
    ...
    m_tSnsAttr.eSnsMode = AX_SNS_LINEAR_MODE;
...
AX_VOID CUxe::InitMipiRxAttr() {
    ...
    if (m_eImgFormat == AX_FORMAT_BAYER_RAW_8BPP) {
    ...
    } else if (m_eImgFormat == AX_FORMAT_YUV422_SEMIPLANAR_10BIT_P010) {
        m_tMipiRxDev.tMipiAttr.eLaneNum = AX_MIPI_DATA_LANE_8;
        if (60 == m_nFps) {
        ...
        if (m_nW == 1920 && m_nH == 1080) {
            m_tMipiRxDev.tMipiAttr.nDataRate = 713;
        }
    }
...
AX_VOID CUxe::InitDevAttr() {
    ...
    if (m_eImgFormat == AX_FORMAT_BAYER_RAW_8BPP) {
        ...
        m_tDevAttr.nConvFactor = 5;

        m_tDevAttr.eDevWorkMode = AX_VIN_DEV_WORK_MODE_4MULTIPLEX;
        m_tDevFrmIntAttr.bImgRgnIntEn[0] = AX_TRUE;
        m_tDevFrmIntAttr.bImgRgnIntEn[1] = AX_TRUE;
        m_tDevFrmIntAttr.bImgRgnIntEn[2] = AX_TRUE;
        m_tDevFrmIntAttr.bImgRgnIntEn[3] = AX_TRUE;

        if (m_eImgFormat == AX_FORMAT_YUV422_SEMIPLANAR_10BIT_P010) {
            ...
            m_tDevAttr.tMipiIntfAttr.szImgDt[1] = AX_MIPI_CSI_DT_YUV422_10BIT;
            m_tDevAttr.tMipiIntfAttr.szImgDt[2] = AX_MIPI_CSI_DT_YUV422_10BIT;
            m_tDevAttr.tMipiIntfAttr.szImgDt[3] = AX_MIPI_CSI_DT_YUV422_10BIT;
        } else if (m_eImgFormat == AX_FORMAT_YUV422_INTERLEAVED_YUYV) {
            m_tDevAttr.tMipiIntfAttr.szImgVc[0] = 0;
        ...
        m_tDevAttr.tDevImgRgn[0].nStartX = 0;
        m_tDevAttr.tDevImgRgn[0].nStartY = 0;
        m_tDevAttr.tDevImgRgn[0].nWidth = m_nW;
        m_tDevAttr.tDevImgRgn[0].nHeight = m_nH / 4;
        if (m_eImgFormat == AX_FORMAT_YUV422_SEMIPLANAR_10BIT_P010) {
            m_tDevAttr.nWidthStride[0] = m_nW * 2;
        } else {
            m_tDevAttr.nWidthStride[0] = m_nW;
        }

        m_tDevAttr.tDevImgRgn[1].nStartX = 0;
        m_tDevAttr.tDevImgRgn[1].nStartY = m_nH / 4 * 1;
        m_tDevAttr.tDevImgRgn[1].nWidth = m_nW;
        m_tDevAttr.tDevImgRgn[1].nHeight = m_nH / 4;
        if (m_eImgFormat == AX_FORMAT_YUV422_SEMIPLANAR_10BIT_P010) {
            m_tDevAttr.nWidthStride[1] = m_nW * 2;
        } else {
            m_tDevAttr.nWidthStride[1] = m_nW;
        }

        m_tDevAttr.tDevImgRgn[2].nStartX = 0;
        m_tDevAttr.tDevImgRgn[2].nStartY = m_nH / 4 * 2;
        m_tDevAttr.tDevImgRgn[2].nWidth = m_nW;
        m_tDevAttr.tDevImgRgn[2].nHeight = m_nH / 4;
        if (m_eImgFormat == AX_FORMAT_YUV422_SEMIPLANAR_10BIT_P010) {
            m_tDevAttr.nWidthStride[2] = m_nW * 2;
        } else {
            m_tDevAttr.nWidthStride[2] = m_nW;
        }

        m_tDevAttr.tDevImgRgn[3].nStartX = 0;
        m_tDevAttr.tDevImgRgn[3].nStartY = m_nH / 4 * 3;
        m_tDevAttr.tDevImgRgn[3].nWidth = m_nW;
        m_tDevAttr.tDevImgRgn[3].nHeight = m_nH / 4;
        if (m_eImgFormat == AX_FORMAT_YUV422_SEMIPLANAR_10BIT_P010) {
            m_tDevAttr.nWidthStride[3] = m_nW * 2;
        } else {
            m_tDevAttr.nWidthStride[3] = m_nW;
        }
    }
...
AX_VOID CUxe::InitPipeAttr() {
    ...
    for (AX_U8 i = 0; i < m_tSnsCfg.nPipeCount; i++) {
        ...
        tPipeAttr.ePipeWorkMode = AX_VIN_PIPE_NORMAL_MODE0;
...
AX_VOID CUxe::InitChnAttr() {
    ...
    for (AX_U8 i = 0; i < m_tSnsCfg.nPipeCount; i++) {
        ...
        AX_VIN_CHN_ATTR_T arrChnAttr[AX_VIN_CHN_ID_MAX] = {{.nWidth = m_nW,   //(AX_U32)tPipeAttr.arrChannelAttr[0].nWidth,   // 3840
