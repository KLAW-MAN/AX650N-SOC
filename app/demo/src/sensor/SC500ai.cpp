...
AX_SNS_HDR_MODE_E CSC500ai::GetMaxHdrMode() {
...
AX_VOID CSC500ai::InitSnsLibraryInfo(AX_VOID) {
    m_tSnsLibInfo.strLibName = "libsns_sc500ai.so";
    m_tSnsLibInfo.strObjName = "gSnssc500aiObj";
}
...
AX_VOID CSC500ai::InitMipiRxAttr() {
    ...
    m_tMipiRxDev.tMipiAttr.nDataRate = (AX_SNS_LINEAR_MODE == m_tSnsCfg.eSensorMode) ? 396 : 810;
...
AX_VOID CSC500ai::InitDevAttr() {
    ...
    for (AX_U8 i = 0; i < AX_HDR_CHN_NUM; i++) {
        m_tDevAttr.tDevImgRgn[i] = {0, 0, 2880, 1620};
    }
...
AX_VOID CSC500ai::InitPipeAttr() {
    ...
    for (AX_U8 i = 0; i < m_tSnsCfg.nPipeCount; i++) {
        ...
        tPipeAttr.tPipeImgRgn = {0, 0, 2880, 1620};
        tPipeAttr.eBayerPattern = AX_BP_RGGB;
        tPipeAttr.nWidthStride = 2880;
...
AX_VOID CSC500ai::InitTriggerAttr() {
}