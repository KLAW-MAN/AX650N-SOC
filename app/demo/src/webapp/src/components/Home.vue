      ...
      <el-header>
        ...
        <div class="header_nav">
          <el-menu router :default-active="activePath" @select="handleSelect" mode="horizontal" background-color="#4c4c4c" text-color="#fff" active-text-color="#fff">
            <el-menu-item index="/preview" @click="saveNavState('/preview')" style="height: 50px;line-height: 50px;">Preview</el-menu-item>
            <el-menu-item index="/setting" @click="saveNavState('/setting')" style="height: 50px;line-height: 50px;">Configure</el-menu-item>
          </el-menu>
        </div>
        <div class="header_right" align="right">
            ...
            <span>admin</span>
            <el-tooltip effect="dark" content="Logout" placement="bottom-end" :enterable="false">
              <el-button type="text" @click="logout" icon="el-icon-switch-button"></el-button>
            </el-tooltip>
        </div>
      </el-header>
      <el-main class="main-container">
        <router-view :mode_option="mode_option_data" :home_dual_mode="sns_mode" :pano_sns_id="pano_snsId" :sensor_select="sns_Select"></router-view>
      </el-main>
    </el-container>
  </div>
</template>
...
export default {
  data () {
    return {
      ...
      sns_Select: parseInt(window.sessionStorage.getItem('snsSelect')),
      isForceStop: false
    }
  },
  created () {
    ...
    baseUrl = baseUrl.substring(0, baseUrl.indexOf('/#/') + 1) + 'action'
