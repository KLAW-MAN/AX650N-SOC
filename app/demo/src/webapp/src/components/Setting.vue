        ...
        <div class="left-container">
          <el-menu router :default-active="activePath" class="setting_menu" background-color="#4c4c4c" text-color="#fff" active-text-color="#ffd04b">
            ...
            <el-menu-item index="/setting/overlay" @click="saveNavState('/setting/overlay')" v-show="formData.capInfo.support_page_overlay > 0">
              ...
              <span slot="title">OSD</span>
            </el-menu-item>
            <el-menu-item index="/setting/record" @click="saveNavState('/setting/record')" v-show="formData.capInfo.support_page_record > 0">
            ...
            <el-menu-item index="/setting/playback" @click="saveNavState('/setting/playback')" v-show="formData.capInfo.support_page_playback > 0">
              ...
              <span slot="title">Replay</span>
            </el-menu-item>
          </el-menu>
        </div>
      </el-aside>
      <el-main>
        <router-view :setting_sns_count="formData.capInfo.sns_count" :img_multi_mode="formData.capInfo.img_page_support_multi_sns"></router-view>
      </el-main>
    </el-container>
  </div>
</template>
    ...
    async getInfo () {
      try {
        ...
        if (res.meta.status === 200) {
          this.formData.capInfo.sns_count = res.data.capInfo.sns_count
          this.formData.capInfo.img_page_support_multi_sns = res.data.capInfo.img_page_support_multi_sns
          ...
          this.formData.capInfo.support_page_record = res.data.capInfo.support_page_record
