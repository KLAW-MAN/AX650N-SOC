  ...
  <el-container class="playback_container">
    <el-main class="playback_view_left_container">
      <el-container class="preview_container">
        <el-main ref="myMain" class="video_container">
          <video id="myVideo" class="frtVideo" controls="control" muted playsinline oncontextmenu="return false;">
          </video>
        </el-main>
      </el-container>
    </el-main>
    <el-aside class="panel_container" width="350px">
      <div class="panel_view">
        <p class="notice" v-show="play_list.length == 0">No Record File</p>
        <div class="">
          <input id="chkAll" type="checkbox" style="margin-left: 10px" @change="selectAll()">Select All&nbsp;
          <el-button type="primary" size="small" icon="el-icon-delete"  @click="removeSelected()"></el-button>
          <el-button type="primary" size="small" icon="el-icon-download" @click="downloadSelected()"></el-button>
        </div>
        <div :for="index" v-for="(item, index) in play_list" :key="index" :class="{'record_normal':!item.focus, 'record_play':item.focus}">
          ...
          <el-button type="info" size="mini" class="download_btn" icon="el-icon-download" @click="download(index)"></el-button>
        </div>
      </div>
    </el-aside>
  </el-container>
  ...
  methods: {
    ...
    entry_record_files() {
      ...
      window.webkitRequestFileSystem(window.PERSISTENT, this.rec_threshold,
        function (fs) {
          _self._fs = fs;
          fs.root.getDirectory('frt_video', {create: true}, function(dirEntry) {
            ...
            dirReader.readEntries(function(entries) {
              ...
              if (entries.length > 0) {
                ...
                for(var i = 0; i < entries.length; i++) {
                  var entry = entries[i];
                  if (entry.isFile){
                    fs.root.getFile('frt_video/' + entry.name, {}, function(fileEntry) {
    ...
    load_record_file(fileNm, bPlay=false) {
      ...
      this._fs.root.getFile('frt_video/' + fileNm, {}, function(fileEntry) {
    ...
    download(index) {
      var _self = this;
      if (index < this.play_list.length) {
        this._fs.root.getFile('frt_video/' + this.play_list[index].name, {}, function(fileEntry) {
          fileEntry.file(function (file) {
    ...
    removeSelected() {
      ...
      if (confirm('Are you sure you want to delete the file?') == true) {
        ...
        for (var _file of _sel) {
          this._fs.root.getFile('frt_video/' + _file, {}, function(fileEntry) {
            fileEntry.remove(function() {
              console.log(fileEntry.toURL() + ' is removed');
            });
          }, _self.errorHandler);
