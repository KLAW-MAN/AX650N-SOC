...
AX_ULONG AX_OSAL_DEV_usr_virt_to_phys(AX_ULONG virt)
{
	...
	if (pgd_none(*pgd)) {
		printk("no mapped in pg,dvirt addr 0x%08lx\n", virt);
		return 0;
	}

	p4d = p4d_offset(pgd, virt);
	if (p4d_none(*p4d)) {
		printk("no mapped in p4d,dvirt addr 0x%08lx\n", virt);
		return 0;
	}

	pud = pud_offset(p4d, virt);
	if (pud_none(*pud)) {
		printk("not mapped in pud! addr 0x%08lx\n", virt);
		return 0;
	}

	pmd = pmd_offset(pud, virt);
	if (pmd_none(*pmd)) {
		printk("not mapped in pmd! addr 0x%08lx\n", virt);
		return 0;
	}

	pte = pte_offset_map(pmd, virt);
	if (pte_none(*pte)) {
		printk("not mapped in pte!addr 0x%08lx\n", virt);
		pte_unmap(pte);
		return 0;
	}
