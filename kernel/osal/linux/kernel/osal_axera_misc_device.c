/*
 * linux/drivers/char/axdev.c
 *
 * AXERA driver base frameforwk
 * Changed for axera devices
 */
...
//static AX_PROC_DIR_ENTRY_T *proc_dir_entry = NULL;
/*
...
int axdev_register(struct axdev_device *axdev)
{
	...
	if (err < 0) {
		ax_dev_minors[axdev->minor >> 3] &= ~(1 << (axdev->minor & 7));
		printk
		    ("Func[%s], ERROR, Failed to register device of axdev [minor id=%d]\n",
		     __func__, axdev->minor);
		goto out;
	}
	/*step 5/7 register driver of axdev-device */
	pdrv =
	    axdev_driver_register(axdev->devfs_name, axdev->owner,
				  axdev->drvops);
	if (IS_ERR(pdrv)) {
		...
		err = PTR_ERR(pdrv);
		printk
		    ("Func[%s], ERROR, Failed to register driver of axdev [minor id=%d]\n",
		     __func__, axdev->minor);
		goto out;
	}
