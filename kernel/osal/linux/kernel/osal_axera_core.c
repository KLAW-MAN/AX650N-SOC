			     ...
			     char *buf)
{
	...
	int len = snprintf(buf, PAGE_SIZE, "axdev:%s\n", pdev->devfs_name);
...
/*
static struct device_attribute axdev_dev_attrs[] = {
    __ATTR_RO(modalias),
    __ATTR_NULL,
};
*/
...
{
	struct axdev_device *pdev = to_axdev_device(dev);
	return (strncmp(pdev->devfs_name, drv->name, sizeof(pdev->devfs_name))
		== 0);
}
...
int axdev_device_register(struct axdev_device *pdev)
{
	/*
	   ...
	   pdev->device.devt    = MKDEV(axdev_DEVICE_MAJOR, pdev->minor);
	...
	if (rval) {
		AXDEV_LOG_DEBUG("failed to get dev region for %s.\n",
				pdev->devfs_name);
		return rval;
	}
	...
	if (rval) {
		AXDEV_LOG_ERR("cdev_add failed for %s, error = %d.\n",
	...
	//pdev->device.class = pdev->axdev_class;
	pdev->device.parent = NULL;
	dev_set_drvdata(&pdev->device, NULL);
	//dev_set_drvdata(&pdev->device, pdev);
	...
	if (rval) {
		AXDEV_LOG_ERR("device_register failed for %s, error = %d.\n",
			      pdev->devfs_name, rval);
		goto FAIL1;
	}
...
struct axdev_driver *axdev_driver_register(const char *name,
					   struct module *owner,
					   struct axdev_ops *pmops)
{
	int ret;
	...
	pdrv =
	    kzalloc(sizeof(struct axdev_driver) +
		    strnlen(name, HIMIDIA_MAX_DEV_NAME_LEN), GFP_KERNEL);
	if (!pdrv)
	...
	if (ret) {
		AXDEV_LOG_ERR("Error, Failed to register driver[%s] \n", name);
		kfree(pdrv);
		return ERR_PTR(ret);
	}
