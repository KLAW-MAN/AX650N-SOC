...
static int AX_OSAL_DEV_comon_target(struct device *dev, unsigned long *freq, u32 flags)
{
	...
	((struct AX_DEVFREQ_DEV_PROFILE *)pdev->axera_devdfs_ptr)->target((AX_VOID *) dev, rate, volt);
	dev_pm_opp_put(opp);
	return 0;
}
...
{
	struct platform_device *pdev = to_platform_device(dev);
	*freq = ((struct AX_DEVFREQ_DEV_PROFILE *)pdev->axera_devdfs_ptr)->get_cur_freq((AX_VOID *) dev);
	return 0;
}
...
AX_S32 AX_OSAL_DEV_devm_devfreq_add_device(AX_VOID * pdev, struct AX_DEVFREQ_DEV_PROFILE *ax_profile,
