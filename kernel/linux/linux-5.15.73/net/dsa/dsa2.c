...
static int dsa_port_setup(struct dsa_port *dp)
{
	...
	if (dp->gpiod_reset != NULL) {
		gpiod_direction_output(dp->gpiod_reset, 0);

		/* For a complete PHY reset, must be asserted low for at least 10ms. */
		gpiod_set_value_cansleep(dp->gpiod_reset, 0);
		msleep(15);
		gpiod_set_value_cansleep(dp->gpiod_reset, 1);
		msleep(75);
	}
				     ...
				     struct device_node *dn)
{
	...
	for_each_available_child_of_node(ports, port) {
		...
		gpiod_reset = devm_gpiod_get_from_of_node(ds->dev, port,
						    "phy-rst-gpio",
						    0, GPIOD_OUT_LOW,
						    "phy_reset");
		if (PTR_ERR_OR_ZERO(gpiod_reset)) {
			dev_err(ds->dev,
				"Request failed for reset GPIO: %d\n", err);
			dp->gpiod_reset = NULL;
		} else {
			dp->gpiod_reset = gpiod_reset;
		}
	}
