...
#ifndef _AX_KERNEL_PRINTK_RINGBUFFER_H
#define _AX_KERNEL_PRINTK_RINGBUFFER_H
 ...
 */
struct ax_printk_info {
	u64	seq;		/* sequence number */
	u64	ts_nsec;	/* timestamp in nanoseconds */
	u16	text_len;	/* length of text message */
	u8	mid;		/* module id */
	u8	flags:5;	/* internal record flags */
	u8	level:3;	/* axsyslog level */
	u8      cpu_id;         /* cpu id*/
 ...
 *
 * Writers:
 * Using ax_prb_rec_init_wr(), a writer sets @text_buf_size before calling
 * prb_reserve(). On success, prb_reserve() sets @info and @text_buf to
 * buffers reserved for that writer.
 *
 * Readers:
 * Using ax_prb_rec_init_rd(), a reader sets all fields before calling
 * ax_prb_read_valid(). Note that the reader provides the @info and @text_buf,
...
struct ax_prb_desc {
	...
	struct ax_prb_data_blk_lpos	text_blk_lpos;
};

/* A ringbuffer of "ID + data" elements. */
struct ax_prb_data_ring {
...
/* A ringbuffer of "struct ax_prb_desc" elements. */
struct ax_prb_desc_ring {
	unsigned int		count_bits;
	struct ax_prb_desc		*descs;
 ...
 *        record was created.
 */
struct ax_printk_ringbuffer {
	struct ax_prb_desc_ring	desc_ring;
	struct ax_prb_data_ring	text_data_ring;
	atomic_long_t		fail;
};
 ...
 */
struct ax_prb_reserved_entry {
	struct ax_printk_ringbuffer	*rb;
...
enum ax_desc_state {
	ax_desc_miss		=  -1,	/* ID mismatch (pseudo state) */
	ax_desc_reserved	= 0x0,	/* reserved, in use by writer */
	ax_desc_committed	= 0x1,	/* committed by writer, could get reopened */
	ax_desc_finalized	= 0x2,	/* committed, no further modification allowed */
	ax_desc_reusable	= 0x3,	/* free, not yet used by any writer */
};
 ...
 *     descriptor in the array. But it allows ax_prb_read_valid() to correctly
...
#define DESC0_SV(ct_bits)	DESC_SV(DESC0_ID(ct_bits), ax_desc_reusable)

/*
 * Define a ringbuffer with an external text data buffer. The same as
 * AX_DEFINE_PRINTKRB() but requires specifying an external buffer for the
 * text data.
 *
 * Note: The specified external buffer must be of the size:
 *       2 ^ (descbits + avgtextbits)
 */
#define _AX_DEFINE_PRINTKRB(name, descbits, avgtextbits, text_buf)			\
static struct ax_prb_desc _##name##_descs[_DESCS_COUNT(descbits)] = {				\
...
static struct ax_printk_info _##name##_infos[_DESCS_COUNT(descbits)] = {				\
...
static struct ax_printk_ringbuffer name = {							\
	.desc_ring = {										\
		.count_bits	= descbits,							\
...
/**
 * AX_DEFINE_PRINTKRB() - Define a ringbuffer.
 *
 ...
 * such that it is ready for immediate use. See _AX_DEFINE_PRINTKRB() for a
 * variant where the text data buffer can be specified externally.
 */
#define AX_DEFINE_PRINTKRB(name, descbits, avgtextbits)				\
static char _##name##_text[1U << ((avgtextbits) + (descbits))]			\
			__aligned(__alignof__(unsigned long));			\
_AX_DEFINE_PRINTKRB(name, descbits, avgtextbits, &_##name##_text[0])

/* Writer Interface */

/**
 * ax_prb_rec_init_wr() - Initialize a buffer for writing records.
 *
 * @r:             The record to initialize.
 * @text_buf_size: The needed text buffer size.
 */
static inline void ax_prb_rec_init_wr(struct ax_printk_record *r,
				   unsigned int text_buf_size)
{
	r->info = NULL;
	r->text_buf = NULL;
...
void ax_prb_init(struct ax_printk_ringbuffer *rb,
	      ...
	      struct ax_prb_desc *descs, unsigned int descs_count_bits,
	      struct ax_printk_info *infos);
...
/**
 * ax_prb_rec_init_rd() - Initialize a buffer for reading records.
 *
 * @r:             The record to initialize.
 ...
 */
static inline void ax_prb_rec_init_rd(struct ax_printk_record *r,
				   struct ax_printk_info *info,
				   char *text_buf, unsigned int text_buf_size)
{
	r->info = info;
...
/**
 * ax_prb_for_each_record() - Iterate over the records of a ringbuffer.
 *
 ...
 * @r:    A ax_printk_record to store the record on each iteration.
 *
 ...
 * Context: Any context.
 */
#define ax_prb_for_each_record(from, rb, s, r) \
for ((s) = from; ax_prb_read_valid(rb, s, r); (s) = (r)->info->seq + 1)

/**
 * ax_prb_for_each_info() - Iterate over the meta data of a ringbuffer.
 *
 * @from: The sequence number to begin with.
 * @rb:   The ringbuffer to iterate over.
 * @s:    A u64 to store the sequence number on each iteration.
 * @i:    A ax_printk_info to store the record meta data on each iteration.
 ...
 * Context: Any context.
 */
#define ax_prb_for_each_info(from, rb, s, i, lc) \
for ((s) = from; ax_prb_read_valid_info(rb, s, i, lc); (s) = (i)->seq + 1)
