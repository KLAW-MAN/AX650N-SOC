...
static int ax_plane_atomic_check(struct drm_plane *plane, struct drm_atomic_state *state)
{
				   ...
				   struct drm_atomic_state *state)
{
	...
	DRM_DEBUG("[plane:%d:%s:%d]\n", plane->base.id, plane->name, plane->type);

	DRM_DEBUG("width = %d, height = %d, pitches[0, 1] = [%d, %d]\n",
		  fb->width, fb->height, fb->pitches[0], fb->pitches[1]);
	DRM_DEBUG("src_x = %d, src_y = %d, src_w = %d, src_h = %d\n",
		  new_state->src_x, new_state->src_y,
		  new_state->src_w, new_state->src_h);
	DRM_DEBUG("crtc_x = %d, crtc_y = %d, crtc_w = %d, crtc_h = %d\n",
	...
	DRM_DEBUG("type:%d, fmt:0x%x, stride: {%d, %d}, pddr: {0x%llx, 0x%llx}\n",
		  ax_plane->fb.type, ax_plane->fb.format,
		  ax_plane->fb.stride_y, ax_plane->fb.stride_c,
				 ...
				 struct drm_property *property, uint64_t val)
{
	...
	DRM_DEBUG("[plane:%d:%s %s:%llx]\n", plane->base.id, plane->name, property->name, val);
