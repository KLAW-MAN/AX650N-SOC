	...
	if (!(dp_funs && dp_funs->dpu_mem_alloc)) {
		...
		ret =
		    dp_funs->dpu_mem_alloc(dp_dev->data,
					   ...
					   "drm-gem");
		if (ret) {
			DRM_ERROR("failed to allocate %zu byte dma buffer from cmm\n",
				  obj->size);
			return -ENOMEM;
		}
	}
...
static const struct drm_gem_object_funcs ax_gem_object_funcs = {
	.free = ax_gem_free_object,
