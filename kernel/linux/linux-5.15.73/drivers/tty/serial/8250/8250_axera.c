...
#define PERIPH_GLOBAL_BASE		0x2002000
#define PERIPH_GLOBAL_LEN		0x10000
#define PERIPH_UART_MODE_OFFSET		0x20
...
static void ax8250_tx_wait_empty(struct uart_port *p)
{
	struct uart_8250_port *up = up_to_u8250p(p);
...
static void ax8250_serial_out38x(struct uart_port *p, int offset, int value)
{
	struct ax8250_data *d = to_ax8250_data(p->private_data);

	/* Allow the TX to drain before we reconfigure */
	if (offset == UART_LCR)
		ax8250_tx_wait_empty(p);
...
#ifdef CONFIG_64BIT
static unsigned int ax8250_serial_inq(struct uart_port *p, int offset)
{
	unsigned int value;
...
static void ax8250_serial_outq(struct uart_port *p, int offset, int value)
{
	struct ax8250_data *d = to_ax8250_data(p->private_data);
...
static void ax8250_serial_out32be(struct uart_port *p, int offset, int value)
{
	struct ax8250_data *d = to_ax8250_data(p->private_data);
...
static unsigned int ax8250_serial_in32be(struct uart_port *p, int offset)
{
...
static void ax8250_clk_work_cb(struct work_struct *work)
{
	struct ax8250_data *d = work_to_ax8250_data(work);
...
static int ax8250_clk_notifier_cb(struct notifier_block *nb,
				  unsigned long event, void *data)
{
	struct ax8250_data *d = clk_to_ax8250_data(nb);

	/*
	 * We have no choice but to defer the uartclk update due to two
	 * deadlocks. First one is caused by a recursive mutex lock which
	 * happens when clk_set_rate() is called from ax8250_set_termios().
	 * Second deadlock is more tricky and is caused by an inverted order of
	 * the clk and tty-port mutexes lock. It happens if clock rate change
	 * is requested asynchronously while set_termios() is executed between
	 * tty-port mutex lock and clk_set_rate() function invocation and
	 * vise-versa. Anyway if we didn't have the reference clock alteration
	 * in the ax8250_set_termios() method we wouldn't have needed this
...
static bool ax8250_idma_filter(struct dma_chan *chan, void *param)
{
...
static void ax8250_quirks(struct uart_port *p, struct ax8250_data *data)
{
	if (p->dev->of_node) {
		...
		if (of_device_is_compatible(np, "cavium,octeon-3860-uart")) {
			p->serial_in = ax8250_serial_inq;
			p->serial_out = ax8250_serial_outq;
		...
		if (of_device_is_big_endian(p->dev->of_node)) {
			...
			p->serial_in = ax8250_serial_in32be;
			p->serial_out = ax8250_serial_out32be;
		}
		if (of_device_is_compatible(np, "marvell,armada-38x-uart"))
			p->serial_out = ax8250_serial_out38x;
	...
	if (platform_get_resource_byname(to_platform_device(p->dev),
		...
		data->data.dma.fn = ax8250_idma_filter;
	}
}
...
static int ax8250_probe(struct platform_device *pdev)
{
	...
	if (!err) {
		...
		void __iomem *reg = ioremap(PERIPH_GLOBAL_BASE, PERIPH_GLOBAL_LEN);
	...
	INIT_WORK(&data->clk_work, ax8250_clk_work_cb);
	data->clk_notifier.notifier_call = ax8250_clk_notifier_cb;
...
static const struct acpi_device_id ax8250_acpi_match[] = {
	{ "INT33C4", 0 },
	{ "INT33C5", 0 },
	{ "INT3434", 0 },
	{ "INT3435", 0 },
...
MODULE_DEVICE_TABLE(acpi, ax8250_acpi_match);
...
static struct platform_driver ax8250_platform_driver = {
	.driver = {
		...
		.acpi_match_table = ax8250_acpi_match,
	},
	.probe			= ax8250_probe,
