...
static int rtl9010_modify_changed(struct phy_device *phydev, u32 regnum, u16 mask, u16 set)
{
	...
	ret = phy_write(phydev, 0x1b, regnum);
	if (ret < 0)
		return ret;
	ret = phy_modify_changed(phydev, 0x1c, mask, set);
...
static int rtl9010_config_init(struct phy_device *phydev)
{
...
#ifdef CONFIG_DWMAC_AXERA
		/* enable rgmii tx delay, 2ns */
		val_txdly = RTL9010_TX_DELAY;
		val_rxdly = RTL9010_RX_DELAY;
#else
		val_txdly = 0;
		val_rxdly = 0;
#endif
	...
	ret |= rtl9010_modify_changed(phydev, 0xd084, BIT(14) | BIT(15), val_txdly);
	...
	ret = rtl9010_modify_changed(phydev, 0xd04a, RTL9010_RX_DELAY, val_rxdly);
	if (ret < 0) {
		dev_err(dev, "Failed to update the RX delay register\n");
		return ret;
	} else if (ret) {
	...
	if (ret < 0) {
		dev_err(dev, "Failed to close spread spectrum\n");
		return ret;
	}
...
static int rtl9010_config_aneg(struct phy_device *phydev)
{
	int ret;
...
static int rtl9010_read_status(struct phy_device *phydev)
{
	...
	phydev->duplex = DUPLEX_FULL;
	ret = phy_read_paged(phydev, 0xa60, 0x10) & 0xff;
	if (ret == 0x44) {
		phydev->speed = SPEED_100;
	} else if (ret == 0x37) {
		phydev->speed = SPEED_1000;
...
static int rtl9010_config_intr(struct phy_device *phydev)
{
	u16 val;
	int err;
	...
	return phy_write_paged(phydev, 0xa42, RTL9000A_GINER, val);
}
...
static int rtl9010_probe(struct phy_device *phydev)
{
	...
	if (of_property_read_bool(dev->of_node, "realtek,as-master")) {
...
static struct phy_driver realtek_drvs[] = {
	{
		...
		.name		= "RTL9010 Ethernet",
		.features       = PHY_GBIT_FEATURES,
		.probe		= rtl9010_probe,
		.config_init	= rtl9010_config_init,
		.config_aneg	= rtl9010_config_aneg,
		.read_status	= rtl9010_read_status,
		.config_intr	= rtl9010_config_intr,
		.suspend	= genphy_suspend,
		.resume		= genphy_resume,
		.read_page	= rtl821x_read_page,
		.write_page	= rtl821x_write_page,
