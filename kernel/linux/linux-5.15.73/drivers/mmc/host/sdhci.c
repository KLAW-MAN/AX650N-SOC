	...
	struct mmc_data *data, int sg_count)
{
	...
	for_each_sg(data->sg, sg, host->sg_count, i) {
		...
		while (len > host->max_adma) {
			int n = 32 * 1024; /* 32KiB*/
			if (((addr & (SDHCI_ADMA_BOUNDARY_SIZE - 1)) + n) > SDHCI_ADMA_BOUNDARY_SIZE) {
				offset = SDHCI_ADMA_BOUNDARY_SIZE - (addr & (SDHCI_ADMA_BOUNDARY_SIZE - 1));
				__sdhci_adma_write_desc(host, &desc, addr, offset, ADMA2_TRAN_VALID);
				addr += offset;
				n -= offset;
			}
		...
		/* tran, valid */
		if (len) {
			if (((addr & (SDHCI_ADMA_BOUNDARY_SIZE - 1)) + len) > SDHCI_ADMA_BOUNDARY_SIZE) {
				offset = SDHCI_ADMA_BOUNDARY_SIZE - (addr & (SDHCI_ADMA_BOUNDARY_SIZE - 1));
				__sdhci_adma_write_desc(host, &desc, addr, offset, ADMA2_TRAN_VALID);
				addr += offset;
				len -= offset;
