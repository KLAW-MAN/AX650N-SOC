...
static inline void axi_chan_disable(struct axi_dma_chan *chan)
{
	...
	spin_lock_irqsave(&chan->chip->reg_lock, flags);
	val = axi_dma_ioread32(chan->chip, DMAC_CHEN);
	val &= ~(BIT(chan->id) << DMAC_CHAN_EN_SHIFT);
	val |= BIT(chan->id) << DMAC_CHAN_EN_WE_SHIFT;
	axi_dma_iowrite32(chan->chip, DMAC_CHEN, val);
	spin_unlock_irqrestore(&chan->chip->reg_lock, flags);
}
...
static int axi_dma_hw_handshake_sel(struct axi_dma_chan *chan)
{
	u32 val, shift;
	...
	if (chan->id / 4) {
		val =
		    ioread32(chan->chip->regs_glb + COMM_SYS_DMA_FLASH_DW_SEL1);
		val &= ~(0x7f << shift);
		val |= ((chan->hw_handshake_num & 0x7f) << shift);
		iowrite32(val,
			  chan->chip->regs_glb + COMM_SYS_DMA_FLASH_DW_SEL1);
	} else {
		val =
		    ioread32(chan->chip->regs_glb + COMM_SYS_DMA_FLASH_DW_SEL0);
		val &= ~(0x7f << shift);
		val |= ((chan->hw_handshake_num & 0x7f) << shift);
		iowrite32(val,
			  chan->chip->regs_glb + COMM_SYS_DMA_FLASH_DW_SEL0);
	}
...
static int dma_chan_pause(struct dma_chan *dchan)
{
	...
	spin_lock_irqsave(&chan->vc.lock, flags);
	spin_lock(&chan->chip->reg_lock);
	atomic_set(&chan->in_use, 0);
	val = axi_dma_ioread32(chan->chip, DMAC_CHEN);
	val |= BIT(chan->id) << DMAC_CHAN_SUSP_SHIFT |
	    BIT(chan->id) << DMAC_CHAN_SUSP_WE_SHIFT;
	axi_dma_iowrite32(chan->chip, DMAC_CHEN, val);
	spin_unlock(&chan->chip->reg_lock);
	do {
...
static int dw_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR_OR_NULL(mem)) {
		dev_err(chip->dev, "platform_get_resource regs_glb\n");
		return -EINVAL;
	}

	chip->regs_glb = devm_ioremap(chip->dev, mem->start, resource_size(mem));
	if (IS_ERR(chip->regs_glb)) {
		dev_err(chip->dev, "devm_ioremap_resource regs_glb\n");
		return PTR_ERR(chip->regs_glb);
	}
...
subsys_initcall_sync(dw_dmac_init);
