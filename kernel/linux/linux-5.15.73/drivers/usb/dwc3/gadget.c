...
static int dwc3_gadget_resize_tx_fifos(struct dwc3_ep *dep)
{
	...
	if ((dep->endpoint.maxburst > 1 &&
		...
		num_fifos = 3;
        if (dep->dwc->gadget->speed >= USB_SPEED_SUPER
                &&usb_endpoint_xfer_isoc(dep->endpoint.desc)) {
            num_fifos = 5;
        }
...
static int __dwc3_gadget_ep_enable(struct dwc3_ep *dep, unsigned int action)
{
	...
	if (usb_endpoint_xfer_isoc(desc)) {
		uint32_t value = dwc3_readl(dwc->regs, DWC3_GTXFIFOPRIDEV);
		dwc3_writel(dwc->regs, DWC3_GTXFIFOPRIDEV, value | BIT((dep->number / 2 * 2) >> 1));
	}
...
static int __dwc3_gadget_ep_disable(struct dwc3_ep *dep)
{
	...
	reg = dwc3_readl(dwc->regs, DWC3_GTXFIFOPRIDEV);
	dwc3_writel(dwc->regs, DWC3_GTXFIFOPRIDEV, reg & ~BIT((dep->number / 2 * 2) >> 1));
