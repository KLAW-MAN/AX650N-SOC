...
void ax_pcie_write_dbi(struct pci_epc *epc, u32 reg, size_t size, u32 val)
{
	struct dw_pcie_ep *ep = epc_get_drvdata(epc);
	struct dw_pcie *pci = to_dw_pcie_from_ep(ep);

	dw_pcie_write_dbi(pci, reg, size, val);
}
EXPORT_SYMBOL(ax_pcie_write_dbi);

u32 ax_pcie_read_dbi(struct pci_epc *epc, u32 reg, size_t size)
{
	struct dw_pcie_ep *ep = epc_get_drvdata(epc);
	struct dw_pcie *pci = to_dw_pcie_from_ep(ep);

	return dw_pcie_read_dbi(pci, reg, size);
}
EXPORT_SYMBOL(ax_pcie_read_dbi);
#endif
