...
static void ax_pcie_ep_spin_lock(unsigned long *flags)
{
	spin_lock_irqsave(&s_shmem_data_lock, *flags);
}

static void ax_pcie_ep_spin_unlock(unsigned long *flags)
{
...
static void axera_pcie_reset_deassert(struct axera_pcie *axera)
{
	udelay(PCIE_PERST_DELAY_US);
	gpiod_set_value(axera->reset, 1);
}
...
static int axera_pcie_host_init(struct pcie_port *pp)
{
	...
	axera_pcie_reset_deassert(axera);
	return 0;
}
...
static int axera_add_pcie_ep(struct axera_pcie *axera,
...
static int axera_pcie_probe(struct platform_device *pdev)
{
	...
	printk("axera pcie%d ep probe", pcie_id);
	...
	reset = devm_gpiod_get_optional(dev, "perst", GPIOD_OUT_LOW);
	if (IS_ERR(reset)) {
		ret = PTR_ERR(reset);
	...
	if (!IS_ENABLED(CONFIG_PCIE_AXERA_EP)) {
		return -ENODEV;
	}

	axera_set_device_type(axera);

	ret = axera_add_pcie_ep(axera, pdev);
	if (ret < 0)
		return ret;
...
module_platform_driver(axera_pcie_driver);

MODULE_DEVICE_TABLE(of, of_axera_pcie_match);

MODULE_AUTHOR("AXERA");
MODULE_DESCRIPTION("AXERA PCIE EP driver");
