...
int pci_setup_device(struct pci_dev *dev)
{
	...
	struct pci_bus_region region;
	struct resource *res;
#if IS_ENABLED(CONFIG_PCIE_AXERA)
	int devctl_val;
#endif
...
#if IS_ENABLED(CONFIG_PCIE_AXERA)
	if (dev->vendor != 0x144d && dev->device != 0xa809) {
		pos = pci_find_capability(dev, PCI_CAP_ID_EXP);
		pci_read_config_dword(dev, pos + 0x8, &devctl_val);
		devctl_val &= ~(0x7 << 12);
		pci_write_config_dword(dev, pos + 0x8, devctl_val);
	}
#endif
...
static struct pci_dev *pci_scan_device(struct pci_bus *bus, int devfn)
{
	struct pci_dev *dev;
	u32 l;
//#ifdef CONFIG_PCIE_AXERA
#if IS_ENABLED(CONFIG_PCIE_AXERA)
#define AX_PCIE_RABAR_BASE	0x278
#define AX_PCIE_RABAR0_CTL	(AX_PCIE_RABAR_BASE + 0x8)
	u32 bz;
#endif
...
//#ifdef CONFIG_PCIE_AXERA
#if IS_ENABLED(CONFIG_PCIE_AXERA)
	if (dev->vendor == 0x1f4b) {
		pci_read_config_dword(dev, AX_PCIE_RABAR0_CTL, &bz);
		bz &= (~0x3f00);
		pci_write_config_dword(dev, AX_PCIE_RABAR0_CTL, bz);
	}
#endif
