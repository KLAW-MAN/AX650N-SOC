...
int md_run(struct mddev *mddev)
{
		...
		mddev->ro = 2; /* read-only, but switch on first write */
	{
		mddev->id = 0;
		if (mddev->level == 5) {
			mddev->id = idr_alloc_cyclic(&mddev_idr, mddev, 1, 16, GFP_KERNEL);
			if (mddev->id == -ENOSPC) {
				pr_warn("md: %s-%d %s Not enough dma channels, will use cpu!\n",\
					__func__, __LINE__, mddev->gendisk->disk_name);
			}
			pr_debug("%s-%d level %d name %s id %d\n", __func__, __LINE__,\
				mddev->level, mddev->gendisk->disk_name, mddev->id);
		}
	}
	err = pers->run(mddev);
	if (err)
