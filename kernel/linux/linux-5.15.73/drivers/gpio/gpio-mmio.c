...
static int bgpio_dir_in(struct gpio_chip *gc, unsigned int gpio)
{
	...
	phy_gpio_number = gc->base + gpio;

	if(((phy_gpio_number <= 84) && (phy_gpio_number >=69)) || ((phy_gpio_number <= 191) && (phy_gpio_number >=160))) {
		if (&dphy_gpio_tree == NULL) {
			printk("phy_gpio_tree not found\n");
			return -1;
		}
     		data = radix_tree_lookup(&dphy_gpio_tree, phy_gpio_number);
		if (data == NULL) {
			printk("%s GPIO%d not found\n",__func__, phy_gpio_number);
			return -1;
		}
		if ((data->id >= 1) && (data->id <= 48))
			axera_phygpio_direction(data->id, AX_INPUT);
	}
#endif
	spin_lock_irqsave(&gc->bgpio_lock, flags);
#ifdef CONFIG_ARCH_AXERA
...
static void bgpio_dir_out(struct gpio_chip *gc, unsigned int gpio, int val)
{
	...
	if (((phy_gpio_number <= 84) && (phy_gpio_number >=69)) || ((phy_gpio_number <= 191) && (phy_gpio_number >=160))) {
		...
		if ((data->id >= 1) && (data->id <= 48))
			axera_phygpio_direction(data->id, AX_OUTPUT);
	}
#endif
	spin_lock_irqsave(&gc->bgpio_lock, flags);
#ifdef CONFIG_ARCH_AXERA
