...
static void pll_set(struct ax_pll_cfg *clk, u32 rate)
{
    u32 val;
    int i;
    struct ax_pll_cfg *pll_cfg = clk;
    int try_count = 10000;
    ...
    fbk_fra = do_div(fbk_int, pll_cfg->ref_rate);
    pre_div = 0;
    ldo_stb_x2_en = 1;
    fbk_fra = ((u64)fbk_fra * (1 << 24) + pll_cfg->ref_rate / 2) / pll_cfg->ref_rate;
    ...
    writel_relaxed(GENMASK(25, 2), pll_cfg->cfg0_reg + 8);
...
#define to_pll_clk(_hw) container_of(_hw, struct ax_pll_cfg, hw)
static int clk_pll_set_rate(struct clk_hw *hw,
                            unsigned long rate,
                            unsigned long parent_rate)
{
    struct ax_pll_cfg *clk = to_pll_clk(hw);
...
static unsigned long clk_pll_recalc_rate(struct clk_hw *hw,
        unsigned long parent_rate)
{
    struct ax_pll_cfg *pll_cfg = to_pll_clk(hw);
    u64 rate;
    u32 fbk_int;
    u32 fbk_fra;
    ...
    rate = fbk_int * pll_cfg->ref_rate + ((u64)fbk_fra * pll_cfg->ref_rate + (1 << 23)) / (1 << 24);
...
void ax_clk_register_pll(struct axera_pll_cfg_reg *clks, int nums, struct axera_clock_data *data)
{
    void __iomem *base = data->base;
    struct ax_pll_cfg *p_clk = NULL;
    struct clk *clk = NULL;
    struct clk_init_data init;
    int i;
...
EXPORT_SYMBOL(ax_clk_register_pll);
