...
int g_pcie_lock;
EXPORT_SYMBOL(g_pcie_lock);

void ax_pcie_sata_spin_lock(struct ahci_host_priv *hpriv, unsigned long *flags)
{
	if (hpriv->pcie_lock) {
		ax_pcie_spin_lock(flags);
	}
}
EXPORT_SYMBOL(ax_pcie_sata_spin_lock);

void ax_pcie_sata_spin_unlock(struct ahci_host_priv *hpriv, unsigned long *flags)
{
	if (hpriv->pcie_lock) {
		ax_pcie_spin_unlock(flags);
	}
}
EXPORT_SYMBOL(ax_pcie_sata_spin_unlock);
#endif
...
#ifdef CONFIG_PCIE_AXERA_650
void write_ahci_hba(void __iomem *host_mmio, u32 addr, u32 data)
{
    writel(data, host_mmio + addr);
}

void set_ini_reg(void __iomem *host_mmio)
{
    //[AHCI_HBA]
    //0xB0 = 0x79E4EFBF  // [31:30]:cfg_bufsize=1 (1024 Byte)
    //0xB0 = 0x39E4EFBF  // [31:30]:cfg_bufsize=0 (512 Byte)
    write_ahci_hba(host_mmio, 0xB0, 0x39E4EFBF);
}
#endif
...
static int ahci_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	{
		dev_info(&pdev->dev, "jmicron, pdev->vendor=0x%X, pdev->device=0x%X \n", pdev->vendor, pdev->device);
		set_ini_reg(hpriv->mmio);
		dev_info(&pdev->dev, "jmicron, set_ini_reg done \n");
	}
#endif
