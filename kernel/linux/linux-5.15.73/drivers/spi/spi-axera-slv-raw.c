...
static void dw_slv_reader(struct dw_poll_priv *priv)
{
	...
	u32 rx_left = priv->rx_entries;
	u32 rx_avail = dw_slv_read(priv->dws_slv, DW_SPI_RXFLR);
	u32 rx_max = rx_left > rx_avail ? rx_avail : rx_left;
	priv->rx_entries = rx_left - rx_max;
	while (rx_max-- && priv->rx) {
...
static void dw_slv_writer(struct dw_poll_priv *priv)
{
	...
	u32 dw = priv->fifowidth >> 3;
	u32 tx_left = priv->tx_entries;
	u32 tx_avail = DW_SPI_FIFO_LEN / dw - dw_slv_read(priv->dws_slv, DW_SPI_TXFLR);
...
static irqreturn_t dw_spi_s_xfer_handler(struct dw_poll_priv *priv_slv)
{
	...
	if (irq_status & SPI_INT_TXEI) {
		new_mask = dw_slv_read(dws_slv, DW_SPI_IMR) & ~SPI_INT_TXEI;
...
static irqreturn_t dw_spi_s_irq(int irq, void *dev_id)
{
	...
	return dw_spi_s_xfer_handler(priv_slv);
}
...
retry:
		if (kfifo_len(&slave_rx_fifo) >= rxsize) {
			int len = kfifo_out(&slave_rx_fifo, input_buf, rxsize);
			if ((len != rxsize) && (len != 0)) {
				pr_err(" %s: %d != %d\n", __func__, len, rxsize);
				mutex_unlock(&priv_slv->stream_mutex);
				return -EFAULT;
...
static int dw_spi_slv_probe(struct platform_device *pdev)
{
	...
	dws_slv->clk = devm_clk_get(&pdev->dev, "periphclk");
