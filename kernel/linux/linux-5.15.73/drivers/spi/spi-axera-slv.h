...
static inline u32 dw_readl(struct dw_spi *dws, u32 offset)
{
	...
	printk("=====> spi reg read: 0x%x, 0x%x\n", offset, val);
	return val;
}

static inline void dw_writel(struct dw_spi *dws, u32 offset, u32 val)
{
	printk("=====> spi reg write: 0x%x, 0x%x\n", offset, val);
...
static inline u32 dw_read_io_reg(struct dw_spi *dws, u32 offset)
{
	...
	case 2:
		val = readw_relaxed(dws->regs + offset);
		printk("=====> spi reg read u16: 0x%x, 0x%x\n", offset, val);
		return val;
	case 4:
	default:
		val = readl_relaxed(dws->regs + offset);
		printk("=====> spi reg read: 0x%x, 0x%x\n", offset, val);
		return val;
	}
}
...
static inline void dw_write_io_reg(struct dw_spi *dws, u32 offset, u32 val)
{
	switch (dws->reg_io_width) {
	case 2:
		printk("=====> spi reg write u16: 0x%x, 0x%x\n", offset, val);
...
extern void dw_slv_spi_update_config(struct dw_spi *dws, struct spi_device *spi,
				 struct dw_spi_cfg *cfg);
extern int dw_slv_spi_check_status(struct dw_spi *dws, bool raw);
