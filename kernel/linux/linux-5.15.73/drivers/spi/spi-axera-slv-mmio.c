...
static int dw_spi_dw_apb_init(struct platform_device *pdev, struct dw_spi_mmio *dwsmmio)
{
...
static int dw_slv_spi_mmio_probe(struct platform_device *pdev)
{
	...
	if (ret)
		return ret;
	dwsmmio->clk = devm_clk_get(&pdev->dev, "periphclk");
	...
	if (0 == dws->max_freq) {
		dev_warn(&pdev->dev, "clk_get_rate = %d\n", dws->max_freq);
		dws->max_freq = 200000000;
	}
...
static const struct of_device_id dw_slv_spi_mmio_of_match[] = {
	{.compatible = "snps,dwc-ssi-slv-1.03a", .data = dw_spi_dwc_ssi_init},
	{ /* end of table */ }
};
