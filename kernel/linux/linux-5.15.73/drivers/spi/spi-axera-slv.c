...
static int stream_cin_gpio = -1;
static bool rx_handshk_gpio_stat = true;
static int slv_rx_handshk_gpio_init(void)
{
	if (stream_cin_gpio < 0) {
		pr_err("none stream ctrl in gpio\n");
		return -1;
	}
	gpio_direction_output(stream_cin_gpio, 1);
...
/*************************************************************************
  SPI slave device must control GPIO level flip when operate read per time.
 *************************************************************************/
static void slv_rx_handshk_gpio_invert(struct dw_spi *dws)
{
	...
	if (rx_handshk_gpio_stat) {
		gpio_set_value(stream_cin_gpio, 0);
		rx_handshk_gpio_stat = false;
	} else {
		gpio_set_value(stream_cin_gpio, 1);
		rx_handshk_gpio_stat = true;
	}
}
/* stream control end */

void dw_spi_host_dump(struct dw_spi *dws)
{
	int i;

	printk("======== SPI DUMP =========\n");
	for (i = 0; i < 6; i++) {
		printk("%08lX ++ %04X: %08X %08X %08X %08X\n", dws->paddr,
		       i * 0x10, dw_readl(dws, (i * 0x10)), dw_readl(dws,
								     (i * 0x10 +
								      0x4)), dw_readl(dws, (i * 0x10 + 0x8)), dw_readl(dws, (i * 0x10 + 0xC)));
	}
	printk("%08lX ++ %04X: %08X %08X %08X %08X\n", dws->paddr, 0xF0,
	       dw_readl(dws, 0xF0), dw_readl(dws, 0xF4), dw_readl(dws, 0xF8), dw_readl(dws, 0xFC));
	printk("===========================\n");
}
...
int dw_slv_spi_check_status(struct dw_spi *dws, bool raw)
{
	u32 irq_status;
	int ret = 0;
...
EXPORT_SYMBOL_GPL(dw_slv_spi_check_status);

static irqreturn_t dw_spi_transfer_handler(struct dw_spi *dws)
{
	u16 irq_status = dw_readl(dws, DW_SPI_ISR);

	if (dw_slv_spi_check_status(dws, false)) {
...
void dw_slv_spi_update_config(struct dw_spi *dws, struct spi_device *spi, struct dw_spi_cfg *cfg)
{
...
#ifdef ENHANCED_SPI_SUPPORT
		if (qmode) {
			cr0 |= SPI_FRF_SPI_QUAD << DWC_SSI_CTRLR0_SPI_FRF_OFFSET;
		} else {
			cr0 |= SPI_FRF_SPI_STANDARD << DWC_SSI_CTRLR0_SPI_FRF_OFFSET;
		}

		cr0 &= ~(SPI_SLVOE_MASK | SPI_TMOD_MASK);
...
EXPORT_SYMBOL_GPL(dw_slv_spi_update_config);
...
/*
	if (dws->tx) {
		slv_rx_handshk_gpio_invert(dws, 1);
	}
	if (dws->rx) {
		slv_rx_handshk_gpio_invert(dws, 0);
	}
*/
#else
	if (dws->tx) {
...
static int dw_spi_poll_transfer(struct dw_spi *dws, struct spi_transfer *transfer)
{
	...
	do {
		...
		ret = dw_slv_spi_check_status(dws, true);
		if (ret)
			return ret;
	} while (dws->rx_len);
...
static int dw_spi_transfer_one(struct spi_controller *master, struct spi_device *spi, struct spi_transfer *transfer)
{
	...
	if (4 == transfer->rx_nbits || 4 == transfer->tx_nbits) {
		//need set pinmux to switch 4bit simplex;
		qmode = 1;
	} else
		qmode = 0;	//need set pinmux to switch 1bit duplex (default);
	if (transfer->rx_buf && (NULL == transfer->tx_buf)) {
		cfg.tmode = SPI_TMOD_RO;
	} else if (transfer->tx_buf && (NULL == transfer->rx_buf)) {
...
#ifdef ENHANCED_SPI_SUPPORT
	} else if (SPI_TMOD_TO == cfg.tmode) {
		dws->tx = (void *)transfer->tx_buf;
		dws->tx_len = transfer->len / dws->n_bytes;
		dws->rx = transfer->rx_buf;
		dws->rx_len = 0;
		if (qmode) {
			txftlr = (((dws->tx_len > 0xf) ? 0xf : (dws->tx_len - 1)) << SPI_TXFTHR_OFFSET | ((dws->tx_len > 0x4) ? 0x4 : (dws->tx_len - 1)));
		}
	} else if (SPI_TMOD_RO == cfg.tmode) {
		dws->tx = (void *)transfer->tx_buf;
		dws->tx_len = 0;
		dws->rx = transfer->rx_buf;
		dws->rx_len = transfer->len / dws->n_bytes;
		if (qmode)
	...
	dw_slv_spi_update_config(dws, spi, &cfg);
...
static void dw_spi_handle_err(struct spi_controller *master, struct spi_message *msg)
{
	struct dw_spi *dws = spi_controller_get_devdata(master);
...
{
#ifdef ENHANCED_SPI_SUPPORT
	if (op->data.buswidth > 4 || op->addr.buswidth > 1 || op->dummy.buswidth > 1 || op->cmd.buswidth > 1)
#else
...
static int dw_spi_exec_mem_op(struct spi_mem *mem, const struct spi_mem_op *op)
{
	...
	dw_slv_spi_update_config(dws, mem->spi, &cfg);
...
static int dw_spi_setup(struct spi_device *spi)
{
	...
	if (!chip) {
		...
		if (device_property_read_u32(&spi->dev, "rx-sample-delay-ns", &rx_sample_dly_ns) != 0)
...
static void spi_hw_init(struct device *dev, struct dw_spi *dws)
{
	...
	if (!dws->fifo_len) {
		...
		dev_err(dev, "Detected FIFO size: %u bytes\n", dws->fifo_len);
	}
	...
	if (!(dws->caps & DW_SPI_CAP_DWC_SSI)) {
		...
		dev_err(dev, "DW_SPI_CTRLR0 :0x%x\n", tmp);
		spi_enable_chip(dws, 0);
		...
		if (!(cr0 & SPI_DFS_MASK)) {
			...
			dev_err(dev, "Detected 32-bits max data frame size\n");
		}
	} else {
		dws->caps |= DW_SPI_CAP_DFS32;
	}
...
int dw_slv_spi_add_host(struct device *dev, struct dw_spi *dws)
{
	...
	device_property_read_u32(dev, "stream-ctl-enable", &stream_ctl_flag);
	if (1 == stream_ctl_flag) {
		stream_gpio = of_get_named_gpio(dev->of_node, "stream-rx-gpio", 0);
		...
		if (gpio_is_valid(stream_gpio)) {
			...
			pr_err("devm_gpio_request error\n");
			if (ret)
				goto err_free_master;
		}
		stream_cin_gpio = stream_gpio;
	}
...
void dw_slv_spi_remove_host(struct dw_spi *dws)
{
	...
	if (gpio_is_valid(stream_cin_gpio))
		devm_gpio_free(&dws->master->dev, stream_cin_gpio);
}
