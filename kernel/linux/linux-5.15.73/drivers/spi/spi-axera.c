...
int dw_ahb_spi_check_status(struct dw_spi *dws, bool raw)
{
	u32 irq_status;
	int ret = 0;
...
EXPORT_SYMBOL_GPL(dw_ahb_spi_check_status);

static irqreturn_t dw_spi_transfer_handler(struct dw_spi *dws)
{
	u16 irq_status = dw_readl(dws, DW_SPI_ISR);

	if (dw_ahb_spi_check_status(dws, false)) {
...
void dw_ahb_spi_update_config(struct dw_spi *dws, struct spi_device *spi,
...
#ifdef ENHANCED_SPI_SUPPORT
	else if ((cfg->tmode == SPI_TMOD_TO) && qmode) {
		if (dws->tx_len < 2) {
			dev_err(&dws->master->dev, "TXONLY xfer len = %d < 2\n", dws->tx_len);
			return;
		}
		dw_writel(dws, DW_SPI_CTRLR1, dws->tx_len - 2);
	}
#endif
...
EXPORT_SYMBOL_GPL(dw_ahb_spi_update_config);
				...
				struct spi_transfer *transfer)
{
	...
	do {
		...
		ret = dw_ahb_spi_check_status(dws, true);
		if (ret)
			return ret;
	} while (dws->rx_len);
		...
		struct spi_device *spi, struct spi_transfer *transfer)
{
	...
	if (transfer->rx_buf && (NULL == transfer->tx_buf)) {
		...
		cfg.ndf = transfer->len / dws->n_bytes;
	...
	dws->rx_len = dws->tx_len;
		...
		dws->rx_len = 0;
		if (qmode) {
			txftlr = (((dws->tx_len > 0xf) ? 0xf : (dws->tx_len - 1)) << SPI_TXFTHR_OFFSET |
					 ((dws->tx_len > 0x4) ? 0x4 : (dws->tx_len - 1)));
	...
	dw_ahb_spi_update_config(dws, spi, &cfg);
#ifdef ENHANCED_SPI_SUPPORT
	if (qmode && dw_readl(dws, DW_SPI_SPI_CTRLR0) != spicr0)
		dw_writel(dws, DW_SPI_SPI_CTRLR0, spicr0);
#endif
...
static int dw_spi_exec_mem_op(struct spi_mem *mem, const struct spi_mem_op *op)
{
	...
	dw_ahb_spi_update_config(dws, mem->spi, &cfg);
