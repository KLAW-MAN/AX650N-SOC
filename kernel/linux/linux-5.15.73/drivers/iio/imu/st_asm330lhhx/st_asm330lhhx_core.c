...
static int st_asm330lhhx_get_int_reg(struct st_asm330lhhx_hw *hw, u8 *drdy_reg)
{
    ...
    printk("vision_d:%s asm330 int pin%d \n",__func__,int_pin);
					       ...
					       enum st_asm330lhhx_sensor_id id)
{
	...
	if (!iio_dev)
		return NULL;
    printk("vision_d:%s init channels id =%d \n",__func__,id);
...
/*
	err = st_asm330lhhx_power_enable(hw);   //pwr control by HW,not software
	if (err != 0)
		return err;
*/
	...
	st_asm330lhhx_get_properties(hw);   //get hw->module_id ,default is 1

	err = st_asm330lhhx_get_odr_calibration(hw);    //get hw->ts_delta_ns
	if (err < 0)
		return err;
	...
	err = st_asm330lhhx_init_device(hw);    //init device reg
	if (err < 0)
		return err;
...
/*	if (hw->irq > 0) {
	...
	if (hw->irq > 0) {
		err = st_asm330lhhx_buffers_setup(hw);  //config IRQ, buffer/enable node
		if (err < 0)
			return err;
	}
	...
	for (i = ST_ASM330LHHX_ID_GYRO; i < ST_ASM330LHHX_ID_MAX; i++) {
        ...
        //register iio device,create nodes.iio_devs init on line 2298
...
#ifdef CONFIG_IIO_ST_ASM330LHHX_EN_BASIC_FEATURES
	err = st_asm330lhhx_probe_event(hw);    //regist fops node about trigger and event(ff/sc/wk/6d)
	if (err < 0)
		return err;

	err = st_asm330lhhx_post_init_device(hw);//set trigger default value
	if (err < 0)
		return err;
#endif /* CONFIG_IIO_ST_ASM330LHHX_EN_BASIC_FEATURES */
