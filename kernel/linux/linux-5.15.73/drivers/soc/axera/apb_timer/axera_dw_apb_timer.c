...
static irqreturn_t dw_apb_timer_irq(int irq, void *data)
{
	...
	dw_timer = (struct axera_dw_apb_timer *)data;

	for (i = 0; i < PER_TIMER_CHANNEL_COUNT; i++) {
		int_status = apbt_readl(dw_timer, APBTMR_N_INIT_STATUS(i));
		if (!int_status)
			continue;
		channel_id = i;
...
u32 *timer_open(unsigned int dev_id, unsigned int channel_id)
{
	...
	clk_prepare_enable(timer_p[dev_id]->clk[channel_id]);
	clk_set_rate(timer_p[dev_id]->clk[channel_id], CLK_SEL_FREQ);
...
void timer_close(u32 *timer_handle)
{
	...
	if (id == PER_TIMER_CHANNEL_COUNT) {
		...
		clk_disable_unprepare(dw_timer->clk[channel_id]);
	}
...
static int axera_dw_timer_probe(struct platform_device *pdev)
{
	...
	timer_p[index]->base = devm_ioremap(&pdev->dev, res_base->start, resource_size(res_base));
	...
	for (i = 0; i < PER_TIMER_CHANNEL_COUNT; i++) {
		sprintf(buf, "timer-ch%d-reset", i);
		timer_p[index]->reset[i] = devm_reset_control_get_optional(&pdev->dev, buf);
		if (IS_ERR(timer_p[index]->reset[i])) {
			pr_err("axera get %s failed\n", buf);
			return PTR_ERR(timer_p[index]->reset[i]);
		}
		reset_control_deassert(timer_p[index]->reset[i]);
	}
	...
	for (i = 0; i < PER_TIMER_CHANNEL_COUNT; i++) {
		sprintf(buf, "timer-ch%d-clk", i);
		timer_p[index]->clk[i] = devm_clk_get(&pdev->dev, buf);
		if (IS_ERR(timer_p[index]->clk[i])) {
			pr_err("axera get %s fail\n", buf);
			return PTR_ERR(timer_p[index]->clk[i]);
		}
	}


	timer_p[index]->irq = platform_get_irq(pdev, 0);
	...
	err = devm_request_irq(&pdev->dev, timer_p[index]->irq, dw_apb_timer_irq,
