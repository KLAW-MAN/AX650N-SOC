...
static int dw_tdm_dai_probe(struct snd_soc_dai *dai)
{
	struct dw_i2s_dev *dev = snd_soc_dai_get_drvdata(dai);

	snd_soc_dai_init_dma_data(dai,
				  &dev->play_dma_data,
				  &dev->capture_dma_data);
...
static int dw_tdm_resume(struct snd_soc_component *component)
{
	...
	if (dev->pin_sd_res && dev->pin_g3_res)
...
static int dw_tdm_probe(struct platform_device *pdev)
{
	...
	if (dev->hdmi_i2s) {
		ret = ax_audio_reset_hw(pdev, "rst");
		if (0 != ret) {
			pr_err("rst hw err!\n");
			return -1;
		}
	} else {
		ret = ax_audio_reset_hw(pdev, "prst");
		if (0 != ret) {
			pr_err("prst hw err!\n");
			return -1;
		}
	...
	/*
	 * Use probe to init dma data before any pcm operation, due to
	 * soc_pcm_trigger first call soc_pcm_components_open then snd_soc_dai_startup
	 * in start process.
	 */
	dw_i2s_dai->probe = &dw_tdm_dai_probe;
	...
	dev->pin_sd_res = platform_get_resource(pdev, IORESOURCE_MEM, 1);
	dev->pin_g3_res = platform_get_resource(pdev, IORESOURCE_MEM, 2);
	if (dev->pin_sd_res && dev->pin_g3_res) {
