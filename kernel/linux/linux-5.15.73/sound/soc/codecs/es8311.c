...
static int es8311_set_dai_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
	...
	u8 daciface = 0;
	printk("Enter into %s()\n", __func__);
	iface    = snd_soc_component_read(codec, ES8311_RESET_REG00);
	adciface = snd_soc_component_read(codec, ES8311_SDPOUT_REG0A);
	daciface = snd_soc_component_read(codec, ES8311_SDPIN_REG09);
	...
	iface    = snd_soc_component_read(codec, ES8311_CLK_MANAGER_REG06);
	/* clock inversion */
			...
			struct snd_soc_dai *dai)
{
	...
	*/
	if(coeff >= 0) {
		regv = snd_soc_component_read(codec, ES8311_CLK_MANAGER_REG02) & 0x07;
		...
		regv = snd_soc_component_read(codec, ES8311_CLK_MANAGER_REG05) & 0x00;
	...
	if(substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		iface = snd_soc_component_read(codec, ES8311_SDPOUT_REG0A) & 0xE3;
		/* bit size */
		switch (params_format(params)) {
		case SNDRV_PCM_FORMAT_S16_LE:
			iface |= 0x0c;
			...
			enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_ON:
		...
		regv = snd_soc_component_read(codec, ES8311_SYSTEM_REG14) & 0xCF;
		regv |= 0x1A;
			...
			struct device_attribute *attr, char *_buf)
{
	...
	ret = sprintf(_buf, "%s(): get 0x%04x=0x%04x\n",
			...
			snd_soc_component_read(es8311_codec, cur_reg));
	return ret;
}
/*
			...
			const char *buf, size_t count)
{
	...
	if (flag) {
		...
		do {
			value_r = 0;
			value_r = snd_soc_component_read(es8311_codec, reg);
