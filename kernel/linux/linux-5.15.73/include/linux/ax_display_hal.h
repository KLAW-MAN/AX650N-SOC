...
#define MODE_FLAG_INTERLACE			(1 << 0)	/* use interlace mode */
#define MODE_FLAG_BT_EXT_SYNC			(1 << 1)	/* use v/h sync */
#define MODE_FLAG_BT_SINGLE_CHN			(1 << 2)	/* use single-chn for bt1120 */
#define MODE_FLAG_BT_8BIT_LOW			(1 << 3)
#define MODE_FLAG_BT_FIELD1_FIRST		(1 << 4)	/* field1 transmits first. */
...
enum {
	AX_DISP_OFF_TYPE_NORMAL,
	AX_DISP_OFF_TYPE_LOW_LATENCY,
	AX_DISP_OFF_TYPE_BUTT,
};
...
struct ax_display_funcs {
	...
	void (*dispc_enable)(void *priv_data);
	void (*dispc_disable)(void *priv_data, u32 type);
	...
	void (*dispc_fb_release)(void *priv_data);
