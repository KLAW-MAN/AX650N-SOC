...
void pll_set(AX_PLL_ID_E id, u32 clk)
{
	...
	value = readl(AON_SYS_PLL_BASE_ADDR + pll_cfg->re_open_reg);
	value &= ~(1 << pll_cfg->re_open_bit);
	writel(value,AON_SYS_PLL_BASE_ADDR + pll_cfg->re_open_reg);

/*	clrbits_le32(AON_SYS_PLL_BASE_ADDR + pll_cfg->re_open_reg,
		     1 << pll_cfg->re_open_bit);
*/

	value = readl(AON_SYS_PLL_BASE_ADDR + pll_cfg->on_reg);
	value &= ~(1 << pll_cfg->on_bit);
	writel(value,AON_SYS_PLL_BASE_ADDR + pll_cfg->on_reg);
	/*on set to 0 */
