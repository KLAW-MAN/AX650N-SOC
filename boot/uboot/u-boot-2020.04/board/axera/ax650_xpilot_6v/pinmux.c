...
unsigned int ax750A_xpilot_alpha_pinmux[] = {
#include "ax750A_xpilot_alpha_pinmux.h"
};

unsigned int ax750A_xpilot_beta_pinmux[] = {
#include "ax750A_xpilot_beta_pinmux.h"
};
...
typedef enum xpilot_board_type {
	AX750A_ALPHA = 0,
	AX750A_BETA = 1,
} xpilot_board_type_t;
...
struct pinmux ax650A_pinmux_tbl[] = {
	[AX750A_ALPHA] =
		{ax750A_xpilot_alpha_pinmux,
			sizeof(ax750A_xpilot_alpha_pinmux) / sizeof(unsigned int)},
	[AX750A_BETA] =
		{ax750A_xpilot_beta_pinmux,
			sizeof(ax750A_xpilot_beta_pinmux) / sizeof(unsigned int)},
};
...
int pinmux_init(void)
{
	...
	for (i = 0; i < ax650A_pinmux_tbl[index].size; i += 2) {
		writel(ax650A_pinmux_tbl[index].data[i + 1],
		       (long)ax650A_pinmux_tbl[index].data[i]);

		if((((long)ax650A_pinmux_tbl[index].data[i] & PINMUX_ADDR_MASK) == EMMC_PINMUX_ADDR) &&
			(((ax650A_pinmux_tbl[index].data[i + 1] >> 16) & 0x7) != 0)) {
				emmc_phy_flag = 1;
		}
		if((((long)ax650A_pinmux_tbl[index].data[i] & PINMUX_ADDR_MASK) == SD_PINMUX_ADDR) &&
			(((ax650A_pinmux_tbl[index].data[i + 1] >> 16) & 0x7) != 0)) {
				sd_phy_flag = 1;
		}
		if((((long)ax650A_pinmux_tbl[index].data[i] & PINMUX_ADDR_MASK) == SDIO_PINMUX_ADDR) &&
			(((ax650A_pinmux_tbl[index].data[i + 1] >> 16) & 0x7) != 0)) {
				sdio_phy_flag = 1;
		}

		if (((long)ax650A_pinmux_tbl[index].data[i] & RTC_SYS_GLB_MASK) == RTC_SYS_GLB) {
			if (((long)ax650A_pinmux_tbl[index].data[i] < (RTC_SYS_GLB + RTC_SYS_GLB_FUNC)) &&
				((long)ax650A_pinmux_tbl[index].data[i + 1] != RTC_SYS_PROPERTY_DEF)) {
				rtc_sys_flag = 1;
			}
			if (((long)ax650A_pinmux_tbl[index].data[i] >= (RTC_SYS_GLB + RTC_SYS_GLB_FUNC)) &&
				((long)ax650A_pinmux_tbl[index].data[i + 1])) {
				rtc_sys_flag = 1;
			}
		}
	}
