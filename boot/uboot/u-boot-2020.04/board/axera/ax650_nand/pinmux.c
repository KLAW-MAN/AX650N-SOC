...
#include "ax650A_demo_NAND_pinmux.h"
};

unsigned int ax650A_evb_pinmux[] = {
#include "ax650A_NAND_pinmux.h"
};

unsigned int ax650N_demo_pinmux[] = {
#include "ax650N_demo_NAND_pinmux.h"
};

unsigned int ax650N_evb_pinmux[] = {
#include "ax650N_NAND_pinmux.h"
};
...
#define REG_EMMC_PIN_CTRL	0x04251e04
int pinmux_init(void)
{
	int index, i;
	int emmc_phy_flag = 0, sd_phy_flag = 0, sdio_phy_flag = 0;
	index = get_board_id();
	index = AX650A_EVB;
	if (index < 0 || index > 8)
		return 0;
	writel((1 << 17), REG_EMMC_PIN_CTRL);
	/* enable rtc sys */
