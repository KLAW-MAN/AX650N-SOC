...
int ext4fs_memory_dump(struct blk_desc *fs_dev_desc, unsigned long int addr, unsigned long int size, unsigned long int info_addr,
		unsigned long int info_size,char *filename,char *info_name)
{
	struct fstype_info *info;
	int ret;
	int part = 2;
	...
	if (!info->probe(fs_dev_desc, &fs_partition)) {
		...
		fs_type = info->fstype;
		fs_dev_part = part;
		printf("ext4 mount probe ok\n");
		fs_write(info_name, info_addr, offset, info_size, &len);
	}
