...
#define UART_BASE_CLK     		(CONFIG_SYS_NS16550_CLK)
#define UART_BAUDATE      		(CONFIG_BAUDRATE)
...
static void empty_check(ulong base, u32 timeout_us)
{
	int status;
	do {
...
void uart_putc(ulong base, const char ch, u32 timeout_us)
{
	empty_check(base, timeout_us);
	writel(ch, base + THR_R);
}

void uart_init(ulong base)
{
