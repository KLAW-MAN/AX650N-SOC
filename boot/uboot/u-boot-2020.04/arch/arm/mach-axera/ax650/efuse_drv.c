...
unsigned int reg_read(uint64_t addr)
{
	return *(volatile unsigned int *)addr;
}

void reg_write(unsigned int value, uint64_t addr)
{
...
{
	int val;
	val = reg_read(EFSC_INTR_STATUS + EFSC0_BASE);
	if (val & (1 << 2)) {
...
int ax_efuse_read(int blk, int *data)
{
	...
	*data = reg_read(EFSC_SHADOW_0 + EFSC0_BASE + blk * 4);
	return ret;
}
...
int ax_efuse_write(int blk, int data)
{
	...
	val = (blk << 3);
	/*passwd */
	reg_write(EFSC_PASS_WD, EFSC_PASSWORD + EFSC0_BASE);
	/*pgm_value */
	reg_write(data, EFSC_PGM_VALUE + EFSC0_BASE);
	/*apb ctrl blk_addr[18:3],mode[2:0] 0: pgm mode */
	reg_write(val, EFSC_APB_CTRL + EFSC0_BASE);
	/*efs start */
	reg_write(1, EFSC_EFS_START + EFSC0_BASE);
	/*clr efs start */
	reg_write(0, EFSC_EFS_START + EFSC0_BASE);
	for (i = 0; i < EFSC_TIMEOUT; i++) {
		if (reg_read(EFSC_INTR_STATUS + EFSC0_BASE) & 0x1)
			break;
	}
	if (i >= EFSC_TIMEOUT) {
	...
	reg_write(0xf, EFSC_INT_CLR + EFSC0_BASE);
	/*passwd */
	reg_write(0, EFSC_PASSWORD + EFSC0_BASE);
	/*clear pgm done */
	reg_write(0, EFSC_APB_CTRL + EFSC0_BASE);
	return ret;
}
