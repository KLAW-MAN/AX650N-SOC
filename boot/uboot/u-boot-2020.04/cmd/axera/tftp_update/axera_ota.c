...
static long ota_tftp_download_file(char * cmd)
{
	int ret;
	printf("%s\n", cmd);
...
static int ota_update_repatition(struct update_part_info *pheader)
{
	int ret;
...
static int ota_update_repartition(struct update_part_info *pheader)
{
	int ret = -1;

	switch (boot_info_data.storage_sel) {
	case STORAGE_TYPE_EMMC:
		ret = ota_update_repatition(pheader);
		break;
	case STORAGE_TYPE_NAND:
		//ret = ota_update_repatition_nand(pheader);
		printf("%s: Not support nand flash.\n", __FUNCTION__);
		break;
	case STORAGE_TYPE_NOR:
		//ret = ota_update_repatition_nor(pheader);
		printf("%s: Not support nor flash.\n", __FUNCTION__);
		break;
	case STORAGE_TYPE_SSD:
		ret = ota_update_repatition(pheader);
		break;
	default:
		printf("%s: storage_sel %d error\n", __FUNCTION__, boot_info_data.storage_sel);
		break;
	}
...
int run_ota_update(const ota_info_t info, struct update_part_info *pheader)
{
	...
	if (NULL == ota_repartition) {
		...
		ret = ota_update_repartition(pheader);
		if (ret) {
			printf("repartition error %d\n", ret);
			return ret;
		}
		printf("ota_update_repartition return %d\n", ret);
	}
