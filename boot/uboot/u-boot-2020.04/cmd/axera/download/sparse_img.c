...
int sparse_info_init_ssd(struct sparse_storage *sparse_info, const char *part_name)
{
	struct blk_desc *blk_dev_desc = NULL;
	disk_partition_t part_info;
...
int sparse_info_init(struct sparse_storage *info, const char *name)
{
	...
	case STORAGE_TYPE_SSD:
		ret = sparse_info_init_ssd(info, name);
		break;
	default:
...
#if defined(CONFIG_TARGET_AX650_SSD)
		bootargs = BOOTARGS_SSD;
#elif defined(CONFIG_TARGET_AX650_PIPRO_BOX)
		bootargs = BOOTARGS_PIPRO;
#elif defined(CONFIG_TARGET_AX650_MASTER)
		bootargs = BOOTARGS_MASTER;
#else
