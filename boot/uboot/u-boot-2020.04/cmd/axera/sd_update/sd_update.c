...
static int sd_update_to_ssd(struct sd_update_file *pfile, int wr_len)
{
	lbaint_t start_lba, blkcnt_lb, base_lba;
	lbaint_t written_lb;
	lbaint_t wr_blkcnt_lb;
...
static int sd_update_to_storage(struct sd_update_file *pfile, int len)
{
	...
	case STORAGE_TYPE_SSD:
		ret = sd_update_to_ssd(pfile, len);
		break;
	default:
		ret = -1;
		break;
...
int do_sd_update(cmd_tbl_t * cmdtp, int flag, int argc, char *const argv[])
{
	...
	if (fat_register_device(sd_desc, 0)) {
		printf("sd register part0 fat fail, try part1\n");
	...
	ret = update_parse_part_info(&pbin_info, sd_desc);
#else
	...
	if (ret) {
		printf("sd parse config file fail, exit sd update, normal boot\n");
		goto normal_boot;
	...
	env_save();
	printf("sdupdate flag write\n");

	reboot();

	/* if update suceed, it will reboot, won't come here */
normal_boot:
	if(boot_info_data.dl_channel == DL_CHAN_SD) {
		boot_info_data.mode = SD_BOOT_MODE;
		printf("after update, and enter sd boot\n");
		run_command_list("sd_boot", -1, 0);
	} else {
		printf("after update, and enter axera boot\n");
...
free_node:
	...
	if (!strcmp(env_get("sdupdate"), "retry")) {
		...
		printf("retry error, set sdupdate env to %s", env_get("sdupdate"));
	} else {
		env_set("sdupdate", "retry");
		printf("first error, set sdupdate env to %s", env_get("sdupdate"));
	}
	env_save();
#else
	writel(SD_UPDATE_FAIL, SD_UPDATE_STATUS_ADDR);
#endif
	return -1;
}
