...
{
	u32 hw_version;
	struct dw_mipi_dsi_dphy_timing *timing = &dsi->dphy_timing;
...
static int dw_mipi_dsi_timing_fixup(struct dw_mipi_dsi *dsi,
				struct display_timing *timings,
				struct display_timing *adjusted_timings)
{
	int ret;
	u32 tmp, lbcc, htotal, vtotal, adjusted_vtotal, clk_khz, adjusted_clk_khz;
	const struct mipi_dsi_phy_ops *phy_ops = dsi->phy_ops;
	struct mipi_dsi_device *device = dsi->device;
	...
	if (!(device->mode_flags & MIPI_DSI_MODE_VIDEO_BURST)) {
		clk_khz = timings->pixelclock.typ / 1000;
		vtotal = timings->vactive.typ + timings->vfront_porch.typ +
			 timings->vback_porch.typ + timings->vsync_len.typ;
		htotal = timings->hactive.typ + timings->hfront_porch.typ +
			 timings->hback_porch.typ + timings->hsync_len.typ;
		lbcc = dw_mipi_dsi_get_hcomponent_lbcc(dsi, timings, htotal);
		tmp = htotal * dsi->lane_mbps * MSEC_PER_SEC / 8;
		adjusted_clk_khz = tmp / lbcc;
		if (tmp % lbcc)
			adjusted_clk_khz += 1;

		tmp = (adjusted_clk_khz * vtotal) + (clk_khz + 1) / 2;
		adjusted_vtotal = tmp / clk_khz;
		if (adjusted_vtotal < vtotal) {
			tmp = vtotal - adjusted_vtotal;
			if (tmp < timings->vback_porch.typ) {
				adjusted_timings->vback_porch.typ -= tmp;
			} else if (tmp < timings->vsync_len.typ) {
				adjusted_timings->vsync_len.typ -= tmp;
			} else if (tmp < timings->vfront_porch.typ) {
				adjusted_timings->vfront_porch.typ -= tmp;
			} else {
				dev_err(dsi->dev, "reduced rows:%d failed for adjusted_timings\n", tmp);
				return -EINVAL;
			}

			adjusted_timings->pixelclock.typ = adjusted_clk_khz * 1000;
		}
	}

	adjusted_timings->flags = DISPLAY_FLAGS_HSYNC_HIGH | DISPLAY_FLAGS_VSYNC_HIGH | DISPLAY_FLAGS_DE_HIGH;
			  ...
			  unsigned max_data_lanes, const struct mipi_dsi_phy_ops *phy_ops)
{
	...
	ret = dw_mipi_dsi_timing_fixup(dsi, timings, adjusted_timings);
	if (ret)
		return ret;

	dw_mipi_dsi_bridge_set(dsi, adjusted_timings);
