...
static int nvme_get_info_from_identify(struct nvme_dev *dev)
{
	...
	invalidate_dcache_range((unsigned long)ctrl,
			(unsigned long)ctrl + sizeof(struct nvme_id_ctrl));
...
static int nvme_blk_probe(struct udevice *udev)
{
	...
	invalidate_dcache_range((unsigned long)(id),
		(unsigned long)(id) + sizeof(struct nvme_id_ns));
...
static int nvme_probe(struct udevice *udev)
{
	...
	invalidate_dcache_range((unsigned long)(ndev->prp_pool),
			 (unsigned long)(ndev->prp_pool) + MAX_PRP_POOL);
