...
#ifdef CONFIG_TARGET_AX650_HAPS
		       ...
		       const void *dout, void *din, unsigned long flags)
{
...
#endif
#if !defined (SPI_MST_USE_DMA) && defined (CONFIG_TARGET_AX650_HAPS)
	int temp_len;
#endif
	...
	if (priv->len >= txrx_dlr) {
		if (tx) {
			...
			axi_dma_xfer_start(DMAC_CHAN_1, (u64)tx, ((u64)priv->regs + DW_SPI_DR), priv->len,
			dma_width, dma_width, burst_len, DMA_MEM_TO_DEV, SSI_DMA_TX_REQ);
		} else {
			axi_dma_xfer_start(DMAC_CHAN_1, ((u64)priv->regs + DW_SPI_DR), (u64)rx, priv->len,
		...
		ret = axi_dma_wait_xfer_done(DMAC_CHAN_1);
	} else
#endif
	{
	/* Enable controller after writing control registers */
...
static int dw_spi_set_speed(struct udevice *bus, uint speed)
{
	...
	dw_write(priv, DW_SPI_RX_SAMPLE_DLY, 0x1);//50=200/4
