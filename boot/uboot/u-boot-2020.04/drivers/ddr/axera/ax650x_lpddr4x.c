...
static void ddr_phy_init(int idx)
{
	...
	if(swap_mode == 0) {
		...
		lp4_ddrphy_addr_swap_750(phy_base_addr);
		lp4_ddrphy_data_swap_750(phy_base_addr);
	} else {
...
void ddr_config_get(void)
{
	...
	/* adc_read_boardid(1,&adc_data); //Users can read adc values to be compatible with different DDR types */
}
