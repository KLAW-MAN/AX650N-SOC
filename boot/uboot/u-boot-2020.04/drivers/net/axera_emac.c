...
#define EMAC_EPHY_REF_EB 	(COMM_CK_RST_CFG+0x38)
...
static void emac_rgmii_set_tx_speed(int bus_id, int speed)
{
	...
	default:
		printk("only support EMAC0 and EMAC1\n");
		break;
	}
}
...
static void emac1_rmii_set_speed(int speed)
{
	...
	case SPEED_100:
		...
		break;
	default:
		printf("EMAC:un-supported emac1 rmii phy speed\n");
		return;
	}
...
static void emac_rmii_set_speed(int bus_id, int speed)
{
	...
	default:
		printk("EMAC:only support EMAC0 and EMAC1 rmii\n");
		break;
	}
}
...
static void emac_sw_rst(int bus_id)
{
	...
	{
		//printf("reset EMAC0...\n");
		addr = (void *)(EMAC0_MM_SW_RST0);
		emac_rst = EMAC0_SW_RST;
	}
	else if (bus_id == EMAC1)
	{
		//printf("reset EMAC1...\n");
		addr = (void *)(EMAC1_PP_SW_RST0);
		emac_rst = EMAC1_SW_RST;
	}
	else
	{
		printf("EMAC:only support EMAC0 and EMAC1 reset\n");
		return;
	}
...
static void emac0_clk_init(void)
{
	u32 value;
	void *addr;

	//printf("configure EMAC0 clk...\n");

	/* enable aclk & pclk */
	addr = (void *)(EMAC0_MM_CLK_EB1);
	value = readl(addr);
	value |= (0x1 << EMAC0_ACLK);
	writel(value, addr);

	/* enable ephy ref */
	addr = (void *)(EMAC_EPHY_REF_EB);
	value = readl(addr);
	//if ((value>>9) & 0x1 == 0x0)
	value |= (0x1 << 9);
	writel(value, addr);
	...
	/* enable rgmii tx clk */
	addr =  (void *)(EMAC0_MM_CLK_EB0);
	value = readl(addr);
	value |= (0x1 << EMAC0_RGMII_TX_EB);
	writel(value, addr);

	/* configure rmii clk */
	addr = (void *)(EMAC1_PP_CLK_EB0);
	value = readl(addr);
...
static void emac1_clk_init(void)
{
	u32 value;
	void *addr;

	//printf("configure EMAC1 clk...\n");

	/* enable aclk & pclk */
	addr = (void *)(EMAC1_PP_CLK_EB1);
	value = readl(addr);
...
static void emac0_phy_select(phy_interface_t interface)
{
	u32 value;
	void *addr;

	addr = (void *)(EMAC0_MM_REG);
	value = readl(addr);

	if (interface == PHY_INTERFACE_MODE_RGMII)
	{
		value |= (0x1 << EMAC0_PHY_IF_SEL);
		//printf("EMAC0: select RGMII mode\n");
	}
	else if (interface == PHY_INTERFACE_MODE_RMII)
	{
		value &= (~(0x1 << EMAC0_PHY_IF_SEL));
		//printf("EMAC0: select RMII mode\n");
	}
	else if (interface == PHY_INTERFACE_MODE_GMII)
	{
		printf("EMAC0: select GMII mode for haps\n");
		return;
	}
	else
	{
		printf("EMAC0: don't support this phy interface\n");
		return;
	}
...
static void emac1_phy_select(phy_interface_t interface)
{
	u32 value;
	void *addr;

	addr = (void *)(EMAC1_PP_REG);
	value = readl(addr);

	if (interface == PHY_INTERFACE_MODE_RGMII)
	{
		value |= (0x1 << EMAC1_PHY_IF_SEL);
		//printk("EMAC1: select RGMII mode\n");
	}
	else if (interface == PHY_INTERFACE_MODE_RMII)
	{
		value &= (~(0x1 << EMAC1_PHY_IF_SEL));
		//printk("EMAC1: select RMII mode\n");
	}
	else if (interface == PHY_INTERFACE_MODE_GMII)
	{
		printk("EMAC1: select GMII mode for haps\n");
		return;
	}
	else
	{
		printk("EMAC1: don't support this phy interface\n");
		return;
	}
...
static int eqos_start_clks_ax650(struct udevice *dev)
{
	...
	if (bus_id == EMAC0)
	{
		//printf("EMAC:configure EMAC0 clk...\n");
		emac0_clk_init();
	}
	else if (bus_id == EMAC1)
	{
		//printf("EMAC:configure EMAC1 clk...\n");
		emac1_clk_init();
	}
	else
	{
		printf("WARNING: only support EMAC0 and EMAC1\n");
		return -1;
	}
	return 0;
}
...
static void select_phy_interface(struct udevice *dev, phy_interface_t interface)
{
	...
	{
	case EMAC0:
		emac0_phy_select(interface);
		break;

	case EMAC1:
		emac1_phy_select(interface);
		break;

	default:
		printf("EMAC:only support EMAC0 and EMAC1 Phy\n");
		break;
	}
}

#ifdef USE_CLK_FROM_DTS
static void eqos_stop_clks_ax650(struct udevice *dev)
{
	struct eqos_priv *eqos = dev_get_priv(dev);
...
void emac_phy_reset(struct udevice *dev)
{
	struct eqos_priv *eqos = dev_get_priv(dev);
...
static int eqos_start_resets_ax650(struct udevice *dev)
{
	struct eqos_priv *eqos = dev_get_priv(dev);

	emac_sw_rst(eqos->id);
#ifndef CONFIG_AXERA_EMAC_HAPS
	emac_phy_reset(dev);  //reset emac phy
#endif
	return 0;
}

static int eqos_stop_resets_ax650(struct udevice *dev)
{
	return 0;
}
...
#ifdef USE_CLK_FROM_DTS
static ulong eqos_get_tick_clk_rate_ax650(struct udevice *dev)
{
	struct eqos_priv *eqos = dev_get_priv(dev);
...
static void eqos_inval_desc_ax650(void *desc)
{
...
static void eqos_flush_desc_ax650(void *desc)
{
#ifndef CONFIG_SYS_NONCACHED_MEMORY
	unsigned long start = rounddown((unsigned long)desc, ARCH_DMA_MINALIGN);
	unsigned long end = roundup((unsigned long)desc + EQOS_DESCRIPTOR_SIZE,
...
static void eqos_inval_buffer_ax650(void *buf, size_t size)
{
...
static void eqos_flush_buffer_ax650(void *buf, size_t size)
{
	unsigned long start = rounddown((unsigned long)buf, ARCH_DMA_MINALIGN);
	unsigned long end = roundup((unsigned long)buf + size,
				    ARCH_DMA_MINALIGN);
...
#ifndef CONFIG_SYS_NONCACHED_MEMORY
#ifndef CONFIG_AXERA_EMAC_USE_CACHED_DESC
	debug("%s: recovery dcache for emac abnormal exit\n", __func__);
	dcache_enable();
#endif
#endif
	pr_err("FAILED: %d\n", ret);
...
static int eqos_probe_resources_ax650(struct udevice *dev)
{
	struct eqos_priv *eqos = dev_get_priv(dev);
	int ret;
...
static phy_interface_t eqos_get_interface_ax650(struct udevice *dev)
{
	const char *phy_mode;
...
static int eqos_remove_resources_ax650(struct udevice *dev)
{
#ifdef USE_CLK_FROM_DTS
	struct eqos_priv *eqos = dev_get_priv(dev);
...
static struct eqos_ops eqos_ax650_ops = {
	.eqos_inval_desc = eqos_inval_desc_ax650,
	.eqos_flush_desc = eqos_flush_desc_ax650,
	.eqos_inval_buffer = eqos_inval_buffer_ax650,
	.eqos_flush_buffer = eqos_flush_buffer_ax650,
	.eqos_probe_resources = eqos_probe_resources_ax650,
	.eqos_remove_resources = eqos_remove_resources_ax650,
	.eqos_stop_resets = eqos_stop_resets_ax650,
	.eqos_start_resets = eqos_start_resets_ax650,
	.eqos_stop_clks = eqos_stop_clks_ax650,
	.eqos_start_clks = eqos_start_clks_ax650,
	.eqos_set_clk_speed = eqos_set_clk_speed_ax650,
	.eqos_get_tick_clk_rate = eqos_get_tick_clk_rate_ax650
};


static const struct eqos_config eqos_ax650_config = {
...
#endif
	.interface = eqos_get_interface_ax650,
	.ops = &eqos_ax650_ops
};
