...
#ifdef CONFIG_TARGET_AX650_XPILOT_6V
	if (get_board_id()) {
		rst_str = "beta-phy-rst-gpio";
	}
#endif
	err = gpio_request_by_name(pdev, rst_str, 0,
				   &port_pdata->phy_reset_gpio,
				   GPIOD_IS_OUT | GPIOD_IS_OUT_ACTIVE);
	if (err) {
		port_pdata->phy_reset_gpio.dev = NULL;
	}
...
static int dsa_port_probe(struct udevice *pdev)
{
	...
	if (port_pdata->phy_reset_gpio.dev != NULL) {
		dm_gpio_set_value(&port_pdata->phy_reset_gpio, 0);
		mdelay(100);
		dm_gpio_set_value(&port_pdata->phy_reset_gpio, 1);
		mdelay(200);
	}
	...
	if (ops->port_probe) {
		err = ops->port_probe(pdev, port_pdata->index,
				      port_pdata->phy);
		if (err)
			return err;
	}
...
U_BOOT_DRIVER(dsa_port) = {
	...
	.ofdata_to_platdata = dsa_port_of_to_pdata,
...
UCLASS_DRIVER(dsa) = {
	...
	.per_device_auto_alloc_size = sizeof(struct dsa_priv),
	.per_device_platdata_auto_alloc_size = sizeof(struct dsa_pdata),
	.per_child_platdata_auto_alloc_size = sizeof(struct dsa_port_pdata),
