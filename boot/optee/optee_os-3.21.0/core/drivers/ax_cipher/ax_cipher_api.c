...
uint32_t ax_cipher_get_random_number(uint8_t * p_random_number, uint32_t size)
{
	...
	if(i == MAX_TRNG_CONFIG_TIMES) {
		ax_cipher_err("trng config failed!\n");
		ax_cipher_unlock();
		ax_cipher_cmm_free(&cmm);
		return ret;
	}
...
int32_t ax_cipher_huk_init(void)
{
	int as_id;
	int ret;
	ret = ax_cipher_asset_search(PUBLIC_DATA_ASSET_NUMBER, &as_id);
	...
	ret = ax_cipher_asset_search(0, &as_id);
	if (ret == AX_CIPHER_SUCCESS) {
		ret = ax_cipher_otp_public_init();
		return ret;
	}
	ret = ax_cipher_provision_random_rootkey();
	if (ret < 0) {
		ax_cipher_err("fail:%d\n", ret);
		return ret;
	}
	ret = ax_cipher_otp_public_init();
	return ret;
...
int32_t ax_cipher_huk_read(uint8_t * huk, uint32_t count)
{
	int ret;
	ret = ax_cipher_otp_public_read(huk, count);
	return ret;
}
