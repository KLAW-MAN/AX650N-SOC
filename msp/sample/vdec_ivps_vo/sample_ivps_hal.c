...
static AX_U32 gIvpsStartGrpId = 0;
static AX_U32 gIvpsGrpNum = 0;
...
int SampleIVPS_Init(AX_BOOL bEnaIvpsBakFrm, AX_U32 u32StartGrpId, AX_U32 u32GrpNum, AX_U32 u32WinW, AX_U32 u32WinH)
{
    ...
    int ch = 0;
    AX_U32 i = 0;
    IVPS_GRP ivpsGrpId = u32StartGrpId;
    ...
    stPipelineAttr.tFilter[ch][0].tFRC.fSrcFrameRate = 25;
    stPipelineAttr.tFilter[ch][0].tFRC.fDstFrameRate = 25;
    stPipelineAttr.tFilter[ch][0].nDstPicWidth = u32WinW;
    stPipelineAttr.tFilter[ch][0].nDstPicHeight = u32WinH;
    ...
    for (i = 0; i < u32GrpNum; i++) {
        ivpsGrpId = i + u32StartGrpId;
        ...
        SAMPLE_LOG("ivps start grp %d +++++++++++\n", ivpsGrpId);
    }

    gIvpsGrpNum = u32GrpNum;
    gIvpsStartGrpId = u32StartGrpId;
...
AX_S32 SampleIvpsExit(AX_BOOL bEnaIvpsBakFrm)
{
    ...
    int ch = 0;
    AX_U32 i = 0;
    IVPS_GRP ivpsGrpId = gIvpsStartGrpId;

    SAMPLE_LOG("start +++++++++++");
    for (i = 0; i < gIvpsGrpNum; i++) {
        ivpsGrpId = gIvpsStartGrpId + i;
        ch = 0;
...
AX_S32 SampleIvpsReset()
{
    ...
    for (i = 0; i < gIvpsGrpNum; i++) {
        axRet = AX_IVPS_ResetGrp(gIvpsStartGrpId + i);
        if (0 != axRet) {
            SAMPLE_ERR_LOG("AX_IVPS_DestoryGrp axRet:%#x\n", axRet);
        }
    }
