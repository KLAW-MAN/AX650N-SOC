...
static AX_VOID SAMPLE_VSYNC_CallBack(AX_U32 u32Seq, AX_U32 u32Sec, AX_U32 u32USec, AX_VOID *pPrivateData)
{
     ...
     SAMPLE_PRT("[%06ld.%06ld] u32Seq:%d, u32Sec:%d, u32USec:%d\n", start.tv_sec, start.tv_nsec/1000, u32Seq, u32Sec, u32USec);
}
...
static AX_VO_PCLK_ADJ_TYPE_E g_enAdjType = AX_VO_PCLK_ADJ_TYPE_FVCO;
static AX_VO_PCLK_ADJ_MODE_E g_enAdjMode = AX_VO_PCLK_ADJ_MODE_FRA;
...
static AX_VOID *SAMPLE_VO_CTRL_THREAD(AX_VOID *pData)
{
    ...
    AX_PCLK_ADJ_PARAM_T stPclkAdjParam = {0};
    ...
    while (pstSampleVoCtrlParam->u32ThreadForceStop && !bQuit) {
        ...
        case 'a':
        case 'A':
            s32Ret = AX_VO_PCLK_AdjGet(0, g_enAdjType, &stPclkAdjParam);
            SAMPLE_PRT("PCLK-Adj before enAdjType:%d, u32Fref:%d, u32Fvco:%d, u32PreDiv:%d, u32PostDiv:%d, u32FbkInt:0x%x, u32FbkFra:0x%x\n", g_enAdjType,
                       stPclkAdjParam.u32Fref, stPclkAdjParam.u32Fvco,
                       stPclkAdjParam.u32PreDiv, stPclkAdjParam.u32PostDiv,
                       stPclkAdjParam.u32FbkInt, stPclkAdjParam.u32FbkFra);

            if (key == 'a') {
                stPclkAdjParam.u32FbkFra += 0x40000;
            } else {
                if (stPclkAdjParam.u32FbkFra) {
                    stPclkAdjParam.u32FbkFra -= 0x40000;
                }
            }

            s32Ret = AX_VO_PCLK_AdjSet(0, g_enAdjType, g_enAdjMode, &stPclkAdjParam);
	    SAMPLE_PRT("PCLK-Adj enAdjType:%d, enAdjMode:%d, u32FbkInt:0x%x, u32FbkFra:0x%x %s\n",
                       g_enAdjType, g_enAdjMode,
                       stPclkAdjParam.u32FbkInt, stPclkAdjParam.u32FbkFra,
                       s32Ret ? "failed" : "success");
	    s32Ret = AX_VO_PCLK_AdjGet(0, g_enAdjType, &stPclkAdjParam);
            SAMPLE_PRT("PCLK-Adj after enAdjType:%d, u32Fref:%d, u32Fvco:%d, u32PreDiv:%d, u32PostDiv:%d, u32FbkInt:0x%x, u32FbkFra:0x%x\n", g_enAdjType,
                       stPclkAdjParam.u32Fref, stPclkAdjParam.u32Fvco,
                       stPclkAdjParam.u32PreDiv, stPclkAdjParam.u32PostDiv,
                       stPclkAdjParam.u32FbkInt, stPclkAdjParam.u32FbkFra);
            break;
...
AX_S32 VoInit(SAMPLE_VO_CONFIG_S *pstVoConf, AX_U32 u32WinNum)
{
    ...
    for (i = 0; i < pstVoConf->u32LayerNr; i++) {
        ...
        case 1:
            ...
            pstVoLayerConf->s32Chns = 1;
            break;
        case 2:
            pstVoLayerConf->enVoMode = VO_MODE_2MUX;
            pstVoLayerConf->s32Chns = 2;
            break;
        case 3:
        case 4:
            pstVoLayerConf->enVoMode = VO_MODE_4MUX;
            pstVoLayerConf->s32Chns = 4;
            break;
        case 5:
        case 6:
        case 7:
        case 8:
            pstVoLayerConf->enVoMode = VO_MODE_8MUX;
            pstVoLayerConf->s32Chns = 8;
            break;
        case 9:
            pstVoLayerConf->enVoMode = VO_MODE_9MUX;
            pstVoLayerConf->s32Chns = 9;
            break;
        default:
            pstVoLayerConf->enVoMode = VO_MODE_16MUX;
            pstVoLayerConf->s32Chns = 16;
            break;
        }
    ...
    if (g_vo_debug) {
        pstVoConf->stVoDev[0].u32Flags |= AX_VO_FLAG_PCLK_ADJ;
        pstVoConf->stVoDev[1].u32Flags |= AX_VO_FLAG_PCLK_ADJ;
    }
