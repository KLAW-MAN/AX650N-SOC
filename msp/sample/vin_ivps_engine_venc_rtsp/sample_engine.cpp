...
AX_S32 SAMPLE_ENGINE_Inference(AX_VIDEO_FRAME_T *pFrame, SAMPLE_ENGINE_Results *pResults)
{
    ...
    if (stride == pFrame->u32Width * 3) {
        memcpy(dst, src, stride * pFrame->u32Height);
    }
    else {
        for (size_t i = 0; i < pFrame->u32Height; i++)
        {
            memcpy(dst + i * pFrame->u32Width * 3, src + i * stride, stride);
        }
    }
    ...
    for (AX_S32 i = 0; i < pResults->obj_count; i++)
    {
        ...
        // printf("x y w h: %d %d %d %d\n", pResults->objs[i].x, pResults->objs[i].y, pResults->objs[i].width, pResults->objs[i].height);
    }
