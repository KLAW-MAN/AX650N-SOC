...
#define AX_SAMPLE_LOG_TAG "sample_vin_ivps_engine_venc_rtsp"
...
AX_S32 SAMPLE_CASE_OS08A20_VIN_IVPS_ENGINE_VENC_RTSP(SAMPLE_PARAM_T *pSampleParam)
{
    AX_S32 nRet = 0;
    ...
    nRet = SAMPLE_Ivps2VencInit(pIVPSGrp, AX_TRUE, pSampleParam->bVencSelect);
    if (0 != nRet) {
    ...
    // channel 0
    for (int chn_idx = 0; chn_idx < pIVPSGrp->tPipelineAttr.nOutChnNum - 1; chn_idx++) {
        stAttr[chn_idx].nChannel = chn_idx;
        stAttr[chn_idx].stVideoAttr.bEnable = AX_TRUE;
        stAttr[chn_idx].stVideoAttr.ePt = PT_H264;
    }
    AX_Rtsp_Init(&pIVPSGrp->pRtspObj, &stAttr[0], pIVPSGrp->tPipelineAttr.nOutChnNum - 1, 0);
    ...
    // if (0 != nRet) {
    //     ALOGE("AX_ENGINE_Init error. ret:0x%x", nRet);
    //     goto EXIT_FAIL7;
    // }

    nRet = SAMPLE_ENGINE_Load(SAMPLE_ENGINE_MODEL_FILE);
    if (0 != nRet) {
        ALOGE("SAMPLE_ENGINE_Load error. ret:0x%x", nRet);
        goto EXIT_FAIL6;
    }

    SAMPLE_IVPS_ThreadStart(pIVPSGrp);
...
EXIT_FAIL6:
    SAMPLE_Ivps2VencDeinit(pIVPSGrp, AX_TRUE, pSampleParam->bVencSelect);
EXIT_FAIL5:
...
SAMPLE_PARAM_T gSampleParam = {
    SAMPLE_CASE_OS08A20_VIN_IVPS_ENGINE_VENC_RTSP_E,
    COMMON_VIN_SENSOR,
    AX_SNS_LINEAR_MODE,
    AX_TRUE,
    0,
...
int main(int argc, char *argv[])
{
    ...
    while ((c = getopt_long(argc, argv, "c:v:h:n:s:", long_options, &option_index)) != -1) {
        isExit = AX_FALSE;
        switch (c) {
        case 'c':
        ...
        case 'n':
            ...
            break;
        case 's':
            gSampleParam.bVencSelect = atoi(optarg);
            break;
    ...
    if (gSampleParam.eSampleCase == SAMPLE_CASE_OS08A20_VIN_IVPS_ENGINE_VENC_RTSP_E) {
        SAMPLE_CASE_OS08A20_VIN_IVPS_ENGINE_VENC_RTSP(&gSampleParam);
    }
    exit(0);
}
