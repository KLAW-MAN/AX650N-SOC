...
AX_F32 sample_awb_int_convert_to_float(AX_S32 p, AX_S32 int_bit, AX_S32 frac_bit, AX_BOOL signed_value)
{
    AX_F32 result = 0.0;
...
AX_U32 sample_awb_float_convert_to_int(AX_F32 value, AX_U32 int_bit, AX_U32 frac_bit, AX_BOOL signed_value)
{
    AX_U32 result = 0;
    if ((int_bit + frac_bit + signed_value) > 32) {
        printf("%s, invalid parameters\n", __func__);
    ...
    AX_ISP_AWB_RESULT_T *pAwbResult)
{
    ...
    pAwbResult->nRgain = sample_awb_float_convert_to_int(pStatus->rGain, 4, 8, AX_FALSE);
    pAwbResult->nGrGain = sample_awb_float_convert_to_int(pStatus->grGain, 4, 8, AX_FALSE);
    pAwbResult->nGbGain = sample_awb_float_convert_to_int(pStatus->gbGain, 4, 8, AX_FALSE);
    pAwbResult->nBgain = sample_awb_float_convert_to_int(pStatus->bGain, 4, 8, AX_FALSE);
    pAwbResult->nColorTemp = 5000;
...
)
{
    ...
    pCtx->tStatus.blkColNum = pAwbStat->tAwbGridStats.nZoneColSize;
    pCtx->tStatus.blkRowNum = pAwbStat->tAwbGridStats.nZoneRowSize;;
    ...
    for (int i = 0; i < pCtx->tStatus.blkRowNum ; i++) {
        ...
        // Calculate the R, G, B, Y Values of Each AWB Statistics Block.
            if(pAwbStat->tAwbGridStats.tAwbGridStats[i * (pCtx->tStatus.blkColNum) + j].nUnSatGridNum[0][0] == 0 &&
                pAwbStat->tAwbGridStats.tAwbGridStats[i * (pCtx->tStatus.blkColNum) + j].nUnSatGridNum[0][1] == 0 &&
                pAwbStat->tAwbGridStats.tAwbGridStats[i * (pCtx->tStatus.blkColNum) + j].nUnSatGridNum[0][2] == 0 &&
                pAwbStat->tAwbGridStats.tAwbGridStats[i * (pCtx->tStatus.blkColNum) + j].nUnSatGridNum[0][3] == 0) {

                pCtx->tStatus.blkRg[i*pCtx->tStatus.blkColNum+j] = 0.0f;
                pCtx->tStatus.blkBg[i*pCtx->tStatus.blkColNum+j] = 0.0f;
                pCtx->tStatus.blkY[i*pCtx->tStatus.blkColNum+j] = 0.0f;

                continue;
            }

            AX_U32 gridNum = pAwbStat->tAwbGridStats.tAwbGridStats[i * (pCtx->tStatus.blkColNum) + j].nUnSatGridNum[0][0];
            rSum  =  pAwbStat->tAwbGridStats.tAwbGridStats[i * (pCtx->tStatus.blkColNum) + j].nUnSatGridSum[0][0];
            grSum  =  pAwbStat->tAwbGridStats.tAwbGridStats[i * (pCtx->tStatus.blkColNum) + j].nUnSatGridSum[0][1];
            gbSum =  pAwbStat->tAwbGridStats.tAwbGridStats[i * (pCtx->tStatus.blkColNum) + j].nUnSatGridSum[0][2];
            bSum =  pAwbStat->tAwbGridStats.tAwbGridStats[i * (pCtx->tStatus.blkColNum) + j].nUnSatGridSum[0][3];

            float gSum = 0.5 * (grSum + gbSum);
            float y = (0.299 * rSum + 0.587 * gSum + 0.114 * bSum) / gridNum;

            // In HDR mode, the Y of the AWB statistic needs to be reduced by 256 times. SDR mode is not required
            if(pCtx->tStatus.eSnsMode != AX_SNS_LINEAR_MODE) {
                y /= 256.0;
            }
            ...
            if (gSum < 0.00001) {
                ...
                pCtx->tStatus.blkRg[i] = rSum / gSum;
                pCtx->tStatus.blkBg[i] = bSum / gSum;
            }
        }
...
{
    pAwbResult->nRgain  = sample_awb_float_convert_to_int(pStatus->rGain, 4, 8, false);
    pAwbResult->nGrGain = sample_awb_float_convert_to_int(pStatus->grGain, 4, 8, false);
    pAwbResult->nGbGain = sample_awb_float_convert_to_int(pStatus->gbGain, 4, 8, false);
    pAwbResult->nBgain  = sample_awb_float_convert_to_int(pStatus->bGain, 4, 8, false);
    pAwbResult->nColorTemp  = (AX_U16) (pStatus->prevCct);
    pAwbResult->nSatDiscnt  = 100;
    printf("AWB[pipe=%d]: Gains (R, Gr, Gb, B) = (%.3f, %.3f, %.3f, %.3f),  CCT = %.2f \n",
           pipe, pStatus->rGain, pStatus->grGain, pStatus->gbGain, pStatus->bGain, pStatus->prevCct);
    return 0;
}
...
)
{
    ...
    sample_awb_gen_api_output(pipe, &(pCtx->tStatus) , pAwbResult);
