        ...
        SAMPLE_VIN_PARAM_T *pVinParam, COMMON_SYS_ARGS_T *pCommonArgs)
{
    ...
    pCam->eHdrMode = eHdrMode;
    pCam->eSysMode = eSysMode;
    pCam->tPipeAttr[pCam->nPipeId].bAiIspEnable = pVinParam->bAiispEnable;
        ...
        SAMPLE_VIN_PARAM_T *pVinParam, COMMON_SYS_ARGS_T *pCommonArgs)
{
    ...
    SAMPLE_PIPE_INFO_T tSamplePipeInfo_hdr_2x[] = {
        {AX_VIN_PIPE_NORMAL_MODE0, SAMPLE_PIPE_MODE_VIDEO, AX_TRUE,  "null.bin"},                                  /* t3dnr */
    ...
    for (i = 0; i < pCommonArgs->nCamCnt; i++) {
        ...
        for (AX_S32 j = 0; j < pCam->tDevBindPipe.nNum; j++) {
            ...
            strncpy(pCam->tPipeInfo[j].szBinPath, (pSamplePipeInfo + pCam->nPipeId)->szBinPath, sizeof(pCam->tPipeInfo[j].szBinPath));
        }
        pCam->nPipeId = i;
        ...
        SAMPLE_VIN_PARAM_T *pVinParam, COMMON_SYS_ARGS_T *pCommonArgs)
{
    ...
    SAMPLE_PIPE_INFO_T tSamplePipeInfo[] = {
        ...
        {AX_VIN_PIPE_NORMAL_MODE0, SAMPLE_PIPE_MODE_VIDEO, AX_FALSE, "/opt/etc/os08a20_sdr_t2dnr.bin"},
        {AX_VIN_PIPE_NORMAL_MODE0, SAMPLE_PIPE_MODE_FLASH_SNAP, AX_TRUE, "/opt/etc/os08a20_sdr_ai2dnr.bin"},
    };
    pCommonArgs->nCamCnt = 1;
...
int main(int argc, char *argv[])
{
    ...
    if (tVinParam.eSysCase >= SAMPLE_VIN_DOUBLE_OS08A20_MULTIPLE_PIPE || tVinParam.eSysCase <= SAMPLE_VIN_NONE) {
        COMM_ISP_PRT("error sys case : %d\n", tVinParam.eSysCase);
        exit(0);
    }

    if (tVinParam.eSysMode >= COMMON_VIN_TPG || tVinParam.eSysMode <= COMMON_VIN_NONE) {
        COMM_ISP_PRT("error sys work mode : %d\n", tVinParam.eSysMode);
        exit(0);
    }
    if (tVinParam.eHdrMode >= AX_SNS_HDR_3X_MODE || tVinParam.eHdrMode <= AX_SNS_MODE_NONE) {
        COMM_ISP_PRT("error sys SDR/HDR mode : %d\n", tVinParam.eHdrMode);
        exit(0);
    }
