...
static AX_SENSOR_REGISTER_FUNC_T *pSnsHandle[AX_VIN_MAX_PIPE_NUM];

static SAMPLE_AEC_CTX_T g_tAeAlgCtx[AX_VIN_MAX_PIPE_NUM];
...
AX_S32 sample_ae_sns_get_param(AX_U8       pipe, AX_SNS_EXP_CTRL_PARAM_T *pSnsParam)
{
    AX_S32 result = 0;

    if (AX_NULL != pSnsHandle[pipe]->pfn_sensor_get_hw_exposure_params) {
        result = pSnsHandle[pipe]->pfn_sensor_get_hw_exposure_params(pipe, pSnsParam);
        if (SAMPLE_SUCCESS != result) {
            return SAMPLE_FAILED;
        }
    } else {
...
{
    AX_S32 result = 0;
    AX_SNS_EXP_CTRL_PARAM_T tSnsParam;
    tSnsParam.sns_dev_attr.eSnsMode = AX_SNS_LINEAR_MODE;

    if (AX_NULL != pSnsHandle[pipe]->pfn_sensor_get_hw_exposure_params) {
        result = pSnsHandle[pipe]->pfn_sensor_get_hw_exposure_params(pipe, &tSnsParam);
        if (SAMPLE_SUCCESS != result) {
            return SAMPLE_FAILED;
        }
    } else {
...
)
{
    ...
    switch (tSnsParam.eSnsHcgLcgMode) {
    case AX_HCG_MODE:
        *phcgEn = 1;
...
)
{
    ...
    pCtxManual->fGain    = pCtxStatus->snsRegAgain[AX_SNS_HDR_FRAME_L];
    pCtxManual->fDgain   = pCtxStatus->snsDgain[AX_SNS_HDR_FRAME_L];
    ...
    pCtxManual->fIntegrationTime = max(pCtxStatus->snsShutter[AX_SNS_HDR_FRAME_L], pCtxConfig->IntegrationTimeLimit.fMin);
    AX_U32 lineNum = (AX_U32)(pCtxManual->fIntegrationTime / pCtxConfig->flineTime);
    AX_F32 lineNum_frac = pCtxManual->fIntegrationTime / pCtxConfig->flineTime - (AX_F32)lineNum;
    if( lineNum_frac >= 0.5){
        lineNum += 1;
    }
    pCtxManual->fIntegrationTime = lineNum * pCtxConfig->flineTime;
    ...
    if (pCtxConfig->eSnsMode == AX_SNS_LINEAR_MODE) {
        ...
        tGainTbl.fGain[AXAE_LONG_FRAME] = pCtxManual->fGain;
        sample_ae_sns_set_again(pipe, &tGainTbl);

        tGainTbl.fGain[AXAE_LONG_FRAME] = pCtxManual->fDgain;
        sample_ae_sns_set_dgain(pipe, &tGainTbl);

        tShutterTbl.fIntTime[AXAE_LONG_FRAME] = lineNum;
        tShutterTbl.fHdrRatio[AXAE_LONG_FRAME] = pAlgCtx->tConfig.hdrRatio;
        ...
        // Set Gains and Shutter
        tGainTbl.fGain[AXAE_LONG_FRAME] = pCtxManual->fGain;
        tGainTbl.fGain[AXAE_MEDIUM_FRAME] = pCtxManual->fGain;
        sample_ae_sns_set_again(pipe, &tGainTbl);

        tGainTbl.fGain[AXAE_LONG_FRAME] = pCtxManual->fDgain;
        tGainTbl.fGain[AXAE_MEDIUM_FRAME] = pCtxManual->fDgain;
        sample_ae_sns_set_dgain(pipe, &tGainTbl);

        tShutterTbl.fIntTime[AXAE_LONG_FRAME] = lineNum;
        tShutterTbl.fHdrRatio[AXAE_LONG_FRAME] = pAlgCtx->tConfig.hdrRatio;
        tShutterTbl.fIntTime[AXAE_MEDIUM_FRAME] = lineNum / pAlgCtx->tConfig.hdrRatio;
        tShutterTbl.fHdrRatio[AXAE_MEDIUM_FRAME] = pAlgCtx->tConfig.hdrRatio;
        sample_ae_sns_set_shutter(pipe, &tShutterTbl);
        ...
        tGainTbl.fGain[AXAE_LONG_FRAME] = pCtxManual->fGain;
        tGainTbl.fGain[AXAE_MEDIUM_FRAME] = pCtxManual->fGain;
        tGainTbl.fGain[AXAE_SHORT_FRAME] = pCtxManual->fGain;
        sample_ae_sns_set_again(pipe, &tGainTbl);

        tGainTbl.fGain[AXAE_LONG_FRAME] = pCtxManual->fDgain;
        tGainTbl.fGain[AXAE_MEDIUM_FRAME] = pCtxManual->fDgain;
        tGainTbl.fGain[AXAE_SHORT_FRAME] = pCtxManual->fDgain;
        sample_ae_sns_set_dgain(pipe, &tGainTbl);
        ...
        tShutterTbl.fIntTime[AXAE_MEDIUM_FRAME] = lineNum / pAlgCtx->tConfig.hdrRatio;
        tShutterTbl.fHdrRatio[AXAE_MEDIUM_FRAME] = pAlgCtx->tConfig.hdrRatio;
        tShutterTbl.fIntTime[AXAE_SHORT_FRAME] = lineNum;
        tShutterTbl.fHdrRatio[AXAE_SHORT_FRAME] = pAlgCtx->tConfig.hdrRatio;
        sample_ae_sns_set_shutter(pipe, &tShutterTbl);
    }
...
)
{
    ...
    snsDgain        = tSnsParam.sns_ae_param.fCurDGain[AXAE_LONG_FRAME];
    snsRegAgain     = tSnsParam.sns_ae_param.fCurAGain[AXAE_LONG_FRAME];
    ...
    snsShutter = tSnsParam.sns_ae_param.fCurIntegrationTime[AXAE_LONG_FRAME] * tSnsParam.fTimePerLine;

    // Total EV = Total Gain * Integration Time
    totalExposure = totalGain * snsShutter;

    // Update Context's Exposure Settings
    pAlgCtx->tStatus.snsDgain[AX_SNS_HDR_FRAME_L]        = snsDgain;
    pAlgCtx->tStatus.snsRegAgain[AX_SNS_HDR_FRAME_L]     = snsRegAgain;
    pAlgCtx->tStatus.snsTotalRegGain[AX_SNS_HDR_FRAME_L] = snsTotalRegGain;
    pAlgCtx->tStatus.snsTotalAgain[AX_SNS_HDR_FRAME_L]   = snsTotalAgain;
    pAlgCtx->tStatus.snsTotalGain[AX_SNS_HDR_FRAME_L]    = snsTotalGain;
    pAlgCtx->tStatus.totalGain[AX_SNS_HDR_FRAME_L]       = totalGain;
    pAlgCtx->tStatus.snsShutter[AX_SNS_HDR_FRAME_L]      = snsShutter;
    pAlgCtx->tStatus.totalExposure[AX_SNS_HDR_FRAME_L]   = totalExposure;
    ...
    pAlgCtx->tStatus.hdrHwMaxShutter = tSnsParam.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[AX_SNS_HDR_FRAME_L] * tSnsParam.fTimePerLine;
    pAlgCtx->tStatus.algoMaxShutter = min(pAlgCtx->tStatus.hdrHwMaxShutter, pAlgCtx->tConfig.maxUserShutter);
    ...
    if (hdrMode == AX_SNS_LINEAR_MODE) {
        ...
        shortShutter  = tSnsParam.sns_ae_param.fCurIntegrationTime[AXAE_MEDIUM_FRAME] * tSnsParam.fTimePerLine;
        shortDgain    = tSnsParam.sns_ae_param.fCurDGain[AXAE_MEDIUM_FRAME];
        shortRegAgain = tSnsParam.sns_ae_param.fCurAGain[AXAE_MEDIUM_FRAME];
        ...
        // Update Context's Exposure Settings
        pAlgCtx->tStatus.snsDgain[AX_SNS_HDR_FRAME_M] 	   = shortDgain;
        pAlgCtx->tStatus.snsRegAgain[AX_SNS_HDR_FRAME_M]	   = shortRegAgain;
        pAlgCtx->tStatus.snsTotalRegGain[AX_SNS_HDR_FRAME_M] = shortTotalRegGain;
        pAlgCtx->tStatus.snsTotalAgain[AX_SNS_HDR_FRAME_M]   = shortTotalAgain;
        pAlgCtx->tStatus.snsTotalGain[AX_SNS_HDR_FRAME_M]    = shortSnsTotalGain;
        pAlgCtx->tStatus.totalGain[AX_SNS_HDR_FRAME_M]	   = shortTotalGain;
        pAlgCtx->tStatus.snsShutter[AX_SNS_HDR_FRAME_M]	   = shortShutter;
        pAlgCtx->tStatus.totalExposure[AX_SNS_HDR_FRAME_M]   = shortTotalExposure;
        ...
        vsShutter  = tSnsParam.sns_ae_param.fCurIntegrationTime[AXAE_SHORT_FRAME] * tSnsParam.fTimePerLine;
        vsDgain    = tSnsParam.sns_ae_param.fCurDGain[AXAE_SHORT_FRAME];
        vsRegAgain = tSnsParam.sns_ae_param.fCurAGain[AXAE_SHORT_FRAME];
        ...
        // Update Context's Exposure Settings
        pAlgCtx->tStatus.snsDgain[AX_SNS_HDR_FRAME_S] 	   = vsDgain;
        pAlgCtx->tStatus.snsRegAgain[AX_SNS_HDR_FRAME_S]	   = vsRegAgain;
        pAlgCtx->tStatus.snsTotalRegGain[AX_SNS_HDR_FRAME_S] = vsTotalRegGain;
        pAlgCtx->tStatus.snsTotalAgain[AX_SNS_HDR_FRAME_S]   = vsTotalAgain;
        pAlgCtx->tStatus.snsTotalGain[AX_SNS_HDR_FRAME_S]    = vsSnsTotalGain;
        pAlgCtx->tStatus.totalGain[AX_SNS_HDR_FRAME_S]	   = vsTotalGain;
        pAlgCtx->tStatus.snsShutter[AX_SNS_HDR_FRAME_S]	   = vsShutter;
        pAlgCtx->tStatus.totalExposure[AX_SNS_HDR_FRAME_S]   = vsTotalExposure;
...
(
    ...
    pCtxConfig->antiFlickerParam.antiflickerMode = 1;
    ...
    //Init Hardware limit.
    pCtxConfig->IntegrationTimeLimit.fMax =  tSnsParam.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[AXAE_LONG_FRAME] * tSnsParam.fTimePerLine;
    pCtxConfig->IntegrationTimeLimit.fMin =  tSnsParam.sns_ae_limit.tIntTimeRange.fMinIntegrationTime[AXAE_LONG_FRAME] * tSnsParam.fTimePerLine;
    pCtxConfig->IntegrationTimeLimit.fStep = tSnsParam.sns_ae_param.fIntegrationTimeIncrement[AXAE_LONG_FRAME];

    pCtxConfig->AgainLimit.fMax = tSnsParam.sns_ae_limit.fMaxAgain[AXAE_LONG_FRAME];
    pCtxConfig->AgainLimit.fMin = tSnsParam.sns_ae_limit.fMinAgain[AXAE_LONG_FRAME];
    pCtxConfig->AgainLimit.fStep = tSnsParam.sns_ae_param.fAGainIncrement[AXAE_LONG_FRAME];

    pCtxConfig->DgainLimit.fMax = tSnsParam.sns_ae_limit.fMaxDgain[AXAE_LONG_FRAME];
    pCtxConfig->DgainLimit.fMin = tSnsParam.sns_ae_limit.fMinDgain[AXAE_LONG_FRAME];
    pCtxConfig->DgainLimit.fStep = tSnsParam.sns_ae_param.fDGainIncrement[AXAE_LONG_FRAME];

    pCtxConfig->IspDgainLimit.fMax = 64.0f;
    pCtxConfig->IspDgainLimit.fMin = 1.0f;
    pCtxConfig->IspDgainLimit.fStep = tSnsParam.sns_ae_param.fIspDGainIncrement[AXAE_LONG_FRAME];

    pCtxConfig->sIntegrationTime.fMax =  tSnsParam.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[AXAE_MEDIUM_FRAME]  * tSnsParam.fTimePerLine;
    pCtxConfig->sIntegrationTime.fMin =  tSnsParam.sns_ae_limit.tIntTimeRange.fMinIntegrationTime[AXAE_MEDIUM_FRAME]  * tSnsParam.fTimePerLine;
    pCtxConfig->sIntegrationTime.fStep = tSnsParam.sns_ae_param.fIntegrationTimeIncrement[AXAE_MEDIUM_FRAME];

    pCtxConfig->sGain.fMax = tSnsParam.sns_ae_limit.fMaxAgain[AXAE_MEDIUM_FRAME];
    pCtxConfig->sGain.fMin = tSnsParam.sns_ae_limit.fMinAgain[AXAE_MEDIUM_FRAME];
    pCtxConfig->sGain.fStep = tSnsParam.sns_ae_param.fAGainIncrement[AXAE_MEDIUM_FRAME];

    pCtxConfig->sDgain.fMax = tSnsParam.sns_ae_limit.fMaxDgain[AXAE_MEDIUM_FRAME];
    pCtxConfig->sDgain.fMin = tSnsParam.sns_ae_limit.fMinDgain[AXAE_MEDIUM_FRAME];
    pCtxConfig->sDgain.fStep = tSnsParam.sns_ae_param.fDGainIncrement[AXAE_MEDIUM_FRAME];

    pCtxConfig->sVsintegrationTime.fMax =  tSnsParam.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[AXAE_SHORT_FRAME]  * tSnsParam.fTimePerLine;
    pCtxConfig->sVsintegrationTime.fMin =  tSnsParam.sns_ae_limit.tIntTimeRange.fMinIntegrationTime[AXAE_SHORT_FRAME]  * tSnsParam.fTimePerLine;
    pCtxConfig->sVsintegrationTime.fStep = tSnsParam.sns_ae_param.fIntegrationTimeIncrement[AXAE_SHORT_FRAME];

    pCtxConfig->sVsgain.fMax = tSnsParam.sns_ae_limit.fMaxAgain[AXAE_SHORT_FRAME];
    pCtxConfig->sVsgain.fMin = tSnsParam.sns_ae_limit.fMinAgain[AXAE_SHORT_FRAME];
    pCtxConfig->sVsgain.fStep = tSnsParam.sns_ae_param.fAGainIncrement[AXAE_SHORT_FRAME];

    pCtxConfig->sVsdgain.fMax = tSnsParam.sns_ae_limit.fMaxDgain[AXAE_SHORT_FRAME];
    pCtxConfig->sVsdgain.fMin = tSnsParam.sns_ae_limit.fMinDgain[AXAE_SHORT_FRAME];
    pCtxConfig->sVsdgain.fStep = tSnsParam.sns_ae_param.fDGainIncrement[AXAE_SHORT_FRAME];

    pCtxConfig->fHwMinHdrRatio   = tSnsParam.sns_ae_limit.fMinRatio;
    pCtxConfig->fHwMaxHdrRatio   = tSnsParam.sns_ae_limit.fMaxRatio;
    pCtxConfig->fTotalGainMin    = pCtxConfig->AgainLimit.fMin * pCtxConfig->DgainLimit.fMin * pCtxConfig->IspDgainLimit.fMin;
    pCtxConfig->fTotalGainMax    = pCtxConfig->AgainLimit.fMax * pCtxConfig->DgainLimit.fMax * pCtxConfig->IspDgainLimit.fMax * tSnsParam.fSnsHcgLcgRatio;
    pCtxConfig->fSnsTotalGainMin = pCtxConfig->AgainLimit.fMin * pCtxConfig->DgainLimit.fMin;
    pCtxConfig->fSnsTotalGainMax = pCtxConfig->AgainLimit.fMax * pCtxConfig->DgainLimit.fMax* tSnsParam.fSnsHcgLcgRatio;

    // flineTime is time of each line
    pCtxConfig->flineTime = tSnsParam.fTimePerLine;
    pCtxConfig->fReadLineTime = tSnsParam.fLineTime;
    ...
    pCtxConfig->fHcglcgratio = tSnsParam.fSnsHcgLcgRatio;
...
(
    ...
    pAlgStatus->currHcgLcg   = tSnsParam.eSnsHcgLcgMode;
    pAlgStatus->currHcgRatio = pAlgConfig->againRatio_hcg2lcg;
...
{
    pAeResult->nIntTime[AX_SNS_HDR_FRAME_L]  = pStatus->snsShutter[AX_SNS_HDR_FRAME_L];
    pAeResult->nAgain[AX_SNS_HDR_FRAME_L]    = sample_ae_float_convert_to_int(pStatus->snsTotalAgain[AX_SNS_HDR_FRAME_L], 22, 10, false);
    pAeResult->nDgain[AX_SNS_HDR_FRAME_L]    = sample_ae_float_convert_to_int(pStatus->snsDgain[AX_SNS_HDR_FRAME_L], 22, 10, false);
    pAeResult->nTotalGain[AX_SNS_HDR_FRAME_L]    = sample_ae_float_convert_to_int(pStatus->totalGain[AX_SNS_HDR_FRAME_L], 22, 10, false);

    pAeResult->nIntTime[AX_SNS_HDR_FRAME_M]  = pStatus->snsShutter[AX_SNS_HDR_FRAME_M];
    pAeResult->nAgain[AX_SNS_HDR_FRAME_M]    = sample_ae_float_convert_to_int(pStatus->snsTotalAgain[AX_SNS_HDR_FRAME_M], 22, 10, false);
    pAeResult->nDgain[AX_SNS_HDR_FRAME_M]    = sample_ae_float_convert_to_int(pStatus->snsDgain[AX_SNS_HDR_FRAME_M], 22, 10, false);
    pAeResult->nTotalGain[AX_SNS_HDR_FRAME_M]    = sample_ae_float_convert_to_int(pStatus->totalGain[AX_SNS_HDR_FRAME_M], 22, 10, false);

    pAeResult->nIntTime[AX_SNS_HDR_FRAME_S]  = pStatus->snsShutter[AX_SNS_HDR_FRAME_S];
    pAeResult->nAgain[AX_SNS_HDR_FRAME_S]    = sample_ae_float_convert_to_int(pStatus->snsTotalAgain[AX_SNS_HDR_FRAME_S], 22, 10, false);
    pAeResult->nDgain[AX_SNS_HDR_FRAME_S]    = sample_ae_float_convert_to_int(pStatus->snsDgain[AX_SNS_HDR_FRAME_S], 22, 10, false);
    pAeResult->nTotalGain[AX_SNS_HDR_FRAME_S]    = sample_ae_float_convert_to_int(pStatus->totalGain[AX_SNS_HDR_FRAME_S], 22, 10, false);

    pAeResult->nIntTime[AX_SNS_HDR_FRAME_VS] = pStatus->snsShutter[AX_SNS_HDR_FRAME_VS];
    pAeResult->nAgain[AX_SNS_HDR_FRAME_VS]   = sample_ae_float_convert_to_int(pStatus->snsTotalAgain[AX_SNS_HDR_FRAME_VS], 22, 10, false);
    pAeResult->nDgain[AX_SNS_HDR_FRAME_VS]   = sample_ae_float_convert_to_int(pStatus->snsDgain[AX_SNS_HDR_FRAME_VS] , 22, 10, false);
    pAeResult->nTotalGain[AX_SNS_HDR_FRAME_VS]   = sample_ae_float_convert_to_int(pStatus->totalGain[AX_SNS_HDR_FRAME_VS] , 22, 10, false);

    pAeResult->nIspGain     = sample_ae_float_convert_to_int(pStatus->ispGain, 22, 10, false);

    pAeResult->nHcgLcgMode  = (AX_U8) pStatus->currHcgLcg;
    ...
    if (nHdrMode == 1){
        printf("AE[pipe=%d] Result: shutter=%f, Again=%.2f, ispGain=%.2f, lux=%.2f, Luma=%.2f \n",
            pipe, pStatus->snsShutter[AX_SNS_HDR_FRAME_L],  pStatus->snsTotalAgain[AX_SNS_HDR_FRAME_L],
            pStatus->ispGain, pStatus->lux, pStatus->meanLuma);
    }
    else if(nHdrMode == 2){
        printf("AE[pipe=%d] Result: shutter=%f, Again=%.2f, ispGain=%.2f, lux=%.2f, Luma=%.2f , realhdrRealRatioLtoS = %0.2f\n",
            pipe, pStatus->snsShutter[AX_SNS_HDR_FRAME_L],  pStatus->snsTotalAgain[AX_SNS_HDR_FRAME_L],
            pStatus->ispGain, pStatus->lux, pStatus->meanLuma,pStatus->hdrRealRatioLtoS);
    }
    else if(nHdrMode == 3){
        printf("AE[pipe=%d] Result: shutter=%f, Again=%.2f, ispGain=%.2f, lux=%.2f, Luma=%.2f , realhdrRealRatioLtoS = %0.2f, realhdrRealRatioStoVS = %0.2f,\n",
            pipe, pStatus->snsShutter[AX_SNS_HDR_FRAME_L],  pStatus->snsTotalAgain[AX_SNS_HDR_FRAME_L],
...
)
{
    ...
    // Update Gain Limits
    AX_SNS_EXP_CTRL_PARAM_T tSnsParam = {0};

    nRet = AX_ISP_GetSnsAttr(pipe, &tSnsParam.sns_dev_attr);
    if (0 != nRet) {
        COMM_ISP_PRT("AX_ISP_GetSnsAttr failed, nRet=0x%x.\n", nRet);
          return -1;
    }


    for(int i = 0 ;i<tSnsParam.sns_dev_attr.eSnsMode;i++){
        tSnsParam.sns_ae_limit.fMinAgain[i] = 1.0f;
        tSnsParam.sns_ae_limit.fMaxAgain[i] = 25.5f;
        tSnsParam.sns_ae_limit.fMinDgain[i] = 1.0f;
        tSnsParam.sns_ae_limit.fMaxDgain[i] = 25.5f;
        tSnsParam.sns_ae_limit.tIntTimeRange.fMinIntegrationTime[i] = 1;
        tSnsParam.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[i] = 33000;
        tSnsParam.sns_ae_param.fCurAGain[i] = 2.0f;
        tSnsParam.sns_ae_param.fCurDGain[i] = 1.0f;
        tSnsParam.sns_ae_param.fCurIspDGain[i] = 1.0f;
        tSnsParam.sns_ae_param.fCurIntegrationTime[i] = 10000;
        tSnsParam.sns_ae_param.fAGainIncrement[i] = 1.0f;
        tSnsParam.sns_ae_param.fDGainIncrement[i] = 1.0f;
        tSnsParam.sns_ae_param.fIspDGainIncrement[i] = 1.0f;
        tSnsParam.sns_ae_param.fIntegrationTimeIncrement[i] = 10;

    }

    tSnsParam.sns_ae_limit.fMinRatio = 5.0f;
    tSnsParam.sns_ae_limit.fMaxRatio = 16.0f;
    ...
    tSnsParam.fSnsHcgLcgRatio = 4.05f;

    if (pipe >= AX_VIN_MAX_PIPE_NUM) {
        return -1;
    }
    ...
    if (pCtxConfig->antiFlickerParam.antiflickerMode == AX_ISP_FLICKER_AUTO_DET
        || pCtxConfig->antiFlickerParam.antiflickerMode == AX_ISP_FLICKER_AUTO_ANTI){
        AX_ISP_IQ_AE_STAT_PARAM_T ae_param;
        AX_ISP_IQ_GetAeStatParam(pipe, &ae_param);
        printf("after nRoiRegionNumV:%d, nRoiRegionNumH:%d\n",ae_param.tGridRoi[1].nRoiRegionNumV,ae_param.tGridRoi[1].nRoiRegionNumH);

        AX_AE_AFD_LOG_T AFD_Log_Param;
        AFD_Log_Param.nValidPipeCnt = 1;
        AFD_Log_Param.nAeLogLevel = SYS_LOG_WARN;
        AFD_Log_Param.nAeLogTarget = SYS_LOG_TARGET_SYSLOG;

        AX_ISP_ALG_Ae_AFDInit(pipe, &AFD_Log_Param, ae_param.tGridRoi[1].nRoiRegionNumV, &(pCtxStatus->tAFDOut));
    }

    // set again, dgain, uHcg_lcg, shutter
    pCtxStatus->snsDgain[AX_SNS_HDR_FRAME_L] = 1;
    pCtxStatus->snsRegAgain[AX_SNS_HDR_FRAME_L] = 5;
    pCtxStatus->snsShutter[AX_SNS_HDR_FRAME_L] = 10000;    // 10ms
    pCtxStatus->totalGain[AX_SNS_HDR_FRAME_L] = 5.0;
...
)
{
    SAMPLE_AEC_CTX_T *pAlgCtx = sample_ae_get_ctx_handle(pipe);
    AX_S32 result = 0;
    AX_AE_GRID_STATS_T const *luma_data;
    AX_U32 nZoneRow = 0;
    AX_U32 nZoneCol = 0;
    AX_S32 aeBlockNum = 0;
    ...
    for (AX_S32 i = 0; i < nZoneRow; i++) {
        for (AX_S32 j = 0; j < nZoneCol; j++) {
            AX_F32 tempR_sum = (float)(luma_data[i * nZoneCol + j].nGridSum[0] >> 6) +
                               (luma_data[i * nZoneCol + j].nGridSum[0] & 0x03) / 64.0;
            AX_F32 tempB_sum = (float)(luma_data[i * nZoneCol + j].nGridSum[1] >> 6) +
                               (luma_data[i * nZoneCol + j].nGridSum[1] & 0x03) / 64.0;
            AX_F32 tempGr_sum = (float)(luma_data[i * nZoneCol + j].nGridSum[2] >> 6) +
                                (luma_data[i * nZoneCol + j].nGridSum[2] & 0x03) / 64.0;
            AX_F32 tempGb_sum = (float)(luma_data[i * nZoneCol + j].nGridSum[3] >> 6) +
                                (luma_data[i * nZoneCol + j].nGridSum[3] & 0x03) / 64.0;
...
)
{
    ...
    pCtxStatus->AxPreEV[snsDelayFrame] = pCtxStatus->totalExposure[AX_SNS_HDR_FRAME_L];
...
)
{
    ...
    if ( pCtxConfig->antiFlickerParam.antiflickerMode == AX_ISP_FLICKER_DISABLE)
    {
        Tint = max_shutter_th;
    }else{
...
)
{
     ...
     * in dark environment, it will not reach the maximum integration time due to UI accuracy(U32).
     */
    AX_U32 lineNum = (AX_U32)(SplitIntegrationTime / pCtxConfig->flineTime);
    AX_F32 lineNum_frac = SplitIntegrationTime / pCtxConfig->flineTime - (AX_F32)lineNum;
    if( lineNum_frac >= 0.5){
        lineNum += 1;
    }
    ...
    if (pCtxConfig->eSnsMode == AX_SNS_LINEAR_MODE) {
        ...
        // Set Gains and Shutter
        tGainTbl.fGain[AXAE_LONG_FRAME] = snsRegAgain;
        sample_ae_sns_set_again(pipe, &tGainTbl);

        tGainTbl.fGain[AXAE_LONG_FRAME] = snsDGain;
        sample_ae_sns_set_dgain(pipe, &tGainTbl);

        tShutterTbl.fIntTime[AXAE_LONG_FRAME] = lineNum;
        tShutterTbl.fHdrRatio[AXAE_LONG_FRAME] = suggestHdrRatio;
        sample_ae_sns_set_shutter(pipe, &tShutterTbl);
    } else if (pCtxConfig->eSnsMode == AX_SNS_HDR_2X_MODE) {
        // Set LCG / HCG
        sample_ae_sns_set_lcg_hcg(pipe, HcgLcg_Mode);
        ...
        // Set Gains and Shutter
        tGainTbl.fGain[AXAE_LONG_FRAME] = snsRegAgain;
        tGainTbl.fGain[AXAE_MEDIUM_FRAME] = snsRegAgain;
        sample_ae_sns_set_again(pipe, &tGainTbl);

        tGainTbl.fGain[AXAE_LONG_FRAME] = snsDGain;
        tGainTbl.fGain[AXAE_MEDIUM_FRAME] = snsDGain;
        sample_ae_sns_set_dgain(pipe, &tGainTbl);

        tShutterTbl.fIntTime[AXAE_LONG_FRAME] = lineNum;
        tShutterTbl.fHdrRatio[AXAE_LONG_FRAME] = suggestHdrRatio;
        tShutterTbl.fIntTime[AXAE_MEDIUM_FRAME] = lineNum / suggestHdrRatio;
        tShutterTbl.fHdrRatio[AXAE_MEDIUM_FRAME] = suggestHdrRatio;
        sample_ae_sns_set_shutter(pipe, &tShutterTbl);
    } else if (pCtxConfig->eSnsMode == AX_SNS_HDR_3X_MODE) {
        ...
        tGainTbl.fGain[AXAE_LONG_FRAME] = snsRegAgain;
        tGainTbl.fGain[AXAE_MEDIUM_FRAME] = snsRegAgain;
        tGainTbl.fGain[AXAE_SHORT_FRAME] = snsRegAgain;
        sample_ae_sns_set_again(pipe, &tGainTbl);

        tGainTbl.fGain[AXAE_LONG_FRAME] = snsDGain;
        tGainTbl.fGain[AXAE_MEDIUM_FRAME] = snsDGain;
        tGainTbl.fGain[AXAE_SHORT_FRAME] = snsDGain;
        sample_ae_sns_set_dgain(pipe, &tGainTbl);
        ...
        tShutterTbl.fHdrRatio[AXAE_MEDIUM_FRAME] = suggestHdrRatio;
        tShutterTbl.fIntTime[AXAE_SHORT_FRAME] = lineNum;
        tShutterTbl.fHdrRatio[AXAE_SHORT_FRAME] = suggestHdrRatio;
        sample_ae_sns_set_shutter(pipe, &tShutterTbl);
    }
...
)
{
    ...
    pAlgCtx->tStatus.lux =   pAlgCtx->tStatus.meanLuma * pAlgCtx->tConfig.luxK
                             / pAlgCtx->tStatus.totalExposure[AX_SNS_HDR_FRAME_L];
...
(
    ...
    if (pAlgCtx->tConfig.antiFlickerParam.antiflickerMode == AX_ISP_FLICKER_AUTO_DET || pAlgCtx->tConfig.antiFlickerParam.antiflickerMode == AX_ISP_FLICKER_AUTO_ANTI) {
        AX_F32 fLineTime = 0.0f;
        fLineTime = pAlgCtx->tConfig.fReadLineTime;
        ...
        AX_ISP_ALG_Ae_AFDRun(pipe, &AFD_Log_Param, pAeInputInfo, pCtxStatus->lux, fLineTime, &(pCtxConfig->tAFDParams), &(pCtxStatus->tAFDOut));
        printf("pCtxStatus->tAFDOut.nFlickerFreq:%d\n",pCtxStatus->tAFDOut.nFlickerFreq);
    }

    if (pAlgCtx->bEnable && pipe == 0) {
