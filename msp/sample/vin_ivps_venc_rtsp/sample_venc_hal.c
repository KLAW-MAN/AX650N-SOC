...
static AX_S32 SAMPLE_VencInit(AX_S32 nChnIdx, AX_U16 width, AX_U16 height, AX_MOD_ID_E enModId, AX_BOOL bSelect)
{
...
static AX_S32 SAMPLE_VencDeinit(AX_S32 nChnIdx, AX_BOOL bSelect)
{
    AX_S32 s32Ret = AX_SUCCESS;
...
AX_S32 SAMPLE_Ivps2VencInit(SAMPLE_GRP_T *pGrp, AX_BOOL bVencMode, AX_BOOL bVencSelect)
{
    ...
    for (chn_idx = 0; chn_idx < pGrp->tPipelineAttr.nOutChnNum; chn_idx++) {
        ...
        if (0 != ret) {
            ALOGE("AX_SYS_Link err. ret:%d. src: modId %d grpId %d chnId %d. dst: modId %d grpId %d chnId %d", ret,
                  tSrcMod.enModId, tSrcMod.s32GrpId, tSrcMod.s32ChnId, tDstMod.enModId, tDstMod.s32GrpId,
                  tDstMod.s32ChnId);
            return ret;
        }

        ret = SAMPLE_VencInit(tDstMod.s32ChnId, nWidth, nHeight, dstModId, bVencSelect);
        if (0 != ret) {
    ...
    if (bVencSelect) {
        gstVencSelectPara.chnNum = pGrp->tPipelineAttr.nOutChnNum;
...
AX_S32 SAMPLE_Ivps2VencDeinit(SAMPLE_GRP_T *pGrp, AX_BOOL bVencMode, AX_BOOL bVencSelect)
{
    ...
    for (chn_idx = 0; chn_idx < pGrp->tPipelineAttr.nOutChnNum; chn_idx++) {
        ...
        SAMPLE_VencDeinit(tDstMod.s32ChnId, bVencSelect);
