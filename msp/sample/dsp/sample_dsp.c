...
#endif
static AX_S32 SAMPLE_DSP_Init(AX_DSP_ID_E dspid, char *itcm, char *sram, char *dtcm, char *dtcm2)
{
    AX_S32 ret;
...
extern int SAMPLE_DSP_TestCvtBayer_YUV(AX_DSP_ID_E dspid);
extern int SAMPLE_DSP_Test_SAD_EX(AX_DSP_ID_E dspid);
extern int SAMPLE_DSP_Test_RGB_DEINTERLEAVE(AX_DSP_ID_E dspid);
extern int SAMPLE_DSP_Test_RGB_INTERLEAVE(AX_DSP_ID_E dspid);
extern int SAMPLE_DSP_jointLR(AX_DSP_ID_E dspid);
extern int SAMPLE_DSP_TestDFT(AX_DSP_ID_E dspid);
extern int SAMPLE_DSP_TestResizeRGB(AX_DSP_ID_E dspid);
extern int SAMPLE_DSP_TestCvtColor_NV12_I420(AX_DSP_ID_E dspid);
extern int SAMPLE_DSP_TestResizeCvtColor_NV12_RGB(AX_DSP_ID_E dspid);
...
int main(int argc, AX_CHAR *argv[])
{
    ...
    if ((argc != 3) && (argc != 5)) {
        ...
        printf("sample_dsp itcm.bin sram.bin dtcm.bin dtcm2.bin\n");
        return -1;
    }
    if(argc == 3) {
    ...
    if(argc == 5) {
        printf("itcm file path: %s, sram file path: %s, dtcm file path: %s, dtcm2 file path: %s\n", argv[1], argv[2], argv[3], argv[4]);
    }
    ...
    for (i = 0; i < AX_DSP_ID_BUTT; i++) {
        if(argc == 3) {
            ret = SAMPLE_DSP_Init(i, argv[1], argv[2], 0, 0);
        }
        if(argc == 5) {
            ret = SAMPLE_DSP_Init(i, argv[1], argv[2], argv[3], argv[4]);
        }
        if (ret != AX_DSP_SUCCESS) {
    ...
    ret = SAMPLE_DSP_TestResizeRGB(0);
    if (ret)
        printf("SAMPLE_DSP0_TestResizeRGB FAILED\n");

    ret = SAMPLE_DSP_TestCvtColor(1);
    if (ret)
        printf("SAMPLE_DSP1_TestCvtColor FAILED\n");

    ret = SAMPLE_DSP_Test_SAD_EX(1);
    if (ret)
        printf("SAMPLE_DSP_Test_SAD_EX FAILED\n");

    ret = SAMPLE_DSP_Test_RGB_DEINTERLEAVE(1);
    if (ret)
        printf("SAMPLE_DSP1_Test_RGB_DEINTERLEAVE FAILED\n");

    ret = SAMPLE_DSP_Test_RGB_INTERLEAVE(1);
    if (ret)
        printf("SAMPLE_DSP1_Test_RGB_INTERLEAVE FAILED\n");
    ...
    for (i = 0; i < 1; i++) {
        ret = SAMPLE_DSP_TestDFT(i);
        if (ret)
            printf("SAMPLE_DSP%d_TestDFT FAILED\n", i);
        else
            printf("SAMPLE_DSP%d_TestDFT PASS\n", i);
    }
    SAMPLE_DSP_TestCvtBayer_YUV(0);

    ret = SAMPLE_DSP_TestCvtColor_NV12_I420(0);
    if (ret)
        printf("SAMPLE_DSP0_TestCvtColor_NV12_I420 FAILED\n");

    ret = SAMPLE_DSP_TestResizeCvtColor_NV12_RGB(0);
    if (ret)
        printf("SAMPLE_DSP_TestResizeCvtColor_NV12_RGB FAILED\n");
