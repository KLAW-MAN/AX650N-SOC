...
typedef enum
{
    ...
    AX_ENGINE_ABST_NONE = 2, /* not allocate buffer */
...
static INPUT_OUTPUT_ALLOC_STRATEGY strategy_none = std::make_pair(AX_ENGINE_ABST_NONE, AX_ENGINE_ABST_NONE);
...
{
    if (strategy_none.first != AX_ENGINE_ABST_NONE) {
    ...
    if (strategy_none.second != AX_ENGINE_ABST_NONE) {
...
static inline int prepare_io(AX_ENGINE_IO_INFO_T *info, AX_ENGINE_IO_T *io_data, INPUT_OUTPUT_ALLOC_STRATEGY strategy)
{
    ...
    for (uint i = 0; i < info->nInputSize; ++i)
    {
        ...
        if (strategy.first == AX_ENGINE_ABST_NONE)
        {
            continue;
        }
        else if (strategy.first == AX_ENGINE_ABST_CACHED)
        {
    ...
    for (uint i = 0; i < info->nOutputSize; ++i)
    {
        ...
        if (strategy.second == AX_ENGINE_ABST_NONE)
        {
            continue;
        }
        else if (strategy.second == AX_ENGINE_ABST_CACHED)
        {
            ret = AX_SYS_MemAllocCached((AX_U64 *)(&buffer->phyAddr), &buffer->pVirAddr, meta.nSize, AX_CMM_ALIGN_SIZE, (const AX_S8 *)(AX_CMM_SESSION_NAME));
        }
        else
...
int ax_runner_ax650::sub_init()
{
    ...
    if (!_parepare_io)
    {
        for (size_t grpid = 0; grpid < io_count; grpid++)
        {
            ...
            ret = prepare_io(m_handle->io_info[grpid], &m_handle->io_data[grpid], strategy_none);
            if (0 != ret)
            {
...
bool ax_runner_ax650::update_input_io_data(unsigned int index, unsigned long long phyAddr, void *virAddr)
{
    if (index >= m_handle->io_data[0].nInputSize)
    {
        ALOGE("input index %d out of range", index);
        return false;
    }

    m_handle->io_data[0].pInputs[index].phyAddr = (AX_ADDR)phyAddr;
    m_handle->io_data[0].pInputs[index].pVirAddr = virAddr;

    return true;
}

bool ax_runner_ax650::update_output_io_data(unsigned int index, unsigned long long phyAddr, void *virAddr)
{
    if (index >= m_handle->io_data[0].nOutputSize)
    {
        ALOGE("output index %d out of range", index);
        return false;
    }

    m_handle->io_data[0].pOutputs[index].phyAddr = (AX_ADDR)phyAddr;
    m_handle->io_data[0].pOutputs[index].pVirAddr = virAddr;
