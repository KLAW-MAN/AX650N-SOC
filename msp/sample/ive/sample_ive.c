...
static AX_CHAR optstr[] = "?::c:e:m:t:i:o:w:h:p:a:f:";
static const struct option long_options[] = {
    ...
    {"vnpu_affinity", required_argument, NULL, 'f'},
...
AX_VOID SAMPLE_IVE_Usage(AX_CHAR* pchPrgName)
{
    ...
    printf("\t-a | --align_need:Does the width/height/stride need to be aligned automatically, default:0.\n"
        "\t\t  0-no; 1-yes.\n");
    printf("\t-f | --vnpu_affinity: config VNPU affinity , default: 0.\n"
        "\t\t  0bxxx, x:1- select some vnpus; x:0 - not select some vnpus.\n");
...
int main(int argc, char *argv[])
{
    ...
    while ((s32Ret = getopt_long(argc, argv, optstr, long_options, NULL)) != -1) {
        ...
        case 'a':
            ...
            break;
        case 'f':
            g_u32NpuAffinity = (AX_U32)atoi(optarg);
            break;
        case '?':
        default:
            SAMPLE_IVE_Usage(argv[0]);
    ...
    case 17: { //MatMul
            ...
            for (int i = 0; i < AX_ENGINE_VIRTUAL_NPU_BUTT; i++) {
                if (g_u32NpuAffinity) {
                    stNpuAttr.eHardMode = AX_ENGINE_VIRTUAL_NPU_STD;
                } else {
