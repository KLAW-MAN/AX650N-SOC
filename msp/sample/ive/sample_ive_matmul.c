...
typedef struct axTEST_MAT_MUL_T {
    ...
    AX_IVE_MAU_MATMUL_CTRL_T stMauMatMulCtrl;
    AX_IVE_NPU_MATMUL_CTRL_T stNpuMatMulCtrl;
    AX_IVE_NPU_MATMUL_CTRL_AF_T stNpuMatMulCtrlAF;
} TEST_MAT_MUL_T;
...
static AX_CHAR g_ModelDir[MAX_NAME_LEN] = {0};
...
static AX_S32 SAMPLE_IVE_MatMul_Reference(AX_IVE_MAU_BLOB_T *pData1, AX_IVE_MAU_BLOB_T *pData2, AX_IVE_MAU_BLOB_T *pDataOut, AX_U32 u32Engine)
{
    ...
    if (pData1->u8ShapeSize != MAT_DIM || pData2->u8ShapeSize != MAT_DIM || pDataOut->u8ShapeSize != MAT_DIM) {
        SAMPLE_IVE_PRT("Only support shape_size = %d\n", MAT_DIM);
        return AX_FAILURE;
    }
...
static AX_S32 SAMPLE_IVE_TestNpuMatMul_ParseParams(TEST_MAT_MUL_T* pstTestMatMul, AX_CHAR *pchParamsList)
{
    cJSON *root = NULL;
    AX_CHAR *buf = NULL;
    FILE *fp = fopen(pchParamsList, "r");
    ...
    SAMPLE_IVE_CHECK_EXPR_GOTO(!item, PARSE_FAIL, "Error:parse param data_type failed!\n");
    if (g_u32NpuAffinity) {
        pstTestMatMul->stNpuMatMulCtrlAF.enDataType = item->valueint;
    } else {
        pstTestMatMul->stNpuMatMulCtrl.enDataType = item->valueint;
    }
    item = cJSON_GetObjectItem(root, "k_size");
    SAMPLE_IVE_CHECK_EXPR_GOTO(!item, PARSE_FAIL, "Error:parse param k_size failed!\n");
    if (g_u32NpuAffinity) {
        pstTestMatMul->stNpuMatMulCtrlAF.s32KSize = item->valueint;
    } else {
        pstTestMatMul->stNpuMatMulCtrl.s32KSize = item->valueint;
    }
    item = cJSON_GetObjectItem(root, "model_dir");
    SAMPLE_IVE_CHECK_EXPR_GOTO(!item, PARSE_FAIL, "Error:parse param model_dir failed!\n");
    strncpy(g_ModelDir, item->valuestring, MAX_NAME_LEN - 1);
    if (g_u32NpuAffinity) {
       pstTestMatMul->stNpuMatMulCtrlAF.pchModelDir = g_ModelDir;
    } else {
       pstTestMatMul->stNpuMatMulCtrl.pchModelDir = g_ModelDir;
    }
    item = cJSON_GetObjectItem(root, "type_in");
    SAMPLE_IVE_CHECK_EXPR_GOTO(!item, PARSE_FAIL, "Error:parse param type_in failed!\n");
    g_enDataTypeIn = (AX_IVE_MAU_DATA_TYPE_E)item->valueint;
...
static AX_S32 Sample_MatMul_Check_Result(TEST_MAT_MUL_T* pstTestMatMul, AX_U32 u32Engine, AX_BOOL bCheck)
{
    if (bCheck) {
        AX_IVE_MAU_BLOB_T res_reference = {0};
        AX_S32 ref_shape[MAT_DIM] = {g_MatBShape[0], g_MatQShape[0]};
        if (u32Engine == AX_IVE_ENGINE_NPU) {
            ref_shape[0] = g_MatQShape[0];
            ref_shape[1] = g_MatBShape[0];
        }
        ...
        if (u32Engine != AX_IVE_ENGINE_NPU) {
            SAMPLE_IVE_MatMul_Reference(&pstTestMatMul->stMatMulInput.stMatB, &pstTestMatMul->stMatMulInput.stMatQ, &res_reference, u32Engine);
        } else {
            SAMPLE_IVE_MatMul_Reference(&pstTestMatMul->stMatMulInput.stMatQ, &pstTestMatMul->stMatMulInput.stMatB, &res_reference, u32Engine);
        }
        if (u32Engine == AX_IVE_ENGINE_NPU || pstTestMatMul->stMauMatMulCtrl.bEnableMulRes) {
        ...
        if (u32Engine != AX_IVE_ENGINE_NPU && pstTestMatMul->stMauMatMulCtrl.bEnableTopNRes) {
            AX_IVE_MAU_BLOB_T topn_reference = {0};
            AX_S32 topn_shape[MAT_DIM] = {g_MatQShape[0], pstTestMatMul->stMauMatMulCtrl.s32TopN};
            ...
            SAMPLE_IVE_Toptopn_reference(&res_reference, &topn_reference, g_MatQShape[0], pstTestMatMul->stMauMatMulCtrl.s32TopN, pstTestMatMul->stMauMatMulCtrl.enOrder);
...
******************************************************************************/
static AX_S32 SAMPLE_IVE_TestMatMul_Init(TEST_MAT_MUL_T* pstTestMatMul, AX_CHAR *pchParamsList, AX_U32 u32Engine)
{
    ...
    if (pchParamsList) {
        if (u32Engine != AX_IVE_ENGINE_NPU) {
            ...
            s32Ret = SAMPLE_IVE_TestNpuMatMul_ParseParams(pstTestMatMul, pchParamsList);
        }
        if (AX_SUCCESS != s32Ret)
            return s32Ret;
    } else {
        if (u32Engine != AX_IVE_ENGINE_NPU) {
            pstTestMatMul->stMauMatMulCtrl.enMauId = AX_IVE_MAU_ID_0;
            pstTestMatMul->stMauMatMulCtrl.s32DdrReadBandwidthLimit = 0;
            pstTestMatMul->stMauMatMulCtrl.bEnableMulRes = AX_TRUE;
            pstTestMatMul->stMauMatMulCtrl.bEnableTopNRes = AX_TRUE;
            pstTestMatMul->stMauMatMulCtrl.enOrder = AX_IVE_MAU_ORDER_ASCEND;
            pstTestMatMul->stMauMatMulCtrl.s32TopN = 32;
            ...
            g_enDataTypeIn = AX_IVE_MAU_DT_SINT8;
            g_enDataTypeMulRes = AX_IVE_MAU_DT_FLOAT32;
            if (g_u32NpuAffinity) {
                pstTestMatMul->stNpuMatMulCtrlAF.pchModelDir = "/opt/data/ive/matmul/matmul_models/";
                pstTestMatMul->stNpuMatMulCtrlAF.enDataType = AX_IVE_MAU_DT_SINT8;
                pstTestMatMul->stNpuMatMulCtrlAF.s32KSize = 256;
                pstTestMatMul->stNpuMatMulCtrlAF.enNpuSet = g_u32NpuAffinity;
                g_MatQShape[0] = 256;
                g_MatQShape[1] = 256;
                g_MatBShape[0] = 1024;
                g_MatBShape[1] = 256;
            } else {
                pstTestMatMul->stNpuMatMulCtrl.pchModelDir = "/opt/data/ive/matmul/matmul_models/";
                pstTestMatMul->stNpuMatMulCtrl.enDataType = AX_IVE_MAU_DT_SINT8;
                pstTestMatMul->stNpuMatMulCtrl.s32KSize = 256;
                g_MatQShape[0] = 256;
                g_MatQShape[1] = 256;
                g_MatBShape[0] = 1e4;
                g_MatBShape[1] = 256;
    ...
    if (u32Engine != AX_IVE_ENGINE_NPU) {
        ...
        AX_S32 ref_shape[MAT_DIM] = {g_MatQShape[0], g_MatBShape[0]};
        s32Ret = SAMPLE_COMM_IVE_CreateMauBlob(&(pstTestMatMul->stMatMulOutput.stMulRes), g_enDataTypeMulRes, ref_shape, MAT_DIM, AX_FALSE);
        if (AX_SUCCESS != s32Ret) {
            SAMPLE_IVE_PRT("Error(%#x),Create mul result data failed!\n", s32Ret);
            SAMPLE_IVE_TestMatMul_Uninit(pstTestMatMul);
...
static AX_S32 SAMPLE_IVE_TestMatMulProc(TEST_MAT_MUL_T* pstTestMatMul, AX_U32 u32Engine)
{
    ...
    if (u32Engine != AX_IVE_ENGINE_NPU) {
        ...
        if (g_u32NpuAffinity) {
            s32Ret = AX_IVE_NPU_CreateMatMulHandle_Affinity(&hHandle, &pstTestMatMul->stNpuMatMulCtrlAF);
            if (AX_SUCCESS != s32Ret) {
                SAMPLE_IVE_PRT("Error(%#x),AX_IVE_NPU_CreateMatMulHandle_Affinity failed!\n",s32Ret);
                return s32Ret;
            }
        } else {
...
AX_VOID SAMPLE_IVE_MatMul_TEST(AX_U32 u32Engine, AX_CHAR *pchParamsList)
{
    ...
    s32Ret = SAMPLE_IVE_TestMatMul_Init(&s_stTestMatMul, pchParamsList, u32Engine);
    if (AX_SUCCESS != s32Ret) {
        SAMPLE_IVE_PRT("Error(%#x),SAMPLE_IVE_TestMatMul_Init failed!\n", s32Ret);
        return;
    }

    s32Ret = SAMPLE_IVE_TestMatMulProc(&s_stTestMatMul, u32Engine);
    if (AX_SUCCESS == s32Ret)
