...
int ToActualStride(AX_IMG_FORMAT_E format)
{
    switch (format)
    {
    case AX_FORMAT_ARGB8888:
    ...
    case AX_FORMAT_BITMAP:
        return 1;
    default:
        ALOGE("illegal format:%x is not support!", format);
        return 8;
    }
}

int CalStoragePlanarNum(AX_IMG_FORMAT_E format)
{
    switch (format)
    {
    case AX_FORMAT_YUV420_PLANAR:
...
AX_VOID SaveFile(AX_VIDEO_FRAME_T *tDstFrame, AX_S32 nGrpIdx, AX_S32 nChnIdx,
                 char *pFilePath, char *pFileName, char *pCaseId)
{
    AX_U32 nPixelSize;
    AX_S32 s32Ret1 = 0;
    AX_S32 s32Ret2 = 0;
    ...
    nPixelStride = tDstFrame->u32PicStride[0] * 8 / ToActualStride(tDstFrame->enImgFormat);
    printf("SaveFileExt nPixelSize: %d u32FrameSize: %d Height:%d Width:%d nPixelStride:%d PhyAddrY:%llx PhyAddrUV:%llx Format:%d\n",
           nPixelSize, tDstFrame->u32FrameSize, tDstFrame->u32Height, tDstFrame->u32Width, nPixelStride,
    ...
    sprintf(szOutImgFile, "%s/%s_GRP%dCHN%d_%dx%d.%s", pFilePath, pFileName, nGrpIdx, nChnIdx,
            nPixelStride, tDstFrame->u32Height, global_format2string(tDstFrame->enImgFormat));
                    ...
                    char *pFilePath, char *pFileName, int nInputW, int nInputH)
{
    ...
    SaveFile(ptDstFrame, nGrpIdx, nChnIdx, pFilePath, szOutImgName, NULL);
}
...
char *FrameInfoGet(char *optArg, AX_VIDEO_FRAME_T *ptFrame)
{
    ...
    ptFrame->u32PicStride[0] = tImage.tImage.nStride * ToActualStride(tImage.tImage.eFormat) / 8;
...
int compare_md5(const unsigned char *md5_actual, const char *md5_expected) {
    char md5_actual_str[MD5_DIGEST_LENGTH * 2 + 1];
    bin_to_hex(md5_actual, md5_actual_str, MD5_DIGEST_LENGTH);
    printf("Actual MD5: %s Expected MD5: %s\n", md5_actual_str, md5_expected);
    return strcmp(md5_actual_str, md5_expected) == 0;
}
...
int IVPS_FileMD5Check(const char *pFilePath, char *pCaseId, int nGrpId, int nChnId)
{
    ...
    if (file == NULL) {
        ALOGE("Error opening file %s ", pFilePath);
        return 1;
    }
    ...
    if (compare_md5(md5, gMD5Info[i].md5_expected)) {
        ALOGI("Case:%s MD5 check success", pCaseId);
    } else {
        ALOGE("Case:%s MD5 check fail!", pCaseId);
        return -3;
    }
    printf("Case MD5 check success\n");
    return 0;
}