...
#define iniparser_getval(val, func, ini, default, tmpstr, fmt, ...) \
    ...
    typedef struct
    {
        ...
        AX_BOOL bGdcDewarp;   /* --GdcDewarp */
        AX_U8 bPyraLite;   /* --pyralite */
        AX_BOOL bPyraMode;   /* --pyralite_mode */
        AX_BOOL bFewPool;   /* --FewPool test */
        AX_BOOL bUserFRC;   /* --user frame rate ctrl test */
        AX_BOOL bGetCycle;   /* --GetEngineDutyCycle test */
        AX_BOOL bProc;          /* --proc */
        AX_BOOL bLDCH;          /* --LDCH */
        AX_U8 nOutChnNum;
        IVPS_GRP nIvpsGrp;
