        ...
        for (i = 0; i < pThis->tPipelineAttr.nOutChnNum; i++)
        {
            if (!pThis->arrOutChns[i].bEmpty)
            {
                ...
                if (pThis->bSaveFile) {
                    ...
                    if (tDstFrame[i].u64SeqNum > 10 && i == 0) {
                        SaveFile(&tDstFrame[i], pThis->nIvpsGrp, i, pThis->pFilePath, "PIPELINE", pThis->pCaseId);
                    } else {
                        SaveFile(&tDstFrame[i], pThis->nIvpsGrp, i, pThis->pFilePath, "PIPELINE", NULL);
                    }
                }
...
AX_S32 IVPS_StartGrp(SAMPLE_IVPS_GRP_T *p, AX_BOOL bUserFRC)
{
    ...
    if (bUserFRC) {
        ...
        ret = AX_IVPS_SetGrpUserFRC(p->nIvpsGrp, &FrameRateAttr);
         if (IVPS_SUCC != ret)
        {
            ALOGE("AX_IVPS_SetChnUserFrameRate(Grp: %d) failed, ret=0x%x.", p->nIvpsGrp, ret);
            return -1;
        }
        for (IVPS_CHN chn = 0; chn < p->tPipelineAttr.nOutChnNum; chn++)
        {
            printf("AX_IVPS_SetChnUserFRC:%d \n", chn);
            ret = AX_IVPS_SetChnUserFRC(p->nIvpsGrp, chn, &FrameRateAttr);
            if (IVPS_SUCC != ret)
            {
                ALOGE("AX_IVPS_SetChnUserFrameRate(Grp: %d) failed, ret=0x%x.", p->nIvpsGrp, ret);
                return -1;
