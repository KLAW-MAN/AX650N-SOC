...
/*
 * AX_IVPS_AlphaBlendingAsync
 * Function: Overlay two images.
 * Note: Stride should be 16 Byte aligned.
 *       The u64PhyAddr[0] of ptDst should be set.
 */
static AX_S32 AX_IVPS_AlphaBlendingAsync(IVPS_ENGINE_ID_E eEngineId,
                                    const AX_VIDEO_FRAME_T *ptSrc,
                                    const AX_VIDEO_FRAME_T *ptOverlay,
                                    const AX_IVPS_POINT_T tOffset, AX_U8 nAlpha,
                                    AX_VIDEO_FRAME_T *ptDst)
{
    ...
    {
    case IVPS_ENGINE_ID_TDP:
        ret = AX_IVPS_AlphaBlendingAsyncTdp(ptSrc, ptOverlay, tOffset, nAlpha, ptDst);
        break;
    case IVPS_ENGINE_ID_VGP:
        ret = AX_IVPS_AlphaBlendingVgp(ptSrc, ptOverlay, tOffset, nAlpha, ptDst);
        break;
...
AX_S32 SAMPLE_Pyra_Gen(const AX_VIDEO_FRAME_T *ptSrcFrame, char *strFilePath, AX_BOOL bPyraMode)
{
    ...
    AX_BLK BlkId[2];
    AX_U32 nImgSize;
    ALOGI("input u32Width :%d nHeight=%d w nStride=%d", ptSrcFrame->u32Width, ptSrcFrame->u32Height, ptSrcFrame->u32PicStride[0]);
    ALOGI("input addr0=%llx addr1=%llx enImgFormat:%d", ptSrcFrame->u64PhyAddr[0], ptSrcFrame->u64PhyAddr[1], ptSrcFrame->enImgFormat);
    if (bPyraMode) {
        nImgSize = ptSrcFrame->u32Height * ptSrcFrame->u32PicStride[0] / 4;
    } else {
#ifdef PYRA_INPUT_P010
        nImgSize = ptSrcFrame->u32Height * ptSrcFrame->u32PicStride[0] * 1.5;
#else
        nImgSize = ptSrcFrame->u32Height * ptSrcFrame->u32PicStride[0] * 1.5;
#endif
    }
...
#ifdef PYRA_INPUT_P010
    tSrcFrame.nStride = ptSrcFrame->u32PicStride[0] / 2;
#else
    ...
    ALOGI("Src0=%llx Src1=%llx", tSrcFrame.nPhyAddr[0], tSrcFrame.nPhyAddr[1]);
    if (!bPyraMode) {
        if (ptSrcFrame->enImgFormat == AX_FORMAT_YUV420_SEMIPLANAR) {
                ALOGE("Src0=%llx Src1=%llx", tSrcFrame.nPhyAddr[0], tSrcFrame.nPhyAddr[1]);
                tSrcFrame.nPhyAddr[1] = ptSrcFrame->u64PhyAddr[0] + ptSrcFrame->u32Width * ptSrcFrame->u32Height;
        } else {
                ALOGE("Src0=%llx Src1=%llx", tSrcFrame.nPhyAddr[0], tSrcFrame.nPhyAddr[1]);
                tSrcFrame.nPhyAddr[1] = ptSrcFrame->u64PhyAddr[0] + ptSrcFrame->u32Width * ptSrcFrame->u32Height * 2;
        }
    }
    ALOGI("Src0=%llx Src1=%llx", tSrcFrame.nPhyAddr[0], tSrcFrame.nPhyAddr[1]);
...
#ifdef PYRA_INPUT_P010
    tPyraDstFrame[0].nStride = ptSrcFrame->u32PicStride[0] / 4;
#else
    ...
    ALOGI("nWidth:%d nHeight=%d nStride=%d", tPyraDstFrame[0].nWidth, tPyraDstFrame[0].nHeight, tPyraDstFrame[0].nStride);
...
#ifdef PYRA_INPUT_P010
    tPyraDstFrame[1].nStride = ptSrcFrame->u32PicStride[0] / 2;
#else
    ...
    tPyraDstFrame[0].tCompressInfo.enCompressMode = AX_COMPRESS_MODE_LOSSY;
    tPyraDstFrame[0].tCompressInfo.u32CompressLevel = 4;
    tPyraDstFrame[0].nPhyAddr[1] = tDstFrame[0].u64PhyAddr[0] + tPyraDstFrame[0].nHeight * tPyraDstFrame[0].nStride;
    tPyraDstFrame[1].tCompressInfo.enCompressMode = AX_COMPRESS_MODE_LOSSY;
    tPyraDstFrame[1].tCompressInfo.u32CompressLevel = 4;
    tPyraDstFrame[1].nPhyAddr[1] = tDstFrame[1].u64PhyAddr[0] + tPyraDstFrame[1].nHeight * tPyraDstFrame[1].nStride;
    ALOGI("nWidth:%dnHeight=%d nStride=%d", tPyraDstFrame[1].nWidth, tPyraDstFrame[1].nHeight, tPyraDstFrame[1].nStride);
    ...
    for (i = 0; i < 2; i++) {
        ...
        ALOGI("nHeight=%d nStride=%d  w:%d H:%d", tPyraDstFrame[i].nHeight, tPyraDstFrame[i].nStride, tDstFrame[i].u32Width, tDstFrame[i].u32Height);
        sprintf(strFileName, "pyralite_gen%d_", i);
        SaveFile(&tDstFrame[i], 0, 0, strFilePath, strFileName, NULL);
...
AX_S32 SAMPLE_Pyra_Rcn(const AX_VIDEO_FRAME_T *ptGauFrame, AX_VIDEO_FRAME_T *ptLapFrame, char *strFilePath, AX_BOOL bPyraMode)
{
    ...
    SaveFile(&tDstFrame, 0, 0, strFilePath, "pyralite_rcn_", NULL);
    ret = AX_POOL_ReleaseBlock(BlkId);
    if (ret)
    {
        ALOGE("Rls BlkId fail, ret=0x%x", ret);
...
/*
 * SAMPLE_IVPS_AlphaBlendingAsync
 * Function: Overlay two images and save output file.
 * Note: Stride should be 16 Byte aligned.
 *       The u64PhyAddr[0] of ptDst should be set.
 */
AX_S32 SAMPLE_IVPS_AlphaBlendingAsync(IVPS_ENGINE_ID_E eEngineId, const AX_VIDEO_FRAME_T *ptSrcFrame,
                                 AX_VIDEO_FRAME_T *ptOverlay, AX_U8 nAlpha, char *strFilePath,  char *pCaseId)
{
    ...
    ret = AX_IVPS_AlphaBlendingAsync(eEngineId, ptSrcFrame, ptOverlay, tOffset, nAlpha, &tDstFrame);
    if (ret)
    {
        ALOGE("ret=0x%x", ret);
        return ret;
                             ...
                             AX_OSD_BMP_ATTR_T arrBmp[], AX_U32 nNum, char *strFilePath, char *pCaseId)
{
    ...
    {
    case IVPS_ENGINE_ID_TDP:
        ret = AX_IVPS_DrawOsdTdp(ptSrc, arrBmp, nNum);
        break;
    case IVPS_ENGINE_ID_VGP:
        ret = AX_IVPS_DrawOsdVgp(ptSrc, arrBmp, nNum);
        break;
    default:
        ret = -1;
        break;
...
static AX_S32 LDCHSingleCore(const AX_VIDEO_FRAME_T *ptSrc, AX_VIDEO_FRAME_T *ptDst,
                   AX_IVPS_DEWARP_ATTR_T *ptDewarpAttr, char *pStorePath, char *fileName, char *pCaseId)
{
    AX_U64 nTimeStart = 0, nTimeDiff = 0;
    AX_BLK BlkId = 0;
    AX_S32 ret = 0;
    IVPS_BLK_T BlkInfo[16];

    BlkInfo[0].nSize = CalcImgSize(ptDst->u32PicStride[0], ptDst->u32Width, ptDst->u32Height, ptDst->enImgFormat, 16);

    CHECK_RESULT(BufPoolBlockAddrGet(-1, BlkInfo[0].nSize, &ptDst->u64PhyAddr[0], (AX_VOID **)(&ptDst->u64VirAddr[0]), &BlkId));
    ptDst->u64PhyAddr[1] = ptDst->u64PhyAddr[0] + ptDst->u32PicStride[0] * ALIGN_UP(ptDst->u32Height, 16);

    memset((AX_VOID *)((AX_LONG)ptDst->u64VirAddr[0]), 0x00, BlkInfo[0].nSize);
    nTimeStart = GetTickCount();
    ret = AX_IVPS_Dewarp(ptSrc, ptDst, ptDewarpAttr);
    nTimeDiff = GetTickCount() - nTimeStart;
    if (nTimeDiff > 100000) {
        ALOGE("time cost:%lld > 100 ms", nTimeDiff);
    }
 ...
 * SAMPLE_IVPS_LDCH()
 */
static AX_S32 SAMPLE_IVPS_LDCH(const AX_VIDEO_FRAME_T *ptFrame, const AX_IVPS_LDCH_ATTR_T *ptLdcHAttr, char *pStorePath, char *pCaseId)
{
    AX_S32 i, k, ret = 0;
    char fileName[64] = {0};
    AX_VIDEO_FRAME_T tDstFrame = {0};
    AX_IVPS_DEWARP_ATTR_T tDewarpAttr = {0};
    AX_IVPS_MAP_USER_ATTR_T tMapUserAttr = {0};

    tDstFrame.u32Width = ptLdcHAttr->nDstWidth;
    tDstFrame.u32Height = ptLdcHAttr->nDstHeight;
    tDstFrame.u32PicStride[0] = ALIGN_UP(tDstFrame.u32Width, 16);
    tDstFrame.enImgFormat = AX_FORMAT_YUV420_SEMIPLANAR;

    tMapUserAttr.pUserMap = malloc(33 * 33 * 16);
    AX_IVPS_LDCH_MeshGen(&tMapUserAttr, ptLdcHAttr);

    tDewarpAttr.tMapUserAttr = tMapUserAttr;
    tDewarpAttr.eDewarpType = AX_IVPS_DEWARP_MAP_USER;
    tDewarpAttr.eImgFormat = ptFrame->enImgFormat;
    tDewarpAttr.nDstWidth = ptFrame->u32Width;
    tDewarpAttr.nDstHeight = ptFrame->u32Height;
    tDewarpAttr.nDstStride = ALIGN_UP(ptFrame->u32PicStride[0], 128);

    ALOGI("tDstFrame.u32Width:%d, tDstFrame.u32Height:%d, ptFrame->enImgFormat:%d, %d, %d, %d\n",
           tDstFrame.u32Width, tDstFrame.u32Height, ptFrame->enImgFormat, ptFrame->u32Width, ptFrame->u32Height, ptFrame->u32PicStride[0]);

    snprintf(fileName, 64, "LDCH_%u_%ux%u", ptFrame->u32PicStride[0], ptFrame->u32Width, ptFrame->u32Height);
    CHECK_RESULT(LDCHSingleCore(ptFrame, &tDstFrame, &tDewarpAttr, pStorePath, fileName, pCaseId));

    free(tMapUserAttr.pUserMap);
    return ret;
}

AX_S32 SAMPLE_IVPS_SyncApiLDCH(const IVPS_ARG_T *ptArg, const SAMPLE_IVPS_GRP_T *pSync)
{
    int ret;
    AX_IVPS_LDCH_ATTR_T tLdcHAttr = {0};
    AX_IVPS_POINT_NICE_T tDst, tSrc;

    if (!ptArg->bLDCH)
        return 0;

    tLdcHAttr.nSrcWidth = 3840;
    tLdcHAttr.nSrcHeight = 2160;
    tLdcHAttr.nDstWidth = 3840;
    tLdcHAttr.nDstHeight = 2160;
    tLdcHAttr.nRadius = 1900;
    tLdcHAttr.nHorOffset = 0;
    tLdcHAttr.nVerOffset = 0;
    tLdcHAttr.nOffsetLeft = 0;
    tLdcHAttr.nOffsetRight = 0;

    tDst.fX = 1080.0;
    tDst.fY = 540.0;
    AX_IVPS_LDCH_PointQueryDst2Src(&tDst, &tSrc, &tLdcHAttr);
    ALOGI("tSrc.fX:%f tSrc.fY:%f\n", tSrc.fX, tSrc.fY);

    CHECK_RESULT(SAMPLE_IVPS_LDCH(&pSync->tFrameInput, &tLdcHAttr, pSync->pFilePath, ptArg->pCaseId));
    return 0;
}
