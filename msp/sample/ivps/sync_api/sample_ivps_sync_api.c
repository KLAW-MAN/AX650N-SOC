...
static void *_AlphaBlendingThreadMain(void *Arg)
{
    AX_S32 ret;
    SAMPLE_IVPS_ARGS_T *pstFuncArgs = (SAMPLE_IVPS_ARGS_T *)Arg;

    SAMPLE_IVPS_GRP_T *pGrp = pstFuncArgs->pGrp;
    IVPS_ARG_T *ptArg = pstFuncArgs->ptArg;
    AX_VIDEO_FRAME_T *tOverlay = pstFuncArgs->tOverlay;

    ret = SAMPLE_IVPS_AlphaBlendingAsync(ptArg->nEngineId, &pGrp->tFrameInput, tOverlay, 128, pGrp->pFilePath, ptArg->pCaseId);
    if (ret)
    {
                           ...
                           const SAMPLE_IVPS_SYNC_API_T *ptSyncIntf)
{
    ...
    if (ptArg->bGdcDewarp == 1)
    {
        ret = SAMPLE_IVPS_Dewarp(&pGrp->tFrameInput, pGrp->pFilePath, &gSampleDewarpSync);
        if (ret)
        {
    ...
    if (ptArg->bAsync && ptArg->pOverlayInfo)
    {
        pthread_t GrpTid[2];
        SAMPLE_IVPS_ARGS_T pstFuncArgs;
        char *pOverlayFile = FrameInfoGet(ptArg->pOverlayInfo, &tOverlay);

        FrameBufGet(0, &tOverlay, pOverlayFile);

        pstFuncArgs.pGrp = (SAMPLE_IVPS_GRP_T *)pGrp;
        pstFuncArgs.ptArg = (IVPS_ARG_T *)ptArg;
        pstFuncArgs.tOverlay = &tOverlay;

        for (i = 0; i < 2; i++) {
            pstFuncArgs.nChnIdx = i;
            ret = pthread_create(&GrpTid[i], NULL, _AlphaBlendingThreadMain,
                                 (void *)&pstFuncArgs);
            if (ret) {
                ALOGE("pthread_create failed, ret=0x%x.", ret);
                goto error1;
            }
        }

        for (i = 0; i < 2; i++) {
            ret = pthread_join(GrpTid[i], NULL);
            if (ret) {
                ALOGE("pthread_join FAILED! ret:%d, %s, \n", ret, strerror(ret));
                goto error1;
            }
        }
    }
