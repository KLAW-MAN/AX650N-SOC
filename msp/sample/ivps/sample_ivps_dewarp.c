			       ...
			       char *fileName)
{
	...
	SaveFile(&tDstFrame, 0, 0, pStorePath, fileName, NULL);
...
static AX_S32 SAMPLE_IVPS_DewarpSync(const AX_VIDEO_FRAME_T *ptFrame, char *pStorePath, SAMPLE_IVPS_DEWARP_T *pSampleDewarp)
{
	...
	snprintf(fileName, 64, "Dewarp_s%u_%ux%u", pSampleDewarp->tDewarpAttr.nDstStride,
...
static AX_S32 SAMPLE_IVPS_Fisheye(const AX_VIDEO_FRAME_T *ptSrc, char *pStorePath, SAMPLE_IVPS_DEWARP_T *pSampleDewarp)
{
	...
	SaveFile(&tDst, 0, 0, pStorePath, "Fisheye", NULL);
...
static AX_S32 SAMPLE_IVPS_MapUser(const AX_VIDEO_FRAME_T *ptSrc, char *pStorePath, SAMPLE_IVPS_DEWARP_T *pSampleDewarp)
{
	...
	pGdc->tMapUserAttr.pUserMap = AX_OS_MEM_Malloc(ivps_type, pGdc->tMapUserAttr.nMeshNumV * pGdc->tMapUserAttr.nMeshNumH * 16);
	...
	SaveFile(&tDst, 0, 0, pStorePath, "MapUser", NULL);
	...
	if (pGdc->tMapUserAttr.pUserMap) {
		AX_OS_MEM_Free(ivps_type, pGdc->tMapUserAttr.pUserMap);
	}
	return 0;
}
