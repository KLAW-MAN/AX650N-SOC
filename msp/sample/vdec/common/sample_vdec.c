...
static volatile AX_S32 s_SignalTrigCnt = 0;
    ...
    for (ii = 0; ii < SAMPLE_VDEC_FRAME_CNT; ii++) {
            ...
            if (fp_out == NULL) {
                ...
                if (fp_out == NULL) {
                    ...
                    if (pstVdecCtx->pOutputFilePath[VdChn] != NULL) {
                        ...
                        AX_OS_MEM_Free(AX_ID_USER, pstVdecCtx->pOutputFilePath[VdChn]);
        ...
        if (pstCmd->DestMD5) {
            ...
            if (pstCmd->sWriteFrames) {
                if (pstCmd->bModifyFrmSize && change) {
                    AX_CHAR file_path[128];
                    fflush(fp_out);
                    fclose(fp_out);
                    change = 0;
                    snprintf(file_path, 128, "number%d_w_%d_h_%d.yuv", get_num, pstFrameInfo->stVFrame.u32Width, pstFrameInfo->stVFrame.u32Height);
                    SAMPLE_LOG("Write YUV to: %s\n", file_path);
...
ERR_RET:
    ...
    if (pstCtx_Grp0->u64SelectFrameCnt) {
        ...
        SAMPLE_WARN_LOG("s_ThreadExit:%d, pstGrpSet->u32GrpCount:%d",
...
static AX_S32 __VdecSendEndOfStream(AX_VDEC_GRP VdGrp)
{
    ...
    SAMPLE_LOG_ST("VdGrp=%d, AX_VDEC_SendStream ret:0x%x %s\n",
                  VdGrp, sRet, AX_VdecRetStr(sRet));
    ...
    while (1) {
        ...
        if (pstCmd->bModifyFrmSize) {
            ...
            if (uSendPicNum == 8) {
                ...
                pstVdChnAttr->u32PicWidth = 1392;
                pstVdChnAttr->u32PicHeight = 720;
                pstVdChnAttr->u32FrameStride = 1536;

                sRet = AX_VDEC_SetChnAttr(VdGrp, 1, pstVdChnAttr);
                if (sRet != AX_SUCCESS) {
                    ret = sRet;
             ...
             } else if (uSendPicNum == 16) {
                pstVdChnAttr = pstFuncArgs->pstVdChnAttr[1];

                pstVdChnAttr->enOutputMode = 2;
                pstVdChnAttr->u32PicWidth = 768;
                pstVdChnAttr->u32PicHeight = 480;
                pstVdChnAttr->u32FrameStride = 768;
             ...
             } else if (uSendPicNum == 30) {
                pstVdChnAttr = pstFuncArgs->pstVdChnAttr[1];

                pstVdChnAttr->enOutputMode = 2;
                pstVdChnAttr->u32PicWidth = 1280;
                pstVdChnAttr->u32PicHeight = 720;
                pstVdChnAttr->u32FrameStride = 1280;
        ...
        if (sRet == AX_SUCCESS) {
            ...
            SAMPLE_LOG_ST("VdGrp=%d, AX_VDEC_SendStream ret AX_ERR_VDEC_FLOW_END, uSendPicNum:%d",
                          VdGrp, uSendPicNum);
            break;
        }
                              ...
                              SAMPLE_VDEC_CMD_PARAM_T *pstCmd, AX_U32 uStreamBufSize)
{
    ...
    pstVdGrpAttr->u32RefNum = pstCmd->u32RefNum;
...
static AX_S32 __VdecGrpSendStream(SAMPLE_VDEC_ARGS_T *pstFuncArgs, SAMPLE_STREAM_BUF_T *pstStreamBuf)
{
    ...
    while (!s_ThreadExit && (pstCmd->sLoopDecNum > 0)) {
        ...
        SAMPLE_LOG_ST("VdGrp=%d, AX_VDEC_StopRecvStream Done! sLoopDecNum:%d\n",
                      VdGrp, pstCmd->sLoopDecNum);
        if (s_ThreadExit) {
            break;
        }
...
ERR_RET_STOP_RECV:
    ...
    if (sRet) {
        ...
        pstVdecCtx->GrpStatus[VdGrp] = AX_VDEC_GRP_STOP_RECV;
        SAMPLE_LOG_ST("VdGrp=%d, AX_VDEC_StopRecvStream Done!\n", VdGrp);
    }
ERR_RET:
    s_ThreadExit += 1;
    SAMPLE_NOTICE_LOG("++s_ThreadExit:%d\n", s_ThreadExit);
...
static void *_VdecGroupThreadMain(void *arg)
{
    ...
    const AX_VDEC_STREAM_T *pstStreamBuf = &stStreamBuf;
    ...
    if (pstCmd->bExtractHead) {
        size_t parse_len = stStreamBuf.u32StreamPackLen;
        ...
        sRet = AX_VDEC_ExtractStreamHeaderInfo(pstStreamBuf, pstCmd->enDecType,
                                               pstStreamHeadInfo);
        if (sRet != AX_SUCCESS) {
            SAMPLE_ERR_LOG("VdGrp=%d, AX_VDEC_ExtractStreamHeaderInfo FAILED! ret:0x%x %s\n",
                            VdGrp, sRet, AX_VdecRetStr(sRet));
        } else {
            SAMPLE_LOG_T("VdGrp=%d, pstStreamHeadInfo->u32RefFramesNum:%d, "
    ...
    AX_VDEC_CHN_ATTR_T *pstChnSet = AX_OS_MEM_Calloc(AX_ID_USER, 1, tmp_size);
    if (pstChnSet == NULL) {
    ...
    if (pstChnSet) {
        AX_OS_MEM_Free(AX_ID_USER, pstChnSet);
        pstChnSet = NULL;
    }
    ...
    if (pstStreamMem) {
        if (tStreamBuf.tBufAddr.u64PhyAddr != 0) {
            ...
            AX_OS_MEM_Free(AX_ID_USER, tStreamBuf.tBufAddr.pVirAddr);
        }
    }
...
int Sample_VdecTestBenchMain(SAMPLE_VDEC_CONTEXT_T **ppstSampleCtx)
{
    ...
    for (i = 0; i < pstCmd->uGrpCount; i++) {
        ...
        if (i == 0) {
            ...
            pstVdecGrpArgs->pstCtx = (SAMPLE_VDEC_CONTEXT_T *)AX_OS_MEM_Calloc(AX_ID_USER, 1, tmp_size);
                ...
                if (pstVdecCtx->bArgvAlloc == AX_TRUE) {
                    if (pstVdecCtx->argv != NULL) {
                        if (pstVdecCtx->argv[0] != NULL) {
                            AX_OS_MEM_Free(AX_ID_USER, pstVdecCtx->argv[0]);
                            pstVdecCtx->argv[0] = NULL;
                        }

                        AX_OS_MEM_Free(AX_ID_USER, pstVdecCtx->argv);
...
static void _VdecSigInt(int sigNo)
{
    ...
    if (s_pstVdecCtx == NULL) {
        SAMPLE_LOG_ST("s_pstVdecCtx == NULL\n");
        goto ERR_RET;
    }
    ...
    for (VdGrp = uStartGrpId; VdGrp < (pstCmd->uGrpCount + uStartGrpId); VdGrp++) {
        ...
        else if (s32Ret == AX_ERR_VDEC_UNEXIST) {
            continue;
        }
        else {
        ...
        SAMPLE_LOG_ST("VdGrp=%d, reset done! pstVdecCtx:%p", VdGrp, pstVdecCtx);
    }
ERR_RET:
    s_ThreadExit += 1;
    s_SignalTrigCnt += 1;

    SAMPLE_LOG_ST("Catch signal %d, s_ThreadExit:%d, s_SignalTrigCnt:%d, ==============\n\n\n", sigNo, s_ThreadExit, s_SignalTrigCnt);
...
AX_S32 Sample_VdecTestBenchInit(int argc, char *argv[], SAMPLE_VDEC_CONTEXT_T **ppstVdecCtx)
{
    ...
    pstVdecCtx = (SAMPLE_VDEC_CONTEXT_T *)AX_OS_MEM_Calloc(AX_ID_USER, 1, sizeof(SAMPLE_VDEC_CONTEXT_T));
    if (NULL == pstVdecCtx) {
    ...
    if (pstCmd->sWriteFrames || pstCmd->DestMD5) {
        ...
        if (fp_out == NULL) {
            if (pstCmd->tChnCfg[0].pOutputFilePath != NULL) {
                ...
                AX_OS_MEM_Free(AX_ID_USER, pstCmd->tChnCfg[0].pOutputFilePath);
