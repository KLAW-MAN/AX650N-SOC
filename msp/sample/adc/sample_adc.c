...
int main(int argc, char **argv)
{
    ...
    if (argc == 2) {
        ...
        if (get_adc_value.chan0_val > 0x3ff || get_adc_value.chan1_val > 0x3ff || get_adc_value.chan2_val > 0x3ff)
            fprintf(stderr, "\n\t ADC Driver Test Example Failed.\n");
        else
            fprintf(stderr, "\n\t ADC Driver Test Example Successed.\n");
        fprintf(stderr,
            "\nCurrent ADC date is chan0:0x%x, chan1:0x%x, chan2:0x%x\n",
            get_adc_value.chan0_val, get_adc_value.chan1_val,
            get_adc_value.chan2_val);
