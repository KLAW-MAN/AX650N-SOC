...
static AX_S32 getAddr(ADDR_BUF_T *addrBuf, ADDR_BUF_T *addrBuf2, AX_U32 u32Size)
{
    AX_S32 i, ret;
    AX_U64 phyAddr;
    AX_VOID *src_test_virt = NULL;
...
static AX_VOID freeAddr(ADDR_BUF_T *addrBuf, ADDR_BUF_T *addrBuf2)
{
    AX_S32 i;
    for (i = 0; i < 16; i++) {
...
static AX_S32 AX_DMADIM_FUNC_MEMORY_INIT_SYNC(ADDR_BUF_T *srcBuf, ADDR_BUF_T *dstBuf, AX_U32 u32DescNum)
{
    AX_S32 i, ret = 0, s32DmaChn = 0;
...
static AX_VOID AX_DMADIM_MEMINIT_Signal(AX_DMADIM_XFER_STAT_T *pXferStat, AX_VOID *pCbArg)
{
...
static AX_S32 AX_DMADIM_FUNC_MEMORY_INIT_ASYNC(ADDR_BUF_T *srcBuf, ADDR_BUF_T *dstBuf, AX_U32 u32DescNum)
{
    AX_S32 i, ret = 0, s32DmaChn = 0;
    AX_DMADIM_MSG_T tDmaMsg;
    AX_DMADIM_DESC_T *pDescBuf;
...
static AX_VOID AX_DMADIM_Test_Signal(AX_DMADIM_XFER_STAT_T *pXferStat, AX_VOID *pCbArg)
{
    AX_DMADIM_XFER_STAT_T *tXferStat = (AX_DMADIM_XFER_STAT_T *)pCbArg;
...
static AX_S32 AX_DMADIM_FUNC_CHECKSUM(ADDR_BUF_T *srcBuf, ADDR_BUF_T *dstBuf, AX_U32 u32DescNum, bool bIsSync)
{
    AX_S32 i, ret = 0, s32DmaChn = 0;
    AX_DMADIM_MSG_T tDmaMsg;
    AX_DMADIM_DESC_T *pDescBuf;
...
static AX_S32 AX_DMADIM_FUNC_1D(ADDR_BUF_T *srcBuf, ADDR_BUF_T *dstBuf, AX_U32 u32DescNum, bool bIsSync)
{
    AX_S32 i, s32DmaChn = 0, ret = 0;
    AX_DMADIM_MSG_T tDmaMsg;
    AX_DMADIM_DESC_T *pDescBuf;
    AX_DMADIM_XFER_STAT_T tXferStat;
    memset(&tXferStat, 0, sizeof(AX_DMADIM_XFER_STAT_T));
...
static AX_S32 AX_DMADIM_FUNC_MEMORY_XD(ADDR_BUF_T *srcBuf, ADDR_BUF_T *dstBuf, AX_U8 eDmaMode, AX_U32 u32DescNum,
                                    bool bIsSync)
{
    AX_S32 i, ret = 0, s32DmaChn = 0;
    AX_DMADIM_MSG_T tDmaMsg;
...
static AX_S32 AX_DMA_FUNC_SIMPLIFY(ADDR_BUF_T *srcBuf, ADDR_BUF_T *dstBuf)
{
    AX_S32 ret = 0;
    AX_U32 u32Result;
    ...
    tDimDesc.tSrcInfo.u64PhyAddr = srcBuf[0].aPhy;
    tDimDesc.tDstInfo.u64PhyAddr = dstBuf[0].aPhy;

    ret |= AX_DMA_MemCopy(dstBuf[0].aPhy, srcBuf[0].aPhy, 0x1000);
    ret |= AX_DMA_MemSet(dstBuf[0].aPhy, 0xAA, 0x1000);
    ret |= AX_DMA_CheckSum(&u32Result, dstBuf[0].aPhy, 0x1000);
    ret |= AX_DMA_MemCopyXD(tDimDesc, AX_DMADIM_2D);
    ret |= AX_DMA_MemCopyXD(tDimDesc, AX_DMADIM_3D);
    ret |= AX_DMA_MemCopyXD(tDimDesc, AX_DMADIM_4D);
    return ret;
}
