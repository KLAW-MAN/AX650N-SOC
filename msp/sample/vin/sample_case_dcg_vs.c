...
COMMON_SYS_POOL_CFG_T gtSysCommPoolDoubleOs04a10Hdr[] = {
    {2688, 1520, 2688, AX_FORMAT_YUV420_SEMIPLANAR, 50},        /* vin nv21/nv21 use */
};

COMMON_SYS_POOL_CFG_T gtPrivatePoolDoubleOs04a10Hdr[] = {
    {2688, 1520, 2688, AX_FORMAT_BAYER_RAW_16BPP, 25 * 2},      /* vin raw16 use */
};
...
static AX_U32 __dcg_vs_sensor_init(AX_U8 nPipeId, AX_SENSOR_REGISTER_FUNC_T *ptSnsHdl, AX_SNS_CONNECT_TYPE_E eBusType,
                                   AX_SNS_ATTR_T *ptSnsAttr, AX_SNS_CLK_ATTR_T *ptSnsClkAttr)
{
    ...
    if (ISP_SNS_CONNECT_I2C_TYPE == eBusType) {
        tSnsBusInfo.I2cDev = COMMON_ISP_GetI2cDevNode(nPipeId);
        tSnsBusInfo.busType = ISP_SNS_CONNECT_I2C_TYPE;
    } else {
        tSnsBusInfo.SpiDev.bit4SpiDev = COMMON_ISP_GetSpiDevNode(nPipeId);
    ...
    axRet = AX_ISP_OpenSnsClk(ptSnsClkAttr->nSnsClkIdx, ptSnsClkAttr->eSnsClkRate);
    if (0 != axRet) {
...
static AX_S32 __dcg_vs_cam_open(AX_CAMERA_T *pCam)
{
    ...
    axRet = __dcg_vs_sensor_init(pCam->nPipeId, pCam->ptSnsHdl[pCam->nPipeId], pCam->eBusType, &pCam->tSnsAttr,
                                 &pCam->tSnsClkAttr);
    if (0 != axRet) {
    ...
    if (!pCam->bDevOnly) {
        for (i = 0; i < pCam->tDevBindPipe.nNum; i++) {
            ...
            pCam->tPipeAttr[pCam->nPipeId].eCombMode = pCam->tPipeInfo[i].eCombMode;
            pCam->tPipeAttr[pCam->nPipeId].ePipeWorkMode = pCam->tPipeInfo[i].ePipeWorkMode;

            axRet = COMMON_VIN_SetPipeAttr(pCam->eSysMode, nPipeId, &pCam->tPipeAttr[pCam->nPipeId],
            ...
            if (pCam->bRegisterSns) {
                axRet = __dcg_vs_sensor_register(nPipeId, pCam->ptSnsHdl[pCam->nPipeId], &pCam->tSnsAttr);
                if (0 != axRet) {
                    COMM_CAM_PRT("__dcg_vs_sensor_register failed, ret=0x%x.\n", axRet);
                    return -1;
                }
            }
            ...
            axRet = COMMON_VIN_StartChn(nPipeId, pCam->tChnAttr[pCam->nPipeId], pCam->bChnEnable[pCam->nPipeId]);
            if (0 != axRet) {
...
static AX_S32 __dcg_vs_cam_switch_test(AX_CAMERA_T *pCam)
{
    ...
    if (pCam->tPipeAttr[pCam->nPipeId].eSnsMode == AX_SNS_LINEAR_MODE) {
        ...
        pCam->eHdrMode = AX_SNS_HDR_2X_MODE;
        __set_pipe_hdr_mode(&pCam->tDevBindPipe.nHDRSel[0], AX_SNS_HDR_2X_MODE);
    } else {
    ...
    /* rebuild DCG pipe*/
    axRet = COMMON_VIN_SetPipeAttr(pCam->eSysMode, nPipeId, &pCam->tPipeAttr[pCam->nPipeId],
                                   pCam->tPipeInfo[nPipeId].eFrameSrcType);
    if (0 != axRet) {
        COMM_CAM_PRT("COMMON_ISP_SetPipeAttr failed, ret=0x%x.\n", axRet);
        return -1;
    }
