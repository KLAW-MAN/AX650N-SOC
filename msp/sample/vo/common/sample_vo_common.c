...
static AX_VOID SAMPLE_VO_EXEC_CMD_AFTER_START(SAMPLE_VO_CONFIG_S *pstVoConf)
{
    AX_S32 i;

    for (i = 0; i < pstVoConf->u32VDevNr; i++) {
        AX_CHAR *cCmd = &pstVoConf->stVoDev[i].cCmd[0];
        if (cCmd[0] != 0) {
            SAMPLE_PRT("exec vo%d cmd:%s\n", i, cCmd);
	    system(cCmd);
        }
    }
}
...
static AX_S32 SAMPLE_VO_FB_CONF(AX_U32 u32Type, SAMPLE_FB_CONFIG_S *pstFbConf)
{
    ...
    if (u32Type) { /* GUI */
        ...
        if (u32Fmt == AX_FORMAT_ARGB8888) {
            stCanvas.nStride = u32Width << 2;
        } else if (u32Fmt == AX_FORMAT_ARGB1555) {
            stCanvas.nStride = u32Width << 1;
        } else {
...
static AX_VOID *SAMPLE_VO_CHN_THREAD(AX_VOID *pData)
{
    ...
    s32Ret = AX_VO_GetVideoLayerAttr(u32LayerID, &stLayerAttr);
    if (s32Ret) {
        SAMPLE_PRT("layer%d AX_VO_GetVideoLayerAttr failed, s32Ret = 0x%x\n", u32LayerID, s32Ret);
        goto exit;
    }
    ...
    while (!pstChnThreadParam->u32ThreadForceStop) {
        ...
        if ((stLayerAttr.enSyncMode == AX_VO_LAYER_SYNC_PRIMARY) && (stLayerAttr.u32PrimaryChnId == u32ChnID)) {
            sleep(1);
            SAMPLE_PRT("layer%d-chn%d send frame to primary-chn\n", u32LayerID, u32ChnID);
        }
    ...
    if (AX_COMPRESS_MODE_NONE == tDstFrame->stCompressInfo.enCompressMode) {
        ...
        default:
            if (tDstFrame->u32FrameSize) {
                ...
                fwrite((AX_VOID *)((AX_ULONG)tDstFrame->u64VirAddr[0]), 1, nPixelSize * 3, fp_out);
...
static AX_VOID *SAMPLE_VO_PLAY_CTRL_THREAD(AX_VOID *pData)
{
    ...
    AX_VO_CHN_ATTR_T stChnAttr, stChnAttrTmp;
    ...
    while (!pstChnThreadParam->u32ThreadForceStop) {
        ...
        /* dynamic switch of channel attr test */
        SAMPLE_PRT("layer%d-chn%d dynamic switch of channel attr\n", u32LayerID, u32ChnID);
        stChnAttrTmp = stChnAttr;
        for (j = 0; j < 10; j++) {
            stChnAttrTmp.stRect.u32X += 5;
            stChnAttrTmp.stRect.u32Y += 5;
            stChnAttrTmp.stRect.u32Width -= 10;
            stChnAttrTmp.stRect.u32Height -= 10;
            s32Ret = AX_VO_SetChnAttr(u32LayerID, u32ChnID, &stChnAttrTmp);
            if (s32Ret) {
                SAMPLE_PRT("layer%d-chn%d AX_VO_SetChnAttr failed, s32Ret = 0x%x\n", u32LayerID, u32ChnID, s32Ret);
                goto exit;
            }
...
exit:
    if (s32Ret) {
        AX_VO_ResumeChn(u32LayerID, u32ChnID);
    }
...
static AX_VOID *SAMPLE_VO_PLAY_THREAD(AX_VOID *pData)
{
    ...
    u32FrameRate = pstChnThreadParam->u32FrameRate;

    u64PTS = pstChnThreadParam->u64PTS;

    SAMPLE_PRT("u32FrameRate = %d, u64PTS = %lld, u32FrameMax = %d\n", u32FrameRate, u64PTS, u32FrameMax);
    ...
    while (!pstChnThreadParam->u32ThreadForceStop) {
        ...
        if ((u64SeqNum == 300) && pstChnThreadParam->bChnClean) {
            SAMPLE_PRT("layer%d-chn%d clean and hold 1 frame\n", u32LayerID, u32ChnID);

            s32Ret = AX_VO_ClearChnBuf(u32LayerID, u32ChnID, AX_FALSE);
            if (s32Ret) {
                SAMPLE_PRT("layer%d-chn%d AX_VO_ClearChnBuf failed, s32Ret = 0x%x\n", u32LayerID, u32ChnID, s32Ret);
            }

	    sleep(3);

            s32Ret = AX_VO_ClearChnBuf(u32LayerID, u32ChnID, AX_TRUE);
            if (s32Ret) {
                SAMPLE_PRT("layer%d-chn%d AX_VO_ClearChnBuf failed, s32Ret = 0x%x\n", u32LayerID, u32ChnID, s32Ret);
            }

            SAMPLE_PRT("layer%d-chn%d clean all\n", u32LayerID, u32ChnID);
...
static AX_VOID *SAMPLE_VO_GET_LAYER_FRAME_THREAD(AX_VOID *pData)
{
    ...
    SAMPLE_PRT("layer%d exit, u32GetCnt = %d\n", u32LayerID, u32GetCnt);
...
static AX_VOID *SAMPLE_VO_GET_CHN_FRAME_THREAD(AX_VOID *pData)
{
    ...
    while (!pstThreadParam->u32ThreadForceStop) {
        memset(&stFrame, 0, sizeof(stFrame));
        s32Ret = AX_VO_GetChnFrame(u32LayerID, u32ChnID, &stFrame, 500);
        if (s32Ret) {
            continue;
        }
...
AX_S32 SAMPLE_VO_LAYER(SAMPLE_VO_LAYER_CONFIG_S *pstLayerConf)
{
    ...
    system("cat /proc/ax_proc/pool");
    system("cat /proc/ax_proc/vo/layer_status");
    system("cat /proc/ax_proc/vo/display_status");
...
AX_S32 SAMPLE_VO_LAYER_DISPLAY(SAMPLE_VO_CONFIG_S *pstVoConf)
{
    ...
    SAMPLE_VO_EXEC_CMD_AFTER_START(pstVoConf);
...
static AX_S32 SAMPLE_VDEV_CTRL_TEST(SAMPLE_VO_CONFIG_S *pstVoConf)
{
    AX_S32 i, s32Ret = 0;
    SAMPLE_VO_DEV_CONFIG_S *pstVoDevConf;
    ...
    for (i = 0; i < pstVoConf->u32VDevNr; i++) {
        pstVoDevConf = &pstVoConf->stVoDev[i];
        if (pstVoDevConf->bDpmsCtrlEn) {
            pstVoDevConf->bDpmsCtrlEn = AX_FALSE;
            s32Ret = AX_VO_DpmsOff(pstVoDevConf->u32VoDev);
            if (s32Ret) {
                SAMPLE_PRT("vo%d dpms-off failed, s32Ret:0x%x\n", pstVoDevConf->u32VoDev, s32Ret);
                goto exit;
            }

            SAMPLE_PRT("vo%d dpms-off success\n", pstVoDevConf->u32VoDev);
            sleep(4);

            s32Ret = AX_VO_DpmsOn(pstVoDevConf->u32VoDev);
            if (s32Ret) {
                SAMPLE_PRT("vo%d dpms-on failed, s32Ret:0x%x\n", pstVoDevConf->u32VoDev, s32Ret);
                goto exit;
            }

            SAMPLE_PRT("vo%d dpms-on success\n", pstVoDevConf->u32VoDev);
            sleep(4);
        }

        if (pstVoDevConf->bCrtcCtrlEn) {
            pstVoDevConf->bCrtcCtrlEn = AX_FALSE;

            s32Ret = AX_VO_CrtcOff(pstVoDevConf->u32VoDev);
            if (s32Ret) {
                SAMPLE_PRT("vo%d crtc-off failed, s32Ret:0x%x\n", pstVoDevConf->u32VoDev, s32Ret);
                goto exit;
            }

            SAMPLE_PRT("vo%d crtc-off success\n", pstVoDevConf->u32VoDev);
            sleep(4);

            s32Ret = AX_VO_CrtcOn(pstVoDevConf->u32VoDev);
            if (s32Ret) {
                SAMPLE_PRT("vo%d crtc-on failed, s32Ret:0x%x\n", pstVoDevConf->u32VoDev, s32Ret);
                goto exit;
            }

            SAMPLE_PRT("vo%d crtc-on success\n", pstVoDevConf->u32VoDev);
            sleep(3);
        }

        if (pstVoDevConf->bHsvCtrlEn) {
            AX_VO_CSC_T stCsc, stCscTmp;

            pstVoDevConf->bHsvCtrlEn = AX_FALSE;

            SAMPLE_PRT("vo%d hsv-ctrl start\n", pstVoDevConf->u32VoDev);

            s32Ret = AX_VO_GetCSC(pstVoDevConf->u32VoDev, &stCsc);
            if (s32Ret) {
                SAMPLE_PRT("vo%d get csc failed, s32Ret:0x%x\n", pstVoDevConf->u32VoDev, s32Ret);
                goto exit;
            }
            ...
            for (i = 0; i < 230; i += 1) {
                stCscTmp.u32Luma = (stCscTmp.u32Luma + 1) % 100;
                stCscTmp.u32Contrast = (stCscTmp.u32Contrast + 1) % 100;
                stCscTmp.u32Hue = (stCscTmp.u32Hue + 1) % 100;
                stCscTmp.u32Satuature = (stCscTmp.u32Satuature + 1) % 100;

                s32Ret = AX_VO_SetCSC(pstVoDevConf->u32VoDev, &stCscTmp);
                if (s32Ret) {
                    SAMPLE_PRT("vo%d set csc failed, s32Ret:0x%x\n", pstVoDevConf->u32VoDev, s32Ret);
                    goto exit;
                }
		...
		usleep(20000);
            ...
            s32Ret = AX_VO_SetCSC(pstVoDevConf->u32VoDev, &stCsc);
            if (s32Ret) {
                SAMPLE_PRT("vo%d set csc failed, s32Ret:0x%x\n", pstVoDevConf->u32VoDev, s32Ret);
                goto exit;
            }

            SAMPLE_PRT("vo%d hsv-ctrl success\n", pstVoDevConf->u32VoDev);
        }
    }
...
AX_S32 SAMPLE_VO_PLAY(SAMPLE_VO_CONFIG_S *pstVoConf)
{
    ...
    for (i = 0; i < pstVoConf->u32LayerNr; i++) {
        ...
        for (j = 0; j < pstVoLayerConf->s32Chns; j++) {
            ...
            pstChnThreadParam->u64PTS = pstVoLayerConf->u64PTS;
            strcpy(pstChnThreadParam->chnFileName, pstVoLayerConf->chnFileName);
            pstChnThreadParam->u32ThreadForceStop = 0;
            pstChnThreadParam->u32UserPoolId = pstVoLayerConf->u32ChnPoolId;
	    pstChnThreadParam->bChnClean = ((j == 0) && pstVoLayerConf->bChnClean) ? AX_TRUE : AX_FALSE;
    ...
    while (!gLoopExit) {
        ...
        if (u32Count > 35) {
            s32Ret = SAMPLE_VDEV_CTRL_TEST(pstVoConf);
            if (s32Ret) {
                break;
            }
        }
