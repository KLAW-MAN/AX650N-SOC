...
static struct ax_img_fmt_name vo_img_fmt_str[] = {
    ...
    {AX_FORMAT_YUV422_SEMIPLANAR_10BIT_P010, "AX_FORMAT_YUV422_SEMIPLANAR_10BIT_P010"},
...
static AX_CHAR *vo_hdmi_out_fmt[AX_HDMI_VIDEO_MODE_BUTT] = {
    [AX_HDMI_VIDEO_MODE_UNUSED] = "AX_HDMI_VIDEO_MODE_UNUSED",
    [AX_HDMI_VIDEO_MODE_RGB444] = "AX_HDMI_VIDEO_MODE_RGB444",
    [AX_HDMI_VIDEO_MODE_YUV444] = "AX_HDMI_VIDEO_MODE_YUV444",
    [AX_HDMI_VIDEO_MODE_YUV422] = "AX_HDMI_VIDEO_MODE_YUV422",
    [AX_HDMI_VIDEO_MODE_YUV420] = "AX_HDMI_VIDEO_MODE_YUV420",
};
...
static AX_CHAR *key_str[AX_VO_INI_K_BUTT] = {
    ...
    "disp_hdmi_mode",
    "disp_dpms_ctrl_enable",
    "disp_crtc_ctrl_enable",
    "disp_hsv_ctrl_enable",
    "disp_wbc_enable",
    "disp_wbc_type",
    "disp_wbc_mode",
    "disp_wbc_frame_rate",
...
static AX_VOID VO_INI_KVAL_SPLIT(AX_CHAR *pStr, AX_CHAR *pSeparator, SPLIT_STR_T *pstSplitStr)
{
    AX_CHAR *pStrTmp = NULL, *pSaveStr = NULL;

    memset(pstSplitStr, 0, sizeof(*pstSplitStr));

    pStrTmp = strtok_r(pStr, pSeparator, &pSaveStr);
    while (pStrTmp && pstSplitStr->u32StrNr <= SAMPLE_VO_DEV_MAX) {
        SAMPLE_PRT("split:%s\n", pStrTmp);
        pstSplitStr->pStr[pstSplitStr->u32StrNr++] = pStrTmp;
        pStrTmp = strtok_r(NULL, pSeparator, &pSaveStr);
    }
}
                                 ...
                                 AX_VO_INI_SECTION_E type, AX_VO_INI_KEY_E key)
{
    ...
    if (key == AX_VO_INI_K_D_CMD_AFTER_START) {
        pSeparator =  ",";
    }

    VO_INI_KVAL_SPLIT(pstr, pSeparator, &stSplitStr);
    ...
    case AX_VO_INI_K_C_INITIAL_PTS:
        if (type == AX_VO_INI_S_LAYER_DISPLAY) {
            pVoConfig = (SAMPLE_VO_CONFIG_S *)pConfig;
            u32ValNr = stSplitStr.u32StrNr;
            for (i = 0; i < u32ValNr; i++) {
                pLayerConf = &pVoConfig->stVoLayer[i];
                pLayerConf->u64PTS = strtoul(stSplitStr.pStr[i], NULL, 0);
            }
        } else {
            pLayerConf = (SAMPLE_VO_LAYER_CONFIG_S *)pConfig;
            pLayerConf->u64PTS = strtoul(stSplitStr.pStr[0], NULL, 0);
        }
        break;
    ...
    case AX_VO_INI_K_L_CHN_CLEAN_CTRL:
        pVoConfig = (SAMPLE_VO_CONFIG_S *)pConfig;
        u32ValNr = stSplitStr.u32StrNr;
        for (i = 0; i < u32ValNr; i++) {
            pLayerConf = &pVoConfig->stVoLayer[i];
            pLayerConf->bChnClean = strtoul(stSplitStr.pStr[i], NULL, 0) ? AX_TRUE : AX_FALSE;
        }
        break;

    case AX_VO_INI_K_L_BATCH_CTRL:
        pVoConfig = (SAMPLE_VO_CONFIG_S *)pConfig;
        u32ValNr = stSplitStr.u32StrNr;
        for (i = 0; i < u32ValNr; i++) {
            pLayerConf = &pVoConfig->stVoLayer[i];
            pLayerConf->bUseBatch = strtoul(stSplitStr.pStr[i], NULL, 0) ? AX_TRUE : AX_FALSE;
        }
        break;
    ...
    case AX_VO_INI_K_D_DPMS_CTRL_ENABLE:
        if (type == AX_VO_INI_S_LAYER_DISPLAY) {
            pVoConfig = (SAMPLE_VO_CONFIG_S *)pConfig;
            u32ValNr = stSplitStr.u32StrNr;
            for (i = 0; i < u32ValNr; i++) {
                pVoConfig->stVoDev[i].bDpmsCtrlEn = strtoul(stSplitStr.pStr[i], NULL, 0) ? AX_TRUE : AX_FALSE;
            }
        }
        break;

    case AX_VO_INI_K_D_CRTC_CTRL_ENABLE:
        if (type == AX_VO_INI_S_LAYER_DISPLAY) {
            pVoConfig = (SAMPLE_VO_CONFIG_S *)pConfig;
            u32ValNr = stSplitStr.u32StrNr;
            for (i = 0; i < u32ValNr; i++) {
                pVoConfig->stVoDev[i].bCrtcCtrlEn = strtoul(stSplitStr.pStr[i], NULL, 0) ? AX_TRUE : AX_FALSE;
            }
        }
        break;

    case AX_VO_INI_K_D_HSV_CTRL_ENABLE:
        if (type == AX_VO_INI_S_LAYER_DISPLAY) {
            pVoConfig = (SAMPLE_VO_CONFIG_S *)pConfig;
            u32ValNr = stSplitStr.u32StrNr;
            for (i = 0; i < u32ValNr; i++) {
                pVoConfig->stVoDev[i].bHsvCtrlEn = strtoul(stSplitStr.pStr[i], NULL, 0) ? AX_TRUE : AX_FALSE;
            }
        }
        break;
            ...
            for (i = 0; i < u32ValNr; i++) {
                for (csc_matrix = AX_VO_CSC_MATRIX_IDENTITY; csc_matrix < AX_VO_CSC_MATRIX_BUTT; csc_matrix++) {
                    if (!strcmp(stSplitStr.pStr[i], vo_csc_matrix_str[csc_matrix])) {
                        pVoConfig->stVoDev[i].stCsc.enCscMatrix = csc_matrix;
                        pVoConfig->stVoDev[i].bCscEn = true;
                    }
                }
            }
        }
    ...
    case AX_VO_INI_K_D_LUMA:
        if (type == AX_VO_INI_S_LAYER_DISPLAY) {
            ...
            u32ValNr = stSplitStr.u32StrNr;
            for (i = 0; i < u32ValNr; i++) {
                pVoConfig->stVoDev[i].stCsc.u32Luma = strtoul(stSplitStr.pStr[i], NULL, 0);
                pVoConfig->stVoDev[i].bCscEn = true;
            }
        }
        break;
    ...
    case AX_VO_INI_K_D_CONTRAST:
        if (type == AX_VO_INI_S_LAYER_DISPLAY) {
            ...
            u32ValNr = stSplitStr.u32StrNr;
            for (i = 0; i < u32ValNr; i++) {
                pVoConfig->stVoDev[i].stCsc.u32Contrast = strtoul(stSplitStr.pStr[i], NULL, 0);
                pVoConfig->stVoDev[i].bCscEn = true;
            }
        }
        break;
    ...
    case AX_VO_INI_K_D_HUE:
        if (type == AX_VO_INI_S_LAYER_DISPLAY) {
            ...
            u32ValNr = stSplitStr.u32StrNr;
            for (i = 0; i < u32ValNr; i++) {
                pVoConfig->stVoDev[i].stCsc.u32Hue = strtoul(stSplitStr.pStr[i], NULL, 0);
                pVoConfig->stVoDev[i].bCscEn = true;
            }
        }
        break;
    ...
    case AX_VO_INI_K_D_SATUATURE:
        if (type == AX_VO_INI_S_LAYER_DISPLAY) {
            ...
            u32ValNr = stSplitStr.u32StrNr;
            for (i = 0; i < u32ValNr; i++) {
                pVoConfig->stVoDev[i].stCsc.u32Satuature = strtoul(stSplitStr.pStr[i], NULL, 0);
                pVoConfig->stVoDev[i].bCscEn = true;
            }
        }
        break;
    ...
    case AX_VO_INI_K_D_HDMI_OUT_FMT:
        if (type == AX_VO_INI_S_LAYER_DISPLAY) {
            AX_HDMI_VIDEO_MODE_E hdmifmt;
            pVoConfig = (SAMPLE_VO_CONFIG_S *)pConfig;
            u32ValNr = stSplitStr.u32StrNr;
            for (i = 0; i < u32ValNr; i++) {
                for (hdmifmt = AX_HDMI_VIDEO_MODE_UNUSED; hdmifmt < AX_HDMI_VIDEO_MODE_BUTT; hdmifmt++) {
                    if (!strcmp(stSplitStr.pStr[i], vo_hdmi_out_fmt[hdmifmt])) {
                        pVoConfig->stVoDev[i].enHdmiOutFmt = hdmifmt;
                        break;
                    }
                }
                if (hdmifmt >= AX_HDMI_VIDEO_MODE_BUTT) {
                    SAMPLE_PRT("Unknown hdmi out format [%s] for dev%d\n",
                               stSplitStr.pStr[i], pVoConfig->stVoDev[i].u32VoDev);
                }
            }
        }
    ...
    case AX_VO_INI_K_D_CMD_AFTER_START:
        if (type == AX_VO_INI_S_LAYER_DISPLAY) {
            pVoConfig = (SAMPLE_VO_CONFIG_S *)pConfig;
            u32ValNr = stSplitStr.u32StrNr;
            for (i = 0; i < u32ValNr; i++) {
                strncpy(pVoConfig->stVoDev[i].cCmd, stSplitStr.pStr[i], CMD_BUF_MAX - 1);
            }
        }
        break;
