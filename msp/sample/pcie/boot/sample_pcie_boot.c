...
static int check_target_dev(int fd, struct boot_attr *attr, int target_id)
{
    int ret;
    int i;
    ...
    for (i = 0; i < MAX_DEVICE_NUM; i++) {
        if (target_id == attr->remote_devices[i].id) {
            return 0;
        }
    }
    ...
    while ((Opt = getopt(argc, argv, "fpsrn:t:d:b:z:h")) != -1) {
        ...
        case 't':
            if (optarg != NULL) {
                ...
                if(check_target_dev(dev_fd, &attr, TargetId)) {
                    printf("not found dev:0x%x \n", TargetId);
                    goto out;
                }
                attr.id = TargetId;
                printf("TargetId = %d\n", TargetId);
