...
static int SAMPLE_CIPHER_RsaPkcs1v15(int id)
{
    int index;
    int failed = 0;
    AX_CIPHER_SIG_DATA_T signature;
...
static AX_U8 pk_msg_4bytes[] = {"abcd"};

static AX_U8 pk_msg_128bytes [] = {
    "abcdddddddjjjjjjjmmmmmmnnnnnnooooabcdddddddjjjjjjjmmmmmmnnnnnnoooojjjjzzzzzzqelcabcdddddddjjjjjjjmmmmmmnnnnnnooooabcdddddddjjjjj"
};

static AX_U8 pk_msg_256bytes [] = {
    "abcdddddddjjjjjjjmmmmmmnnnnnnooooabcdddddddjjjjjjjmmmmmmnnnnnnoooojjjjzzzzzzqelcabcdddddddjjjjjjjmmmmmmnnnnnnooooabcdddddddjjjjj"
    "abcdddddddjjjjjjjmmmmmmnnnnnnooooabcdddddddjjjjjjjmmmmmmnnnnnnoooojjjjzzzzzzqelcabcdddddddjjjjjjjmmmmmmnnnnnnooooabcdddddddjjjjj"
};

static AX_U8 pk_msg_384bytes [] = {
    "abcdddddddjjjjjjjmmmmmmnnnnnnooooabcdddddddjjjjjjjmmmmmmnnnnnnoooojjjjzzzzzzqelcabcdddddddjjjjjjjmmmmmmnnnnnnooooabcdddddddjjjjj"
    "abcdddddddjjjjjjjmmmmmmnnnnnnooooabcdddddddjjjjjjjmmmmmmnnnnnnoooojjjjzzzzzzqelcabcdddddddjjjjjjjmmmmmmnnnnnnooooabcdddddddjjjjj"
    "abcdddddddjjjjjjjmmmmmmnnnnnnooooabcdddddddjjjjjjjmmmmmmnnnnnnoooojjjjzzzzzzqelcabcdddddddjjjjjjjmmmmmmnnnnnnooooabcdddddddjjjjj"
};

static const TestVectorRSAPk_t testVectorsRsaPk[] = {
    {
    ...
    },
    {
        ...
        pk_msg_384bytes,
        sizeof(pk_msg_384bytes) - 1,
    },
};

static int SAMPLE_CIPHER_Rsa_Public_Key()
{
    AX_CIPHER_RSA_PUB_ENC_T pub_enc;
    AX_CIPHER_RSA_PRI_ENC_T pri_enc;
    int ret;
    AX_U32 msg_len = 0;
    AX_U8 digest_out[384];
    AX_U8 msg_out[384];
    AX_U32 out_len;
    AX_U8 *pk_msg;

    for (int i = 0; i < sizeof(testVectorsRsaPk) / sizeof(testVectorsRsaPk[0]); i++) {
        memset(&pub_enc, 0, sizeof(pub_enc));
        pub_enc.enScheme = testVectorsRsaPk[i].enScheme;
        pub_enc.pubKey.modulusBits = testVectorsRsaPk[i].Key.ModulusBytes * 8;
        pub_enc.pubKey.modulusData = (AX_U8 *)testVectorsRsaPk[i].Key.Modulus_p;
        pub_enc.pubKey.publicExponentBytes = testVectorsRsaPk[i].Key.PublicExponentBytes;
        pub_enc.pubKey.exponentData = (AX_U8 *)testVectorsRsaPk[i].Key.PublicExponent_p;

        memset(&pri_enc, 0, sizeof(pri_enc));
        pri_enc.enScheme = testVectorsRsaPk[i].enScheme;
        pri_enc.priKey.modulusBits = testVectorsRsaPk[i].Key.ModulusBytes * 8;
        pri_enc.priKey.modulusData = (AX_U8 *)testVectorsRsaPk[i].Key.Modulus_p;
        pri_enc.priKey.privateExponentBytes = testVectorsRsaPk[i].Key.PrivateExponentBytes;
        pri_enc.priKey.exponentData = (AX_U8 *)testVectorsRsaPk[i].Key.PrivateExponent_p;

        pk_msg = (AX_U8 *)testVectorsRsaPk[i].msg;
        msg_len = testVectorsRsaPk[i].msgBytes;

        ret = AX_CIPHER_RsaPublicEncrypt(&pub_enc, pk_msg, msg_len, digest_out, &out_len);
        if (ret) {
            printf("AX_CIPHER_RsaPublicEncrypt failed ret = 0x%08x\n", ret);
            return ret;
        }

        ret = AX_CIPHER_RsaPrivateDecrypt(&pri_enc, digest_out, out_len, msg_out, &out_len);
        if (ret) {
            printf("AX_CIPHER_RsaPublicEncrypt failed ret = 0x%08x\n", ret);
            return ret;
        }

        if (0 != memcmp(msg_out, pk_msg, msg_len)) {
            printf("%s, rsa public key compare fail %d: msg_len = %u, enScheme = %u, key_len = %u\n",
                    __func__, __LINE__, msg_len, testVectorsRsaPk[i].enScheme, testVectorsRsaPk[i].Key.ModulusBytes);
            return -1;
        }

        ret = AX_CIPHER_RsaPrivateEncrypt(&pri_enc, pk_msg, msg_len, digest_out, &out_len);
        if (ret) {
            printf("AX_CIPHER_RsaPublicEncrypt failed ret = 0x%08x\n", ret);
            return ret;
        }

        ret = AX_CIPHER_RsaPublicDecrypt(&pub_enc, digest_out, out_len, msg_out, &out_len);
        if (ret) {
            printf("AX_CIPHER_RsaPublicEncrypt failed ret = 0x%08x\n", ret);
            return ret;
        }
