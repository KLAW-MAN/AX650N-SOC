...
/* default param */
#include "os08b10_sdr.h"
#include "os08b10_hdr_2x.h"

#include "ax_module_version.h"
/****************************************************************************
 * golbal variables  and macro definition
 ****************************************************************************/
SNS_STATE_OBJ *g_szOs08b10Ctx[AX_VIN_MAX_PIPE_NUM] = {AX_NULL};

#define SENSOR_GET_CTX(dev, pstCtx) (pstCtx = g_szOs08b10Ctx[dev])
#define SENSOR_SET_CTX(dev, pstCtx) (g_szOs08b10Ctx[dev] = pstCtx)
#define SENSOR_RESET_CTX(dev) (g_szOs08b10Ctx[dev] = AX_NULL)

const char axera_sns_os08b10_version[] = AXERA_MODULE_VERSION;
...
static AX_S32 sensor_ctx_init(ISP_PIPE_ID nPipeId)
{
    ...
    SNS_DBG("os08b10 sensor_ctx_init. ret = %d\n", ret);
    ...
    if (AX_NULL == sns_obj) {
        ...
        if (AX_NULL == sns_obj) {
            SNS_ERR("malloc g_szOs08b10Ctx failed\n");
 ...
 ****************************************************************************/
static AX_S32 os08b10_get_chipid(ISP_PIPE_ID nPipeId, AX_S32 *pSnsId)
{
    AX_U32 sensor_id = 0;
    AX_U16 val = 0;
    ...
    sensor_id |= os08b10_reg_read(nPipeId, 0x300A) << 16;
    sensor_id |= os08b10_reg_read(nPipeId, 0x300B) << 8;
    sensor_id |= os08b10_reg_read(nPipeId, 0x300C);

    SNS_DBG("%s: sensor os08b10 id: 0x%x\n", __func__, sensor_id);

    if (sensor_id != OS08B10_SENSOR_CHIP_ID) {
        SNS_ERR("%s: Failed to read sensor os08b10 id\n", __func__);
...
static void os08b10_init(ISP_PIPE_ID nPipeId)
{
    AX_S32 nRet = 0;
    AX_S32 nSnsId = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
    ...
    /* 2. i2c init */
    os08b10_sensor_i2c_init(nPipeId);

    nRet = os08b10_get_chipid(nPipeId, &nSnsId);
    if (nRet != SNS_SUCCESS) {
        SNS_ERR("can't find os08b10 sensor id.\n");
    } else {
        SNS_DBG("os08b10 check chip id success.\n");
    }

    /* 3. config settings  */
    os08b10_write_settings(nPipeId);

    /* 4. refresh ae param */
    os08b10_cfg_aec_param(nPipeId);

    /* 5. refresh ae regs table */
    os08b10_sns_refresh_all_regs_from_tbl(nPipeId);
...
static void os08b10_exit(ISP_PIPE_ID nPipeId)
{
    if (nPipeId < 0 || (nPipeId >= AX_VIN_MAX_PIPE_NUM)) {
        return;
    }

    os08b10_sensor_i2c_exit(nPipeId);
    sensor_ctx_exit(nPipeId);
    return;
}


static AX_S32 os08b10_sensor_streaming_ctrl(ISP_PIPE_ID nPipeId, AX_U32 on)
{
    AX_S32 result = 0;
    ...
    if (1 == on) {
        result = os08b10_write_register(nPipeId, 0x0100, 0x01); // stream on
        SNS_DBG("sensor stream on!\n");
    } else {
        result = os08b10_write_register(nPipeId, 0x0100, 0x00); // stream off
...
static AX_S32 os08b10_sensor_set_mode(ISP_PIPE_ID nPipeId, AX_SNS_ATTR_T *sns_mode)
{
    AX_S32 width;
    AX_S32 height;
    AX_S32 hdrmode;
    ...
    if (width == 3840 && height == 2160 && hdrmode == 1 && IS_SNS_FPS_EQUAL(framerate, 25.0)) {
        sns_setting_index = e_OS08B10_4lane_3840x2160_Linear_10bit_25fps_1440Mbps;
    }  else if (width == 3840 && height == 2160 && hdrmode == 2 && IS_SNS_FPS_EQUAL(framerate, 25.0)) {
        sns_setting_index = e_OS08B10_4lane_3840x2160_2hdr_10bit_25fps_1440Mbps;
    }  else if (width == 3840 && height == 2160 && hdrmode == 1 && IS_SNS_FPS_EQUAL(framerate, 30.0)) {
        sns_setting_index = e_OS08B10_4lane_3840x2160_Linear_10bit_30fps_1440Mbps;
    }  else if (width == 3840 && height == 2160 && hdrmode == 2 && IS_SNS_FPS_EQUAL(framerate, 30.0)) {
        sns_setting_index = e_OS08B10_4lane_3840x2160_2hdr_10bit_30fps_1440Mbps;
    }  else if (width == 3840 && height == 2160 && hdrmode == 1 && IS_SNS_FPS_EQUAL(framerate, 50.0)) {
        sns_setting_index = e_OS08B10_4lane_3840x2160_Linear_10bit_50fps_1440Mbps;
    } else {
...
static AX_S32 os08b10_sensor_get_mode(ISP_PIPE_ID nPipeId, AX_SNS_ATTR_T *pSnsMode)
{
    AX_S32 nRet = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
...
static AX_S32 os08b10_testpattern_ctrl(ISP_PIPE_ID nPipeId, AX_U32 on)
{
    SNS_CHECK_VALUE_RANGE_VALID(nPipeId, 0, AX_VIN_MAX_PIPE_NUM - 1);
    ...
    if (1 == on) {
        /* enable test-pattern */
        os08b10_write_register(nPipeId, 0x5080, 0x80); /* long  frame*/
        os08b10_write_register(nPipeId, 0x50c0, 0x80); /* short frame*/
    } else {
        /* disable test-pattern */
        os08b10_write_register(nPipeId, 0x5080, 0x00); /* long  frame*/
        os08b10_write_register(nPipeId, 0x50c0, 0x00); /* short frame*/
    }
 ...
 ****************************************************************************/
static AX_S32 os08b10_get_isp_default_params(ISP_PIPE_ID nPipeId, AX_SENSOR_DEFAULT_PARAM_T *ptDftParam)
{
    AX_S32 nRet = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
...
static AX_S32 os08b10_get_isp_black_level(ISP_PIPE_ID nPipeId, AX_SNS_BLACK_LEVEL_T *ptBlackLevel)
{
    SNS_STATE_OBJ *sns_obj = AX_NULL;
...
AX_SYS_API_PUBLIC AX_SENSOR_REGISTER_FUNC_T gSnsos08b10Obj = {

    /* sensor ctrl */
    .pfn_sensor_chipid                      = os08b10_get_chipid,
    .pfn_sensor_init                        = os08b10_init,
    .pfn_sensor_exit                        = os08b10_exit,
    .pfn_sensor_reset                       = os08b10_reset,
    .pfn_sensor_streaming_ctrl              = os08b10_sensor_streaming_ctrl,
    .pfn_sensor_testpattern                 = os08b10_testpattern_ctrl,

    .pfn_sensor_set_mode                    = os08b10_sensor_set_mode,
    .pfn_sensor_get_mode                    = os08b10_sensor_get_mode,

    .pfn_sensor_set_fps                     = AX_NULL,

    /* communication : register read/write */
    .pfn_sensor_set_bus_info                = os08b10_set_bus_info,
    .pfn_sensor_write_register              = os08b10_write_register,
    .pfn_sensor_read_register               = os08b10_read_register,

    /* default param */
    .pfn_sensor_get_default_params          = os08b10_get_isp_default_params,
    .pfn_sensor_get_black_level             = os08b10_get_isp_black_level,

    /* ae ctrl */
    .pfn_sensor_get_hw_exposure_params      = os08b10_get_hw_exposure_params,
    .pfn_sensor_get_gain_table              = os08b10_get_sensor_gain_table,
    .pfn_sensor_set_again                   = os08b10_set_again,
    .pfn_sensor_set_dgain                   = os08b10_set_dgain,
    .pfn_sensor_hcglcg_ctrl                 = os08b10_hcglcg_ctrl,

    .pfn_sensor_set_integration_time        = os08b10_set_integration_time,
    .pfn_sensor_get_integration_time_range  = os08b10_get_integration_time_range,
    .pfn_sensor_set_slow_fps                = os08b10_set_slow_fps,
    .pfn_sensor_get_slow_shutter_param      = os08b10_get_slow_shutter_param,
    .pfn_sensor_get_sns_reg_info            = os08b10_ae_get_sensor_reg_info,
