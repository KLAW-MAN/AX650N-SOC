...
AX_S32 os04a10_set_slow_fps(ISP_PIPE_ID nPipeId, AX_F32 nFps)
{
    ...
    for (i = 0; i < AX_VIN_MAX_PIPE_NUM; i++) {
        if(gOs04a10SlaveMode[i] == AX_TRUE) {
    ...
    if (nPipeId == 0 && sns_obj->sns_attr_param.eMasterSlaveSel == AX_SNS_SYNC_SLAVE) {
        /*when sensor in slave mode , when fps change, vts need modify synchronous */
        tVsyncInfo.nFps = (AX_U32)nFps;
        tVsyncInfo.nSnsCnt = j;
        ax_vsync_ctrl_set_fps(&tVsyncInfo);
    } else {
    ...
    if(AX_SNS_SYNC_SLAVE == sns_obj->sns_mode_obj.eMasterSlaveSel) {
        os04a10_set_fsin_sync_cnt(nPipeId, sns_os04a10params[nPipeId].vts - g_os04a10MasterVsyncStart);
    }
    ...
    if(IS_SNS_FPS_EQUAL(fFps, 25.0) || IS_SNS_FPS_EQUAL(fFps, 50.0)) {  /* PAL */
        if(AX_SNS_LINEAR_MODE == sns_obj->sns_attr_param.eSnsMode) {
            if(AX_RT_RAW10 == sns_obj->sns_attr_param.eRawType) {
                vts = OS04A10_VTS_10BIT_4M25_SDR;  /* SDR 10BIT 4LINE 25fps */
                if(IS_SNS_FPS_EQUAL(fFps, 50.0)) {
                vts = OS04A10_VTS_10BIT_4M50_SDR;  /* SDR 10BIT 4LINE 50fps*/
                }
                if (sns_obj->sns_attr_param.nSettingIndex > e_OS04A10_setting_special_idx) {
        ...
        if(AX_SNS_LINEAR_MODE == sns_obj->sns_attr_param.eSnsMode) {
            if(AX_RT_RAW10 == sns_obj->sns_attr_param.eRawType) {
                ...
                if(IS_SNS_FPS_EQUAL(fFps, 60.0)) {
                    vts = OS04A10_VTS_10BIT_4M60_SDR;  /* SDR 10BIT 4LINE 60fps*/
                }
                if (sns_obj->sns_attr_param.nSettingIndex > e_OS04A10_setting_special_idx) {
