...
#define OS04A10_HDR_2X_LONG_EXP_LINE_MAX(vts)                         ((vts - 10.0f) / 2.0f)
#define OS04A10_HDR_2X_LONG_INTEGRATION_TIME_STEP                     (1.0f)

#define OS04A10_EXP_OFFSET_DCG_HDR_HCG  (0.4f) //unit:line
#define OS04A10_EXP_OFFSET_DCG_HDR_LCG  (0.7f)

typedef struct _OS04A10_DCG_AGAIN_TABLE_T_ {
    float gain;
    AX_U8 again_in;
    AX_U8 again_de;
} OS04A10_DCG_AGAIN_TABLE_T;

typedef struct _OS04A10_DCG_DGAIN_TABLE_T_ {
    float gain;
    AX_U8 dgain_in;
    AX_U8 dgain_de;
    AX_U8 dgain_de2;
} OS04A10_DCG_DGAIN_TABLE_T;

extern SNS_STATE_OBJ *g_szOs04a10Ctx[AX_VIN_MAX_PIPE_NUM];
extern SNSOS04A10_OBJ_T sns_os04a10params[AX_VIN_MAX_PIPE_NUM];
...
static AX_F32 nDcg_AgainTable[SENSOR_MAX_GAIN_STEP];
static AX_F32 nDcg_DgainTable[SENSOR_MAX_GAIN_STEP];

/*user config*/
static AX_F32 gDcgFpsGear[] = {1.00, 2.00, 3.00, 4.00, 5.00, 6.00, 7.00, 8.00, 9.00, 10.00,
                            11.00, 12.00, 13.00, 14.00, 15.00, 16.00, 17.00, 18.00, 19.00, 20.00,
                            21.00, 22.00, 23.00, 24.00, 25.00, 26.00, 27.00, 28.00, 29.00, 30.00
                           };

typedef enum _AX_SNS_DCG_AE_REG_IDX_E_ {
...
static AX_SNS_DRV_DELAY_TABLE_T gOs04a10DcgAeRegsTable[] = {
...
const OS04A10_DCG_AGAIN_TABLE_T os04a10_dcg_again_table[] = {
    {1,       0x01, 0x00},
    {1.0625,  0x01, 0x10},
    {1.125,   0x01, 0x20},
    {1.1875,  0x01, 0x30},
...
const OS04A10_DCG_DGAIN_TABLE_T os04a10_dcg_dgain_table[] = {
...
AX_F32 os04a10_again2value_dcg(float gain, AX_U8 *again_in, AX_U8 *again_de)
{
    AX_U32 i;
    AX_U32 count;
    AX_U32 ret_value = 0;
    ...
    count = sizeof(os04a10_dcg_again_table) / sizeof(os04a10_dcg_again_table[0]);

    for (i = 0; i < count; i++) {
        if (gain > os04a10_dcg_again_table[i].gain) {
            continue;
        } else {
            *again_in = os04a10_dcg_again_table[i].again_in;
            *again_de = os04a10_dcg_again_table[i].again_de;
            SNS_DBG("again=%f, again_in=0x%x, again_de=0x%x\n", gain, *again_in, *again_de);
            return os04a10_dcg_again_table[i].gain;
        }
    }
...
AX_F32 os04a10_dgain2value_dcg(float gain, AX_U8 *dgain_in, AX_U8 *dgain_de, AX_U8 *dgain_de2)
{
    AX_U32 i;
    AX_U32 count;
    AX_U32 ret_value = 0;
    ...
    count = sizeof(os04a10_dcg_dgain_table) / sizeof(os04a10_dcg_dgain_table[0]);

    for (i = 0; i < count; i++) {
        if (gain > os04a10_dcg_dgain_table[i].gain) {
            continue;
        } else {
            *dgain_in = os04a10_dcg_dgain_table[i].dgain_in;
            *dgain_de = os04a10_dcg_dgain_table[i].dgain_de;
            *dgain_de2 = os04a10_dcg_dgain_table[i].dgain_de2;
            SNS_DBG("dgain=%f, dgain_in=0x%x, dgain_de=0x%x dgain_de2=0x%x\n", gain, *dgain_in, *dgain_de, *dgain_de2);

            return os04a10_dcg_dgain_table[i].gain;
        }
    }
...
AX_S32 os04a10_get_gain_table_dcg(AX_SNS_AE_GAIN_TABLE_T *params)
{
    AX_U32 i;
    AX_S32 ret = 0;
    if (!params)
    ...
    params->nAgainTableSize = sizeof(os04a10_dcg_again_table) / sizeof(os04a10_dcg_again_table[0]);
    params->nDgainTableSize = sizeof(os04a10_dcg_dgain_table) / sizeof(os04a10_dcg_dgain_table[0]);

    for (i = 0; i < params->nAgainTableSize ; i++) {
        nDcg_AgainTable[i] = os04a10_dcg_again_table[i].gain;
        params->pAgainTable = nDcg_AgainTable;
    }

    for (i = 0; i < params->nDgainTableSize ; i++) {
        nDcg_DgainTable[i] = os04a10_dcg_dgain_table[i].gain;
        params->pDgainTable = nDcg_DgainTable;
    }

    return ret;
}

AX_S32 os04a10_cfg_aec_param_dcg(ISP_PIPE_ID nPipeId)
{
    ...
    if (sns_obj->sns_attr_param.eSnsOutputMode == AX_SNS_DCG_HDR) {
        sns_os04a10params[nPipeId].line_period = sns_os04a10params[nPipeId].hts / sns_os04a10params[nPipeId].sclk * SNS_1_SECOND_UNIT_US;
    } else if (sns_obj->sns_attr_param.eSnsOutputMode == AX_SNS_DCG_VS_HDR) {
        sns_os04a10params[nPipeId].line_period = 2 * sns_os04a10params[nPipeId].hts / sns_os04a10params[nPipeId].sclk * SNS_1_SECOND_UNIT_US;
    } else {
        // wrong hdr mode
    }
    ...
    /* sensor integrationtime limit*/
    sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMinIntegrationTime[HDR_LONG_FRAME_IDX] = OS04A10_HDR_2X_LONG_EXP_LINE_MIN;
    sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[HDR_LONG_FRAME_IDX] = OS04A10_HDR_2X_LONG_EXP_LINE_MAX(sns_os04a10params[nPipeId].vts);
    ...
    /* sensor medium integrationtime limit dcg*/
    sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMinIntegrationTime[HDR_MEDIUM_FRAME_IDX] = OS04A10_HDR_2X_LONG_EXP_LINE_MIN;
    sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[HDR_MEDIUM_FRAME_IDX] = OS04A10_HDR_2X_LONG_EXP_LINE_MAX(sns_os04a10params[nPipeId].vts);
    sns_obj->ae_ctrl_param.sns_ae_param.fIntegrationTimeIncrement[HDR_MEDIUM_FRAME_IDX] = OS04A10_HDR_2X_LONG_INTEGRATION_TIME_STEP;

    sns_obj->ae_ctrl_param.fSnsHcgLcgRatio = 4.1953125f;
    sns_obj->ae_ctrl_param.eSnsHcgLcgMode = AX_LCG_NOTSUPPORT_MODE;
    sns_obj->ae_ctrl_param.sns_ae_param.fIntegrationTimeOffset[HDR_LONG_FRAME_IDX] = OS04A10_EXP_OFFSET_DCG_HDR_HCG;
    sns_obj->ae_ctrl_param.sns_ae_param.fIntegrationTimeOffset[HDR_MEDIUM_FRAME_IDX] = OS04A10_EXP_OFFSET_DCG_HDR_LCG;
...
AX_S32 os04a10_sns_update_regidx_table_dcg(ISP_PIPE_ID nPipeId, AX_U8 nRegIdx, AX_U8 nRegValue)
{
    AX_S32 i = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
    SNS_CHECK_VALUE_RANGE_VALID(nPipeId, 0, AX_VIN_MAX_PIPE_NUM - 1);
...
AX_S32 os04a10_get_sensor_gain_table_dcg(ISP_PIPE_ID nPipeId, AX_SNS_AE_GAIN_TABLE_T *params)
{
    AX_S32 result = 0;
    SNS_CHECK_PTR_VALID(params);
    SNS_CHECK_VALUE_RANGE_VALID(nPipeId, 0, AX_VIN_MAX_PIPE_NUM - 1);

    result = os04a10_get_gain_table_dcg(params);
    return result;

    return SNS_SUCCESS;
}

AX_S32 os04a10_set_again_dcg(ISP_PIPE_ID nPipeId, AX_SNS_AE_GAIN_CFG_T *ptAGain)
{
    AX_U8 Gain_in;
    AX_U8 Gain_de;
    AX_S32 result = 0;
    ...
    gain_value = os04a10_again2value_dcg(nGainFromUser, &Gain_in, &Gain_de);
    if (gain_value == -1) {
        SNS_ERR("new gain match failed \n");
    } else {
        sns_obj->ae_ctrl_param.sns_ae_param.fCurAGain[HDR_LONG_FRAME_IDX] = gain_value;
        result = os04a10_sns_update_regidx_table_dcg(nPipeId, OS04A10_LONG_AGAIN_H_IDX, (Gain_in & 0x1F));
        result |= os04a10_sns_update_regidx_table_dcg(nPipeId, OS04A10_LONG_AGAIN_L_IDX, (Gain_de & 0xF0));
        if (result != 0) {
    ...
    if (gain_value == -1) {
        ...
        result = os04a10_sns_update_regidx_table_dcg(nPipeId, OS04A10_SHORT_AGAIN_H_IDX, (Gain_in & 0x1F));
        result = os04a10_sns_update_regidx_table_dcg(nPipeId, OS04A10_SHORT_AGAIN_L_IDX, (Gain_de & 0xF0));
        if (result != 0) {
...
AX_S32 os04a10_set_dgain_dcg(ISP_PIPE_ID nPipeId, AX_SNS_AE_GAIN_CFG_T *ptDGain)
{
    AX_U8 Gain_in;
    AX_U8 Gain_de;
    AX_U8 Gain_de2;
    ...
    gain_val = os04a10_dgain2value_dcg(nGainFromUser, &Gain_in, &Gain_de, &Gain_de2);
    if (gain_val == -1) {
        SNS_ERR("new gain match failed \n");
    } else {
        sns_obj->ae_ctrl_param.sns_ae_param.fCurDGain[HDR_LONG_FRAME_IDX] = gain_val;
        result = os04a10_sns_update_regidx_table_dcg(nPipeId, OS04A10_LONG_DGAIN_H_IDX, (Gain_in & 0x0F));
        result = os04a10_sns_update_regidx_table_dcg(nPipeId, OS04A10_LONG_DGAIN_M_IDX, (Gain_de & 0xFF));
        result = os04a10_sns_update_regidx_table_dcg(nPipeId, OS04A10_LONG_DGAIN_L_IDX, (Gain_de & 0xC0));
        if (result != 0) {
            SNS_ERR("write hw failed %d \n", result);
            return result;
        }
    ...
    if (gain_val == -1) {
        ...
        result = os04a10_sns_update_regidx_table_dcg(nPipeId, OS04A10_SHORT_DGAIN_H_IDX, (Gain_in & 0x0F));
        result = os04a10_sns_update_regidx_table_dcg(nPipeId, OS04A10_SHORT_DGAIN_M_IDX, (Gain_de & 0xFF));
        result = os04a10_sns_update_regidx_table_dcg(nPipeId, OS04A10_SHORT_DGAIN_L_IDX, (Gain_de & 0xC0));
        if (result != 0) {
            SNS_ERR("write hw failed %d \n", result);
            return result;
        }
...
AX_S32 os04a10_get_integration_time_range_dcg(ISP_PIPE_ID nPipeId, AX_F32 fHdrRatio,
        AX_SNS_AE_INT_TIME_RANGE_T *ptIntTimeRange)
{
    ...
    if (sns_obj->sns_attr_param.eSnsOutputMode == AX_SNS_DCG_HDR) {
        ...
        sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[HDR_MEDIUM_FRAME_IDX] = OS04A10_SDR_EXP_LINE_MAX(sns_os04a10params[nPipeId].vts);
    } else if (sns_obj->sns_attr_param.eSnsOutputMode == AX_SNS_DCG_VS_HDR) {
        sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[HDR_LONG_FRAME_IDX] = OS04A10_HDR_2X_LONG_EXP_LINE_MAX(sns_os04a10params[nPipeId].vts);
        sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[HDR_MEDIUM_FRAME_IDX] = OS04A10_HDR_2X_LONG_EXP_LINE_MAX(sns_os04a10params[nPipeId].vts);
    }
...
AX_S32 os04a10_set_integration_time_dcg(ISP_PIPE_ID nPipeId, AX_SNS_AE_SHUTTER_CFG_T *ptIntTime)
{
    AX_U8 ex_h;
    AX_U8 ex_l;
    AX_U32 ex_ival = 0;
    ...
    os04a10_sns_update_regidx_table_dcg(nPipeId, OS04A10_LONG_EXP_LINE_H_IDX, ex_h);
    os04a10_sns_update_regidx_table_dcg(nPipeId, OS04A10_LONG_EXP_LINE_L_IDX, ex_l);
...
AX_S32 os04a10_get_hw_exposure_params_dcg(ISP_PIPE_ID nPipeId, AX_SNS_EXP_CTRL_PARAM_T *ptAeCtrlParam)
{
    SNS_STATE_OBJ *sns_obj = AX_NULL;
...
AX_S32 os04a10_sns_refresh_all_regs_from_tbl_dcg(ISP_PIPE_ID nPipeId)
{
    AX_S32 i = 0;
    AX_U32 nNum = 0;
    AX_U32  nRegValue;
    ...
    nNum = sizeof(gOs04a10DcgAeRegsTable) / sizeof(gOs04a10DcgAeRegsTable[0]);

    for (i = 0; i < nNum; i++) {
        nRegValue = os04a10_reg_read(nPipeId, gOs04a10DcgAeRegsTable[i].nRegAddr);
        sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].nRegAddr = gOs04a10DcgAeRegsTable[i].nRegAddr;
        sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].nData = nRegValue;

        SNS_DBG(" nRegAddr 0x%x, nRegValue 0x%x\n", gOs04a10DcgAeRegsTable[i].nRegAddr, gOs04a10DcgAeRegsTable[i].nRegValue);
    }

    return SNS_SUCCESS;
}

AX_S32 os04a10_sns_update_init_exposure_reg_dcg(ISP_PIPE_ID nPipeId)
{
    int i = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
    SNS_CHECK_VALUE_RANGE_VALID(nPipeId, 0, AX_VIN_MAX_PIPE_NUM - 1);
...
static AX_VOID _sns_reg_info_update_dcg(ISP_PIPE_ID nPipeId, SNS_STATE_OBJ *sns_obj, AX_BOOL *bUpdateReg)
{
    AX_S32 i = 0;
...
static AX_VOID _sns_reg_info_init_dcg(ISP_PIPE_ID nPipeId, SNS_STATE_OBJ *sns_obj)
{
    AX_S32 i = 0;

    sns_obj->sztRegsInfo[0].eSnsType = ISP_SNS_CONNECT_I2C_TYPE;
    sns_obj->sztRegsInfo[0].tComBus.I2cDev = os04a10_get_bus_num(nPipeId);
    sns_obj->sztRegsInfo[0].nRegNum = sizeof(gOs04a10DcgAeRegsTable) / sizeof(gOs04a10DcgAeRegsTable[0]);
    sns_obj->sztRegsInfo[0].nCfg2ValidDelayMax = 2;

    for (i = 0; i < sns_obj->sztRegsInfo[0].nRegNum; i++) {
        sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].bUpdate = AX_TRUE;
        sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].nDevAddr = OS04A10_SLAVE_ADDR;
        ...
        sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].nDelayFrmNum = gOs04a10DcgAeRegsTable[i].nDelayFrmNum;
...
AX_S32 os04a10_ae_get_sensor_reg_info_dcg(ISP_PIPE_ID nPipeId, AX_SNS_REGS_CFG_TABLE_T *ptSnsRegsInfo)
{
    AX_S32 nRet = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
    ...
    if ((AX_FALSE == sns_obj->bSyncInit) || (AX_FALSE == ptSnsRegsInfo->bConfig)) {
        ...
        _sns_reg_info_init_dcg(nPipeId, sns_obj);

        bUpdateReg = AX_TRUE;
        sns_obj->bSyncInit = AX_TRUE;
        os04a10_sns_update_init_exposure_reg_dcg(nPipeId);
    } else {
        _sns_reg_info_update_dcg(nPipeId, sns_obj, &bUpdateReg);
    }
...
static AX_U32 os04a10_set_vts_dcg(ISP_PIPE_ID nPipeId, AX_U32 vts)
{
    AX_U8 vts_h;
    AX_U8 vts_l;
    AX_S32 result = 0;
    ...
    result |= os04a10_sns_update_regidx_table_dcg(nPipeId, OS04A10_VTS_H_IDX, vts_h);
    result |= os04a10_sns_update_regidx_table_dcg(nPipeId, OS04A10_VTS_L_IDX, vts_l);

    return result;
}

AX_S32 os04a10_get_fps_dcg(ISP_PIPE_ID nPipeId, AX_F32 *pFps)
{
    SNS_STATE_OBJ *sns_obj = AX_NULL;
...
AX_S32 os04a10_set_fps_dcg(ISP_PIPE_ID nPipeId, AX_F32 fFps)
{
    AX_S32 result = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
    ...
    if(IS_SNS_FPS_EQUAL(fFps, 25.0) || IS_SNS_FPS_EQUAL(fFps, 50.0)) {  /* PAL */
        if(AX_SNS_DCG_VS_HDR == sns_obj->sns_attr_param.eSnsOutputMode) {
            sns_os04a10params[nPipeId].vts = OS04A10_VTS_10BIT_4M25_DCG_VS_HDR_2X;
        } else if(AX_SNS_DCG_HDR == sns_obj->sns_attr_param.eSnsOutputMode) {
            sns_os04a10params[nPipeId].vts = OS04A10_VTS_10BIT_4M25_DCG_HDR_2X;
        }
    }

    if(IS_SNS_FPS_EQUAL(fFps, 30.0) || IS_SNS_FPS_EQUAL(fFps, 60.0)) {  /* NTSC */
        if(AX_SNS_DCG_VS_HDR == sns_obj->sns_attr_param.eSnsOutputMode) {
            sns_os04a10params[nPipeId].vts = OS04A10_VTS_10BIT_4M30_DCG_VS_HDR_2X;
        } else if(AX_SNS_DCG_HDR == sns_obj->sns_attr_param.eSnsOutputMode) {
            sns_os04a10params[nPipeId].vts = OS04A10_VTS_10BIT_4M30_DCG_HDR_2X;
        }
    }

    /* set sensor vts */
    result = os04a10_set_vts_dcg(nPipeId, sns_os04a10params[nPipeId].vts);
        if (result != 0) {
 ...
 ****************************************************************************/
static AX_S32 sensor_ctx_init_dcg(ISP_PIPE_ID nPipeId)
{
    SNS_STATE_OBJ *sns_obj = AX_NULL;
    AX_S32 ret = 0;

    SNS_DBG("os04a10 sensor_ctx_init_dcg. ret = %d\n", ret);
    SNS_CHECK_VALUE_RANGE_VALID(nPipeId, 0, AX_VIN_MAX_PIPE_NUM - 1);
...
static AX_VOID sensor_ctx_exit_dcg(ISP_PIPE_ID nPipeId)
{
    SNS_STATE_OBJ *sns_obj = AX_NULL;
 ...
 ****************************************************************************/
static AX_S32 os04a10_get_chipid_dcg(ISP_PIPE_ID nPipeId, AX_S32 *pSnsId)
{
    AX_U32 sensor_id = 0;
    AX_U16 val = 0;
...
static void os04a10_init_dcg(ISP_PIPE_ID nPipeId)
{
    AX_S32 nRet = 0;
    AX_S32 nImagemode = 0;
    AX_S32 nSnsId = 0;
    ...
    if (AX_NULL == sns_obj) {
        /* contex init */
        nRet = sensor_ctx_init_dcg(nPipeId);
        if (0 != nRet) {
            SNS_ERR("sensor_ctx_init_dcg failed!\n");
            return;
        } else {
            SENSOR_GET_CTX(nPipeId, sns_obj);
        }
    ...
    nRet = os04a10_get_chipid_dcg(nPipeId, &nSnsId);
    if (nRet != SNS_SUCCESS) {
    ...
    /* 4. refresh ae param */
    os04a10_cfg_aec_param_dcg(nPipeId);

    /* 5. refresh ae regs table */
    os04a10_sns_refresh_all_regs_from_tbl_dcg(nPipeId);
...
static void os04a10_exit_dcg(ISP_PIPE_ID nPipeId)
{
    if (nPipeId < 0 || (nPipeId >= AX_VIN_MAX_PIPE_NUM)) {
        return;
    }
    ...
    sensor_ctx_exit_dcg(nPipeId);

    return;
}


static AX_S32 os04a10_sensor_streaming_ctrl_dcg(ISP_PIPE_ID nPipeId, AX_U32 on)
{
    AX_S32 result = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
...
static AX_S32 os04a10_sensor_set_mode_dcg(ISP_PIPE_ID nPipeId, AX_SNS_ATTR_T *sns_mode)
{
    AX_S32 width = 0;
    AX_S32 height = 0;
    AX_S32 hdrmode = 0;
    ...
    AX_SNS_OUTPUT_MODE_E SnsOutputMode;
    AX_S32 nRet = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
    ...
    SnsOutputMode = sns_mode->eSnsOutputMode;
    ...
    if (width == 2688 && height == 1520 && nRawType == 10 && MasterSlaveSel == AX_SNS_MASTER && SnsOutputMode == AX_SNS_DCG_VS_HDR) {
        sns_setting_index = e_OS04A10_4lane_2688x1520_10bit_DCG_VS_HDR_30fps;
    } else if (width == 2688 && height == 1520 && nRawType == 10 && MasterSlaveSel == AX_SNS_MASTER && SnsOutputMode == AX_SNS_DCG_HDR) {
        sns_setting_index = e_OS04A10_4lane_2688x1520_10bit_DCG_HDR_30fps;
    } else {
        SNS_ERR("it's not supported. [%dx%d mode=%d fps=%d raw_type=%d MasterSlaveSel=%d SnsOutputMode=%d] \n",
                width, height, hdrmode, framerate, nRawType, MasterSlaveSel, SnsOutputMode);
        return SNS_ERR_CODE_ILLEGAL_PARAMS;
    }
...
static AX_S32 os04a10_sensor_get_mode_dcg(ISP_PIPE_ID nPipeId, AX_SNS_ATTR_T *pSnsMode)
{
    AX_S32 nRet = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
...
static AX_S32 os04a10_testpattern_ctrl_dcg(ISP_PIPE_ID nPipeId, AX_U32 on)
{
    SNS_CHECK_VALUE_RANGE_VALID(nPipeId, 0, AX_VIN_MAX_PIPE_NUM - 1);
 ...
 ****************************************************************************/
static AX_S32 os04a10_get_isp_default_params_dcg(ISP_PIPE_ID nPipeId, AX_SENSOR_DEFAULT_PARAM_T *ptDftParam)
{
    AX_S32 nRet = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
...
static AX_S32 os04a10_get_black_level_dcg(ISP_PIPE_ID nPipeId, AX_SNS_BLACK_LEVEL_T *ptBlackLevel)
{
    SNS_STATE_OBJ *sns_obj = AX_NULL;
...
AX_SYS_API_PUBLIC AX_SENSOR_REGISTER_FUNC_T gSnsos04a10ObjDcg = {

    /* sensor ctrl */
    .pfn_sensor_reset                       = os04a10_reset,
    .pfn_sensor_chipid                      = os04a10_get_chipid_dcg,
    .pfn_sensor_init                        = os04a10_init_dcg,
    .pfn_sensor_exit                        = os04a10_exit_dcg,
    .pfn_sensor_streaming_ctrl              = os04a10_sensor_streaming_ctrl_dcg,
    .pfn_sensor_testpattern                 = os04a10_testpattern_ctrl_dcg,

    .pfn_sensor_set_mode                    = os04a10_sensor_set_mode_dcg,
    .pfn_sensor_get_mode                    = os04a10_sensor_get_mode_dcg,

    .pfn_sensor_set_fps                     = os04a10_set_fps_dcg,
    .pfn_sensor_get_fps                     = os04a10_get_fps_dcg,
    ...
    /* default param */
    .pfn_sensor_get_default_params          = os04a10_get_isp_default_params_dcg,
    .pfn_sensor_get_black_level             = os04a10_get_black_level_dcg,

    /* ae ctrl */
    .pfn_sensor_get_hw_exposure_params      = os04a10_get_hw_exposure_params_dcg,
    .pfn_sensor_get_gain_table              = os04a10_get_sensor_gain_table_dcg,
    .pfn_sensor_set_again                   = os04a10_set_again_dcg,
    .pfn_sensor_set_dgain                   = os04a10_set_dgain_dcg,
    .pfn_sensor_hcglcg_ctrl                 = AX_NULL,

    .pfn_sensor_set_integration_time        = os04a10_set_integration_time_dcg,
    .pfn_sensor_get_integration_time_range  = os04a10_get_integration_time_range_dcg,
    .pfn_sensor_set_slow_fps                = AX_NULL,
    .pfn_sensor_get_slow_shutter_param      = AX_NULL,
    .pfn_sensor_get_sns_reg_info            = os04a10_ae_get_sensor_reg_info_dcg,
