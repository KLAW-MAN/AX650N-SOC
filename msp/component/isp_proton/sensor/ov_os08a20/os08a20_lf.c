...
typedef struct _SNSOS08A20_LF_OBJ_T_ {
    AX_U32 hts;
    AX_U32 vs_hts;
    AX_U32 vts;
    AX_F32 sclk;
...
typedef struct _OS08A20_LF_GAIN_TABLE_T_ {
    float gain;
    AX_U8 again_in;
    AX_U8 again_de;
    AX_U8 dgain_in;
...
static AX_F32 nLf_AgainTable[SENSOR_MAX_GAIN_STEP];
static AX_F32 nLf_DgainTable[SENSOR_MAX_GAIN_STEP];

/*user config*/
static AX_F32 gLfFpsGear[] = {2.00, 3.00, 4.00, 5.00, 6.00, 7.00, 8.00, 9.00, 10.00,
...
typedef enum _AX_SNS_LF_AE_REG_IDX_E_ {
    ...
    OS08A20_REG_MAX_IDX,
} AX_SNS_LF_AE_REG_IDX_E;

static AX_SNS_DRV_DELAY_TABLE_T gOs08a20LfAeRegsTable[] = {
...
const OS08A20_LF_GAIN_TABLE_T os08a20_lf_gain_table[] = {
...
static AX_F32 os08a20_again2value_lf(float gain, AX_U8 *again_in, AX_U8 *again_de)
{
    AX_U32 i;
    AX_U32 count;
    AX_U32 ret_value = 0;
    ...
    count = sizeof(os08a20_lf_gain_table) / sizeof(os08a20_lf_gain_table[0]);

    for (i = 0; i < count; i++) {
        if (gain > os08a20_lf_gain_table[i].gain) {
            continue;
        } else {
            if (again_in)
                *again_in = os08a20_lf_gain_table[i].again_in;
            if (again_de)
                *again_de = os08a20_lf_gain_table[i].again_de;
            SNS_DBG("again=%f, again_in=0x%x, again_de=0x%x\n", gain, *again_in, *again_de);
            return os08a20_lf_gain_table[i].gain;
        }
    }
...
static AX_F32 os08a20_dgain2value_lf(float gain, AX_U8 *dgain_in, AX_U8 *dgain_de, AX_U8 *dgain_de2)
{
    AX_U32 i;
    AX_U32 count;
    AX_U32 ret_value = 0;
    ...
    for (i = 0; i < count; i++) {
        if (gain > os08a20_lf_gain_table[i].gain) {
            ...
            *dgain_in = os08a20_lf_gain_table[i].dgain_in;
            *dgain_de = os08a20_lf_gain_table[i].dgain_de;
...
static AX_S32 os08a20_get_gain_table_lf(AX_SNS_AE_GAIN_TABLE_T *params)
{
    AX_U32 i;
    AX_S32 ret = 0;
    if (!params)
    ...
    params->nAgainTableSize = sizeof(os08a20_lf_gain_table) / sizeof(os08a20_lf_gain_table[0]);
    params->nDgainTableSize = sizeof(os08a20_lf_gain_table) / sizeof(os08a20_lf_gain_table[0]);

    for (i = 0; i < params->nAgainTableSize ; i++) {
        nLf_AgainTable[i] = os08a20_lf_gain_table[i].gain;
        params->pAgainTable = nLf_AgainTable;
    }

    for (i = 0; i < params->nDgainTableSize ; i++) {
        nLf_DgainTable[i] = os08a20_lf_gain_table[i].gain;
        params->pDgainTable = nLf_DgainTable;
    }
 ...
 ****************************************************************************/
AX_S32 os08a20_cfg_aec_param_lf(ISP_PIPE_ID nPipeId)
{
    ...
    os08a20_get_params_from_setting(nPipeId);
    sns_obj->sns_mode_obj.nVts = sns_os08a20params[nPipeId].vts;

    sns_os08a20params[nPipeId].line_period = sns_os08a20params[nPipeId].hts / sns_os08a20params[nPipeId].sclk * SNS_1_SECOND_UNIT_US;
    if (IS_2DOL_HDR_MODE(sns_obj->sns_mode_obj.eHDRMode)) {
        sns_os08a20params[nPipeId].line_period *= 2;
    }
    ...
    if (IS_LINEAR_MODE(sns_obj->sns_mode_obj.eHDRMode)) {
        ...
        sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[HDR_LONG_FRAME_IDX] = sns_os08a20params[nPipeId].vts - 8;
        ...
        sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMinIntegrationTime[HDR_LONG_FRAME_IDX] = 4;
        sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[HDR_LONG_FRAME_IDX] = sns_os08a20params[nPipeId].vts - 8;
        sns_obj->ae_ctrl_param.sns_ae_param.fIntegrationTimeIncrement[HDR_LONG_FRAME_IDX] = 1;

        sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMinIntegrationTime[HDR_MEDIUM_FRAME_IDX] = 4;
        sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[HDR_MEDIUM_FRAME_IDX] = sns_os08a20params[nPipeId].vts - 8;
    ...
    if (sns_obj->sns_mode_obj.eHDRMode == AX_SNS_LINEAR_MODE) {
        sns_obj->ae_ctrl_param.sns_ae_param.fIntegrationTimeOffset[HDR_LONG_FRAME_IDX] = OS08A20_EXP_OFFSET_SDR;
    } else if (sns_obj->sns_mode_obj.eHDRMode == AX_SNS_HDR_2X_MODE) {
        sns_obj->ae_ctrl_param.sns_ae_param.fIntegrationTimeOffset[HDR_LONG_FRAME_IDX] = OS08A20_EXP_OFFSET_HDR_2STAGGER;
        sns_obj->ae_ctrl_param.sns_ae_param.fIntegrationTimeOffset[HDR_MEDIUM_FRAME_IDX] = OS08A20_EXP_OFFSET_HDR_2STAGGER;
    } else {
        sns_obj->ae_ctrl_param.sns_ae_param.fIntegrationTimeOffset[HDR_LONG_FRAME_IDX] = OS08A20_EXP_OFFSET_SDR;
    }
...
AX_S32 os08a20_sns_update_regidx_table_lf(ISP_PIPE_ID nPipeId, AX_U8 nRegIdx, AX_U8 nRegValue)
{
    AX_S32 i = 0;
    AX_U32 nNum = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
    ...
    nNum = sizeof(gOs08a20LfAeRegsTable) / sizeof(gOs08a20LfAeRegsTable[0]);
...
AX_S32 os08a20_get_sensor_gain_table_lf(ISP_PIPE_ID nPipeId, AX_SNS_AE_GAIN_TABLE_T *params)
{
    AX_S32 result = 0;
    SNS_CHECK_PTR_VALID(params);
    SNS_CHECK_VALUE_RANGE_VALID(nPipeId, 0, AX_VIN_MAX_PIPE_NUM - 1);

    result = os08a20_get_gain_table_lf(params);
    return result;
...
AX_S32 os08a20_set_again_lf(ISP_PIPE_ID nPipeId, AX_SNS_AE_GAIN_CFG_T *ptAGain)
{
    AX_U8 Gain_in;
    AX_U8 Gain_de;
    AX_S32 result = 0;
    ...
    gain_value = os08a20_again2value_lf(nGainFromUser, &Gain_in, &Gain_de);
    if (gain_value == -1) {
        SNS_ERR("new gain match failed \n");
    } else {
        sns_obj->ae_ctrl_param.sns_ae_param.fCurAGain[HDR_LONG_FRAME_IDX] = gain_value;
        result = os08a20_sns_update_regidx_table_lf(nPipeId, OS08A20_LONG_AGAIN_H_IDX, (Gain_in & 0x3F));
        result |= os08a20_sns_update_regidx_table_lf(nPipeId, OS08A20_LONG_AGAIN_L_IDX, (Gain_de & 0xFF));
        if (result != 0) {
...
AX_S32 os08a20_set_dgain_lf(ISP_PIPE_ID nPipeId, AX_SNS_AE_GAIN_CFG_T *ptDGain)
{
    AX_U8 Gain_in;
    AX_U8 Gain_de;
    AX_U8 Gain_de2;
    ...
    gain_val = os08a20_dgain2value_lf(nGainFromUser, &Gain_in, &Gain_de, &Gain_de2);
    if (gain_val == -1) {
        SNS_ERR("new gain match failed \n");
    } else {
        sns_obj->ae_ctrl_param.sns_ae_param.fCurDGain[HDR_LONG_FRAME_IDX] = gain_val;
        result = os08a20_sns_update_regidx_table_lf(nPipeId, OS08A20_LONG_DGAIN_H_IDX, (Gain_in & 0x3F));
        result = os08a20_sns_update_regidx_table_lf(nPipeId, OS08A20_LONG_DGAIN_L_IDX, (Gain_de & 0xFF));
        if (result != 0) {
            SNS_ERR("write hw failed %d \n", result);
            return result;
        }
...
AX_S32 os08a20_get_integration_time_range_lf(ISP_PIPE_ID nPipeId, AX_F32 fHdrRatio,
...
AX_S32 os08a20_set_integration_time_lf(ISP_PIPE_ID nPipeId, AX_SNS_AE_SHUTTER_CFG_T *ptIntTime)
{
    AX_U8 ex_h;
    AX_U8 ex_l;
    AX_U32 ex_ival = 0;
    ...
    os08a20_sns_update_regidx_table_lf(nPipeId, OS08A20_LONG_EXP_LINE_H_IDX, ex_h);
    os08a20_sns_update_regidx_table_lf(nPipeId, OS08A20_LONG_EXP_LINE_L_IDX, ex_l);
...
AX_S32 os08a20_get_hw_exposure_params_lf(ISP_PIPE_ID nPipeId, AX_SNS_EXP_CTRL_PARAM_T *ptAeCtrlParam)
{
    SNS_STATE_OBJ *sns_obj = AX_NULL;
...
AX_U32 os08a20_sns_refresh_all_regs_from_tbl_lf(ISP_PIPE_ID nPipeId)
{
    AX_S32 i = 0;
    AX_U32 nNum = 0;
    AX_U32  nRegValue;
    ...
    for (i = 0; i < nNum; i++) {
        nRegValue = os08a20_reg_read(nPipeId, gOs08a20LfAeRegsTable[i].nRegAddr);
        sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].nRegAddr = gOs08a20LfAeRegsTable[i].nRegAddr;
        sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].nData = nRegValue;

        SNS_DBG(" nRegAddr 0x%x, nRegValue 0x%x\n", gOs08a20LfAeRegsTable[i].nRegAddr, gOs08a20LfAeRegsTable[i].nRegValue);
    }

    return SNS_SUCCESS;
}

AX_S32 os08a20_sns_update_init_exposure_reg_lf(ISP_PIPE_ID nPipeId)
{
    int i = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
    SNS_CHECK_VALUE_RANGE_VALID(nPipeId, 0, AX_VIN_MAX_PIPE_NUM - 1);
    ...
    for (i = 0; i < sns_obj->sztRegsInfo[0].nRegNum; i++) {
        os08a20_write_register(nPipeId, sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].nRegAddr, sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].nData);
        SNS_DBG("Idx = %d, reg addr 0x%x, reg data 0x%x\n",
...
AX_S32 os08a20_ae_get_sensor_reg_info_lf(ISP_PIPE_ID nPipeId, AX_SNS_REGS_CFG_TABLE_T *ptSnsRegsInfo)
{
    AX_S32 i = 0;
    AX_S32 nRet = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
    ...
    if ((AX_FALSE == sns_obj->bSyncInit) || (AX_FALSE == ptSnsRegsInfo->bConfig)) {
        ...
        sns_obj->sztRegsInfo[0].nRegNum = sizeof(gOs08a20LfAeRegsTable) / sizeof(gOs08a20LfAeRegsTable[0]);
        sns_obj->sztRegsInfo[0].nCfg2ValidDelayMax = 2;

        for (i = 0; i < sns_obj->sztRegsInfo[0].nRegNum; i++) {
            sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].bUpdate = AX_TRUE;
            sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].nDevAddr = OS08A20_SLAVE_ADDR;
            sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].nAddrByteNum = OS08A20_ADDR_BYTE;
            sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].nDataByteNum = OS08A20_DATA_BYTE;
            sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].nDelayFrmNum = gOs08a20LfAeRegsTable[i].nDelayFrmNum;
        ...
        os08a20_sns_update_init_exposure_reg_lf(nPipeId);
    } else {
        for (i = 0; i < sns_obj->sztRegsInfo[0].nRegNum; i++) {
...
static AX_U32 os08a20_set_vts_lf(ISP_PIPE_ID nPipeId, AX_U32 vts)
{
    AX_U8 vts_h;
    AX_U8 vts_l;
    AX_S32 result = 0;
    ...
    result |= os08a20_sns_update_regidx_table_lf(nPipeId, OS08A20_VTS_H_IDX, vts_h);
    result |= os08a20_sns_update_regidx_table_lf(nPipeId, OS08A20_VTS_L_IDX, vts_l);

    return result;
}

AX_S32 os08a20_set_slow_fps_lf(ISP_PIPE_ID nPipeId, AX_F32 nFps)
{
    AX_S32 result = 0;
    AX_S32 framerate = 30;
    AX_U32 vts = 0;
    ...
    result = os08a20_set_vts_lf(nPipeId, sns_os08a20params[nPipeId].vts);
    if (result != 0) {
...
AX_S32 os08a20_get_slow_shutter_param_lf(ISP_PIPE_ID nPipeId, AX_SNS_AE_SLOW_SHUTTER_PARAM_T *ptSlowShutterParam)
{
    SNS_STATE_OBJ *sns_obj = AX_NULL;
    ...
    ptSlowShutterParam->nGroupNum = AXSNS_MIN((sizeof(gLfFpsGear) / sizeof(AX_F32)), framerate);
    //ax_sns_quick_sort_float(gLfFpsGear, ptSlowShutterParam->nGroupNum);
    ptSlowShutterParam->fMinFps = AXSNS_MAX(gLfFpsGear[0],
    ...
    for (nfps = 0 ; nfps < ptSlowShutterParam->nGroupNum; nfps++) {
        nVts = 1 * SNS_1_SECOND_UNIT_US / (sns_os08a20params[nPipeId].line_period * gLfFpsGear[nfps]);
        if ((AX_S32)gLfFpsGear[nfps] >= framerate) {
        ...
        if (IS_LINEAR_MODE(sns_obj->sns_mode_obj.eHDRMode)) {
            ...
            ptSlowShutterParam->tSlowShutterTbl[nfps].fMaxIntTime = (AX_U32)((nVts  - 8) * OS08A20_MAX_RATIO / (OS08A20_MAX_RATIO + 1));
        }
        ...
        SNS_DBG("nPipeId = %d, line_period = %.2f, fps = %.2f, fMaxIntTime = %d, vts=0x%x\n",
...
AX_S32 os08a20_get_fps_lf(ISP_PIPE_ID nPipeId, AX_F32 *pFps)
{
    SNS_STATE_OBJ *sns_obj = AX_NULL;
...
AX_S32 os08a20_set_fps_lf(ISP_PIPE_ID nPipeId, AX_F32 fFps)
{
    AX_S32 result = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
 ...
 ****************************************************************************/
static AX_S32 sensor_ctx_init_lf(ISP_PIPE_ID nPipeId)
{
    SNS_STATE_OBJ *sns_obj = AX_NULL;
    AX_S32 ret = 0;

    SNS_DBG("os08a20 sensor_ctx_init_lf. ret = %d\n", ret);
    SNS_CHECK_VALUE_RANGE_VALID(nPipeId, 0, AX_VIN_MAX_PIPE_NUM - 1);
...
static AX_VOID sensor_ctx_exit_lf(ISP_PIPE_ID nPipeId)
{
    SNS_STATE_OBJ *sns_obj = AX_NULL;
 ...
 ****************************************************************************/
static AX_S32 os08a20_get_chipid_lf(ISP_PIPE_ID nPipeId, AX_S32 *pSnsId)
{
    AX_U32 sensor_id = 0;
...
static void os08a20_init_lf(ISP_PIPE_ID nPipeId)
{
    AX_S32 nRet = 0;
    AX_S32 nSnsId = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
    ...
    if (AX_NULL == sns_obj) {
        /* contex init */
        nRet = sensor_ctx_init_lf(nPipeId);
        if (0 != nRet) {
            SNS_ERR("sensor_ctx_init_lf failed!\n");
            return;
        } else {
            SENSOR_GET_CTX(nPipeId, sns_obj);
        }
    ...
    nRet = os08a20_get_chipid_lf(nPipeId, &nSnsId);
    if (nRet != SNS_SUCCESS) {
    ...
    /* 3. config settings  */
    os08a20_write_settings(nPipeId);

    /* 4. refresh ae param */
    os08a20_cfg_aec_param_lf(nPipeId);

    /* 5. refresh ae regs table */
    os08a20_sns_refresh_all_regs_from_tbl_lf(nPipeId);
...
static void os08a20_exit_lf(ISP_PIPE_ID nPipeId)
{
    if (nPipeId < 0 || (nPipeId >= AX_VIN_MAX_PIPE_NUM)) {
        return;
    }
    ...
    sensor_ctx_exit_lf(nPipeId);

    return;
}

static AX_S32 os08a20_sensor_streaming_ctrl_lf(ISP_PIPE_ID nPipeId, AX_U32 on)
{
    AX_S32 result = 0;
...
static AX_S32 os08a20_sensor_set_mode_lf(ISP_PIPE_ID nPipeId, AX_SNS_ATTR_T *sns_mode)
{
    AX_S32 width;
    AX_S32 height;
    AX_S32 hdrmode;
    ...
    if (width == 3840 && height == 2160 && hdrmode == 1 && eRawType == AX_RT_RAW12 && IS_SNS_FPS_EQUAL(framerate, 25.0)) {
...
static AX_S32 os08a20_sensor_get_mode_lf(ISP_PIPE_ID nPipeId, AX_SNS_ATTR_T *pSnsMode)
{
    AX_S32 nRet = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
...
static AX_S32 os08a20_testpattern_ctrl_lf(ISP_PIPE_ID nPipeId, AX_U32 on)
{
    SNS_CHECK_VALUE_RANGE_VALID(nPipeId, 0, AX_VIN_MAX_PIPE_NUM - 1);
 ...
 ****************************************************************************/
static AX_S32 os08a20_get_isp_default_params_lf(ISP_PIPE_ID nPipeId, AX_SENSOR_DEFAULT_PARAM_T *ptDftParam)
{
    AX_S32 nRet = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
...
static AX_S32 os08a20_get_isp_black_level_lf(ISP_PIPE_ID nPipeId, AX_SNS_BLACK_LEVEL_T *ptBlackLevel)
{
    SNS_STATE_OBJ *sns_obj = AX_NULL;
...
/* long frame sensor object */
AX_SYS_API_PUBLIC AX_SENSOR_REGISTER_FUNC_T gSnsos08a20ObjLf = {
    /* sensor ctrl */
    .pfn_sensor_chipid                      = os08a20_get_chipid_lf,
    .pfn_sensor_init                        = os08a20_init_lf,
    .pfn_sensor_exit                        = os08a20_exit_lf,
    .pfn_sensor_reset                       = os08a20_reset,
    .pfn_sensor_streaming_ctrl              = os08a20_sensor_streaming_ctrl_lf,
    .pfn_sensor_testpattern                 = os08a20_testpattern_ctrl_lf,

    .pfn_sensor_set_mode                    = os08a20_sensor_set_mode_lf,
    .pfn_sensor_get_mode                    = os08a20_sensor_get_mode_lf,

    .pfn_sensor_set_fps                     = os08a20_set_fps_lf,
    .pfn_sensor_get_fps                     = os08a20_get_fps_lf,
    ...
    /* default param */
    .pfn_sensor_get_default_params          = os08a20_get_isp_default_params_lf,
    .pfn_sensor_get_black_level             = os08a20_get_isp_black_level_lf,

    /* ae ctrl */
    .pfn_sensor_get_hw_exposure_params      = os08a20_get_hw_exposure_params_lf,
    .pfn_sensor_get_gain_table              = os08a20_get_sensor_gain_table_lf,
    .pfn_sensor_set_again                   = os08a20_set_again_lf,
    .pfn_sensor_set_dgain                   = os08a20_set_dgain_lf,
    .pfn_sensor_hcglcg_ctrl                 = AX_NULL,

    .pfn_sensor_set_integration_time        = os08a20_set_integration_time_lf,
    .pfn_sensor_get_integration_time_range  = os08a20_get_integration_time_range_lf,
    .pfn_sensor_set_slow_fps                = os08a20_set_slow_fps_lf,
    .pfn_sensor_get_slow_shutter_param      = os08a20_get_slow_shutter_param_lf,
    .pfn_sensor_get_sns_reg_info            = os08a20_ae_get_sensor_reg_info_lf,
