...
#define OS08A20_MAX_RATIO       (32.0f)
 ...
 ****************************************************************************/
AX_S32 os08a20_get_params_from_setting(ISP_PIPE_ID nPipeId)
{
    AX_S32 ret = 0;
    AX_U32 reg_cnt = 0;
    ...
    ret = os08a20_select_setting(nPipeId, &setting, &reg_cnt);
    if(ret) {
        return ret;
    }

    ret |= os08a20_get_vts_from_setting(nPipeId, setting, reg_cnt, &vts);
    ret |= os08a20_get_hts_from_setting(nPipeId, setting, reg_cnt, &hts);
    ret |= os08a20_get_sclk_from_setting(nPipeId, setting, reg_cnt, &sclk);
    if(ret) {
        return ret;
    }

    sns_os08a20params[nPipeId].vts = vts;
    sns_os08a20params[nPipeId].hts = hts;
    sns_os08a20params[nPipeId].sclk = sclk;
...
AX_S32 os08a20_cfg_aec_param(ISP_PIPE_ID nPipeId)
{
    ...
    sns_obj->ae_ctrl_param.fLineTime = sns_os08a20params[nPipeId].line_period;
    ...
    if (IS_LINEAR_MODE(sns_obj->sns_mode_obj.eHDRMode)) {
        sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMinIntegrationTime[HDR_LONG_FRAME_IDX] = 2;
        ...
        sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMinIntegrationTime[HDR_MEDIUM_FRAME_IDX] = 2;
...
AX_S32 os08a20_set_integration_time(ISP_PIPE_ID nPipeId, AX_SNS_AE_SHUTTER_CFG_T *ptIntTime)
{
    ...
    SNS_INFO("userExpline:%f-%f, userRatio:%f, curExpline:%.2f-%.2f(%.2f-%.2f), curRatio:%.2f\n",
...
AX_U32 os08a20_sns_refresh_all_regs_from_tbl(ISP_PIPE_ID nPipeId)
{
    ...
    for (i = 0; i < nNum; i++) {
        ...
        sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].nRegAddr = gOs08a20AeRegsTable[i].nRegAddr;
...
AX_S32 os08a20_ae_get_sensor_reg_info(ISP_PIPE_ID nPipeId, AX_SNS_REGS_CFG_TABLE_T *ptSnsRegsInfo)
{
    ...
    if ((AX_FALSE == sns_obj->bSyncInit) || (AX_FALSE == ptSnsRegsInfo->bConfig)) {
        ...
        for (i = 0; i < sns_obj->sztRegsInfo[0].nRegNum; i++) {
            ...
            sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].nDelayFrmNum = gOs08a20AeRegsTable[i].nDelayFrmNum;
...
AX_S32 os08a20_get_fps(ISP_PIPE_ID nPipeId, AX_F32 *pFps)
{
    SNS_STATE_OBJ *sns_obj = AX_NULL;
...
AX_S32 os08a20_get_slow_shutter_param(ISP_PIPE_ID nPipeId, AX_SNS_AE_SLOW_SHUTTER_PARAM_T *ptSlowShutterParam)
{
    ...
    SNS_INFO("pipe:%d, line_period:%f, max_ratio:%.2f\n", nPipeId, sns_os08a20params[nPipeId].line_period, OS08A20_MAX_RATIO);
    ...
    for (nfps = 0 ; nfps < ptSlowShutterParam->nGroupNum; nfps++) {
        ...
        SNS_INFO("fps = %f, max_expline = %.2f(%.2f), vts = 0x%x\n",
                ...
                ptSlowShutterParam->tSlowShutterTbl[nfps].fMaxIntTime * sns_os08a20params[nPipeId].line_period,
                nVts);
    }
...
AX_S32 os08a20_set_fps(ISP_PIPE_ID nPipeId, AX_F32 fFps)
{
    ...
    if (AX_SNS_HDR_2X_MODE == sns_obj->sns_attr_param.eSnsMode) {
        ...
        } else if ( (AX_RT_RAW10 == sns_obj->sns_attr_param.eRawType) && (IS_SNS_FPS_EQUAL(fFps, 60.0)) ) {
            sns_os08a20params[nPipeId].vts = OS08A20_VTS_10BIT_8M60_SDR;
        } else if ( (AX_RT_RAW10 == sns_obj->sns_attr_param.eRawType) && (IS_SNS_FPS_EQUAL(fFps, 50.0)) ) {
            sns_os08a20params[nPipeId].vts = OS08A20_VTS_10BIT_8M50_SDR;
        }
    }
