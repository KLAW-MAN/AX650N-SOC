...
AX_U32 os08a20_sns_refresh_all_regs_from_tbl_sf(ISP_PIPE_ID nPipeId)
{
    ...
    for (i = 0; i < nNum; i++) {
        ...
        sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].nRegAddr = gOs08a20SfAeRegsTable[i].nRegAddr;
...
AX_S32 os08a20_ae_get_sensor_reg_info_sf(ISP_PIPE_ID nPipeId, AX_SNS_REGS_CFG_TABLE_T *ptSnsRegsInfo)
{
    ...
    if ((AX_FALSE == sns_obj->bSyncInit) || (AX_FALSE == ptSnsRegsInfo->bConfig)) {
        ...
        for (i = 0; i < sns_obj->sztRegsInfo[0].nRegNum; i++) {
            ...
            sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].nDelayFrmNum = gOs08a20SfAeRegsTable[i].nDelayFrmNum;
...
AX_S32 os08a20_get_fps_sf(ISP_PIPE_ID nPipeId, AX_F32 *pFps)
{
    SNS_STATE_OBJ *sns_obj = AX_NULL;
...
AX_SYS_API_PUBLIC AX_SENSOR_REGISTER_FUNC_T gSnsos08a20ObjSf = {
    ...
    .pfn_sensor_get_fps                     = os08a20_get_fps_sf,
    ...
    .pfn_sensor_get_black_level             = os08a20_get_isp_black_level_sf,
