...
AX_SNS_COMMBUS_T gsc410gsBusInfo[AX_VIN_MAX_PIPE_NUM] = {{{1}}, {{2}}};

AX_S32 sc410gs_set_bus_info(ISP_PIPE_ID nPipeId, AX_SNS_COMMBUS_T tSnsBusInfo)
{
    SNS_CHECK_VALUE_RANGE_VALID(nPipeId, 0, AX_VIN_MAX_PIPE_NUM - 1);

    /* out-of-order: before set_mode */
    if (tSnsBusInfo.busType == ISP_SNS_CONNECT_I2C_TYPE) {
        gsc410gsBusInfo[nPipeId].I2cDev = tSnsBusInfo.I2cDev;
        SNS_DBG("pipe:%d, I2cDev:%d\n", nPipeId, gsc410gsBusInfo[nPipeId].I2cDev);
    } else {
        gsc410gsBusInfo[nPipeId].SpiDev.bit4SpiDev = tSnsBusInfo.SpiDev.bit4SpiDev;
        gsc410gsBusInfo[nPipeId].SpiDev.bit4SpiCs = tSnsBusInfo.SpiDev.bit4SpiCs;
        SNS_DBG("pipe:%d, SpiDev:%d, SpiCs:%d\n", nPipeId,
            gsc410gsBusInfo[nPipeId].SpiDev.bit4SpiDev,
            gsc410gsBusInfo[nPipeId].SpiDev.bit4SpiCs);
    }

    gsc410gsBusInfo[nPipeId].busType = tSnsBusInfo.busType;
...
AX_S32 sc410gs_get_bus_num(ISP_PIPE_ID nPipeId)
{
    ...
    if (gsc410gsBusInfo[nPipeId].busType == ISP_SNS_CONNECT_I2C_TYPE) {
        SNS_DBG("pipe:%d, I2cDev:%d\n", nPipeId, gsc410gsBusInfo[nPipeId].I2cDev);
        return gsc410gsBusInfo[nPipeId].I2cDev;
    } else {
        SNS_DBG("pipe:%d, SpiDev:%d, SpiCs:%d\n", nPipeId,
            gsc410gsBusInfo[nPipeId].SpiDev.bit4SpiDev,
            gsc410gsBusInfo[nPipeId].SpiDev.bit4SpiCs);
        return gsc410gsBusInfo[nPipeId].SpiDev.bit4SpiDev;
    }
}
...
AX_S32 sc410gs_i2c_init(ISP_PIPE_ID nPipeId)
{
    ...
    sns_obj->sns_i2c_obj.slave_addr = SC410GS_SLAVE_ADDR;
    sns_obj->sns_i2c_obj.address_byte = SC410GS_ADDR_BYTE;
    sns_obj->sns_i2c_obj.data_byte = SC410GS_DATA_BYTE;
    sns_obj->sns_i2c_obj.swap_byte = SC410GS_SWAP_BYTES;

    sns_obj->sns_i2c_obj.sns_i2c_bnum = sc410gs_get_bus_num(nPipeId);

    sns_obj->sns_i2c_obj.sns_i2c_fd = i2c_init(sns_obj->sns_i2c_obj.sns_i2c_bnum,
                                      sns_obj->sns_i2c_obj.slave_addr);

    SNS_DBG("pipe:%d, sc410gs i2c init finish, i2c bus %d \n", nPipeId, sns_obj->sns_i2c_obj.sns_i2c_bnum);
...
AX_S32 sc410gs_spi_init(ISP_PIPE_ID nPipeId)
{
    ...
    sns_obj->sns_spi_obj.sns_spi_bnum = gsc410gsBusInfo[nPipeId].SpiDev.bit4SpiDev;
    sns_obj->sns_spi_obj.sns_spi_cs = gsc410gsBusInfo[nPipeId].SpiDev.bit4SpiCs;
    ...
    SNS_DBG("pipe:%d, spi init finish, spi bus %d \n", nPipeId, sns_obj->sns_spi_obj.sns_spi_bnum);
...
static AX_S32 sc410gs_spi_write(AX_S32 fd, AX_U32 devaddr, AX_U32 dev_addr_bytes,
                        ...
                        AX_U32 regdata, AX_U32 data_bytes)
{
    ...
    transfer.speed_hz = 1000000; //as kernel will set 10M, but 10M read is invalid
...
static AX_S32 sc410gs_spi_read(int fd, AX_U32 devaddr, AX_U32 dev_addr_bytes,
                        AX_U32 regaddr, AX_U32 reg_addr_bytes,
                        AX_U32 *regdata, AX_U32 data_bytes)
{
    struct spi_ioc_transfer transfer;
...
AX_S32 sc410gs_read_register(ISP_PIPE_ID nPipeId, AX_U32 nAddr, AX_U32 *pData)
{
    ...
    if (gsc410gsBusInfo[nPipeId].busType == ISP_SNS_CONNECT_I2C_TYPE) {
        ...
        dev_addr = ((SC410GS_DEVICEID << 1) | SC410GS_SPI_READ) & 0XFF;
        devaddr_bytes = 1;
        regaddr_bytes = 2;
        data_bytes = 1;
        nRet = sc410gs_spi_read(sns_obj->sns_spi_obj.sns_spi_fd,
...
AX_S32 sc410gs_write_register(ISP_PIPE_ID nPipeId, AX_U32 nAddr, AX_U32 nData)
{
    ...
    if (gsc410gsBusInfo[nPipeId].busType == ISP_SNS_CONNECT_I2C_TYPE) {
        ...
        nRet = i2c_write(sns_obj->sns_i2c_obj.sns_i2c_fd, sns_obj->sns_i2c_obj.slave_addr, nAddr,
                        sns_obj->sns_i2c_obj.address_byte,
                        (AX_U8 *)(&nData), sns_obj->sns_i2c_obj.data_byte, sns_obj->sns_i2c_obj.swap_byte);
        ...
        dev_addr = ((SC410GS_DEVICEID << 1) | SC410GS_SPI_WIRTE) & 0xFF;
        devaddr_bytes = 1;
        regaddr_bytes = 2;
        data_bytes = 1;
        nRet = sc410gs_spi_write(sns_obj->sns_spi_obj.sns_spi_fd,
                        dev_addr, devaddr_bytes,
                        nAddr, regaddr_bytes,
                        nData, data_bytes);
        if (nRet < 0)
...
AX_S32 sc410gs_hw_reset(AX_U32 gpio_num, AX_U32 gpio_out_val)
{
    FILE *fp = AX_NULL;
    char file_name[50];
    char buf[10];
...
AX_S32 sc410gs_reset(ISP_PIPE_ID nPipeId, AX_U32 nResetGpio)
{
    ...
    SNS_DBG("pipe:%d, nResetGpio:%d\n", nPipeId, nResetGpio);

    result |= sc410gs_hw_reset(nResetGpio, 0);
    usleep(5 * 1000);
    result |= sc410gs_hw_reset(nResetGpio, 1);
    usleep(10 * 1000);
...
AX_S32 sc410gs_get_vts_from_setting(ISP_PIPE_ID nPipeId, camera_i2c_reg_array *setting, AX_U32 reg_cnt, AX_U32 *vts)
{
    ...
    for (i = 0; i < reg_cnt; i++) {
        if((setting + i)->addr == SC410GS_VTS_H) {
            vts_h = (setting + i)->value;
            mask |= 1;
        } else if((setting + i)->addr == SC410GS_VTS_L) {
...
AX_S32 sc410gs_select_setting(ISP_PIPE_ID nPipeId, camera_i2c_reg_array **setting, AX_U32 *cnt)
{
    ...
    switch (sns_obj->eImgMode) {
        case e_SC410GS_mipi_4line_1744x2336_linear_10bit_50fps_704Mbps_trigger:
            *setting = SC410GS_mipi_4line_1744x2336_linear_10bit_50fps_704Mbps_trigger;
            *cnt = sizeof(SC410GS_mipi_4line_1744x2336_linear_10bit_50fps_704Mbps_trigger) / sizeof(camera_i2c_reg_array);
            break;
...
AX_S32 sc410gs_write_settings(ISP_PIPE_ID nPipeId)
{
    ...
    for (i = 0; i < reg_cnt; i++) {
        sc410gs_write_register(nPipeId, (setting + i)->addr, ((setting + i)->value));
#ifdef SENSOR_DEBUG
        usleep(2 * 1000);
        AX_U8 val = sc410gs_reg_read(nPipeId, (default_setting + i)->addr);
