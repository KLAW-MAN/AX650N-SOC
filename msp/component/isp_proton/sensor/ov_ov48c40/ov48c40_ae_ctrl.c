...
#define OV48C40_MAX_VTS         (0xFFFF)
#define OV48C40_MAX_RATIO       (32.0f)
#define OV48C40_MIN_RATIO       (1.0f)

#define OV48C40_MIN_AGAIN       (1.0f)
#define OV48C40_MAX_AGAIN       (15.5f)

#define OV48C40_MIN_DGAIN       (1.0f)
#define OV48C40_MAX_DGAIN       (15.99f)

typedef struct SENSOR_OV48C40_S {
    AX_U32 hts;
    AX_U32 vs_hts;
    AX_U32 vts;
    AX_F32 sclk;
...
typedef struct _OV48C40_AGAIN_TABLE_T_ {
    float gain;
    AX_U8 again_in;
    AX_U8 again_de;
} OV48C40_AGAIN_TABLE_T;

typedef struct _OV48C40_DGAIN_TABLE_T_ {
    float gain;
    AX_U8 dgain_in;
    AX_U8 dgain_de;
    AX_U8 dgain_de2;
} OV48C40_DGAIN_TABLE_T;
...
static SNSOV48C40_OBJ sns_ov48c40params[AX_VIN_MAX_PIPE_NUM];
...
AX_U32 g_ov48c40MasterVsyncStart = 0x08;  //from setting

typedef enum _AX_SNS_AE_REG_IDX_E_ {
    OV48C40_LONG_EXP_LINE_H_IDX = 0,
    OV48C40_LONG_EXP_LINE_M_IDX,
    OV48C40_LONG_EXP_LINE_L_IDX,
    OV48C40_LONG_AGAIN_H_IDX,
    OV48C40_LONG_AGAIN_L_IDX,
    OV48C40_LONG_DGAIN_H_IDX,
    OV48C40_LONG_DGAIN_M_IDX,
    OV48C40_LONG_DGAIN_L_IDX,

    OV48C40_SHORT_EXP_LINE_H_IDX,
    OV48C40_SHORT_EXP_LINE_M_IDX,
    OV48C40_SHORT_EXP_LINE_L_IDX,
    OV48C40_SHORT_AGAIN_H_IDX,
    OV48C40_SHORT_AGAIN_L_IDX,
    OV48C40_SHORT_DGAIN_H_IDX,
    OV48C40_SHORT_DGAIN_M_IDX,
    OV48C40_SHORT_DGAIN_L_IDX,

    OV48C40_VTS_H_IDX,
    OV48C40_VTS_L_IDX,

    OV48C40_GROUP1_IDX,
    OV48C40_HCG_LCG_REG0_IDX,
    OV48C40_GROUP2_IDX,
    OV48C40_GROUP3_IDX,
...
static AX_SNS_DRV_DELAY_TABLE_T gOv48c40AeRegsTable[] = {
    /* regs index */                /* regs addr */                 /*regs value*/    /*Delay Frame Num*/
    {OV48C40_LONG_EXP_LINE_H_IDX,   OV48C40_LONG_EXP_LINE_H,        0,          0},
    {OV48C40_LONG_EXP_LINE_M_IDX,   OV48C40_LONG_EXP_LINE_M,        0,          0},
    {OV48C40_LONG_EXP_LINE_L_IDX,   OV48C40_LONG_EXP_LINE_L,        0,          0},
    {OV48C40_LONG_AGAIN_H_IDX,      OV48C40_LONG_AGAIN_H,           0,          0},
    {OV48C40_LONG_AGAIN_L_IDX,      OV48C40_LONG_AGAIN_L,           0,          0},
    {OV48C40_LONG_DGAIN_H_IDX,      OV48C40_LONG_DGAIN_H,           0,          0},
    {OV48C40_LONG_DGAIN_M_IDX,      OV48C40_LONG_DGAIN_M,           0,          0},
    {OV48C40_LONG_DGAIN_L_IDX,      OV48C40_LONG_DGAIN_L,           0,          0},

    {OV48C40_SHORT_EXP_LINE_H_IDX,  OV48C40_SHORT_EXP_LINE_H,       0,          0},
    {OV48C40_SHORT_EXP_LINE_M_IDX,  OV48C40_SHORT_EXP_LINE_M,       0,          0},
    {OV48C40_SHORT_EXP_LINE_L_IDX,  OV48C40_SHORT_EXP_LINE_L,       0,          0},
    {OV48C40_SHORT_AGAIN_H_IDX,     OV48C40_SHORT_AGAIN_H,          0,          0},
    {OV48C40_SHORT_AGAIN_L_IDX,     OV48C40_SHORT_AGAIN_L,          0,          0},
    {OV48C40_SHORT_DGAIN_H_IDX,     OV48C40_SHORT_DGAIN_H,          0,          0},
    {OV48C40_SHORT_DGAIN_M_IDX,     OV48C40_SHORT_DGAIN_M,          0,          0},
    {OV48C40_SHORT_DGAIN_L_IDX,     OV48C40_SHORT_DGAIN_L,          0,          0},

    {OV48C40_VTS_H_IDX,             OV48C40_VTS_H,                  0,          0},
    {OV48C40_VTS_L_IDX,             OV48C40_VTS_L,                  0,          0},

    /* group hold */
    {OV48C40_GROUP1_IDX,            OV48C40_GROUP_ACCESS1,          0,          0},
    {OV48C40_HCG_LCG_REG0_IDX,      OV48C40_HCG_LCG_REG0,           0,          0},
    {OV48C40_GROUP2_IDX,            OV48C40_GROUP_ACCESS2,          0,          0},
    {OV48C40_GROUP3_IDX,            OV48C40_GROUP_ACCESS3,          0,          0},

};

const OV48C40_AGAIN_TABLE_T ov48c40_again_table[] = {
    {1,       0x01, 0x00},
    {1.0625,  0x01, 0x10},
    {1.125,   0x01, 0x20},
    {1.1875,  0x01, 0x30},
...
const OV48C40_DGAIN_TABLE_T ov48c40_dgain_table[] = {
...
AX_F32 ov48c40_again2value(float gain, AX_U8 *again_in, AX_U8 *again_de)
{
    AX_U32 i;
    AX_U32 count;
    AX_U32 ret_value = 0;
    ...
    count = sizeof(ov48c40_again_table) / sizeof(ov48c40_again_table[0]);

    for (i = 0; i < count; i++) {
        if (gain > ov48c40_again_table[i].gain) {
            continue;
        } else {
            *again_in = ov48c40_again_table[i].again_in;
            *again_de = ov48c40_again_table[i].again_de;
            SNS_DBG("again=%f, again_in=0x%x, again_de=0x%x\n", gain, *again_in, *again_de);
            return ov48c40_again_table[i].gain;
        }
    }
...
AX_F32 ov48c40_dgain2value(float gain, AX_U8 *dgain_in, AX_U8 *dgain_de, AX_U8 *dgain_de2)
{
    AX_U32 i;
    AX_U32 count;
    AX_U32 ret_value = 0;
    ...
    count = sizeof(ov48c40_dgain_table) / sizeof(ov48c40_dgain_table[0]);

    for (i = 0; i < count; i++) {
        if (gain > ov48c40_dgain_table[i].gain) {
            continue;
        } else {
            *dgain_in = ov48c40_dgain_table[i].dgain_in;
            *dgain_de = ov48c40_dgain_table[i].dgain_de;
            *dgain_de2 = ov48c40_dgain_table[i].dgain_de2;
            SNS_DBG("dgain=%f, dgain_in=0x%x, dgain_de=0x%x dgain_de2=0x%x\n", gain, *dgain_in, *dgain_de, *dgain_de2);

            return ov48c40_dgain_table[i].gain;
        }
    }
...
AX_S32 ov48c40_get_gain_table(AX_SNS_AE_GAIN_TABLE_T *params)
{
    AX_U32 i;
    AX_S32 ret = 0;
    if (!params)
    ...
    params->nAgainTableSize = sizeof(ov48c40_again_table) / sizeof(ov48c40_again_table[0]);
    params->nDgainTableSize = sizeof(ov48c40_dgain_table) / sizeof(ov48c40_dgain_table[0]);

    for (i = 0; i < params->nAgainTableSize ; i++) {
        nAgainTable[i] = ov48c40_again_table[i].gain;
        params->pAgainTable = nAgainTable;
    }

    for (i = 0; i < params->nDgainTableSize ; i++) {
        nDgainTable[i] = ov48c40_dgain_table[i].gain;
...
static AX_SNS_HCGLCG_MODE_E ov48c40_get_hcglcg_mode(ISP_PIPE_ID nPipeId)
{
    AX_S32 value = 0;
    AX_U8 data = 0;

    value = ov48c40_reg_read(nPipeId, OV48C40_HCG_LCG_REG0);
    if (value == -1) {
        SNS_ERR("OV48C40_HCG_LCG_REG0 reg value:0x%x\n", value);
...
AX_S32 ov48c40_cfg_aec_param(ISP_PIPE_ID nPipeId)
{
    ...
    sns_ov48c40params[nPipeId].hts = ov48c40_get_hts(nPipeId);
    sns_ov48c40params[nPipeId].vs_hts = ov48c40_get_vs_hts(nPipeId);
    sns_ov48c40params[nPipeId].vts = ov48c40_get_vts(nPipeId);
    sns_ov48c40params[nPipeId].sclk = ov48c40_get_sclk(nPipeId);

    if (IS_LINEAR_MODE(sns_obj->sns_mode_obj.eHDRMode)) {
        if (sns_obj->sns_mode_obj.nWidth == 8000 && sns_obj->sns_mode_obj.nHeight == 6000) {
            sns_ov48c40params[nPipeId].line_period = (float)SNS_1_SECOND_UNIT_US / sns_obj->sns_mode_obj.fFrameRate / AXSNS_DIV_0_TO_1(sns_ov48c40params[nPipeId].vts);
        } else if ((sns_obj->sns_mode_obj.nWidth == 4000 && sns_obj->sns_mode_obj.nHeight == 3000)
            || (sns_obj->sns_mode_obj.nWidth == 3840 && sns_obj->sns_mode_obj.nHeight == 2160)) {
            sns_ov48c40params[nPipeId].line_period = (float)SNS_1_SECOND_UNIT_US / sns_obj->sns_mode_obj.fFrameRate / AXSNS_DIV_0_TO_1(sns_ov48c40params[nPipeId].vts);
        }
    ...
    sns_obj->ae_ctrl_param.fTimePerLine = sns_ov48c40params[nPipeId].line_period;

    /* sensor again  limit*/
    sns_obj->ae_ctrl_param.sns_ae_limit.fMinAgain[HDR_LONG_FRAME_IDX] = OV48C40_MIN_AGAIN;
    sns_obj->ae_ctrl_param.sns_ae_limit.fMaxAgain[HDR_LONG_FRAME_IDX] = OV48C40_MAX_AGAIN;
    sns_obj->ae_ctrl_param.sns_ae_param.fAGainIncrement[HDR_LONG_FRAME_IDX] = (AX_F32)1 / 16;

    /* sensor dgain  limit*/
    sns_obj->ae_ctrl_param.sns_ae_limit.fMinDgain[HDR_LONG_FRAME_IDX] = OV48C40_MIN_DGAIN;
    sns_obj->ae_ctrl_param.sns_ae_limit.fMaxDgain[HDR_LONG_FRAME_IDX] = OV48C40_MAX_DGAIN;
    sns_obj->ae_ctrl_param.sns_ae_param.fDGainIncrement[HDR_LONG_FRAME_IDX] = (AX_F32)1 / 1024;

    /* sensor medium again limit*/
    sns_obj->ae_ctrl_param.sns_ae_limit.fMinAgain[HDR_MEDIUM_FRAME_IDX] = OV48C40_MIN_AGAIN;
    sns_obj->ae_ctrl_param.sns_ae_limit.fMaxAgain[HDR_MEDIUM_FRAME_IDX] = OV48C40_MAX_AGAIN;
    sns_obj->ae_ctrl_param.sns_ae_param.fAGainIncrement[HDR_MEDIUM_FRAME_IDX] = (AX_F32)1 / 16;

    /* sensor medium dgain limit*/
    sns_obj->ae_ctrl_param.sns_ae_limit.fMinDgain[HDR_MEDIUM_FRAME_IDX] = OV48C40_MIN_DGAIN;
    sns_obj->ae_ctrl_param.sns_ae_limit.fMaxDgain[HDR_MEDIUM_FRAME_IDX] = OV48C40_MAX_DGAIN;
    sns_obj->ae_ctrl_param.sns_ae_param.fDGainIncrement[HDR_MEDIUM_FRAME_IDX] = (AX_F32)1 / 1024;

    sns_obj->ae_ctrl_param.sns_ae_limit.fMinRatio = OV48C40_MIN_RATIO;
    sns_obj->ae_ctrl_param.sns_ae_limit.fMaxRatio = OV48C40_MAX_RATIO;

    if (IS_LINEAR_MODE(sns_obj->sns_mode_obj.eHDRMode)) {
        if (sns_obj->sns_mode_obj.nWidth == 8000 && sns_obj->sns_mode_obj.nHeight == 6000) {
            sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMinIntegrationTime[HDR_LONG_FRAME_IDX] = 16;
            sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[HDR_LONG_FRAME_IDX] = sns_ov48c40params[nPipeId].vts - 32;
            sns_obj->ae_ctrl_param.sns_ae_param.fIntegrationTimeIncrement[HDR_LONG_FRAME_IDX] = 1;
        } else if ((sns_obj->sns_mode_obj.nWidth == 4000 && sns_obj->sns_mode_obj.nHeight == 3000)
            || (sns_obj->sns_mode_obj.nWidth == 3840 && sns_obj->sns_mode_obj.nHeight == 2160)) {
            sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMinIntegrationTime[HDR_LONG_FRAME_IDX] = 8;
            sns_obj->ae_ctrl_param.sns_ae_limit.tIntTimeRange.fMaxIntegrationTime[HDR_LONG_FRAME_IDX] = sns_ov48c40params[nPipeId].vts - 16;
            sns_obj->ae_ctrl_param.sns_ae_param.fIntegrationTimeIncrement[HDR_LONG_FRAME_IDX] = 2;
        }
    } else if (IS_2DOL_HDR_MODE(sns_obj->sns_mode_obj.eHDRMode)) {
    ...
    sns_obj->ae_ctrl_param.fSnsHcgLcgRatio = 4.532227f;
    sns_obj->ae_ctrl_param.eSnsHcgLcgMode = ov48c40_get_hcglcg_mode(nPipeId);
    SNS_DBG("eSnsHcgLcgMode = %d, fSnsHcgLcgRatio = %f \n", sns_obj->ae_ctrl_param.eSnsHcgLcgMode, sns_obj->ae_ctrl_param.fSnsHcgLcgRatio);

    if (sns_obj->sns_mode_obj.eHDRMode == AX_SNS_LINEAR_MODE) {
        if (sns_obj->sns_mode_obj.nWidth == 8000 && sns_obj->sns_mode_obj.nHeight == 6000) {
            sns_obj->ae_ctrl_param.sns_ae_param.fIntegrationTimeOffset[HDR_LONG_FRAME_IDX] = 1;
        } else if ((sns_obj->sns_mode_obj.nWidth == 4000 && sns_obj->sns_mode_obj.nHeight == 3000)
            || (sns_obj->sns_mode_obj.nWidth == 3840 && sns_obj->sns_mode_obj.nHeight == 2160)) {
            sns_obj->ae_ctrl_param.sns_ae_param.fIntegrationTimeOffset[HDR_LONG_FRAME_IDX] = 2;
        }
    ...
    SNS_DBG("inttime min = %.2f, max = %.2f, line_period=%f, vts = %d \n",
            ...
            sns_ov48c40params[nPipeId].line_period, sns_ov48c40params[nPipeId].vts);

    return SNS_SUCCESS;
}

AX_S32 ov48c40_sns_update_regidx_table(ISP_PIPE_ID nPipeId, AX_U8 nRegIdx, AX_U8 nRegValue)
{
    AX_S32 i = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
    SNS_CHECK_VALUE_RANGE_VALID(nPipeId, 0, AX_VIN_MAX_PIPE_NUM - 1);
...
AX_S32 ov48c40_get_sensor_gain_table(ISP_PIPE_ID nPipeId, AX_SNS_AE_GAIN_TABLE_T *params)
{
    AX_S32 result = 0;
    SNS_CHECK_PTR_VALID(params);
    SNS_CHECK_VALUE_RANGE_VALID(nPipeId, 0, AX_VIN_MAX_PIPE_NUM - 1);

    result = ov48c40_get_gain_table(params);
    return result;

    return SNS_SUCCESS;
}

AX_S32 ov48c40_set_again(ISP_PIPE_ID nPipeId, AX_SNS_AE_GAIN_CFG_T *ptAGain)
{
    AX_U8 Gain_in;
    AX_U8 Gain_de;
    AX_S32 result = 0;
    ...
    gain_value = ov48c40_again2value(nGainFromUser, &Gain_in, &Gain_de);
    if (gain_value == -1) {
        SNS_ERR("new gain match failed \n");
    } else {
        sns_obj->ae_ctrl_param.sns_ae_param.fCurAGain[HDR_LONG_FRAME_IDX] = gain_value;
        result = ov48c40_sns_update_regidx_table(nPipeId, OV48C40_LONG_AGAIN_H_IDX, (Gain_in & 0x7F));
        result |= ov48c40_sns_update_regidx_table(nPipeId, OV48C40_LONG_AGAIN_L_IDX, (Gain_de & 0xFE));
        if (result != 0) {
    ...
    if (IS_HDR_MODE(sns_obj->sns_mode_obj.eHDRMode)) {
        ...
        if (gain_value == -1) {
            ...
            result = ov48c40_sns_update_regidx_table(nPipeId, OV48C40_SHORT_AGAIN_H_IDX, (Gain_in & 0x7F));
            result = ov48c40_sns_update_regidx_table(nPipeId, OV48C40_SHORT_AGAIN_L_IDX, (Gain_de & 0xFE));
            if (result != 0) {
...
AX_S32 ov48c40_set_dgain(ISP_PIPE_ID nPipeId, AX_SNS_AE_GAIN_CFG_T *ptDGain)
{
    AX_U8 Gain_in;
    AX_U8 Gain_de;
    AX_U8 Gain_de2;
    ...
    gain_val = ov48c40_dgain2value(nGainFromUser, &Gain_in, &Gain_de, &Gain_de2);
    if (gain_val == -1) {
        SNS_ERR("new gain match failed \n");
    } else {
        sns_obj->ae_ctrl_param.sns_ae_param.fCurDGain[HDR_LONG_FRAME_IDX] = gain_val;
        result = ov48c40_sns_update_regidx_table(nPipeId, OV48C40_LONG_DGAIN_H_IDX, (Gain_in & 0x0F));
        result = ov48c40_sns_update_regidx_table(nPipeId, OV48C40_LONG_DGAIN_M_IDX, (Gain_de & 0xFF));
        result = ov48c40_sns_update_regidx_table(nPipeId, OV48C40_LONG_DGAIN_L_IDX, (Gain_de2 & 0xC0));
        if (result != 0) {
            SNS_ERR("write hw failed %d \n", result);
            return result;
        }
    ...
    if (IS_HDR_MODE(sns_obj->sns_mode_obj.eHDRMode)) {
        ...
        if (gain_val == -1) {
            ...
            result = ov48c40_sns_update_regidx_table(nPipeId, OV48C40_SHORT_DGAIN_H_IDX, (Gain_in & 0x0F));
            result = ov48c40_sns_update_regidx_table(nPipeId, OV48C40_SHORT_DGAIN_M_IDX, (Gain_de & 0xFF));
            result = ov48c40_sns_update_regidx_table(nPipeId, OV48C40_SHORT_DGAIN_L_IDX, (Gain_de2 & 0xC0));
            if (result != 0) {
                SNS_ERR("write hw failed %d \n", result);
                return result;
            }
...
AX_S32 ov48c40_get_integration_time_range(ISP_PIPE_ID nPipeId, AX_F32 fHdrRatio,
...
AX_S32 ov48c40_set_integration_time(ISP_PIPE_ID nPipeId, AX_SNS_AE_SHUTTER_CFG_T *ptIntTime)
{
    AX_U8 ex_h;
    AX_U8 ex_m;
    AX_U8 ex_l;
    ...
    ex_h = (ex_ival & 0xff0000) >> 16;
    ex_m = (ex_ival & 0xff00) >> 8;
    ex_l = (ex_ival & 0xff) >> 0;

    ov48c40_sns_update_regidx_table(nPipeId, OV48C40_LONG_EXP_LINE_H_IDX, ex_h);
    ov48c40_sns_update_regidx_table(nPipeId, OV48C40_LONG_EXP_LINE_M_IDX, ex_m);
    ov48c40_sns_update_regidx_table(nPipeId, OV48C40_LONG_EXP_LINE_L_IDX, ex_l);

    sns_obj->ae_ctrl_param.sns_ae_param.fCurIntegrationTime[HDR_LONG_FRAME_IDX] = fExpLineFromUser;
    SNS_DBG("fExpLineFromUser %f, ex_ival 0x%x, 0x%x 0x%x 0x%x\n",
            fExpLineFromUser,
            ex_ival, ex_h, ex_m, ex_l);
    if (IS_HDR_MODE(sns_obj->sns_mode_obj.eHDRMode)) {
        ...
        ov48c40_sns_update_regidx_table(nPipeId, OV48C40_SHORT_EXP_LINE_H_IDX, ex_h);
        ov48c40_sns_update_regidx_table(nPipeId, OV48C40_SHORT_EXP_LINE_M_IDX, ex_m);
        ov48c40_sns_update_regidx_table(nPipeId, OV48C40_SHORT_EXP_LINE_L_IDX, ex_l);
...
AX_S32 ov48c40_get_hw_exposure_params(ISP_PIPE_ID nPipeId, AX_SNS_EXP_CTRL_PARAM_T *ptAeCtrlParam)
{
    SNS_STATE_OBJ *sns_obj = AX_NULL;
...
AX_S32 ov48c40_hcglcg_ctrl(ISP_PIPE_ID nPipeId, AX_U32 nSnsHcgLcg)
{
    SNS_STATE_OBJ *sns_obj = AX_NULL;
    ...
    if (AX_LCG_MODE == nSnsHcgLcg) {
        ...
        /* group hold */
        ov48c40_sns_update_regidx_table(nPipeId, OV48C40_GROUP1_IDX, 0x00);
        /*delay to N+2 to align with gain and exposure*/
        ov48c40_sns_update_regidx_table(nPipeId, OV48C40_HCG_LCG_REG0_IDX, 0x00);
        ov48c40_sns_update_regidx_table(nPipeId, OV48C40_GROUP2_IDX, 0x10);
        ov48c40_sns_update_regidx_table(nPipeId, OV48C40_GROUP3_IDX, 0xe0);
        ...
        ov48c40_sns_update_regidx_table(nPipeId, OV48C40_GROUP1_IDX, 0x00);
        /*delay to N+2 to align with gain and exposure*/
        ov48c40_sns_update_regidx_table(nPipeId, OV48C40_HCG_LCG_REG0_IDX, 0x07);
        ov48c40_sns_update_regidx_table(nPipeId, OV48C40_GROUP2_IDX, 0x10);
        ov48c40_sns_update_regidx_table(nPipeId, OV48C40_GROUP3_IDX, 0xe0);
    }
...
AX_S32 ov48c40_sns_refresh_all_regs_from_tbl(ISP_PIPE_ID nPipeId)
{
    AX_S32 i = 0;
    AX_U32 nNum = 0;
    AX_U32  nRegValue;
    ...
    nNum = sizeof(gOv48c40AeRegsTable) / sizeof(gOv48c40AeRegsTable[0]);

    for (i = 0; i < nNum; i++) {
        nRegValue = ov48c40_reg_read(nPipeId, gOv48c40AeRegsTable[i].nRegAddr);
        sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].nRegAddr = gOv48c40AeRegsTable[i].nRegAddr;
        sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].nData = nRegValue;

        SNS_DBG(" nRegAddr 0x%x, nRegValue 0x%x\n", gOv48c40AeRegsTable[i].nRegAddr, nRegValue);
    }

    return SNS_SUCCESS;
}

AX_S32 ov48c40_sns_update_init_exposure_reg(ISP_PIPE_ID nPipeId)
{
    int i = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
    SNS_CHECK_VALUE_RANGE_VALID(nPipeId, 0, AX_VIN_MAX_PIPE_NUM - 1);
    ...
    for (i = 0; i < sns_obj->sztRegsInfo[0].nRegNum; i++) {
        ov48c40_write_register(nPipeId, sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].nRegAddr, sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].nData);
        SNS_DBG("Idx = %d, reg addr 0x%x, reg data 0x%x\n",
...
static AX_VOID _sns_reg_info_init(ISP_PIPE_ID nPipeId, SNS_STATE_OBJ *sns_obj)
{
    ...
    sns_obj->sztRegsInfo[0].tComBus.I2cDev = ov48c40_get_bus_num(nPipeId);
    sns_obj->sztRegsInfo[0].nRegNum = sizeof(gOv48c40AeRegsTable) / sizeof(gOv48c40AeRegsTable[0]);
    sns_obj->sztRegsInfo[0].nCfg2ValidDelayMax = 2;

    for (i = 0; i < sns_obj->sztRegsInfo[0].nRegNum; i++) {
        sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].bUpdate = AX_TRUE;
        sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].nDevAddr = OV48C40_SLAVE_ADDR1;
        sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].nAddrByteNum = OV48C40_ADDR_BYTE;
        sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].nDataByteNum = OV48C40_DATA_BYTE;
        sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].nDelayFrmNum = gOv48c40AeRegsTable[i].nDelayFrmNum;
        if (IS_LINEAR_MODE(sns_obj->sns_mode_obj.eHDRMode)) {
        ...
        SNS_DBG("[%2d] nRegAddr 0x%x, nRegValue 0x%x\n", i,
                gOv48c40AeRegsTable[i].nRegAddr, gOv48c40AeRegsTable[i].nRegValue);
    }

    sns_obj->sztRegsInfo[0].sztData.sztI2cData[OV48C40_GROUP1_IDX].nData = 0x00;
    sns_obj->sztRegsInfo[0].sztData.sztI2cData[OV48C40_GROUP2_IDX].nData = 0x10;
    sns_obj->sztRegsInfo[0].sztData.sztI2cData[OV48C40_GROUP3_IDX].nData = 0xe0;
...
static AX_VOID _sns_reg_info_update(ISP_PIPE_ID nPipeId, SNS_STATE_OBJ *sns_obj, AX_BOOL *bUpdateReg)
{
    ...
    for (i = 0; i < sns_obj->sztRegsInfo[0].nRegNum; i++) {
        if (sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].nData == sns_obj->sztRegsInfo[1].sztData.sztI2cData[i].nData) {
            ...
            if (i <= OV48C40_VTS_L_IDX) {
                sns_obj->sztRegsInfo[0].sztData.sztI2cData[i].bUpdate = AX_TRUE;
                *bUpdateReg = AX_TRUE;
            } else {
                sns_obj->sztRegsInfo[0].sztData.sztI2cData[OV48C40_GROUP1_IDX].bUpdate = AX_TRUE;
                sns_obj->sztRegsInfo[0].sztData.sztI2cData[OV48C40_HCG_LCG_REG0_IDX].bUpdate = AX_TRUE;
                sns_obj->sztRegsInfo[0].sztData.sztI2cData[OV48C40_GROUP2_IDX].bUpdate = AX_TRUE;
                sns_obj->sztRegsInfo[0].sztData.sztI2cData[OV48C40_GROUP3_IDX].bUpdate = AX_TRUE;
                *bUpdateReg = AX_TRUE;
                break;
            }
        }
...
AX_S32 ov48c40_ae_get_sensor_reg_info(ISP_PIPE_ID nPipeId, AX_SNS_REGS_CFG_TABLE_T *ptSnsRegsInfo)
{
    AX_S32 nRet = 0;
    SNS_STATE_OBJ *sns_obj = AX_NULL;
    ...
    if ((AX_FALSE == sns_obj->bSyncInit) || (AX_FALSE == ptSnsRegsInfo->bConfig)) {
        ...
        ov48c40_sns_update_init_exposure_reg(nPipeId);
    } else {
...
static AX_U32 ov48c40_set_vts(ISP_PIPE_ID nPipeId, AX_U32 vts)
{
    AX_U8 vts_h;
    AX_U8 vts_l;
    AX_S32 result = 0;
    ...
    result |= ov48c40_sns_update_regidx_table(nPipeId, OV48C40_VTS_H_IDX, vts_h);
    result |= ov48c40_sns_update_regidx_table(nPipeId, OV48C40_VTS_L_IDX, vts_l);

    return result;
}

AX_S32 ov48c40_set_slow_fps(ISP_PIPE_ID nPipeId, AX_F32 nFps)
{
    AX_S32 result = 0;
    AX_S32 framerate = 30;
    AX_U32 vts = 0;
    ...
    if (nFps >= framerate) {
        sns_ov48c40params[nPipeId].vts = vts;
    } else {
        sns_ov48c40params[nPipeId].vts = 1 * SNS_1_SECOND_UNIT_US / (sns_ov48c40params[nPipeId].line_period * nFps);
    }

    if (sns_ov48c40params[nPipeId].vts > OV48C40_MAX_VTS) {
        sns_ov48c40params[nPipeId].vts = OV48C40_MAX_VTS;
        nFps = 1 * SNS_1_SECOND_UNIT_US / (sns_ov48c40params[nPipeId].line_period * sns_ov48c40params[nPipeId].vts);
        SNS_ERR("Beyond minmum fps  %f\n", nFps);
    }

    result = ov48c40_set_vts(nPipeId, sns_ov48c40params[nPipeId].vts);
    if (result != 0) {
    ...
    sns_obj->ae_ctrl_param.sns_ae_param.fCurFps = 1 * SNS_1_SECOND_UNIT_US / (sns_ov48c40params[nPipeId].line_period *
            sns_ov48c40params[nPipeId].vts);

    SNS_DBG("nPipeId = %d, fps(from alg) = %f, current vts = 0x%x\n", nPipeId, nFps, sns_ov48c40params[nPipeId].vts);

    return SNS_SUCCESS;
}

AX_S32 ov48c40_get_slow_shutter_param(ISP_PIPE_ID nPipeId, AX_SNS_AE_SLOW_SHUTTER_PARAM_T *ptSlowShutterParam)
{
     AX_S32 framerate = 30;
    AX_U32 nfps = 0;
    AX_U32 nVts = 0;
    ...
    if (sns_ov48c40params[nPipeId].line_period == 0) {
        SNS_ERR("line_period is zero : %f\n", sns_ov48c40params[nPipeId].line_period);
        return SNS_ERR_CODE_ILLEGAL_PARAMS;
    }
    ...
    ptSlowShutterParam->fMinFps = AXSNS_MAX(gFpsGear[0],
                                            (1 * SNS_1_SECOND_UNIT_US / (sns_ov48c40params[nPipeId].line_period * OV48C40_MAX_VTS)));

    for (nfps = 0 ; nfps < ptSlowShutterParam->nGroupNum; nfps++) {
        nVts = 1 * SNS_1_SECOND_UNIT_US / (sns_ov48c40params[nPipeId].line_period * gFpsGear[nfps]);
        if ((AX_S32)gFpsGear[nfps] >= framerate) {
            nVts = sns_obj->sns_mode_obj.nVts;
        }
        if (nVts > OV48C40_MAX_VTS) {
            nVts = OV48C40_MAX_VTS;
            SNS_WRN("Beyond minmum fps  %f\n", ptSlowShutterParam->fMinFps);
        }

        if (IS_LINEAR_MODE(sns_obj->sns_mode_obj.eHDRMode)) {
            if (sns_obj->sns_mode_obj.nWidth == 8000 && sns_obj->sns_mode_obj.nHeight == 6000) {
                ptSlowShutterParam->tSlowShutterTbl[nfps].fMaxIntTime = nVts - 32;
            } else if ((sns_obj->sns_mode_obj.nWidth == 4000 && sns_obj->sns_mode_obj.nHeight == 3000)
            || (sns_obj->sns_mode_obj.nWidth == 3840 && sns_obj->sns_mode_obj.nHeight == 2160)) {
                ptSlowShutterParam->tSlowShutterTbl[nfps].fMaxIntTime = nVts - 16;
            }
        } else if (IS_2DOL_HDR_MODE(sns_obj->sns_mode_obj.eHDRMode)) {

        }

        ptSlowShutterParam->tSlowShutterTbl[nfps].fRealFps = 1 * SNS_1_SECOND_UNIT_US / (sns_ov48c40params[nPipeId].line_period
                * nVts);
        ...
        SNS_DBG("nPipeId = %d, line_period = %.2f, fps = %.2f, fMaxIntTime = %.2f, vts=0x%x\n",
                nPipeId, sns_ov48c40params[nPipeId].line_period,
...
AX_S32 ov48c40_get_fps(ISP_PIPE_ID nPipeId, AX_F32 *pFps)
{
    SNS_STATE_OBJ *sns_obj = AX_NULL;
...
AX_S32 ov48c40_set_fps(ISP_PIPE_ID nPipeId, AX_F32 fFps)
{
    AX_S32 result = 0;
    AX_U32 vts = 0;
    ...
    if(IS_SNS_FPS_EQUAL(fFps, 25.0) || IS_SNS_FPS_EQUAL(fFps, 50.0)) {  /* PAL */
        if(AX_SNS_LINEAR_MODE == sns_obj->sns_attr_param.eSnsMode) {
            if(AX_RT_RAW10 == sns_obj->sns_attr_param.eRawType) {
                vts = OV48C40_VTS_10BIT_4M25_SDR;  /* SDR 10BIT 4LINE 25fps */
            }
        }
    }

    if(IS_SNS_FPS_EQUAL(fFps, 30.0) || IS_SNS_FPS_EQUAL(fFps, 60.0)) {  /* NTSC */
        if(AX_SNS_LINEAR_MODE == sns_obj->sns_attr_param.eSnsMode) {
            if(AX_RT_RAW10 == sns_obj->sns_attr_param.eRawType) {
                vts = OV48C40_VTS_10BIT_4M30_SDR;  /* SDR 10BIT 4LINE */
            }
        }
    }
    ...
    /* change fps */
    sns_obj->sns_mode_obj.nVts = sns_ov48c40params[nPipeId].vts;
    ...
    SNS_DBG("nPipeId = %d, fps(from alg) = %f, current vts = 0x%x\n", nPipeId, fFps, sns_ov48c40params[nPipeId].vts);
